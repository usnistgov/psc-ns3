  ...
  for (const auto &itBwps:slBwpIds)
    {
      ...
      psfchCallback = std::bind (&NrUePhy::PhyPsfchReceived, nrUeDev->GetPhy (itBwps), std::placeholders::_1, std::placeholders::_2);
      spectrumPhy->SetNrPhyRxSlPsfchCallback (psfchCallback);

      spectrumPhy->SetPhySlHarqFeedbackCallback (MakeCallback (&NrUePhy::EnqueueSlHarqFeedback, nrUeDev->GetPhy (itBwps)));
