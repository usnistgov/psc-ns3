...
void 
NrSlHelper::InstallNrSlDiscoveryConfiguration (NetDeviceContainer relays, NetDeviceContainer remotes, const LteRrcSap::SlDiscConfigCommon discConfig)
{
  NS_LOG_FUNCTION (this);
  
  for (NetDeviceContainer::Iterator i = relays.Begin (); i != relays.End (); ++i)
    {
  ...
  for (NetDeviceContainer::Iterator j = remotes.Begin (); j != remotes.End (); ++j)
    {
