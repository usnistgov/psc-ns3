...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_LOGIC ("Pscch Packet burst size: " << m_nrSlPscchPacketBurstQueue.size () << " number of packets: " << m_nrSlPscchPacketBurstQueue.at (0)->GetNPackets ());
...
NrPhy::NrSlSlotAllocInfoExists (const SfnSf &sfn) const
{
  ...
  for (auto it = m_nrSlPsschPacketBurstQueue.begin (); it != m_nrSlPsschPacketBurstQueue.end (); it++)
    {
