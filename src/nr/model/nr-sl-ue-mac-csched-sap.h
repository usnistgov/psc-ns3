...
public:
  ...
  /**
   * \brief Remove the NR Sidelink logical channel configuration from UE MAC and scheduler
   *
   * \param lcid logical channel ID
   * \param dstL2Id destination layer 2 ID
   */
  virtual void RemoveUeNrSlLcConfigReq (uint8_t lcid, uint32_t dstL2Id) = 0;
};
...
public:
  ...
  /**
   * \brief Send the confirmation about the removal of configuration of LC
   *        to the UE MAC.
   *
   * \param lcid The Logical Channel id
   */
  virtual void RemoveUeNrSlLcConfigCnf (uint8_t lcid) = 0;
