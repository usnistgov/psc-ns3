...
public:
   ...
   */
  typedef void (* AllocateTracedCallback)(uint8_t harqId, uint32_t dstL2Id, Time timeout, std::size_t available);
...
private:
  ...
  TracedCallback<uint8_t, uint32_t, Time, std::size_t> m_allocateTrace; // Trace HARQ ID allocation
