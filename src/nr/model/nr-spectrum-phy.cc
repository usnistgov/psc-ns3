...
NrSpectrumPhy::GetTypeId (void)
{
  static TypeId
    ...
    .AddTraceSource ("TxCtrlTrace",
                     ...
                     "ns3::Time::TracedCallback")
    .AddTraceSource ("TxFeedbackTrace",
...
void
NrSpectrumPhy::StartTxSlFeedback (const std::list<Ptr<NrSlHarqFeedbackMessage> > & feedbackList, const Time &duration)
{
  NS_LOG_FUNCTION (this << " state: " << m_state);
    ...
    case IDLE:
      {
        ...
        Ptr<NrSpectrumSignalParametersSlFeedback> txParams = Create<NrSpectrumSignalParametersSlFeedback> ();
        txParams->duration = duration;
        txParams->txPhy = GetObject<SpectrumPhy> ();
        txParams->psd = m_txPsd;
        txParams->nodeId = GetDevice ()->GetNode ()->GetId ();
  ...
  for (uint16_t i = 0; i < m_slRxSigParamInfo.size (); i++)
    {
      ...
      Ptr<NrSpectrumSignalParametersSlFeedback> nrSlFbRxParams = DynamicCast<NrSpectrumSignalParametersSlFeedback> (params);
  ...
  for (auto &ctrlMsgIt : sortedControlMessages)
    {
      ...
      if (m_slCtrlErrorModelEnabled)
        {
          ...
          outputEmForCtrl = m_slErrorModel->GetTbDecodificationStats (m_slSinrPerceived.at (paramIndex),
      ...
      Ptr<NrSpectrumSignalParametersSlCtrlFrame> ctrlParams = DynamicCast<NrSpectrumSignalParametersSlCtrlFrame> (m_slRxSigParamInfo.at (paramIndex).params);
  ...
  for (uint32_t i = 0; i < m_slRxSigParamInfo.size (); i++)
    {
      ...
      Ptr<NrSpectrumSignalParametersSlDataFrame> dataParams = DynamicCast<NrSpectrumSignalParametersSlDataFrame> (m_slRxSigParamInfo.at (pktIndex).params);
  ...
  for (auto &tbIt : m_slTransportBlocks)
    {
      ...
      if (m_slDataErrorModelEnabled)
        {
          ...
          tbIt.second.outputEmForSci2 = m_slErrorModel->GetTbDecodificationStats (tbIt.second.sinrPerceived,
          ...
          sciF2aCorrupted = m_random->GetValue () > tbIt.second.outputEmForSci2->m_tbler ? false : true;
          ...
          if (m_harqPhyModule->IsPrevDecoded (tbIt.first, sciF2a.GetHarqId ()))
            {
              NS_LOG_DEBUG ("Do not dispatch already decoded TB; may be blind retx: " << tbIt.first);
              continue;
            }
          ...
          //process
          // retrieve HARQ info
          const NrErrorModel::NrErrorModelHistory & harqInfoList = m_harqPhyModule->GetHarqProcessInfoSl (tbIt.first, sciF2a.GetHarqId ());
          if (sciF2a.GetNdi () && harqInfoList.size () > 0)
            {
              ...
              const_cast<NrErrorModel::NrErrorModelHistory &>(harqInfoList) = m_harqPhyModule->GetHarqProcessInfoSl (tbIt.first, sciF2a.GetHarqId ());
            }
          tbIt.second.outputEmForData = m_slErrorModel->GetTbDecodificationStats (tbIt.second.sinrPerceived,
                                                                                  tbIt.second.expectedTb.rbBitmap,
          ...
          if (!rbCollided)
            {
              ...
              for (auto it = tbIt.second.sinrPerceived.ConstValuesBegin (); it != tbIt.second.sinrPerceived.ConstValuesEnd (); it++)
                {
      ...
      GetSecond GetTBInfo;
      // send HARQ feedback (if not already done for this TB)
      if (tbIt.second.isHarqEnabled && ! GetTBInfo (tbIt).harqFeedbackSent && !m_phySlHarqFeedbackCallback.IsNull ())
        {
      ...
      if (!tbIt.second.isDataCorrupted)
        {
          ...
          Ptr<NrSpectrumSignalParametersSlDataFrame> dataParams = DynamicCast<NrSpectrumSignalParametersSlDataFrame> (m_slRxSigParamInfo.at (tbIt.second.pktIndex).params);
          m_nrPhyRxPsschEndOkCallback (dataParams->packetBurst);
        }
    }
...
NrSpectrumPhy::RxSlPsfch (std::vector<uint32_t> paramIndexes)
{
  ...
  for (uint32_t i = 0; i < m_slRxSigParamInfo.size (); i++)
    {
      uint32_t pktIndex = paramIndexes [i];
      Ptr<NrSpectrumSignalParametersSlFeedback> feedbackParams = DynamicCast<NrSpectrumSignalParametersSlFeedback> (m_slRxSigParamInfo.at (pktIndex).params);
      for (auto& it : feedbackParams->feedbackList)
        {
          NS_LOG_DEBUG ("Received RxSlPsfch from node " << feedbackParams->nodeId 
            << " dstL2Id " << it->GetSlHarqFeedback ().m_dstL2Id
            << " harqProcessId " << +it->GetSlHarqFeedback ().m_harqProcessId
            << " bwpIndex " << +it->GetSlHarqFeedback ().m_bwpIndex
  ...
  if (it != m_slTransportBlocks.end ())
    {
      ...
      NS_LOG_WARN ("Variable m_slTransportBlocks is not designed to manage two TBs from same RNTI in same slot");
      NS_LOG_DEBUG ("RNTI " << ueRx->GetPhy (GetBwpId ())->GetRnti ()
                << " failed to add NR SL expected TB from rnti " << rnti << " with Dest id " << dstId
