...
void
NrSlUeMacSchedulerNs3::DoRemoveUeNrSlLcConfigReq (uint8_t lcid, uint32_t dstL2Id)
{
  NS_LOG_FUNCTION (this << lcid << dstL2Id);
  RemoveDstInfo (lcid, dstL2Id);
  //Send confirmation to MAC
  m_nrSlUeMacCschedSapUser->RemoveUeNrSlLcConfigCnf (lcid);
}

void
NrSlUeMacSchedulerNs3::RemoveDstInfo (uint8_t lcid, uint32_t dstL2Id)
{
  NS_LOG_FUNCTION (this << lcid << dstL2Id);
