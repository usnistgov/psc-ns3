...
NrUePhy::EnqueueSlHarqFeedback (const SlHarqInfo &m)
{
  ...
  NS_LOG_DEBUG ("Enqueued SL HARQ " << (m.IsReceivedOk () ? "ACK" : "NACK") << " in slot "
    << m_currentSlot.Normalize () << " for process " << +m.m_harqProcessId);
  ...
  do
    {
      ...
      NS_LOG_DEBUG ("UE" << m_rnti <<
                    ...
                    "-" << varTtiInfo.symStart + varTtiInfo.symLength - 1
                         << "\t start " << Simulator::Now () <<
                    " end " << (Simulator::Now () + varTtiDuration).GetSeconds ());

      Simulator::Schedule (NanoSeconds (1.0), &NrUePhy::SendNrSlDataChannels, this,
                           pktBurst, varTtiDuration - NanoSeconds (2.0), varTtiInfo);
    }
  while (pktBurst = PopPsschPacketBurst ());
  return varTtiDuration;
}
...
NrUePhy::SlFeedback (const NrSlVarTtiAllocInfo &varTtiInfo)
{
  ...
  while (it != m_slHarqFbList.end ())
    {
      if (m_currentSlot.Normalize () >= gap + it->first.Normalize ())
        ...
        {
          NS_LOG_DEBUG ("At slot " << m_currentSlot.Normalize ()
            << "; suppressing (processing delay " << +gap << " slots from "
            << it->first.Normalize () << ") the insertion of HARQ FB for sender RNTI "
            << it->second->GetSlHarqFeedback ().m_txRnti << " dstL2Id "
            << it->second->GetSlHarqFeedback ().m_dstL2Id << " harqProcessId "
            << +it->second->GetSlHarqFeedback ().m_harqProcessId << " bwpIndex "
            << +it->second->GetSlHarqFeedback ().m_bwpIndex << " status "
  ...
  while (it2 != feedbackList.end ())
    {
      ...
      if (duplicateCheck.insert (std::make_pair (rnti, harqProcessId)).second == true)
        {
          NS_LOG_DEBUG ("Preparing HARQ feedback for sender RNTI " << rnti << " HARQ PID " << +harqProcessId);
  ...
  if (uniqueFeedbackList.size ())
    {
      NS_LOG_DEBUG ("UE" << m_rnti <<
                " TXing NR SL FEEDBACK frame for symbols "  << varTtiInfo.symStart <<
                "-" << varTtiInfo.symStart + varTtiInfo.symLength - 1
                     << "\t start " << Simulator::Now ().GetSeconds () <<
                " end " << (Simulator::Now () + varTtiDuration).GetSeconds ());

      Simulator::Schedule (NanoSeconds (1.0), &NrUePhy::SendNrSlFbChannels, this,
                       uniqueFeedbackList, varTtiDuration - NanoSeconds (2.0), varTtiInfo);
    }
  return varTtiDuration;
}

void
NrUePhy::SendNrSlFbChannels (const std::list<Ptr<NrSlHarqFeedbackMessage> > & feedbackList, const Time &varTtiDuration, const NrSlVarTtiAllocInfo &varTtiInfo)
{
  ...
  m_spectrumPhys.at (0)->StartTxSlFeedback (feedbackList, varTtiDuration);
}
...
NrUePhy::PhyPsfchReceived (uint32_t sendingNodeId, SlHarqInfo harqInfo)
{
  ...
  Simulator::ScheduleWithContext (m_netDevice->GetNode ()->GetId (),
                                  ...
                                  m_nrSlUePhySapUser, sendingNodeId, harqInfo);
}
