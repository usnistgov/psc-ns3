...
NrUePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrUePhy")
    ...
    .AddTraceSource ("ReportUeSlRsrpMeasurements",
                     "Report UE sidelink RSRP measurements (dBm).",
                     MakeTraceSourceAccessor (&NrUePhy::m_reportUeSlRsrpMeasurements),
                     "ns3::NrUePhy::SlRsrpTracedCallback")
      ;
  return tid;
}
      ...
      if ((tag.GetLcid () == 4)) 
        {
          ...
          std::map<uint32_t, UeSlRsrpMeasurementsElement>::iterator itRsrp = m_ueSlRsrpMeasurementsMap.find (srcL2Id);
          if (itRsrp == m_ueSlRsrpMeasurementsMap.end ())
            {
              ...
              elt.rsrpNum = 1;
              m_ueSlRsrpMeasurementsMap.insert (std::pair<uint32_t,UeSlRsrpMeasurementsElement> (srcL2Id, elt));
            }
          else
            {
  ...
  if (m_ueSlRsrpMeasurementsEnabled)
    {
      ...
      for (auto it = m_ueSlRsrpMeasurementsMap.begin (); it != m_ueSlRsrpMeasurementsMap.end (); it++)
        { 
          ...
          NS_LOG_INFO (this << " UE L2 Id " << it->first
                            << " averaged RSRP (dBm) " << avgRsrpDbm
