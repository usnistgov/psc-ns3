...
public:
  ...
  virtual void RemoveUeNrSlLcConfigCnf (uint8_t lcId);
...
void
MemberNrSlUeMacCschedSapUser::RemoveUeNrSlLcConfigCnf (uint8_t lcId)
{
  m_mac->DoRemoveUeNrSlLcConfigCnf (lcId);
}
...
NrUeMac::DoRemoveLc (uint8_t lcId)
{
  ...
  NS_ASSERT_MSG (m_lcInfoMap.find (lcId) == m_lcInfoMap.end (), "cannot remove channel because LCID " << lcId << " doesn't exist");
              ...
              for (const auto & itLcRlcPduInfo : currentGrant.slRlcPduInfo)
                {
                  ...
                  if (nrSlHarqId == std::numeric_limits <uint8_t>::max ())
                    {
...
NrUeMac::DoAddNrSlLc (const NrSlUeCmacSapProvider::SidelinkLogicalChannelInfo &slLcInfo, NrSlMacSapUser* msu)
{
  ...
  NS_LOG_DEBUG ("adding LCID " << +slLcInfo.lcId << ", srcL2Id " << slLcInfo.srcL2Id << ", dstL2Id " << slLcInfo.dstL2Id);
...
NrUeMac::DoRemoveNrSlLc (uint8_t slLcId, uint32_t srcL2Id, uint32_t dstL2Id)
{
  ...
  GrantInfoIt_t itGrantInfo = m_grantInfo.find (dstL2Id);
  if (itGrantInfo != m_grantInfo.end ())
    {
      for (auto grantIt = itGrantInfo->second.slotAllocations.begin (); grantIt != itGrantInfo->second.slotAllocations.end (); grantIt++)
        {
          NrSlSlotAlloc currentGrant = *grantIt;
          //Find RLC PDU with matching LCID, source, and destination
          auto itLcRlcPduInfo = currentGrant.slRlcPduInfo.begin ();
          while (itLcRlcPduInfo != currentGrant.slRlcPduInfo.end ())
            {
              if (itLcRlcPduInfo->lcid == slLcId && currentGrant.dstL2Id == dstL2Id && m_srcL2Id == srcL2Id && !m_nrSlHarq->IsHarqIdAvaiable (itGrantInfo->second.nrSlHarqId))
                {
                  m_nrSlHarq->RecvNrSlHarqFeedback (currentGrant.dstL2Id, itGrantInfo->second.nrSlHarqId);
                  itLcRlcPduInfo = currentGrant.slRlcPduInfo.erase (itLcRlcPduInfo);
                }
              else
                {
                  itLcRlcPduInfo++;
      ...
      m_grantInfo.erase (itGrantInfo);
    }
    
  if (srcL2Id == m_srcL2Id)
    {
      m_nrSlUeMacCschedSapProvider->RemoveUeNrSlLcConfigReq (slLcId, dstL2Id);
    }
...
NrUeMac::DoResetNrSlLcMap ()
{
  ...
  while (it != m_nrSlLcInfoMap.end ())
    {
      if (it->first.lcId > 4) //SL DRB LC starts from 5
        {
...
void
NrUeMac::RemoveNrSlDstL2Id (uint32_t dstL2Id)
{
  NS_LOG_FUNCTION (this << dstL2Id);
  bool foundDst = false;
  uint32_t i = 0;
  while (i < m_sidelinkTxDestinations.size())
    {
      if (m_sidelinkTxDestinations[i].first == dstL2Id)
        {
          foundDst = true;
          m_sidelinkTxDestinations.erase (m_sidelinkTxDestinations.begin()+i);
          break;
        }
      else
        {
  ...
  if (!foundDst)
    {
      NS_FATAL_ERROR ("Can't find destination");
    }
...
void
NrUeMac::DoRemoveNrSlRxDstL2Id (uint32_t dstL2Id)
{
  NS_LOG_FUNCTION (this << dstL2Id);
...
void
NrUeMac::DoRemoveUeNrSlLcConfigCnf (uint8_t lcId)
{
