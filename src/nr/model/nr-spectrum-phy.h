...
private:
  ...
  NrPhySlHarqFeedbackCallback m_phySlHarqFeedbackCallback; //!< callback that is notified when the SL HARQ feedback is being generated
  ...
  TracedCallback <Time> m_txFeedbackTrace; //!< trace callback that is notifying when this spectrum phy starts to occupy the channel with transmission of HARQ feedback
...
public:
 ...
 struct SlTransportBlockInfo
 {
   ...
   * \param duration the duration of transmission
   */
  void StartTxSlFeedback (const std::list<Ptr<NrSlHarqFeedbackMessage> > & feedbackList, const Time &duration);
  /**
...
private:
  ...
  Ptr<NrErrorModel> m_slErrorModel {nullptr}; //!< Instance of sidelink error model
  ...
  NrPhyRxSlPsfchCallback m_nrPhyRxSlPsfchCallback; //!< The callback for the NR SL PHY PSFCH successful reception
  /**
