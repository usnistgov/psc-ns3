...
{
    static TypeId tid =
        TypeId("ns3::OutdoorToOutdoorPropagationLossModel")
    ...
    if (dist >= 3)
    {
        if (r <= plos)
        {
            ...
            if (dist <= d1)
            {
                ...
                NS_LOG_INFO(this << "Outdoor LOS (Distance <= " << d1
                                 << ") : the WINNER B1 loss = " << pl_b1);
            }
            else
            {
                pl_b1 = 40 * std::log10(dist) + 7.56 - 17.3 * std::log10(hbs1) -
        ...
        else
        {
            ...
            if ((fc >= 0.758) and (fc <= 0.798))
            {
                ...
                pl_b1 = (44.9 - 6.55 * std::log10(hbs)) * std::log10(dist) +
                        5.83 * std::log10(hbs) + 16.33 + 26.16 * std::log10(fc) + nlos;
                NS_LOG_INFO(this << "Outdoor NLOS (Frequency 0.7 GHz) , the WINNER B1 loss = "
                                 << pl_b1);
            }
            if ((fc >= 1.92) and (fc <= 2.17))
            {
                ...
                pl_b1 = (44.9 - 6.55 * std::log10(hbs)) * std::log10(dist) +
                        5.83 * std::log10(hbs) + 14.78 + 34.97 * std::log10(fc) + nlos;
                NS_LOG_INFO(this << "Outdoor NLOS (Frequency 2 GHz) , the WINNER B1 loss = "
                                 << pl_b1);
            }
        }
    }
