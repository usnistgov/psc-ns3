...
TypeId
OutdoorToIndoorPropagationLossModel::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::OutdoorToIndoorPropagationLossModel")
...
OutdoorToIndoorPropagationLossModel::GetLoss(Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
    ...
    if (rand <= probLos)
    {
        ...
        {
            lossOut = 40 * std::log10(din + dout) + 7.56 - 17.3 * std::log10(hbs1) -
    ...
    else
    {
        ...
        if ((fc >= 0.758) and (fc <= 0.798))
        {
            ...
            lossOut = (44.9 - 6.55 * std::log10(hbs)) * std::log10(din + dout) +
