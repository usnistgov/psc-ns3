 ...
 * \ingroup buildings
 *
 *  \brief The Hybrid3gppPropagationModel is a compound of different models able to evaluate the
 * pathloss in different environments and with buildings (i.e., indoor and outdoor communications).
 *
...
class Hybrid3gppPropagationLossModel : public BuildingsPropagationLossModel
{
  ...
  private:
    ...
    /**
     * Evaluate the path loss based on outdoor propagation loss model from
    ...
    /**
     * Evaluate the path loss based on outdoor to indoor propagation loss model from
    ...
    /**
     * Evaluate the path loss based on urban macro cell propagation loss model from
     * OutdoorToIndoorPropagationLossModel
     *
     * \param a first mobility model
     * \param b second mobility model
    ...
    Ptr<IndoorToIndoorPropagationLossModel>
        m_indoorToIndoor; ///< Pointer to IndoorPropagationLossModel
    Ptr<OutdoorToOutdoorPropagationLossModel>
        m_outdoorToOutdoor; ///< Pointer to OutdoorPropagationLossModel
    Ptr<OutdoorToIndoorPropagationLossModel>
        m_outdoorToIndoor; ///< Pointer to OutdoorToIndoorPropagationLossModel
    Ptr<UrbanMacroCellPropagationLossModel>
        m_urbanMacroCell; ///< Pointer to UrbanMacroCellPropagationLossModel

    double m_frequency; ///< The propagation frequency in Hz
    TracedCallback<double, Ptr<Node>, Ptr<Node>, double, bool, bool>
        m_hybrid3gppPathlossTrace; ///< Trace
    ...
    mutable bool
        m_isMacroComm; ///< True if it is macro cell communication, i.e., eNB<->UE or UE<->eNB
    double m_heightThreshold;  ///< Height threshold for the difference between the height of TX and
                               ///< RX node
