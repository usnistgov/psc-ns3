 ...
 * \ingroup buildings
 *
 * \brief This class implements the urban macro cell propagation model for 2 - 6 GHz frequency range
 * based on 3GPP specifications: 3GPP TR 36.814 channel model for Urban Macro-cell scenario (UMa) /
 * Annex B
 *
 */
class UrbanMacroCellPropagationLossModel : public BuildingsPropagationLossModel
{
  public:
    ...
    /**
     * Calculate the pathloss in dB. The model returns 0 dB loss if the distance between a
     * transmitter and a receiver is less than 10 m. Therefore, a user should carefully deploy the
     * UEs, such that, the distance between an eNB and an UE is 10 m or above.
     *
