...
{
    static TypeId tid =
        TypeId("ns3::UrbanMacroCellPropagationLossModel")
            ...
            .AddTraceSource(
                "LineOfSight",
                "The trace export mobility of the transmitter and receiver, pathloss, shadowing, "
                "probLos"
    ...
    if (dist >= 10)
    {
        if ((rand <= probLos) || (m_isLosEnabled))
        {
            ...
            if (dist <= d1)
            {
                ...
                NS_LOG_INFO(this << "Urban macro cell LOS (Distance <= " << d1
                                 << ") : the pathloss = " << loss);
            }
            else
            {
                loss = 40.0 * std::log10(dist) + 7.8 - 18.0 * std::log10(hbs1) -
                       18.0 * std::log10(hms1) + 2.0 * std::log10(fc);
                NS_LOG_INFO(this << "Urban macro cell LOS (Distance > " << d1
                                 << ") : the pathloss = " << loss);
            }
        }
        ...
        {
            // NLOS
            loss = 161.04 - 7.1 * std::log10(m_streetWidth) + 7.5 * std::log10(m_buildingHeight) -
                   (24.37 - 3.7 * (std::pow(m_buildingHeight / hbs, 2)) * std::log10(hbs)) +
                   (43.42 + 3.1 * std::log10(hbs)) * (std::log10(dist) - 3) +
                   20.0 * std::log10(fc) - (3.2 * std::pow(std::log10(11.75 * hms), 2) - 4.97);
...
double
UrbanMacroCellPropagationLossModel::GetShadowing(Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
    NS_LOG_FUNCTION(this);
    if (m_isShadowingEnabled)
    {
        ...
        NS_ABORT_MSG_IF((!a1 || !b1),
                        "UrbanMacroCellPropagationLossModel only works with MobilityBuildingInfo");
...
double
UrbanMacroCellPropagationLossModel::EvaluateSigma(Ptr<MobilityBuildingInfo> a,
                                                  Ptr<MobilityBuildingInfo> b) const
{
    ...
    if (m_isShadowingEnabled)
    {
        ...
        double probLos =
            std::min((18 / dist), 1.0) * (1 - std::exp(-dist / 63)) + std::exp(-dist / 63);
        double rand = 0.0;
