...
{
    static TypeId tid =
        TypeId("ns3::Parabolic3dAntennaModel")
            ...
            .AddAttribute("ElectricalTilt",
                          "The angle (degrees) that expresses the orientation of the antenna on "
                          "the y-z plane relative to the y axis",
                          DoubleValue(0.0),
...
Parabolic3dAntennaModel::GetGainDb(Angles a)
{
    ...
    double gainDb = m_boresightGain -
                    std::min(-(GetHorizontalGainDb(a) + GetVerticalGainDb(a)), m_maxHAttenuation);
