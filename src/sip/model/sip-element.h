...
class SipElement : public Object
{
  public:
     ...
     */
    typedef void (*DialogStateTracedCallback)(uint16_t callId,
                                              uint32_t lowUri,
                                              uint32_t highUri,
                                              DialogState state);
     ...
     * \param [in] state DialogState
     */
    typedef void (*TransactionStateTracedCallback)(uint16_t callId,
                                                   uint32_t from,
                                                   uint32_t to,
  ...
  protected:
    ...
    struct TupleHash
    {
        std::size_t operator()(const std::tuple<uint16_t, uint32_t, uint32_t>& tuple) const
        {
            ...
            return std::hash<std::size_t>()(static_cast<uint32_t>(
                (std::get<0>(tuple) << 16) | (std::get<1>(tuple) << 8) | std::get<2>(tuple)));
        }
    };
     ...
     */
    void CacheTransactionPacket(TransactionId id,
    ...
    std::unordered_map<uint16_t, Callback<void, const char*, TransactionState>>& GetEventCallbacks(
        void);
    std::unordered_map<uint16_t, Callback<void, Ptr<Packet>, const SipHeader&, TransactionState>>&
    GetReceiveCallbacks(void);
    ...
    TracedCallback<uint16_t, uint32_t, uint32_t, DialogState>
        m_dialogTrace; //!< Trace dialog state change
    TracedCallback<uint16_t, uint32_t, uint32_t, TransactionState>
        m_transactionTrace; //!< Trace transaction state change
