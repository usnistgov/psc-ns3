...
{
    static TypeId tid =
        TypeId("ns3::psc::SchoolShootingApplicationHelper")
            .SetParent<Object>()
...
void
SchoolShootingApplicationHelper::SetTechnologyHelper(
    Ptr<SchoolShootingNetworkTechnologyHelper> techHelper)
{
...
void
SchoolShootingApplicationHelper::SetScenarioDefinitionHelper(
    Ptr<SchoolShootingDefinitionHelper> definitionHelper)
{
...
void
SchoolShootingApplicationHelper::InitApplications()
{
    NS_LOG_FUNCTION(this);
    ...
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_MCPTT) !=
            m_enabledApps.end())
    {
        InitMcpttApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_TACTICAL_TELEMETRY) !=
            m_enabledApps.end())
    {
        InitTacticalTelemetryApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_BIOMETRICS) !=
            m_enabledApps.end())
    {
        InitBiometricApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_SWAT_HELMET_CAMERA) !=
            m_enabledApps.end())
    {
        InitSwatHelmetCameraApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_DEPLOYABLE_CAMERA) !=
            m_enabledApps.end())
    {
        InitDeployableCameraApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_THROW_PHONE) !=
            m_enabledApps.end())
    {
        InitThrowPhoneApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_HELICOPTER) !=
            m_enabledApps.end())
    {
        InitHelicopterApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_EMS_VEHICLES) !=
            m_enabledApps.end())
    {
        InitEmsVehicleApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_VIDEO_CONFERENCE) !=
            m_enabledApps.end())
    {
        InitVideoConferenceApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_AVL) !=
            m_enabledApps.end())
    {
        InitAvlApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_WEB) !=
            m_enabledApps.end())
    {
        InitWebApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_RMS_BCA) !=
            m_enabledApps.end())
    {
        InitRmsBcaApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_SATELLITE) !=
            m_enabledApps.end())
    {
        InitSatelliteApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_BUILDING_PLANS) !=
            m_enabledApps.end())
    {
        InitBuildingPlanApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_INCIDENT_MANAGEMENT) !=
            m_enabledApps.end())
    {
        InitIncidentManagementApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_NG911) !=
            m_enabledApps.end())
    {
        InitNg911App();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_SCHOOL_VIDEO) !=
            m_enabledApps.end())
    {
        InitSchoolVideoApp();
    }
    if (m_enabledApps.empty() ||
        m_enabledApps.find(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_TRAFFIC_VIDEO) !=
            m_enabledApps.end())
    {
        InitTrafficVideoApp();
    }
...
void
SchoolShootingApplicationHelper::InitMcpttApp()
{
    NS_LOG_FUNCTION(this);
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_MCPTT,
        appInfo);

    // Get the address of the PS MCPTT server
    // Interface 0 is localhost, 1 is the p2p device
    Ipv6Address remoteHostAddr = GetTechnologyHelper()
                                     ->GetPsServer()
    ...
    Ptr<McpttServerApp> mcpttServerApp =
        mcpttServerHelper.Install(GetTechnologyHelper()->GetPsServer());
    ...
    {
        mcpttServerApp->SetStartTime(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E2_TaskForce_Arrive)
                .time);
        mcpttServerApp->SetStopTime(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E12_Incident_End)
                .time);
    }
    ...
    mcpttClientHelper.SetMediaSrc("ns3::psc::McpttMediaSrc",
                                  ...
                                  UintegerValue(60),
                                  "DataRate",
                                  DataRateValue(DataRate("12kb/s")));
    mcpttClientHelper.SetPusher("ns3::psc::McpttPusher", "Automatic", BooleanValue(false));

    // Create and install UE applications
    ApplicationContainer mcpttClientAppsSwatTeam1 = mcpttClientHelper.Install(
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::SWAT_TEAM_1)
            .nodes);
    ApplicationContainer mcpttClientAppsSwatTeam2 = mcpttClientHelper.Install(
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::SWAT_TEAM_2)
            .nodes);
    ApplicationContainer mcpttClientAppsSwatTeam3 = mcpttClientHelper.Install(
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::SWAT_TEAM_3)
            .nodes);
    ApplicationContainer mcpttClientAppsSwatTeam4 = mcpttClientHelper.Install(
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::SWAT_TEAM_4)
            .nodes);

    ApplicationContainer mcpttClientAppsFireEms = mcpttClientHelper.Install(
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::FIRE)
            .nodes);
    mcpttClientAppsFireEms.Add(mcpttClientHelper.Install(
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::EMS)
            .nodes));

    ApplicationContainer mcpttClientAppsTaskForce = mcpttClientHelper.Install(
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::TASK_FORCE)
            .nodes);

    // Setup starting pusher activity
    Ptr<McpttPusherOrchestratorSessionCdf> swatTeam1PttActivity =
        CreateObjectWithAttributes<McpttPusherOrchestratorSessionCdf>(
            "ActivityFactor",
            DoubleValue(0.1), // Initial SWAT team actitivy factor
            "Orchestrator",
            PointerValue(CreateObjectWithAttributes<McpttPusherOrchestratorContention>(
                "ContentionProbability",
                DoubleValue(0.0),
                "Orchestrator",
                PointerValue(CreateObjectWithAttributes<McpttPusherOrchestratorSpurtCdf>(
                    "ActivityFactor",
                    DoubleValue(1.0))))));

    Ptr<McpttPusherOrchestratorSessionCdf> swatTeam2PttActivity =
        CreateObjectWithAttributes<McpttPusherOrchestratorSessionCdf>(
            "ActivityFactor",
            DoubleValue(0.1), // Initial SWAT team actitivy factor
            "Orchestrator",
    ...
    Ptr<McpttPusherOrchestratorSessionCdf> swatTeam3PttActivity =
        CreateObjectWithAttributes<McpttPusherOrchestratorSessionCdf>(
            "ActivityFactor",
            DoubleValue(0.1), // Initial SWAT team actitivy factor
            "Orchestrator",
    ...
    Ptr<McpttPusherOrchestratorSessionCdf> swatTeam4PttActivity =
        CreateObjectWithAttributes<McpttPusherOrchestratorSessionCdf>(
            "ActivityFactor",
            DoubleValue(0.1), // Initial SWAT team actitivy factor
            "Orchestrator",
    ...
    Ptr<McpttPusherOrchestratorSessionCdf> fireEmsPttActivity =
        CreateObjectWithAttributes<McpttPusherOrchestratorSessionCdf>(
            "ActivityFactor",
            DoubleValue(0.3), // Initial fire/EMS activity factor
            "Orchestrator",
            PointerValue(CreateObjectWithAttributes<McpttPusherOrchestratorContention>(
                "ContentionProbability",
                DoubleValue(0.0),
    ...
    Ptr<McpttPusherOrchestratorSessionCdf> taskForcePttActivity =
        CreateObjectWithAttributes<McpttPusherOrchestratorSessionCdf>(
            "ActivityFactor",
            DoubleValue(0.05), // Task force activity factor
            "Orchestrator",
            PointerValue(CreateObjectWithAttributes<McpttPusherOrchestratorContention>(
                "ContentionProbability",
                DoubleValue(0.0),
    ...
    {
        mcpttClientApps.Start(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E2_TaskForce_Arrive)
                .time);
        mcpttClientApps.Stop(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E12_Incident_End)
                .time);
    }
    ...
    clientNetDevices.Add(GetTechnologyHelper()->GetNetDevicesForGroup(
        SchoolShootingDefinitionHelper::UserGroups::SWAT_TEAM_1));
    clientNetDevices.Add(GetTechnologyHelper()->GetNetDevicesForGroup(
        SchoolShootingDefinitionHelper::UserGroups::SWAT_TEAM_2));
    clientNetDevices.Add(GetTechnologyHelper()->GetNetDevicesForGroup(
        SchoolShootingDefinitionHelper::UserGroups::SWAT_TEAM_3));
    clientNetDevices.Add(GetTechnologyHelper()->GetNetDevicesForGroup(
        SchoolShootingDefinitionHelper::UserGroups::SWAT_TEAM_4));
    clientNetDevices.Add(GetTechnologyHelper()->GetNetDevicesForGroup(
        SchoolShootingDefinitionHelper::UserGroups::FIRE));
    clientNetDevices.Add(GetTechnologyHelper()->GetNetDevicesForGroup(
        SchoolShootingDefinitionHelper::UserGroups::EMS));
    clientNetDevices.Add(GetTechnologyHelper()->GetNetDevicesForGroup(
        SchoolShootingDefinitionHelper::UserGroups::TASK_FORCE));
    ...
    // This is also where we set when/how PTT activity should change over the course of the
    // simulation
    uint32_t groupId = 0;
    ...
    if (m_testing)
    {
        callHelper.AddCall(mcpttClientAppsTaskForce,
                           mcpttServerApp,
                           groupId++,
                           callType,
                           Seconds(10),
    ...
    else
    {
        callHelper.AddCall(
            ...
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E12_Incident_End)
                .time);
        taskForcePttActivity->StartAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E2_TaskForce_Arrive)
                .time);
        taskForcePttActivity->StopAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E12_Incident_End)
                .time);
    }
    ...
    if (m_testing)
    {
        callHelper.AddCall(mcpttClientAppsSwatTeam1,
                           mcpttServerApp,
                           groupId++,
                           callType,
                           Seconds(15),
                           Seconds(85));
        callHelper.AddCall(mcpttClientAppsSwatTeam2,
                           mcpttServerApp,
                           groupId++,
                           callType,
                           Seconds(15),
                           Seconds(85));
        callHelper.AddCall(mcpttClientAppsSwatTeam3,
                           mcpttServerApp,
                           groupId++,
                           callType,
                           Seconds(15),
                           Seconds(85));
        callHelper.AddCall(mcpttClientAppsSwatTeam4,
                           mcpttServerApp,
                           groupId++,
                           callType,
                           Seconds(15),
    ...
    {
        callHelper.AddCall(
            mcpttClientAppsSwatTeam1,
            mcpttServerApp,
            groupId++,
            callType,
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E3_SWAT_Arrive)
                .time,
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E10_Incident_Secured)
                .time);
        callHelper.AddCall(
            mcpttClientAppsSwatTeam2,
            mcpttServerApp,
            groupId++,
            callType,
            m_scenarioDefinitionHelper->GetScenarioDefinition()
        ...
        callHelper.AddCall(
            ...
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E10_Incident_Secured)
                .time);
        swatTeam1PttActivity->StartAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E3_SWAT_Arrive)
                .time);
        swatTeam1PttActivity->StopAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E10_Incident_Secured)
                .time);
        swatTeam2PttActivity->StartAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E3_SWAT_Arrive)
                .time);
        swatTeam2PttActivity->StopAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E10_Incident_Secured)
                .time);
        swatTeam3PttActivity->StartAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E3_SWAT_Arrive)
                .time);
        swatTeam3PttActivity->StopAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E10_Incident_Secured)
                .time);
        swatTeam4PttActivity->StartAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E3_SWAT_Arrive)
                .time);
        swatTeam4PttActivity->StopAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E10_Incident_Secured)
                .time);
        ...
        Simulator::Schedule(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E7_SWAT_Deploy)
                .time,
            &McpttPusherOrchestratorSessionCdf::SetActivityFactor,
            swatTeam1PttActivity,
            0.5);
        ...
        Simulator::Schedule(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E9_Interdiction)
                .time,
            &McpttPusherOrchestratorSessionCdf::SetActivityFactor,
            swatTeam1PttActivity,
            0.1);
    ...
    if (m_testing)
    {
        callHelper.AddCall(mcpttClientAppsFireEms,
                           mcpttServerApp,
                           groupId++,
                           callType,
                           Seconds(20),
    ...
    {
        callHelper.AddCall(
            mcpttClientAppsFireEms,
            mcpttServerApp,
            groupId++,
            callType,
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E4_FireEMS_Arrive)
                .time,
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E12_Incident_End)
                .time);
        fireEmsPttActivity->StartAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E4_FireEMS_Arrive)
                .time);
        fireEmsPttActivity->StopAt(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E12_Incident_End)
                .time);
        ...
        Simulator::Schedule(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E6_Perimeters_Ready)
                .time,
            &McpttPusherOrchestratorSessionCdf::SetActivityFactor,
            fireEmsPttActivity,
            0.4);
...
void
SchoolShootingApplicationHelper::InitTacticalTelemetryApp()
{
    NS_LOG_FUNCTION(this);
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_TACTICAL_TELEMETRY,
        appInfo);
    ...
    if (m_testing)
    {
        startSwatTeam = CreateObjectWithAttributes<NormalRandomVariable>("Mean",
                                                                         DoubleValue(10),
                                                                         "Variance",
                                                                         DoubleValue(1),
                                                                         "Bound",
                                                                         DoubleValue(1),
                                                                         "Stream",
                                                                         IntegerValue(2690328116));
        endSwatTeam = CreateObjectWithAttributes<NormalRandomVariable>("Mean",
                                                                       DoubleValue(90),
                                                                       "Variance",
                                                                       DoubleValue(1),
                                                                       "Bound",
                                                                       DoubleValue(1),
                                                                       "Stream",
                                                                       IntegerValue(387351077));
    }
    else
    {
        startSwatTeam = CreateObjectWithAttributes<NormalRandomVariable>(
            "Mean",
            DoubleValue(m_scenarioDefinitionHelper->GetScenarioDefinition()
                            ->GetKeyEventTime(SchoolShootingDefinitionHelper::E7_SWAT_Deploy)
                            .time.GetSeconds()),
            "Variance",
            DoubleValue(6.25 * 60),
            "Bound",
            DoubleValue(10 * 60),
            "Stream",
            IntegerValue(1302947634));
        endSwatTeam = CreateObjectWithAttributes<NormalRandomVariable>(
            "Mean",
            DoubleValue(m_scenarioDefinitionHelper->GetScenarioDefinition()
                            ->GetKeyEventTime(SchoolShootingDefinitionHelper::E10_Incident_Secured)
                            .time.GetSeconds()),
            "Variance",
            DoubleValue(12.25 * 60),
            "Bound",
            DoubleValue(10 * 60),
            "Stream",
            IntegerValue(4118718303));
    }

    // Collect all participating nodes into one container
    NodeContainer allSwats;
    allSwats.Add(m_scenarioDefinitionHelper->GetScenarioDefinition()
                     ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::SWAT_TEAM_1)
                     .nodes);
    allSwats.Add(m_scenarioDefinitionHelper->GetScenarioDefinition()
                     ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::SWAT_TEAM_2)
    ...
    for (uint32_t i = 0; i < allSwats.GetN(); i++)
    {
        ...
        AddPscApplications(SchoolShootingApplicationHelper::SCHOOL_SHOOTING_TACTICAL_TELEMETRY,
                           m_genericApplicationHelper->Install(appConfig,
                                                               allSwats.Get(i),
...
void
SchoolShootingApplicationHelper::InitBiometricApp()
{
    NS_LOG_FUNCTION(this);
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_BIOMETRICS,
        appInfo);
    ...
    appConfig->SetApplicationPattern(
        CreateObjectWithAttributes<ConstantRandomVariable>(
            "Constant",
            DoubleValue(6)), // Number of packets to send per session
        CreateObjectWithAttributes<ConstantRandomVariable>(
            "Constant",
            DoubleValue(10)), // Packet interval (in s)
        CreateObjectWithAttributes<ConstantRandomVariable>("Constant",
                                                           DoubleValue(0)), // Time between sessions
    ...
    if (m_testing)
    {
        ...
        endSwatTeam = CreateObjectWithAttributes<NormalRandomVariable>("Mean",
                                                                       ...
                                                                       DoubleValue(1),
                                                                       "Stream",
                                                                       IntegerValue(3155653880));

        startPatients = CreateObjectWithAttributes<NormalRandomVariable>("Mean",
                                                                         DoubleValue(10),
                                                                         "Variance",
                                                                         DoubleValue(1),
                                                                         "Bound",
                                                                         DoubleValue(1),
                                                                         "Stream",
                                                                         IntegerValue(1547664542));
        endPatients = CreateObjectWithAttributes<NormalRandomVariable>("Mean",
                                                                       DoubleValue(90),
                                                                       "Variance",
                                                                       DoubleValue(1),
                                                                       "Bound",
    ...
    else
    {
        ...
        startPatients = CreateObjectWithAttributes<NormalRandomVariable>(
            ...
            DoubleValue(10 * 60),
            "Stream",
            IntegerValue(1811505450));
        endPatients = CreateObjectWithAttributes<NormalRandomVariable>(
            "Mean",
            DoubleValue(m_scenarioDefinitionHelper->GetScenarioDefinition()
                            ->GetKeyEventTime(SchoolShootingDefinitionHelper::E11_EMS_Departure)
                            .time.GetSeconds()),
            "Variance",
            DoubleValue(0),
            "Bound",
    ...
    NodeContainer emsVehicles =
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::EMS_VEHICLES)
            .nodes;
...
void
SchoolShootingApplicationHelper::InitSwatHelmetCameraApp()
{
    NS_LOG_FUNCTION(this);
    ...
    Ptr<UniformRandomVariable> startVar =
        CreateObjectWithAttributes<UniformRandomVariable>("Min",
                                                          DoubleValue(0.0),
                                                          "Max",
                                                          DoubleValue(0.1),
                                                          "Stream",
                                                          IntegerValue(3565260182));
    ...
    for (uint32_t i = 0; i < allSwats.GetN(); i++)
    {
        ...
        // Attributes
        streamSrc->SetAttribute("ReceiverAddress",
                                AddressValue(GetTechnologyHelper()
                                                 ->GetPsServer()
                                                 ->GetObject<Ipv6L3Protocol>()
                                                 ->GetAddress(1, 1)
                                                 .GetAddress()));
        ...
        streamSrc->ReadCustomDistribution(
            std::string("src/psc/examples/video_cdf/"
                        "CdfVideoStreaming-h264-1280x720-30-25-true_packetsize.txt_90_90"),
            std::string("src/psc/examples/video_cdf/"
                        "CdfVideoStreaming-h264-1280x720-30-25-true_packetinterval.txt_90_90"));
        ...
        {
            streamSrc->SetStartTime(Seconds(
                m_scenarioDefinitionHelper->GetScenarioDefinition()
                    ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E7_SWAT_Deploy)
                    .time.GetSeconds() +
                startVar->GetValue()));
            streamSrc->SetStopTime(
                m_scenarioDefinitionHelper->GetScenarioDefinition()
                    ->GetKeyEventTime(
                        SchoolShootingDefinitionHelper::KeyEvents::E10_Incident_Secured)
                    .time);
        }
        ...
        PacketSinkHelper packetSinkHelper("ns3::UdpSocketFactory",
                                          Inet6SocketAddress(Ipv6Address::GetAny(), serverPort));
        streamSink.Add(packetSinkHelper.Install(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::UC_TEAM_VEHICLE)
                .nodes.Get(0)));
        streamSink.Add(packetSinkHelper.Install(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::SA_VEHICLE)
                .nodes.Get(0)));
        ...
        else
        {
            streamSink.Start(
                m_scenarioDefinitionHelper->GetScenarioDefinition()
                    ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E5_UC_Setup)
                    .time);
            streamSink.Stop(
                m_scenarioDefinitionHelper->GetScenarioDefinition()
                    ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E12_Incident_End)
...
void
SchoolShootingApplicationHelper::InitDeployableCameraApp()
{
    // Register application
    PscScenarioDefinition::ApplicationInfo appInfo;
    ...
    NodeContainer deployableCameras =
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::DEPLOYABLE_CAMERA)
            .nodes;
    ...
    for (uint32_t i = 0; i < deployableCameras.GetN(); i++)
    {
        ...
        else
        {
            streamSrc->SetStartTime(Seconds(
                m_scenarioDefinitionHelper->GetScenarioDefinition()
                    ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E8_Offender_Found)
                    .time.GetSeconds() +
                startVar->GetValue()));
            streamSrc->SetStopTime(
                m_scenarioDefinitionHelper->GetScenarioDefinition()
...
void
SchoolShootingApplicationHelper::InitThrowPhoneApp()
{
    NS_LOG_FUNCTION(this);

    // Get all the sender nodes. One member in each team
    NodeContainer offenders =
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::OFFENDER)
            .nodes;
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_THROW_PHONE,
        appInfo);
    ...
    else
    {
        startTime =
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E8_Offender_Found)
                .time;
        stopTime = m_scenarioDefinitionHelper->GetScenarioDefinition()
                       ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E9_Interdiction)
                       .time;
    }
    ...
    Ptr<PscApplicationConfiguration> appConfig =
        CreateObject<PscApplicationConfiguration>(appInfo.name,
    ...
    appConfig->SetApplicationPattern(
        CreateObjectWithAttributes<ConstantRandomVariable>(
            "Constant",
            DoubleValue(1296)), // Number of packets to send per session
        CreateObjectWithAttributes<ConstantRandomVariable>(
            "Constant",
            DoubleValue(0.1111)), // Packet interval (in s)
        CreateObjectWithAttributes<ConstantRandomVariable>(
            "Constant",
            DoubleValue(216)), // Time between sessions
...
void
SchoolShootingApplicationHelper::InitHelicopterApp()
{
    NS_LOG_FUNCTION(this);

    // Get all the sender nodes. One member in each team
    NodeContainer helicopter =
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::HELICOPTER)
            .nodes;
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_HELICOPTER,
        appInfo);
    ...
    {
        streamSrc->SetStartTime(
            Seconds(m_scenarioDefinitionHelper->GetScenarioDefinition()
                        ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E5_UC_Setup)
                        .time.GetSeconds() +
                    startVar->GetValue()));
        streamSrc->SetStopTime(
...
void
SchoolShootingApplicationHelper::InitEmsVehicleApp()
{
    NS_LOG_FUNCTION(this);
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_EMS_VEHICLES,
        appInfo);
    ...
    for (uint32_t i = 0; i < emsVehicles.GetN(); i++)
    {
        ...
        else
        {
            ...
            streamSrc->SetStopTime(
                m_scenarioDefinitionHelper->GetScenarioDefinition()
                    ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E11_EMS_Departure)
                    .time);
        }
...
void
SchoolShootingApplicationHelper::InitVideoConferenceApp()
{
    NS_LOG_FUNCTION(this);

    // Get the nodes participating in the video conference
    NodeContainer ucVehicles =
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::UC_TEAM_VEHICLE)
            .nodes;
    NodeContainer saVehicles =
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::SA_VEHICLE)
            .nodes;
    ...
    // Attributes
    ucStreamSrc->SetAttribute("ReceiverAddress",
                              AddressValue(GetTechnologyHelper()
                                               ->GetPsServer()
                                               ->GetObject<Ipv6L3Protocol>()
                                               ->GetAddress(1, 1)
    ...
    ucStreamSrc->ReadCustomDistribution(
        std::string("src/psc/examples/video_cdf/"
                    "CdfVideoStreaming-h264-1920x1080-30-25-true_packetsize.txt_90_90"),
        std::string("src/psc/examples/video_cdf/"
                    "CdfVideoStreaming-h264-1920x1080-30-25-true_packetinterval.txt_90_90"));
    ...
    // Attributes
    saStreamSrc->SetAttribute("ReceiverAddress",
                              AddressValue(GetTechnologyHelper()
                                               ->GetPsServer()
                                               ->GetObject<Ipv6L3Protocol>()
                                               ->GetAddress(1, 1)
    ...
    saStreamSrc->ReadCustomDistribution(
        std::string("src/psc/examples/video_cdf/"
                    "CdfVideoStreaming-h264-1920x1080-30-25-true_packetsize.txt_90_90"),
        std::string("src/psc/examples/video_cdf/"
                    "CdfVideoStreaming-h264-1920x1080-30-25-true_packetinterval.txt_90_90"));
    ...
    ApplicationContainer streamSink;
    PacketSinkHelper packetSinkHelper("ns3::UdpSocketFactory",
                                      Inet6SocketAddress(Ipv6Address::GetAny(), serverPort + 1));
    streamSink.Add(packetSinkHelper.Install(ucVehicles.Get(0)));

    packetSinkHelper.SetAttribute(
        "Local",
        AddressValue(Inet6SocketAddress(Ipv6Address::GetAny(), serverPort)));
    ...
    Inet6SocketAddress ucSocket(
        ucVehicles.Get(0)->GetObject<Ipv6L3Protocol>()->GetAddress(1, 1).GetAddress(),
        serverPort + 1);
    serverApps.Get(1)->GetObject<UdpGroupEchoServer>()->AddClient(ucSocket);

    Inet6SocketAddress saSocket(
        saVehicles.Get(0)->GetObject<Ipv6L3Protocol>()->GetAddress(1, 1).GetAddress(),
        serverPort);
...
void
SchoolShootingApplicationHelper::InitAvlApp()
{
    NS_LOG_FUNCTION(this);
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_AVL,
        appInfo);
    ...
    appConfig->SetApplicationPattern(
        CreateObjectWithAttributes<ConstantRandomVariable>(
            "Constant",
            DoubleValue(4)), // Number of packets to send per session
        CreateObjectWithAttributes<ConstantRandomVariable>(
            "Constant",
            DoubleValue(15)), // Packet interval (in s)
        CreateObjectWithAttributes<ConstantRandomVariable>("Constant",
                                                           DoubleValue(0)), // Time between sessions
    ...
    if (m_testing)
    {
        startRndVar = CreateObjectWithAttributes<NormalRandomVariable>("Mean",
                                                                       DoubleValue(5),
                                                                       "Variance",
                                                                       DoubleValue(1),
                                                                       "Bound",
                                                                       DoubleValue(1),
                                                                       "Stream",
                                                                       IntegerValue(226200624));
        endRndVar = CreateObjectWithAttributes<NormalRandomVariable>("Mean",
                                                                     DoubleValue(90),
                                                                     "Variance",
                                                                     DoubleValue(1),
                                                                     "Bound",
        ...
        nodes.Add(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::TASK_FORCE_VEHICLES)
                .nodes);
        nodes.Add(m_scenarioDefinitionHelper->GetScenarioDefinition()
                      ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::FIRE_VEHICLES)
                      .nodes);
        nodes.Add(m_scenarioDefinitionHelper->GetScenarioDefinition()
                      ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::EMS_VEHICLES)
                      .nodes);
    ...
    else
    {
        ...
        startRndVar = CreateObjectWithAttributes<NormalRandomVariable>(
            "Mean",
            DoubleValue(m_scenarioDefinitionHelper->GetScenarioDefinition()
                            ->GetKeyEventTime(SchoolShootingDefinitionHelper::E2_TaskForce_Arrive)
                            .time.GetSeconds()),
            "Variance",
            DoubleValue(6.25 * 60),
            "Bound",
            DoubleValue(10 * 60),
            "Stream",
            IntegerValue(2353855280));
        endRndVar = CreateObjectWithAttributes<NormalRandomVariable>(
            "Mean",
            DoubleValue(m_scenarioDefinitionHelper->GetScenarioDefinition()
                            ->GetKeyEventTime(SchoolShootingDefinitionHelper::E12_Incident_End)
                            .time.GetSeconds()),
            "Variance",
            DoubleValue(12.25 * 60),
            "Bound",
        ...
        // 2 fire engines (FIRE_VEHCILES)
        nodes = m_scenarioDefinitionHelper->GetScenarioDefinition()
                    ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::FIRE_VEHICLES)
                    .nodes;
        ...
        startRndVar = CreateObjectWithAttributes<NormalRandomVariable>(
            "Mean",
            DoubleValue(m_scenarioDefinitionHelper->GetScenarioDefinition()
                            ->GetKeyEventTime(SchoolShootingDefinitionHelper::E4_FireEMS_Arrive)
                            .time.GetSeconds()),
            "Variance",
            DoubleValue(6.25 * 60),
            "Bound",
        ...
        startRndVar = CreateObjectWithAttributes<NormalRandomVariable>(
            "Mean",
            DoubleValue(m_scenarioDefinitionHelper->GetScenarioDefinition()
                            ->GetKeyEventTime(SchoolShootingDefinitionHelper::E5_UC_Setup)
                            .time.GetSeconds()),
            "Variance",
            DoubleValue(6.25 * 60),
            "Bound",
        ...
        startRndVar = CreateObjectWithAttributes<NormalRandomVariable>(
            "Mean",
            DoubleValue(m_scenarioDefinitionHelper->GetScenarioDefinition()
                            ->GetKeyEventTime(SchoolShootingDefinitionHelper::E6_Perimeters_Ready)
                            .time.GetSeconds()),
            "Variance",
            DoubleValue(6.25 * 60),
            "Bound",
...
void
SchoolShootingApplicationHelper::InitWebApp()
{
    NS_LOG_FUNCTION(this);
    ...
    appInfo.name = "Web";
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_WEB,
        appInfo);
    ...
    if (m_testing)
    {
        startRndVar = CreateObjectWithAttributes<UniformRandomVariable>("Min",
                                                                        DoubleValue(0),
                                                                        "Max",
                                                                        DoubleValue(1),
                                                                        "Stream",
                                                                        IntegerValue(2353680874));
        endRndVar = CreateObjectWithAttributes<UniformRandomVariable>("Min",
                                                                      DoubleValue(0),
                                                                      "Max",
                                                                      DoubleValue(1),
                                                                      "Stream",
                                                                      IntegerValue(1692196905));
    }
    else
    {
        startRndVar = CreateObjectWithAttributes<UniformRandomVariable>("Min",
                                                                        ...
                                                                        DoubleValue(600),
                                                                        "Stream",
                                                                        IntegerValue(3439410251));
        endRndVar = CreateObjectWithAttributes<UniformRandomVariable>("Min",
                                                                      DoubleValue(0),
                                                                      "Max",
                                                                      DoubleValue(600),
                                                                      "Stream",
                                                                      IntegerValue(13912914));
    }
    ...
    IntelHttpClientHelper graphicalClientHelper(GetTechnologyHelper()
                                                    ->GetPublicServer()
                                                    ->GetObject<Ipv6L3Protocol>()
                                                    ->GetAddress(1, 1)
                                                    .GetAddress());
    IntelHttpClientHelper nonGraphicalClientHelper(GetTechnologyHelper()
                                                       ->GetPublicServer()
                                                       ->GetObject<Ipv6L3Protocol>()
                                                       ->GetAddress(1, 1)
                                                       .GetAddress());
    NodeContainer ucNodes =
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::UC_TEAM_VEHICLE)
            .nodes;

    // Configure one instance of non-graphical random variables for 2x the non-graphical activity
    Ptr<ConstantRandomVariable> nonGraphicalEmbeddedObjectAmountRvs =
    ...
    nonGraphicalHtmlSizeRvs->SetAttribute("Variance",
                                          DoubleValue((632 * 1024 * 0.05) * (632 * 1024 * 0.05)));
    ...
    nonGraphicalServerHelper.Set("EmbeddedObjectAmountRvs",
                                 PointerValue(nonGraphicalEmbeddedObjectAmountRvs));
    ...
    Ptr<NormalRandomVariable> graphicalEmbeddedObjectAmountRvs =
    ...
    graphicalEmbeddedObjectSizeRvs->SetAttribute(
        "Variance",
        DoubleValue((152371.2 * 0.05) * (152371.2 * 0.05)));
    ...
    graphicalEmbeddedObjectAmountRvs->SetAttribute("Variance",
                                                   DoubleValue((15 * 0.05) * (15 * 0.05)));
    ...
    graphicalServerHelper.Set("EmbeddedObjectSizeRvs",
                              PointerValue(graphicalEmbeddedObjectSizeRvs));
    graphicalServerHelper.Set("EmbeddedObjectAmountRvs",
                              PointerValue(graphicalEmbeddedObjectAmountRvs));
...
void
SchoolShootingApplicationHelper::InitRmsBcaApp()
{
    NS_LOG_FUNCTION(this);
    ...
    appInfo.name = "RmsBca";
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_RMS_BCA,
        appInfo);
    ...
    // Send one packet every six seconds so that each computer performs 10 16 kB reads/writes per
    // minute
    appConfig->SetApplicationPattern(
        CreateObjectWithAttributes<ConstantRandomVariable>(
            "Constant",
        ...
        CreateObjectWithAttributes<ConstantRandomVariable>(
            "Constant",
            DoubleValue(6)), // Packet interval (in s)
        CreateObjectWithAttributes<ConstantRandomVariable>("Constant",
                                                           DoubleValue(0)), // Time between sessions
        16384,
        16384);
    ...
    if (m_testing)
    {
        ucStartRndVar = CreateObjectWithAttributes<NormalRandomVariable>("Mean",
                                                                         DoubleValue(5),
                                                                         "Variance",
                                                                         DoubleValue(1),
                                                                         "Bound",
                                                                         DoubleValue(1),
                                                                         "Stream",
                                                                         IntegerValue(3824051190));
        saStartRndVar = CreateObjectWithAttributes<NormalRandomVariable>("Mean",
                                                                         DoubleValue(5),
                                                                         "Variance",
                                                                         DoubleValue(1),
                                                                         "Bound",
...
void
SchoolShootingApplicationHelper::InitSatelliteApp()
{
    NS_LOG_FUNCTION(this);
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_SATELLITE,
        appInfo);
    ...
    appConfig->SetApplicationPattern(CreateObjectWithAttributes<ConstantRandomVariable>(
                                         "Constant",
                                         DoubleValue(1)), // Number of packets to send per session
                                     CreateObjectWithAttributes<ConstantRandomVariable>(
                                         "Constant",
                                         DoubleValue(0)), // Packet interval (in s)
                                     CreateObjectWithAttributes<ConstantRandomVariable>(
                                         "Constant",
                                         DoubleValue(300)), // Time between sessions
...
void
SchoolShootingApplicationHelper::InitBuildingPlanApp()
{
    NS_LOG_FUNCTION(this);
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_BUILDING_PLANS,
        appInfo);
    ...
    appConfig->SetApplicationPattern(CreateObjectWithAttributes<ConstantRandomVariable>(
                                     ...
                                     CreateObjectWithAttributes<ConstantRandomVariable>(
                                         "Constant",
                                         DoubleValue(14700)), // Time between sessions
                                     4608,                    // Client packet size (4.5 kB)
...
void
SchoolShootingApplicationHelper::InitIncidentManagementApp()
{
    NS_LOG_FUNCTION(this);
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_INCIDENT_MANAGEMENT,
        appInfo);
    ...
    appConfig->SetApplicationPattern(
        ...
        CreateObjectWithAttributes<ConstantRandomVariable>(
            "Constant",
            DoubleValue(0.05)), // Packet interval (in s)
        CreateObjectWithAttributes<ConstantRandomVariable>("Constant",
                                                           DoubleValue(0)), // Time between sessions
...
void
SchoolShootingApplicationHelper::InitNg911App()
{
    NS_LOG_FUNCTION(this);
    ...
    appInfo.name = "NG911";
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_NG911,
        appInfo);
    ...
    Ptr<PscVideoStreaming> streamSrc = CreateObject<PscVideoStreaming>();
    // Attributes
    streamSrc->SetAttribute(
        "ReceiverAddress",
        AddressValue(ucNodes.Get(0)->GetObject<Ipv6L3Protocol>()->GetAddress(1, 1).GetAddress()));
    streamSrc->SetAttribute("ReceiverPort", UintegerValue(serverPort));
    streamSrc->SetAttribute("MaxUdpPayloadSize", UintegerValue(1500));
    streamSrc->ReadCustomDistribution(
        std::string("src/psc/examples/video_cdf/"
...
void
SchoolShootingApplicationHelper::InitSchoolVideoApp()
{
    NS_LOG_FUNCTION(this);
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_SCHOOL_VIDEO,
        appInfo);

    // Get all the sender nodes
    NodeContainer schoolCameras =
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::SCHOOL_CAMERAS)
            .nodes;
    //
    // uniform random variable for scattering start of applications
    Ptr<UniformRandomVariable> startVar =
    ...
    for (uint32_t i = 0; i < schoolCameras.GetN(); i++)
    {
        ...
        ApplicationContainer serverApps =
            echoServer.Install(GetTechnologyHelper()->GetSchoolServer());
        ...
        PacketSinkHelper packetSinkHelper("ns3::UdpSocketFactory",
                                          Inet6SocketAddress(Ipv6Address::GetAny(), serverPort));
        ucStreamSink.Add(packetSinkHelper.Install(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::UC_TEAM_VEHICLE)
                .nodes.Get(0)));
        saStreamSink.Add(packetSinkHelper.Install(
            m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::SA_VEHICLE)
                .nodes.Get(0)));
        ...
        else
        {
            ...
            saStreamSink.Start(
                m_scenarioDefinitionHelper->GetScenarioDefinition()
                    ->GetKeyEventTime(
                        SchoolShootingDefinitionHelper::KeyEvents::E6_Perimeters_Ready)
                    .time);
            saStreamSink.Stop(
                m_scenarioDefinitionHelper->GetScenarioDefinition()
                    ->GetKeyEventTime(SchoolShootingDefinitionHelper::KeyEvents::E12_Incident_End)
...
void
SchoolShootingApplicationHelper::InitTrafficVideoApp()
{
    NS_LOG_FUNCTION(this);
    ...
    m_scenarioDefinitionHelper->GetScenarioDefinition()->AddApplication(
        SchoolShootingApplicationHelper::SCHOOL_SHOOTING_TRAFFIC_VIDEO,
        appInfo);

    // Get all the sender nodes
    NodeContainer trafficCameras =
        m_scenarioDefinitionHelper->GetScenarioDefinition()
            ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::TRAFFIC_CAMERAS)
            .nodes;
    //
    // uniform random variable for scattering start of applications
    Ptr<UniformRandomVariable> startVar =
        CreateObjectWithAttributes<UniformRandomVariable>("Min",
                                                          ...
                                                          DoubleValue(0.1),
                                                          "Stream",
                                                          IntegerValue(2502803685));

    // Base port to use by applications
    uint16_t serverPort = m_basePortsMap[SCHOOL_SHOOTING_TRAFFIC_VIDEO];
    Address serverAddress = GetTechnologyHelper()
                                ->GetPublicServer()
                                ->GetObject<Ipv6L3Protocol>()
                                ->GetAddress(1, 1)
                                .GetAddress();
    for (uint32_t i = 0; i < trafficCameras.GetN(); i++)
    {
        ...
        ApplicationContainer serverApps =
            echoServer.Install(GetTechnologyHelper()->GetPublicServer());
        ...
        streamSrc->SetAttribute(
            "Remote",
            AddressValue(Inet6SocketAddress(Ipv6Address::ConvertFrom(serverAddress), serverPort)));
...
SchoolShootingApplicationHelper::EnableApplicationTraces()
{
    ...
    {
        for (auto enabledAppsIt = m_enabledApps.begin(); enabledAppsIt != m_enabledApps.end();
             enabledAppsIt++)
        {
...
SchoolShootingApplicationHelper::EnableApplicationTraces(SchoolSchootingApplicationId appName)
{
    ...
    case SCHOOL_SHOOTING_VIDEO_CONFERENCE:
        ...
        m_traceHelper->EnableVideoConferenceVideoTraces(m_videoConferenceVideoTx,
                                                        m_videoConferenceVideoRx);
        break;
...
void
SchoolShootingApplicationHelper::AddPscApplication(SchoolSchootingApplicationId name,
                                                   Ptr<PscApplication> application)
{
    NS_LOG_FUNCTION(this);
...
void
SchoolShootingApplicationHelper::AddPscApplications(SchoolSchootingApplicationId name,
                                                    ApplicationContainer applications)
{
    ...
    if (it == m_pscApplicationsMap.end())
    {
        ...
        m_pscApplicationsMap.insert(
            std::pair<SchoolSchootingApplicationId, ApplicationContainer>(name, applications));
    }
    else
    {
