...
{
    static TypeId tid =
        TypeId("ns3::psc::SchoolShootingLteHelper")
            ...
            .AddAttribute("DlEarfcn",
                          "Downlink E-UTRA Absolute Radio Frequency Channel Number (EARFCN) as per "
                          "3GPP 36.101 Section 5.7.3.",
                          UintegerValue(5330),
                          MakeUintegerAccessor(&SchoolShootingLteHelper::m_dlEarfcn),
                          MakeUintegerChecker<uint16_t>())
            .AddAttribute("UlEarfcn",
                          "Uplink E-UTRA Absolute Radio Frequency Channel Number (EARFCN) as per "
                          "3GPP 36.101 Section 5.7.3.",
                          UintegerValue(23330),
...
void
SchoolShootingLteHelper::SetScenarioDefinitionHelper(
...
NodeContainer
SchoolShootingLteHelper::GetEnodeBs()
{
    return m_enbNodesContainer;
}

Ptr<Node>
SchoolShootingLteHelper::GetSgw()
{
    return m_epcHelper->GetSgwNode();
}

Ptr<Node>
SchoolShootingLteHelper::GetPgw()
{
    return m_epcHelper->GetPgwNode();
}

Ptr<Node>
SchoolShootingLteHelper::GetPsServer()
{
    return m_psServer;
}

Ptr<Node>
SchoolShootingLteHelper::GetSchoolServer()
{
    return m_schoolServer;
}

Ptr<Node>
SchoolShootingLteHelper::GetCityServer()
{
    return m_cityServer;
}

Ptr<Node>
SchoolShootingLteHelper::GetPublicServer()
{
    return m_publicServer;
}

NetDeviceContainer
SchoolShootingLteHelper::GetNetDevicesForGroup(PscScenarioDefinition::GroupId id)
{
...
void
SchoolShootingLteHelper::InitNetwork()
{
    NS_LOG_FUNCTION(this);
    ...
    Config::SetDefault("ns3::LteEnbRrc::DefaultTransmissionMode",
                       UintegerValue(m_defaultTransmissionMode));
    ...
    for (uint32_t i = 0; i < serverContainer.GetN(); i++)
    {
        ...
        Ptr<Ipv6StaticRouting> remoteHostStaticRouting =
            ipv6RoutingHelper.GetStaticRouting(serverContainer.Get(i)->GetObject<Ipv6>());
        remoteHostStaticRouting->AddNetworkRouteTo("7777:f000::",
                                                   Ipv6Prefix(60),
        ...
        pgwStaticRouting->AddNetworkRouteTo(internetIpIfaces.GetAddress(1, 1),
                                            Ipv6Prefix(128),
                                            internetIpIfaces.GetAddress(1, 1),
                                            2 + i,
                                            0);
    }
...
void
SchoolShootingLteHelper::InitUes()
{
    NS_LOG_FUNCTION(this);
    ...
    m_netDeviceGroups.insert(
        std::pair<SchoolShootingDefinitionHelper::UserGroups, NetDeviceContainer>(
            SchoolShootingDefinitionHelper::UserGroups::UC_TEAM_VEHICLE,
            netdevices));
    ...
    m_netDeviceGroups.insert(
        std::pair<SchoolShootingDefinitionHelper::UserGroups, NetDeviceContainer>(
            SchoolShootingDefinitionHelper::UserGroups::TASK_FORCE_VEHICLES,
            netdevices));
    ...
    m_netDeviceGroups.insert(
        std::pair<SchoolShootingDefinitionHelper::UserGroups, NetDeviceContainer>(
            SchoolShootingDefinitionHelper::UserGroups::SA_VEHICLE,
            netdevices));
    ...
    m_netDeviceGroups.insert(
        std::pair<SchoolShootingDefinitionHelper::UserGroups, NetDeviceContainer>(
            SchoolShootingDefinitionHelper::UserGroups::EMS_VEHICLES,
            netdevices));
    ...
    m_netDeviceGroups.insert(
        std::pair<SchoolShootingDefinitionHelper::UserGroups, NetDeviceContainer>(
            SchoolShootingDefinitionHelper::UserGroups::FIRE_VEHICLES,
            netdevices));
    ...
    m_netDeviceGroups.insert(
        std::pair<SchoolShootingDefinitionHelper::UserGroups, NetDeviceContainer>(
            SchoolShootingDefinitionHelper::UserGroups::HELICOPTER,
            netdevices));
    ...
    m_netDeviceGroups.insert(
        std::pair<SchoolShootingDefinitionHelper::UserGroups, NetDeviceContainer>(
            SchoolShootingDefinitionHelper::UserGroups::DEPLOYABLE_CAMERA,
            netdevices));
    ...
    nodes = m_scenarioDefinitionHelper->GetScenarioDefinition()
                ->GetNodesForGroup(SchoolShootingDefinitionHelper::UserGroups::UC_TEAM)
                .nodes;
    allNodes.Add(nodes);
    netdevices = m_lteHelper->InstallUeDevice(nodes);
    allNetDevices.Add(netdevices);
    m_netDeviceGroups.insert(
        std::pair<SchoolShootingDefinitionHelper::UserGroups, NetDeviceContainer>(
            SchoolShootingDefinitionHelper::UserGroups::UC_TEAM,
            netdevices));
    ...
    m_netDeviceGroups.insert(
        std::pair<SchoolShootingDefinitionHelper::UserGroups, NetDeviceContainer>(
            SchoolShootingDefinitionHelper::UserGroups::OFFENDER,
            netdevices));
    ...
    m_netDeviceGroups.insert(
        std::pair<SchoolShootingDefinitionHelper::UserGroups, NetDeviceContainer>(
            SchoolShootingDefinitionHelper::UserGroups::SCHOOL_CAMERAS,
            netdevices));
    ...
    m_netDeviceGroups.insert(
        std::pair<SchoolShootingDefinitionHelper::UserGroups, NetDeviceContainer>(
            SchoolShootingDefinitionHelper::UserGroups::TRAFFIC_CAMERAS,
            netdevices));

    // TODO: configure batteries

    // For this scenario, the school and traffic cameras are assumed to be connected via wired
    // network to their respective servers so we do not need to create NetDevices. Even if they were
    // wireless, they most likely would be connected to a commercial network
