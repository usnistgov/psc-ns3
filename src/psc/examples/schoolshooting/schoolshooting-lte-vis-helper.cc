...
SchoolShootingLteVisHelper::SchoolShootingLteVisHelper(
    Ptr<PscScenarioDefinition> scenarioDefinition)
    : m_scenarioHelper(nullptr),
      m_accessTimesCollection(nullptr)
{
    ...
    m_orchestrator =
        CreateObject<netsimulyzer::Orchestrator>(m_scenarioDefinition->GetName() + ".json");
...
SchoolShootingLteVisHelper::SchoolShootingLteVisHelper(
    Ptr<SchoolShootingApplicationHelper> scenarioHelper)
    : m_accessTimesCollection(nullptr)
{
...
{
    static TypeId tid =
        TypeId("ns3::psc::SchoolShootingLteVisHelper")
            .SetParent<Object>()
            .AddConstructor<SchoolShootingLteVisHelper>()
            .AddAttribute("Interval",
                          "Time between updates",
                          TimeValue(Seconds(1)),
                          MakeTimeAccessor(&SchoolShootingLteVisHelper::SetInterval),
                          MakeTimeChecker());
...
SchoolShootingLteVisHelper::InitAreas()
{
    ...
    for (it = areas.begin(); it != areas.end(); it++)
    {
        ...
        // Border
        areaLocation->SetAttribute("Fill",
                                   EnumValue(netsimulyzer::RectangularArea::DrawMode::Hidden));
...
SchoolShootingLteVisHelper::InitNodeModels()
{
    ...
    nodeConfigHelper.Set("Model",
                         StringValue("non-distributable/models/characters/rest/Swat_Male.obj"));
    ...
    nodeConfigHelper.Set("Model",
                         StringValue("non-distributable/models/characters/rest/Police_Male.obj"));
    nodeConfigHelper.Set("Name", StringValue(nodeInfo.name));
    nodeConfigHelper.Install(nodeInfo.nodes);
    ...
    nodeConfigHelper.Set("Model",
                         StringValue("non-distributable/models/characters/rest/Gangster.obj"));
    nodeConfigHelper.Set("Name", StringValue(nodeInfo.name));
    nodeConfigHelper.Install(nodeInfo.nodes);

    nodeInfo = m_scenarioDefinition->GetNodesForGroup(SchoolShootingDefinitionHelper::EMS);
    nodeConfigHelper.Set("Model",
                         StringValue("non-distributable/models/characters/rest/Paramedic.obj"));
    nodeConfigHelper.Set("Name", StringValue(nodeInfo.name));
    nodeConfigHelper.Set("Offset", Vector3DValue(Vector3D(0, 0, 0)));
    nodeConfigHelper.Install(nodeInfo.nodes);

    nodeInfo =
        m_scenarioDefinition->GetNodesForGroup(SchoolShootingDefinitionHelper::SCHOOL_CAMERAS);
    nodeConfigHelper.Set("Model",
                         StringValue("non-distributable/models/props/Security_Camera.obj"));
    nodeConfigHelper.Set("Name", StringValue(nodeInfo.name));
    ...
    nodeInfo =
        m_scenarioDefinition->GetNodesForGroup(SchoolShootingDefinitionHelper::TRAFFIC_CAMERAS);
    nodeConfigHelper.Set("Model",
                         StringValue("non-distributable/models/props/Security_Camera.obj"));
    nodeConfigHelper.Set("Name", StringValue(nodeInfo.name));
    nodeConfigHelper.Set("Height", OptionalValue<double>(0.5));
    ...
    nodeConfigHelper.Set(
        "Model",
        StringValue("non-distributable/models/props/vehicle/Police_Helicopter.obj"));
    nodeConfigHelper.Set("Name", StringValue(nodeInfo.name));
    nodeConfigHelper.Set("Height", OptionalValue<double>(3.0));
    nodeConfigHelper.Install(nodeInfo.nodes);

    nodeInfo =
        m_scenarioDefinition->GetNodesForGroup(SchoolShootingDefinitionHelper::TASK_FORCE_VEHICLES);
    nodeConfigHelper.Set(
        "Model",
        StringValue("non-distributable/models/props/vehicle/Police_Car_Heist.obj"));
    nodeConfigHelper.Set("Name", StringValue(nodeInfo.name));
    ...
    nodeInfo =
        m_scenarioDefinition->GetNodesForGroup(SchoolShootingDefinitionHelper::UC_TEAM_VEHICLE);
    ...
    nodeConfigHelper.Set("Model",
                         StringValue("non-distributable/models/props/vehicle/Swat_Van.obj"));
    nodeConfigHelper.Set("Name", StringValue(nodeInfo.name));
    nodeConfigHelper.Set("Height", OptionalValue<double>(3.0));
    nodeConfigHelper.Set("Offset", Vector3DValue(Vector3D(0, 0, -3)));
    nodeConfigHelper.Install(nodeInfo.nodes);
    ...
    nodeInfo =
        m_scenarioDefinition->GetNodesForGroup(SchoolShootingDefinitionHelper::FIRE_VEHICLES);
    nodeConfigHelper.Set("Model",
    ...
    nodeConfigHelper.Set("Model",
                         StringValue("non-distributable/models/props/vehicle/Ambulance.obj"));
    nodeConfigHelper.Set("Name", StringValue(nodeInfo.name));
    ...
    nodeInfo =
        m_scenarioDefinition->GetNodesForGroup(SchoolShootingDefinitionHelper::DEPLOYABLE_CAMERA);
    nodeConfigHelper.Set("Model",
                         StringValue("non-distributable/models/props/Security_Camera.obj"));
    nodeConfigHelper.Set("Name", StringValue(nodeInfo.name));
    nodeConfigHelper.Set("Height", OptionalValue<double>(0.5));
...
{
    NS_LOG_FUNCTION(this);
    std::set<SchoolShootingApplicationHelper::SchoolSchootingApplicationId> enabledApps =
        m_scenarioHelper->GetEnabledApplications();

    if (enabledApps.size() == 0)
    {
        // all apps are enabled
        for (uint16_t appId = SchoolShootingApplicationHelper::SCHOOL_SHOOTING_MCPTT;
             appId < SchoolShootingApplicationHelper::SCHOOL_SHOOTING_LAST;
             appId++)
        {
            EnableApplicationGraphs(
                (SchoolShootingApplicationHelper::SchoolSchootingApplicationId)appId);
        }
    }
    else
    {
        std::set<SchoolShootingApplicationHelper::SchoolSchootingApplicationId>::iterator
            enabledAppsIt;
        for (enabledAppsIt = enabledApps.begin(); enabledAppsIt != enabledApps.end();
             enabledAppsIt++)
        {
...
void
SchoolShootingLteVisHelper::EnableApplicationGraphs(
    SchoolShootingApplicationHelper::SchoolSchootingApplicationId appName)
{
    NS_LOG_FUNCTION(this << appName);
    NS_ABORT_MSG_UNLESS(m_scenarioHelper != nullptr,
                        "Cannot enable application trace if network not deployed");
...
SchoolShootingLteVisHelper::EnableKeyEventLog()
{
    ...
    std::map<PscScenarioDefinition::KeyEventId, PscScenarioDefinition::KeyEventInfo>::iterator
        eventIt;
...
SchoolShootingLteVisHelper::EnableSwatHelmetCameraGraphs()
{
    ...
    NS_ABORT_MSG_UNLESS(
        txApps.GetN() * 2 == rxApps.GetN(),
        "Invalid Helmet camera configuration. Expecting 2 receivers per transmitter");
    for (uint16_t i = 0; i < txApps.GetN(); i++)
    {
        // Create a collection with the Tx and 2 Rxs together
        PointerValue xAxis;
        PointerValue yAxis;
        Ptr<netsimulyzer::SeriesCollection> collection =
            CreateObject<netsimulyzer::SeriesCollection>(m_orchestrator);
        collection->SetAttribute("AutoColor", BooleanValue(true));
        collection->SetAttribute("Name",
                                 StringValue("SWAT Helmet video (node " +
                                             std::to_string(txApps.Get(i)->GetNode()->GetId()) +
                                             ")"));
        ...
        // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, "SWAT Helmet Camera TX (node " + std::to_string (txApps.Get (i)->GetNode
        // ()->GetId ()) + ")");
        Ptr<netsimulyzer::ThroughputSink> sink =
            CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "TX");
        ...
        txXySeries.Get<netsimulyzer::XYSeries>()->SetAttribute(
            "LabelMode",
            EnumValue(netsimulyzer::XYSeries::LabelMode::Hidden));
        txXySeries.Get<netsimulyzer::XYSeries>()->SetAttribute("Color", netsimulyzer::RED_VALUE);
        txApps.Get(i)->TraceConnectWithoutContext(
            "Tx",
            MakeBoundCallback(&TraceTxSwatHelmetCamera, sink));
        collection->Add(sink->GetSeries());

        for (uint16_t j = 0; j < 2; j++)
        {
            // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
            // (m_orchestrator, "SWAT Helmet Camera RX (" + std::to_string (txApps.Get (i)->GetNode
            // ()->GetId ()) + "->" + std::to_string (rxApps.Get (2*i+j)->GetNode ()->GetId ()) +
            // ")");
            Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>(
                m_orchestrator,
                "RX " + std::to_string(rxApps.Get(2 * i + j)->GetNode()->GetId()));
            sink->SetAttribute("Unit", StringValue("kb/s"));
            sink->SetAttribute("Interval", TimeValue(m_interval));
            PointerValue txXySeries;
            sink->GetAttribute("XYSeries", txXySeries);
            ...
            rxApps.Get(2 * i + j)->TraceConnectWithoutContext(
                "Rx",
                MakeBoundCallback(&TraceRxSwatHelmetCamera, sink));
            collection->Add(sink->GetSeries());
        }
...
void
SchoolShootingLteVisHelper::TraceTxSwatHelmetCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                    Ptr<const Packet> p)
{
...
void
SchoolShootingLteVisHelper::TraceRxSwatHelmetCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                    Ptr<const Packet> p,
...
SchoolShootingLteVisHelper::EnableDeployableCameraGraphs()
{
    ...
    NS_ABORT_MSG_UNLESS(
        txApps.GetN() * 2 == rxApps.GetN(),
        "Invalid deployable camera configuration. Expecting 2 receivers per transmitter");
    for (uint16_t i = 0; i < txApps.GetN(); i++)
    {
        // Create a collection with the Tx and 2 Rxs together
        PointerValue xAxis;
        ...
        collection->SetAttribute("AutoColor", BooleanValue(true));
        collection->SetAttribute("Name",
                                 StringValue("Deployable Camera video (node " +
                                             std::to_string(txApps.Get(i)->GetNode()->GetId()) +
                                             ")"));
        collection->GetAttribute("XAxis", xAxis);
        xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name", StringValue("Time (s)"));
        ...
        // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, "Deployable Camera TX (node " + std::to_string (txApps.Get (i)->GetNode
        // ()->GetId ()) + ")");
        Ptr<netsimulyzer::ThroughputSink> sink =
            CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "TX");
        sink->SetAttribute("Unit", StringValue("kb/s"));
        ...
        txApps.Get(i)->TraceConnectWithoutContext(
            "Tx",
            MakeBoundCallback(&TraceTxDeployableCamera, sink));
        collection->Add(sink->GetSeries());

        for (uint16_t j = 0; j < 2; j++)
        {
            // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
            // (m_orchestrator, "Deployable Camera RX (" + std::to_string (txApps.Get (i)->GetNode
            // ()->GetId ()) + "->" + std::to_string (rxApps.Get (2*i+j)->GetNode ()->GetId ()) +
            // ")");
            Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>(
                m_orchestrator,
            ...
            rxApps.Get(2 * i + j)->TraceConnectWithoutContext(
                "Rx",
                MakeBoundCallback(&TraceRxDeployableCamera, sink));
            collection->Add(sink->GetSeries());
        }
        // Add to list of collections
...
void
SchoolShootingLteVisHelper::TraceTxDeployableCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                    Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(sink);
    sink->AddPacketSize(p->GetSize());
...
void
SchoolShootingLteVisHelper::TraceRxDeployableCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                    Ptr<const Packet> p,
                                                    const Address& srcAddrs)
{
    NS_LOG_FUNCTION(sink);
...
SchoolShootingLteVisHelper::EnableThrowPhoneCameraGraphs()
{
    ...
    NS_ABORT_MSG_UNLESS(
        txApps.GetN() * 2 == rxApps.GetN(),
        "Invalid throw phone camera configuration. Expecting 2 receivers per transmitter");
    for (uint16_t i = 0; i < txApps.GetN(); i++)
    {
        // Create a collection with the Tx and 2 Rxs together
        PointerValue xAxis;
        ...
        collection->SetAttribute("AutoColor", BooleanValue(true));
        collection->SetAttribute("Name",
                                 StringValue("Throw phone video (node " +
                                             std::to_string(txApps.Get(i)->GetNode()->GetId()) +
                                             ")"));
        collection->GetAttribute("XAxis", xAxis);
        xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name", StringValue("Time (s)"));
        ...
        // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, "Throw Phone Camera TX (node " + std::to_string (txApps.Get (i)->GetNode
        // ()->GetId ()) + ")");
        Ptr<netsimulyzer::ThroughputSink> sink =
            CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "TX");
        sink->SetAttribute("Unit", StringValue("kb/s"));
        ...
        txApps.Get(i)->TraceConnectWithoutContext(
            "Tx",
            MakeBoundCallback(&TraceTxThrowPhoneCamera, sink));
        collection->Add(sink->GetSeries());

        for (uint16_t j = 0; j < 2; j++)
        {
            // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
            // (m_orchestrator, "Throw Phone Camera RX (" + std::to_string (txApps.Get (i)->GetNode
            // ()->GetId ()) + "->" + std::to_string (rxApps.Get (2*i+j)->GetNode ()->GetId ()) +
            // ")");
            Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>(
                m_orchestrator,
            ...
            rxApps.Get(2 * i + j)->TraceConnectWithoutContext(
                "Rx",
                MakeBoundCallback(&TraceRxThrowPhoneCamera, sink));
            collection->Add(sink->GetSeries());
        }
        // Add to list of collections
...
void
SchoolShootingLteVisHelper::TraceTxThrowPhoneCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                    Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(sink);
    sink->AddPacketSize(p->GetSize());
...
void
SchoolShootingLteVisHelper::TraceRxThrowPhoneCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                    Ptr<const Packet> p,
                                                    const Address& srcAddrs)
{
    NS_LOG_FUNCTION(sink);
...
SchoolShootingLteVisHelper::EnableHelicopterCameraGraphs()
{
    ...
    NS_ABORT_MSG_UNLESS(
        txApps.GetN() * 2 == rxApps.GetN(),
        "Invalid Helicopterera configuration. Expecting 2 receivers per transmitter");
    for (uint16_t i = 0; i < txApps.GetN(); i++)
    {
        // Create a collection with the Tx and 2 Rxs together
        PointerValue xAxis;
        ...
        collection->SetAttribute("AutoColor", BooleanValue(true));
        collection->SetAttribute("Name",
                                 StringValue("Helicopter video (node " +
                                             std::to_string(txApps.Get(i)->GetNode()->GetId()) +
                                             ")"));
        collection->GetAttribute("XAxis", xAxis);
        xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name", StringValue("Time (s)"));
        ...
        // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, "Helicopter Camera TX (node " + std::to_string (txApps.Get (i)->GetNode
        // ()->GetId ()) + ")");
        Ptr<netsimulyzer::ThroughputSink> sink =
            CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "TX");
        sink->SetAttribute("Unit", StringValue("kb/s"));
        ...
        txApps.Get(i)->TraceConnectWithoutContext(
            "Tx",
            MakeBoundCallback(&TraceTxHelicopterCamera, sink));
        collection->Add(sink->GetSeries());

        for (uint16_t j = 0; j < 2; j++)
        {
            // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
            // (m_orchestrator, "Helicopter Camera RX (" + std::to_string (txApps.Get (i)->GetNode
            // ()->GetId ()) + "->" + std::to_string (rxApps.Get (2*i+j)->GetNode ()->GetId ()) +
            // ")");
            Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>(
                m_orchestrator,
            ...
            rxApps.Get(2 * i + j)->TraceConnectWithoutContext(
                "Rx",
                MakeBoundCallback(&TraceRxHelicopterCamera, sink));
            collection->Add(sink->GetSeries());
        }
        // Add to list of collections
...
void
SchoolShootingLteVisHelper::TraceTxHelicopterCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                    Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(sink);
    sink->AddPacketSize(p->GetSize());
...
void
SchoolShootingLteVisHelper::TraceRxHelicopterCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                    Ptr<const Packet> p,
                                                    const Address& srcAddrs)
{
    NS_LOG_FUNCTION(sink);
...
SchoolShootingLteVisHelper::EnableEmsVehicleCameraGraphs()
{
    ...
    NS_ABORT_MSG_UNLESS(
        txApps.GetN() * 2 == rxApps.GetN(),
        "Invalid EMS vehicle camera configuration. Expecting 2 receivers per transmitter");
    for (uint16_t i = 0; i < txApps.GetN(); i++)
    {
        // Create a collection with the Tx and 2 Rxs together
        PointerValue xAxis;
        ...
        collection->SetAttribute("AutoColor", BooleanValue(true));
        collection->SetAttribute("Name",
                                 StringValue("EMS vehicle video (node " +
                                             std::to_string(txApps.Get(i)->GetNode()->GetId()) +
                                             ")"));
        collection->GetAttribute("XAxis", xAxis);
        xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name", StringValue("Time (s)"));
        ...
        // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, "EMS Vehicle Camera TX (node " + std::to_string (txApps.Get (i)->GetNode
        // ()->GetId ()) + ")");
        Ptr<netsimulyzer::ThroughputSink> sink =
            CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "TX");
        sink->SetAttribute("Unit", StringValue("kb/s"));
        ...
        txApps.Get(i)->TraceConnectWithoutContext(
            "Tx",
            MakeBoundCallback(&TraceTxEmsVehicleCamera, sink));
        collection->Add(sink->GetSeries());

        for (uint16_t j = 0; j < 2; j++)
        {
            // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
            // (m_orchestrator, "EMS Vehicle Camera RX (" + std::to_string (txApps.Get (i)->GetNode
            // ()->GetId ()) + "->" + std::to_string (rxApps.Get (2*i+j)->GetNode ()->GetId ()) +
            // ")");
            Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>(
                m_orchestrator,
            ...
            rxApps.Get(2 * i + j)->TraceConnectWithoutContext(
                "Rx",
                MakeBoundCallback(&TraceRxEmsVehicleCamera, sink));
            collection->Add(sink->GetSeries());
        }
        // Add to list of collections
...
void
SchoolShootingLteVisHelper::TraceTxEmsVehicleCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                    Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(sink);
    sink->AddPacketSize(p->GetSize());
...
void
SchoolShootingLteVisHelper::TraceRxEmsVehicleCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                    Ptr<const Packet> p,
                                                    const Address& srcAddrs)
{
    NS_LOG_FUNCTION(sink);
...
SchoolShootingLteVisHelper::EnableVideoConferenceVideoGraphs()
{
    ...
    NS_ABORT_MSG_UNLESS(
        txApps.GetN() == 2 && rxApps.GetN() == 2,
        "Invalid video conference video configuration. Expecting 1 receiver and 1 transmitter");
    for (uint16_t i = 0; i < txApps.GetN(); i++)
    {
        ...
        collection->SetAttribute("AutoColor", BooleanValue(true));
        collection->SetAttribute("Name",
                                 StringValue("Video conference video (node " +
                                             std::to_string(txApps.Get(i)->GetNode()->GetId()) +
                                             ")"));
        collection->GetAttribute("XAxis", xAxis);
        xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name", StringValue("Time (s)"));
        ...
        // Ptr<netsimulyzer::ThroughputSink> txSink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, "Video Conference Video TX (node " + std::to_string (txApps.Get
        // (i)->GetNode ()->GetId ()) + ")");
        Ptr<netsimulyzer::ThroughputSink> txSink =
            CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "TX");
        ...
        txApps.Get(i)->TraceConnectWithoutContext(
            "Tx",
            MakeBoundCallback(&TraceTxVideoConferenceVideo, txSink));
        collection->Add(txSink->GetSeries());

        // Ptr<netsimulyzer::ThroughputSink> rxSink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, "Video Conference Video RX (" + std::to_string (txApps.Get (i)->GetNode
        // ()->GetId ()) + "->" + std::to_string (rxApps.Get ((i + 1) % txApps.GetN ())->GetNode
        // ()->GetId ()) + ")");
        Ptr<netsimulyzer::ThroughputSink> rxSink = CreateObject<netsimulyzer::ThroughputSink>(
            m_orchestrator,
            "RX " + std::to_string(rxApps.Get((i + 1) % txApps.GetN())->GetNode()->GetId()));
        ...
        rxXySeries.Get<netsimulyzer::XYSeries>()->SetAttribute(
            "LabelMode",
            EnumValue(netsimulyzer::XYSeries::LabelMode::Hidden));
        rxApps.Get((i + 1) % txApps.GetN())
            ->TraceConnectWithoutContext("Rx",
                                         MakeBoundCallback(&TraceRxVideoConferenceVideo, rxSink));
...
void
SchoolShootingLteVisHelper::TraceTxVideoConferenceVideo(Ptr<netsimulyzer::ThroughputSink> sink,
                                                        Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(sink);
    sink->AddPacketSize(p->GetSize());
...
void
SchoolShootingLteVisHelper::TraceRxVideoConferenceVideo(Ptr<netsimulyzer::ThroughputSink> sink,
                                                        Ptr<const Packet> p,
                                                        const Address& srcAddrs)
{
    NS_LOG_FUNCTION(sink);
...
SchoolShootingLteVisHelper::EnableSchoolCameraGraphs()
{
    ...
    NS_ABORT_MSG_UNLESS(
        txApps.GetN() * 2 == rxApps.GetN(),
        "Invalid school camera configuration. Expecting 2 receivers per transmitter");
    for (uint16_t i = 0; i < txApps.GetN(); i++)
    {
        // Create a collection with the Tx and 2 Rxs together
        PointerValue xAxis;
        ...
        collection->SetAttribute("AutoColor", BooleanValue(true));
        collection->SetAttribute("Name",
                                 StringValue("School camera video (node " +
                                             std::to_string(txApps.Get(i)->GetNode()->GetId()) +
                                             ")"));
        collection->GetAttribute("XAxis", xAxis);
        xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name", StringValue("Time (s)"));
        ...
        // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, "School Camera TX (node " + std::to_string (txApps.Get (i)->GetNode
        // ()->GetId ()) + ")");
        Ptr<netsimulyzer::ThroughputSink> sink =
            CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "TX");
        sink->SetAttribute("Unit", StringValue("kb/s"));
        ...
        txXySeries.Get<netsimulyzer::XYSeries>()->SetAttribute(
            "LabelMode",
            EnumValue(netsimulyzer::XYSeries::LabelMode::Hidden));
        txApps.Get(i)->TraceConnectWithoutContext("Tx",
                                                  MakeBoundCallback(&TraceTxSchoolCamera, sink));
        collection->Add(sink->GetSeries());

        for (uint16_t j = 0; j < 2; j++)
        {
            // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
            // (m_orchestrator, "School Camera RX (" + std::to_string (txApps.Get (i)->GetNode
            // ()->GetId ()) + "->" + std::to_string (rxApps.Get (2*i+j)->GetNode ()->GetId ()) +
            // ")");
            Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>(
                m_orchestrator,
            ...
            rxApps.Get(2 * i + j)->TraceConnectWithoutContext(
                "Rx",
                MakeBoundCallback(&TraceRxSchoolCamera, sink));
            collection->Add(sink->GetSeries());
        }
        // Add to list of collections
...
void
SchoolShootingLteVisHelper::TraceTxSchoolCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(sink);
    sink->AddPacketSize(p->GetSize());
...
void
SchoolShootingLteVisHelper::TraceRxSchoolCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                Ptr<const Packet> p,
                                                const Address& srcAddrs)
{
    NS_LOG_FUNCTION(sink);
...
SchoolShootingLteVisHelper::EnableTrafficCameraGraphs()
{
    ...
    NS_ABORT_MSG_UNLESS(
        txApps.GetN() * 2 == rxApps.GetN(),
        "Invalid traffic camera configuration. Expecting 2 receivers per transmitter");
    for (uint16_t i = 0; i < txApps.GetN(); i++)
    {
        // Create a collection with the Tx and 2 Rxs together
        PointerValue xAxis;
        ...
        collection->SetAttribute("AutoColor", BooleanValue(true));
        collection->SetAttribute("Name",
                                 StringValue("Traffic camera video (node " +
                                             std::to_string(txApps.Get(i)->GetNode()->GetId()) +
                                             ")"));
        collection->GetAttribute("XAxis", xAxis);
        xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name", StringValue("Time (s)"));
        ...
        // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, "Traffic Camera TX (node " + std::to_string (txApps.Get (i)->GetNode
        // ()->GetId ()) + ")");
        Ptr<netsimulyzer::ThroughputSink> sink =
            CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "TX");
        sink->SetAttribute("Unit", StringValue("kb/s"));
        ...
        txXySeries.Get<netsimulyzer::XYSeries>()->SetAttribute(
            ...
            EnumValue(netsimulyzer::XYSeries::LabelMode::Hidden));
        txApps.Get(i)->TraceConnectWithoutContext("Tx",
                                                  MakeBoundCallback(&TraceTxTrafficCamera, sink));
        collection->Add(sink->GetSeries());

        for (uint16_t j = 0; j < 2; j++)
        {
            // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
            // (m_orchestrator, "Traffic Camera RX (" + std::to_string (txApps.Get (i)->GetNode
            // ()->GetId ()) + "->" + std::to_string (rxApps.Get (2*i+j)->GetNode ()->GetId ()) +
            // ")");
            Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>(
                m_orchestrator,
            ...
            rxApps.Get(2 * i + j)->TraceConnectWithoutContext(
                "Rx",
                MakeBoundCallback(&TraceRxTrafficCamera, sink));
            collection->Add(sink->GetSeries());
        }
        // Add to list of collections
...
void
SchoolShootingLteVisHelper::TraceTxTrafficCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                 Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(sink);
    sink->AddPacketSize(p->GetSize());
...
void
SchoolShootingLteVisHelper::TraceRxTrafficCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                                 Ptr<const Packet> p,
                                                 const Address& srcAddrs)
{
    NS_LOG_FUNCTION(sink);
...
SchoolShootingLteVisHelper::EnableWebGraphs()
{
    ...
    NS_ABORT_MSG_UNLESS(txApps.GetN() == 4 && rxApps.GetN() == 4,
                        "Invalid web configuration. Expecting 8 receivers and 8 transmitters");
    for (uint16_t i = 0; i < txApps.GetN(); i++)
    {
        // Create a collection with the Tx and 2 Rxs together
        PointerValue xAxis;
        ...
        collection->SetAttribute(
            "Name",
            StringValue("Web (node " + std::to_string(txApps.Get(i)->GetNode()->GetId()) + ")"));
        collection->GetAttribute("XAxis", xAxis);
        xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name", StringValue("Time (s)"));
        collection->GetAttribute("YAxis", yAxis);
        yAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name",
        ...
        // Ptr<netsimulyzer::ThroughputSink> txSink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, "Web TX (node " + std::to_string (txApps.Get (i)->GetNode ()->GetId ())
        // + ")");
        Ptr<netsimulyzer::ThroughputSink> txSink =
            CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "TX");
        txSink->SetAttribute("Unit", StringValue("kb/s"));
        ...
        // Ptr<netsimulyzer::ThroughputSink> rxSink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, "Web RX (" + std::to_string (txApps.Get (i)->GetNode ()->GetId ()) +
        // "->" + std::to_string (rxApps.Get ((i + 2) % txApps.GetN ())->GetNode ()->GetId ()) +
        // ")");
        Ptr<netsimulyzer::ThroughputSink> rxSink = CreateObject<netsimulyzer::ThroughputSink>(
            m_orchestrator,
            "RX " + std::to_string(rxApps.Get((i + 2) % txApps.GetN())->GetNode()->GetId()));
        rxSink->SetAttribute("Unit", StringValue("kb/s"));
        rxSink->SetAttribute("Interval", TimeValue(m_interval));
        PointerValue rxXySeries;
        rxSink->GetAttribute("XYSeries", rxXySeries);
        rxXySeries.Get<netsimulyzer::XYSeries>()->SetAttribute(
            "LabelMode",
            EnumValue(netsimulyzer::XYSeries::LabelMode::Hidden));
        rxApps.Get((i + 2) % txApps.GetN())
            ->TraceConnectWithoutContext("Rx", MakeBoundCallback(&TraceRxWeb, rxSink));
        collection->Add(rxSink->GetSeries());
        // Add to list of collections
...
void
SchoolShootingLteVisHelper::TraceTxWeb(Ptr<netsimulyzer::ThroughputSink> sink, Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(sink);
    sink->AddPacketSize(p->GetSize());
}
...
void
SchoolShootingLteVisHelper::TraceRxWeb(Ptr<netsimulyzer::ThroughputSink> sink,
                                       Ptr<const Packet> p,
                                       const Address& srcAddrs)
{
    NS_LOG_FUNCTION(sink);
...
SchoolShootingLteVisHelper::EnableNg911Graphs()
{
    ...
    NS_ABORT_MSG_UNLESS(txApps.GetN() == 1 && rxApps.GetN() == 1,
                        "Invalid NG911 configuration. Expecting 1 receiver and 1 transmitter");
    // Create a collection with the Tx and  Rx together
    PointerValue xAxis;
    PointerValue yAxis;
    Ptr<netsimulyzer::SeriesCollection> collection =
    ...
    collection->SetAttribute("AutoColor", BooleanValue(true));
    collection->SetAttribute("Name",
                             StringValue("NG911 video (node " +
                                         std::to_string(txApps.Get(0)->GetNode()->GetId()) + ")"));
    collection->GetAttribute("XAxis", xAxis);
    xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name", StringValue("Time (s)"));
    collection->GetAttribute("YAxis", yAxis);
    ...
    // Ptr<netsimulyzer::ThroughputSink> txSink = CreateObject<netsimulyzer::ThroughputSink>
    // (m_orchestrator, "NG911 TX (node " + std::to_string (txApps.Get (0)->GetNode ()->GetId ()) +
    // ")");
    Ptr<netsimulyzer::ThroughputSink> txSink =
        CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "TX");
    txSink->SetAttribute("Unit", StringValue("kb/s"));
    ...
    // Ptr<netsimulyzer::ThroughputSink> rxSink = CreateObject<netsimulyzer::ThroughputSink>
    // (m_orchestrator, "NG911 RX (" + std::to_string (txApps.Get (0)->GetNode ()->GetId ()) + "->"
    // + std::to_string (rxApps.Get (0)->GetNode ()->GetId ()) + ")");
    Ptr<netsimulyzer::ThroughputSink> rxSink = CreateObject<netsimulyzer::ThroughputSink>(
        m_orchestrator,
        "RX " + std::to_string(rxApps.Get(0)->GetNode()->GetId()));
    rxSink->SetAttribute("Unit", StringValue("kb/s"));
    rxSink->SetAttribute("Interval", TimeValue(m_interval));
    PointerValue rxXySeries;
    rxSink->GetAttribute("XYSeries", rxXySeries);
...
void
SchoolShootingLteVisHelper::TraceTxNg911(Ptr<netsimulyzer::ThroughputSink> sink,
                                         Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(sink);
    sink->AddPacketSize(p->GetSize());
...
void
SchoolShootingLteVisHelper::TraceRxNg911(Ptr<netsimulyzer::ThroughputSink> sink,
                                         Ptr<const Packet> p,
                                         const Address& srcAddrs)
{
    NS_LOG_FUNCTION(sink);
...
void
SchoolShootingLteVisHelper::EnablePscAppsTrafficGraphs(
    SchoolShootingApplicationHelper::SchoolSchootingApplicationId name)
{
    NS_LOG_FUNCTION(this);
    ...
    if (apps.GetN() == 0)
    {
        NS_LOG_DEBUG("No application " + ai.name +
                     " found. It was either not enabled or it is not a PscApplication");
    }
    for (uint32_t i = 0; i < apps.GetN(); i++)
    {
        ...
        collection->SetAttribute("AutoColor", BooleanValue(true));
        collection->SetAttribute("Name",
                                 StringValue(ai.name + " (node " +
                                             std::to_string(apps.Get(i)->GetNode()->GetId()) +
                                             ")"));
        collection->GetAttribute("XAxis", xAxis);
        xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name", StringValue("Time (s)"));
        collection->GetAttribute("YAxis", yAxis);
        ...
        // Ptr<netsimulyzer::ThroughputSink> sink = CreateObject<netsimulyzer::ThroughputSink>
        // (m_orchestrator, ai.name + " TX (node " + std::to_string (apps.Get (i)->GetNode ()->GetId
        // ()) + ")");
        Ptr<netsimulyzer::ThroughputSink> sink =
            CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "TX");
        sink->SetAttribute("Unit", StringValue("kb/s"));
        ...
        DynamicCast<PscApplication>(apps.Get(i))
            ->TraceConnectWithoutContext("Tx", MakeBoundCallback(&TraceAppRxTraffic, sink));
        collection->Add(sink->GetSeries());

        // sink = CreateObject<netsimulyzer::ThroughputSink> (m_orchestrator, ai.name + " RX (node "
        // + std::to_string (apps.Get (i)->GetNode ()->GetId ()) + ")");
        ...
        rxXySeries.Get<netsimulyzer::XYSeries>()->SetAttribute(
            ...
            EnumValue(netsimulyzer::XYSeries::LabelMode::Hidden));
        DynamicCast<PscApplication>(apps.Get(i))
            ->TraceConnectWithoutContext("Rx", MakeBoundCallback(&TraceAppTxTraffic, sink));
        collection->Add(sink->GetSeries());
        // Add to list of collections
...
void
SchoolShootingLteVisHelper::TraceAppTxTraffic(Ptr<netsimulyzer::ThroughputSink> sink,
                                              std::string appName,
...
void
SchoolShootingLteVisHelper::TraceAppRxTraffic(Ptr<netsimulyzer::ThroughputSink> sink,
                                              std::string appName,
                                              SeqTsSizeHeader header)
{
    NS_LOG_FUNCTION(sink << appName << header);
...
SchoolShootingLteVisHelper::EnableMcpttGraphs()
{
    ...
    for (uint16_t callId = 1; callId <= 6; callId++)
    {
        ...
        // callIt->second.accessTimeGraph->SetAttribute ("Name", StringValue("Call ID " +
        // std::to_string(callId) + " Access time"));
        callIt->second.accessTimeGraph->SetAttribute(
            "Name",
            StringValue("Call ID " + std::to_string(callId)));
        ...
        // callIt->second.m2eLatencyGraph->SetAttribute ("Name", StringValue("Call ID " +
        // std::to_string(callId) + " M2E Latency"));
        callIt->second.m2eLatencyGraph->SetAttribute(
            "Name",
            StringValue("Call ID " + std::to_string(callId)));
    ...
    Config::ConnectWithoutContext("/NodeList/*/ApplicationList/*/$ns3::psc::McpttPttApp/Calls/*/"
                                  "FloorMachine/StateChangeTrace",
                                  MakeCallback(&SchoolShootingLteVisHelper::TraceAccessTime, this));
    Config::ConnectWithoutContext(
        "/NodeList/*/ApplicationList/*/$ns3::psc::McpttPttApp/RxTrace",
        MakeCallback(&SchoolShootingLteVisHelper::TraceMouthToEarLatency, this));
}

void
SchoolShootingLteVisHelper::TraceAccessTime(uint32_t userId,
                                            uint16_t callId,
    ...
    if (callIt != m_callInfoByCallId.end())
    {
        ...
        else
        {
            if (it->second.second == "'U: pending Request'")
            {
                ...
                {
                    callIt->second.accessTimeGraph->Append(
                        Simulator::Now().GetSeconds(),
                        (Simulator::Now() - it->second.first).GetMilliSeconds());
                    callIt->second.accessTimeMap.erase(userId);
                    return;
                }
                else if (newStateName == "'U: has no permission'" ||
                         newStateName == "'Releasing'" || newStateName == "'U: pending Release'")
                {
                    callIt->second.accessTimeMap.erase(userId);
                    return;
                }
...
void
SchoolShootingLteVisHelper::TraceMouthToEarLatency(Ptr<const Application> app,
                                                   uint16_t callId,
                                                   ...
                                                   const TypeId& headerType)
{
    ...
    if (headerType == McpttMediaMsg::GetTypeId())
    {
        ...
        {
            callIt->second.m2eLatencyMap.insert(
                std::make_pair(app->GetNode()->GetId(), talkSpurtStart));
            callIt->second.m2eLatencyGraph->Append(
                Simulator::Now().GetSeconds(),
...
SchoolShootingLteVisHelper::EnableEnbThroughputGraphs()
{
    ...
    NodeContainer enbs =
        m_scenarioHelper->GetTechnologyHelper()->GetObject<SchoolShootingLteHelper>()->GetEnodeBs();
    ...
    for (auto enbIt = enbs.Begin(); enbIt != enbs.End(); enbIt++)
    {
        ...
        for (uint32_t netDevIdx = 0; netDevIdx < node->GetNDevices(); netDevIdx++)
        {
            ...
            if (netDev != nullptr)
            {
                ...
                collection->SetAttribute(
                    "Name",
                    StringValue("eNodeB Cell Id " + std::to_string(netDev->GetCellId()) +
                                " (Node " + std::to_string(node->GetId()) + ")"));
                collection->GetAttribute("XAxis", xAxis);
                xAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute("Name", StringValue("Time (s)"));
                collection->GetAttribute("YAxis", yAxis);
                yAxis.Get<netsimulyzer::ValueAxis>()->SetAttribute(
                    "Name",
                    StringValue("Throughput (kb/s)"));
                ...
                Ptr<netsimulyzer::ThroughputSink> rxSink =
                    CreateObject<netsimulyzer::ThroughputSink>(m_orchestrator, "RX");
                rxSink->SetAttribute("Unit", StringValue("kb/s"));
                rxSink->SetAttribute("Interval", TimeValue(m_interval));
                PointerValue rxXySeries;
                rxSink->GetAttribute("XYSeries", rxXySeries);
                ...
                for (auto ueManagerIt = ueManagers.Begin(); ueManagerIt != ueManagers.End();
                     ueManagerIt++)
                {
                    ...
                    for (auto drbIt = drbMap.Begin(); drbIt != drbMap.End(); drbIt++)
                    {
                        ...
                        rlc.Get<LteRlc>()->TraceConnectWithoutContext(
                            "TxPDU",
                            MakeBoundCallback(&TraceEnbThroughputTx, txSink));
                        rlc.Get<LteRlc>()->TraceConnectWithoutContext(
                            "RxPDU",
                            MakeBoundCallback(&TraceEnbThroughputRx, rxSink));
                    }
                }
            }
        }
...
void
SchoolShootingLteVisHelper::TraceEnbThroughputTx(Ptr<netsimulyzer::ThroughputSink> sink,
                                                 uint16_t rnti,
                                                 uint8_t lcid,
                                                 uint32_t bytes)
{
...
void
SchoolShootingLteVisHelper::TraceEnbThroughputRx(Ptr<netsimulyzer::ThroughputSink> sink,
                                                 uint16_t rnti,
                                                 uint8_t lcid,
                                                 uint32_t bytes,
                                                 uint64_t delay)
