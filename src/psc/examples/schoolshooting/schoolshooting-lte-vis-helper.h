...
class SchoolShootingLteVisHelper : public Object
{
  ...
  protected:
     ...
     */
    static void TraceTxSwatHelmetCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                        Ptr<const Packet> p);
     ...
     */
    static void TraceRxSwatHelmetCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                        Ptr<const Packet> p,
     ...
     * \param p The packet sent
     */
    static void TraceTxDeployableCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                        Ptr<const Packet> p);
     ...
     * \param srcAddrs The address of the sender
     */
    static void TraceRxDeployableCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                        Ptr<const Packet> p,
                                        const Address& srcAddrs);
     ...
     * \param p The packet sent
     */
    static void TraceTxThrowPhoneCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                        Ptr<const Packet> p);
     ...
     * \param srcAddrs The address of the sender
     */
    static void TraceRxThrowPhoneCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                        Ptr<const Packet> p,
                                        const Address& srcAddrs);
     ...
     * \param p The packet sent
     */
    static void TraceTxHelicopterCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                        Ptr<const Packet> p);
     ...
     * \param srcAddrs The address of the sender
     */
    static void TraceRxHelicopterCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                        Ptr<const Packet> p,
                                        const Address& srcAddrs);
     ...
     * \param p The packet sent
     */
    static void TraceTxEmsVehicleCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                        Ptr<const Packet> p);
     ...
     * \param srcAddrs The address of the sender
     */
    static void TraceRxEmsVehicleCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                        Ptr<const Packet> p,
                                        const Address& srcAddrs);
     ...
     * \param p The packet sent
     */
    static void TraceTxVideoConferenceVideo(Ptr<netsimulyzer::ThroughputSink> sink,
                                            Ptr<const Packet> p);
     ...
     * \param srcAddrs The address of the sender
     */
    static void TraceRxVideoConferenceVideo(Ptr<netsimulyzer::ThroughputSink> sink,
                                            Ptr<const Packet> p,
                                            const Address& srcAddrs);
     ...
     * \param srcAddrs The address of the sender
     */
    static void TraceRxSchoolCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                    Ptr<const Packet> p,
                                    const Address& srcAddrs);
     ...
     * \param srcAddrs The address of the sender
     */
    static void TraceRxTrafficCamera(Ptr<netsimulyzer::ThroughputSink> sink,
                                     Ptr<const Packet> p,
                                     const Address& srcAddrs);
     ...
     * \param srcAddrs The address of the sender
     */
    static void TraceRxWeb(Ptr<netsimulyzer::ThroughputSink> sink,
                           Ptr<const Packet> p,
                           const Address& srcAddrs);
     ...
     * \param srcAddrs The address of the sender
     */
    static void TraceRxNg911(Ptr<netsimulyzer::ThroughputSink> sink,
                             Ptr<const Packet> p,
                             const Address& srcAddrs);
     ...
     */
    void EnablePscAppsTrafficGraphs(
     ...
     */
    static void TraceAppTxTraffic(Ptr<netsimulyzer::ThroughputSink> sink,
                                  std::string appName,
     ...
     * \param header The PSC header of the packet received
     */
    static void TraceAppRxTraffic(Ptr<netsimulyzer::ThroughputSink> sink,
                                  std::string appName,
                                  SeqTsSizeHeader header);
     ...
     */
    void TraceAccessTime(uint32_t userId,
                         uint16_t callId,
     ...
     */
    void TraceMouthToEarLatency(Ptr<const Application> app,
                                uint16_t callId,
     ...
     */
    static void TraceEnbThroughputTx(Ptr<netsimulyzer::ThroughputSink> sink,
                                     uint16_t rnti,
                                     uint8_t lcid,
                                     uint32_t bytes);
     ...
     */
    static void TraceEnbThroughputRx(Ptr<netsimulyzer::ThroughputSink> sink,
                                     uint16_t rnti,
                                     uint8_t lcid,
                                     uint32_t bytes,
                                     uint64_t delay);
