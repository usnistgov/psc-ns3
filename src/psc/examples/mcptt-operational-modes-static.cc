    ...
    for (uint32_t i = 0; i < n.GetN(); i++)
    {
        ...
        if (m)
        {
            *os << Simulator::Now().As(Time::S) << " " << std::setw(2) << n.Get(i)->GetId() << " ("
                << std::setw(3) << m->GetPosition().x << "," << std::setw(4) << m->GetPosition().y
                << "," << std::setw(4) << m->GetPosition().z << ")" << std::endl;
        }
    }
}
...
ConnectRelayService(Ptr<NetDevice> ueDevice)
{
    ...
    if (!slUeRrc->IsMonitoringRelayServiceCode(LteSlDiscHeader::DISC_RELAY_ANNOUNCEMENT,
                                               RELAY_SERVICE_CODE))
    {
        NS_LOG_INFO(
            "Starting Relay Service as Remote UE for UE Node ID: " << ueDevice->GetNode()->GetId());
...
Pc5ConnectionStatusTrace(Ptr<McpttServerApp> serverApp,
                         ...
                         uint32_t iface)
{
    NS_LOG_DEBUG("PC5 status changed for srcL2Id " << srcL2Id << " and dstL2Id " << dstL2Id
                                                   << " to " << status);
...
main(int argc, char* argv[])
{
    ...
    Config::SetDefault("ns3::psc::McpttStateMachineStats::OutputFileName",
                       StringValue("mcptt-operational-modes-static-state-machine.dat"));
    Config::SetDefault("ns3::psc::McpttMsgStats::OutputFileName",
                       StringValue("mcptt-operational-modes-static-message.dat"));
    ...
    PrintPositionsAt(ascii.CreateFileStream("mcptt-operational-modes-static-positions.dat"),
                     Seconds(0));
    ...
    callId =
        callHelper.AddCall(team1Apps, serverApp, groupId, callType, callStartTime, callStopTime);
    groupIdToOnNetworkCallId.insert(std::pair<uint32_t, uint16_t>(groupId, callId));

    groupId = 3;
    callId =
        callHelper.AddCall(team3Apps, serverApp, groupId, callType, callStartTime, callStopTime);
    groupIdToOnNetworkCallId.insert(std::pair<uint32_t, uint16_t>(groupId, callId));
    ...
    Simulator::Schedule(relayStartTime,
    ...
    Ptr<OutputStreamWrapper> PC5SignalingPacketTraceStream =
        ascii.CreateFileStream("mcptt-operational-modes-static-pc5-signaling.dat");
    *PC5SignalingPacketTraceStream->GetStream() << "time(s)\ttxId\tRxId\tmsgType" << std::endl;
    ...
    Ptr<OutputStreamWrapper> RsrpMeasurementsStream =
        ascii.CreateFileStream("mcptt-operational-modes-static-rsrp-measurement.dat");
