...
{
    static TypeId tid =
        TypeId("ns3::LteSlUeControllerCamad2019")
            ...
            .AddAttribute("MaxConnectionAttempts",
                          ...
                          MakeDoubleChecker<double>())
            .AddTraceSource(
                "RemoteConnectionStart",
                "Traces when the Remote UE starts the one-to-one connection establishment with a "
                "Relay UE.",
            ...
            .AddTraceSource(
                ...
                "Traces when the Remote UE completes the one-to-one connection establishment with "
                "a Relay UE.",
                MakeTraceSourceAccessor(
                    &LteSlUeControllerCamad2019::m_traceRemoteConnectionEstablished),
...
void
LteSlUeControllerCamad2019::DoRecvRelayServiceDiscovery(uint32_t serviceCode,
                                                        ...
                                                        uint8_t statusIndicator)
{
    ...
    if (m_campaign == "Discovery")
    {
        ...
        if (it == m_dicoveredRelayUeIds.end())
        {
            ...
            // Store
            m_dicoveredRelayUeIds.insert(
                std::pair<uint32_t, Time>(proseRelayUeId, Simulator::Now()));
            ...
            uint32_t nDiscPeriods =
                std::ceil((double)discoverTime.GetMilliSeconds() / m_discPeriodDuration);
            ...
            NS_LOG_INFO(" UE Imsi: "
                        << myImsi << " UE L2Id: " << myL2Id << " discovered proseRelayId "
                        << proseRelayUeId << " nDiscoveredRelays " << m_dicoveredRelayUeIds.size()
                        << " discoverTime (s) " << discoverTime.GetSeconds() << " nDiscPeriods "
                        << nDiscPeriods << " allRelaysDiscovered " << allRelaysDiscovered);

            if (allRelaysDiscovered)
            {
                m_traceDiscoversAllRelays(myL2Id,
                                          proseRelayUeId,
                                          discoverTime.GetSeconds(),
                                          nDiscPeriods);
            }
        }
        else
...
void
LteSlUeControllerCamad2019::DoPc5ConnectionStarted(uint32_t peerUeId,
                                                   uint32_t selfUeId,
...
void
LteSlUeControllerCamad2019::DoPc5SecuredEstablished(uint32_t peerUeId,
                                                    uint32_t selfUeId,
                                                    LteSlUeRrc::RelayRole role)
{
    NS_LOG_FUNCTION(this << peerUeId << selfUeId << role);
    ...
    if (m_campaign == "Discovery")
    {
        NS_LOG_ERROR("This controller should not initiate relay communication when running the "
                     "Discovery Campaign");
        return;
    }
    ...
    if (m_connectingRelayUeId != std::numeric_limits<uint32_t>::max())
    {
        NS_ASSERT_MSG(it == m_lteSlUeNetDeviceMap.end(),
                      "PC5 connection between " << selfUeId << " and " << peerUeId
                                                << " already established");
        NS_ASSERT_MSG(m_connectingRelayUeId == peerUeId,
                      "PC5 connection established to a relay it was not trying to connect");
        m_connectingRelayUeId = std::numeric_limits<uint32_t>::max();
    }

    // If the UE is a Relay UE and has already a connection with that Remote UE,
    // tear it down and proceed to start a new one
    // This happens e.g., when the DirectCommunicationAccept is lost: the Relay established the
    // connection but not the Remote, and the Remote try to connect again before the local release
    // (in the relay) of the previous connection

    // TS 24.334 10.4.2.6.2 Abnormal cases at the target UE
    // "... if the target UE already has an existing link established to the UE known to use this
    // Layer 2 ID and the new request contains an identical User Info as the known user, the UE
    // shall process the new request. However, the target UE shall only delete the existing link
    // context after the new link setup procedure succeeds, or the link keepalive procedure as
    // described in subclause 10.4.3 fails.
    ...
    if (it == m_lteSlUeNetDeviceMap.end())
    {
        ...
        {
            NS_LOG_DEBUG(" " << role << " " << LteSlUeRrc::RemoteUE << " "
                             << bool(role == LteSlUeRrc::RemoteUE));
            // Tracing Remote connection time
            if (role == LteSlUeRrc::RemoteUE)
            {
                m_haveConnected = true;
                double connTimeSeconds =
                    Simulator::Now().GetSeconds() - m_remoteStartedConnectionTime.GetSeconds();
                m_traceRemoteConnectionEstablished(selfUeId,
                                                   peerUeId,
                                                   m_connectionCount,
                                                   connTimeSeconds);
                NS_LOG_DEBUG("selfUeId " << selfUeId << " peerUeId " << peerUeId
                                         << " m_connectionCount " << m_connectionCount
                                         << " connTimeSeconds " << connTimeSeconds);
            }
        }
        ...
        NS_LOG_DEBUG("LteNetDevice address "
                     << m_netDevice->GetAddress() << " Ipv6: "
                     << node->GetObject<Ipv6L3Protocol>()->GetAddress(1, 1).GetAddress());
        ...
        m_lteSlUeNetDeviceMap.insert(
            std::pair<uint32_t, Ptr<LteSlUeNetDevice>>(peerUeId, slNetDev));
        ...
        if (role == LteSlUeRrc::RemoteUE)
        {
            Ipv6InterfaceContainer ifUp =
                m_lteSidelinkHelper->AssignIpv6AddressForRelayCommunication(slNetDev,
                                                                            peerUeId,
                                                                            selfUeId,
                                                                            role);
            uint32_t ipInterfaceIndex = ipv6->GetInterfaceForDevice(slNetDev);
            NS_LOG_DEBUG("New LteSlNetDevice in remote node has interface index "
                         << ipInterfaceIndex);

            // add route
            Ipv6Address ipv6a_relay =
                m_lteSidelinkHelper->GetRelayIpv6AddressFromMap(peerUeId, selfUeId, role);
            ...
            Ptr<LteSlTft> tft = Create<LteSlTft>(LteSlTft::RECEIVE,
                                                 LteSlTft::LOCAL,
                                                 ifUp.GetAddress(0, 1),
                                                 peerUeId);
            m_netDevice->GetNas()->SetForwardUpCallback(
                tft,
                MakeCallback(&LteSlUeNetDevice::Receive, slNetDev));

            // configure TFT to be able to send packets through the sidelink radio bearer
            tft = Create<LteSlTft>(LteSlTft::BIDIRECTIONAL,
                                   LteSlTft::LOCAL,
                                   ifUp.GetAddress(0, 1),
                                   peerUeId);
        ...
        if (role == LteSlUeRrc::RelayUE)
        {
            Ipv6InterfaceContainer ifUp =
                m_lteSidelinkHelper->AssignIpv6AddressForRelayCommunication(slNetDev,
                                                                            ...
                                                                            role);
            uint32_t ipInterfaceIndex = ipv6->GetInterfaceForDevice(slNetDev);
            NS_LOG_DEBUG("New LteSlNetDevice in relay node has interface index "
                         << ipInterfaceIndex);
            // Since it is a relay, it needs to forward packets between sidelink and uplink/downlink
            // netdevices
            ...
            // Ipv6StaticRoutingHelper ipv6RoutingHelper;
            // Ptr<Ipv6StaticRouting> ue2ueStaticRouting = ipv6RoutingHelper.GetStaticRouting
            // (node->GetObject<Ipv6> ()); ue2ueStaticRouting->AddNetworkRouteTo (ifUp.GetAddress
            // (0,1), Ipv6Prefix(64), "::",2, 0);
...
void
LteSlUeControllerCamad2019::DoPc5ConnectionTerminated(uint32_t peerUeId,
                                                      uint32_t selfUeId,
                                                      LteSlUeRrc::RelayRole role)
{
    NS_LOG_FUNCTION(this << peerUeId << selfUeId << role);
    ...
    {
        Ipv6Address ipv6a =
            m_lteSidelinkHelper->GetSelfIpv6AddressFromMap(peerUeId, selfUeId, role);
    ...
    {
        Ipv6Address ipv6a =
            m_lteSidelinkHelper->GetSelfIpv6AddressFromMap(selfUeId, peerUeId, role);

        Ptr<LteSlTft> tft =
            Create<LteSlTft>(LteSlTft::RECEIVE, LteSlTft::REMOTE, ipv6a, Ipv6Prefix(64), peerUeId);
        m_netDevice->GetNas()->RemoveForwardUpCallback(tft);
...
void
LteSlUeControllerCamad2019::DoPc5ConnectionAborted(uint32_t peerUeId,
                                                   uint32_t selfUeId,
                                                   LteSlUeRrc::RelayRole role,
                                                   LteSlO2oCommParams::UeO2ORejectReason reason)
{
...
void
LteSlUeControllerCamad2019::DoRecvRemoteUeReport(uint64_t localImsi,
                                                 uint32_t peerUeId,
                                                 uint64_t remoteImsi)
{
...
uint64_t
LteSlUeControllerCamad2019::DoRelayUeSelection(std::map<uint64_t, double> validRelays,
                                               uint32_t serviceCode,
                                               uint64_t currentRelayId)
{
    ...
    if (m_campaign == "Discovery")
    {
        ...
        // This function will be called upon measurement report and it will return the
        // currentRelayId so that there is no action taken in LteSlUeRrc::RelayUeSelection
    ...
    else
    {
        if (m_campaign == "Connection")
        {
            // In the Connection campaign we are only interested in the first successful connection
            // in simTime If the Remote UE already had a connection, it should not try to connect If
            // the Remote UE has reach the maximum attempts without being successful, it should not
            // try to connect
        ...
        NS_LOG_INFO("Current Relay ID " << currentRelayId << " Selected Relay ID "
                                        << selectedRelayId);
