...
{
    double simTime = 20.0; // Simulation time (in seconds) updated automatically based on number of
                           // nodes and traffic duration
    double appStartTime =
        0.0; // Application start time (in seconds) updated automatically based on number of nodes
    ...
    {
        NS_LOG_INFO(
            "Regular UE " << ry + 1 << " IMSI = ["
                          << regularUeDevs.Get(ry)->GetObject<LteUeNetDevice>()->GetRrc()->GetImsi()
                          << "]");
    }
    for (uint32_t ry = 0; ry < relayUeDevs.GetN(); ry++)
    {
        NS_LOG_INFO(
            "Relay UE " << ry + 1 << " IMSI = ["
                        << relayUeDevs.Get(ry)->GetObject<LteUeNetDevice>()->GetRrc()->GetImsi()
                        << "]");
    }
    for (uint32_t rm = 0; rm < remoteUeDevs.GetN(); rm++)
    {
        NS_LOG_INFO(
            "Remote UE " << rm + 1 << " IMSI = ["
                         << remoteUeDevs.Get(rm)->GetObject<LteUeNetDevice>()->GetRrc()->GetImsi()
                         << "]");
    }
    ...
    {
        remoteClientUdpApps.Get(clientIdx)->SetStartTime(
            Seconds(8.0 + (10.0 * slPeriod / 1000.0) * clientIdx));
    }
    ...
    OnOffHelper clientOnOffHelper("ns3::UdpSocketFactory",
                                  Address(Inet6SocketAddress(remoteHostAddr, serverPort)));
    ...
    clientOnOffApps.Stop(
        Seconds(simTime - 1.0)); // To ensure we receive the packets in simulation time

    std::ostringstream oss;
    Ptr<OutputStreamWrapper> packetTraceStream = ascii.CreateFileStream("AppPacketTrace.txt");
    *packetTraceStream->GetStream()
        << "time(sec)\ttx/rx\tnodeType\tNodeID\tPktSize(bytes)\tPktSeq\tDelay(ms)" << std::endl;
    ...
    if (txType == "Regular")
    {
        ...
        regularUeOnOffApps.Get(0)->TraceConnect(
            "TxWithAddresses",
            oss.str(),
            MakeBoundCallback(&UePacketTrace, packetTraceStream));
        regularClientUdpApps.Get(0)->SetStartTime(
            Seconds(simTime)); // do not pre-register sender node
    }
    else if (txType == "Relay")
    {
        ...
        relayUeOnOffApps.Get(0)->TraceConnect("TxWithAddresses",
                                              oss.str(),
                                              MakeBoundCallback(&UePacketTrace, packetTraceStream));
        relayClientUdpApps.Get(0)->SetStartTime(Seconds(simTime)); // do not pre-register sender
                                                                   // node
    }
    else if (txType == "Remote")
    {
        ...
        remoteUeOnOffApps.Get(0)->TraceConnect(
            "TxWithAddresses",
            oss.str(),
            MakeBoundCallback(&UePacketTrace, packetTraceStream));
        remoteClientUdpApps.Get(0)->SetStartTime(
            Seconds(simTime)); // do not pre-register sender node
    }
    else
    {
    ...
    for (uint32_t serverIdx = 0; serverIdx < regularServerUdpApps.GetN(); serverIdx++)
    {
        ...
        regularServerUdpApps.Get(serverIdx)->TraceConnect(
            "RxWithAddresses",
            oss.str(),
            MakeBoundCallback(&UePacketTrace, packetTraceStream));
        oss.str("");
    ...
    for (uint32_t serverIdx = 0; serverIdx < remoteServerUdpApps.GetN(); serverIdx++)
    {
        ...
        remoteServerUdpApps.Get(serverIdx)->TraceConnect(
            "RxWithAddresses",
            oss.str(),
            MakeBoundCallback(&UePacketTrace, packetTraceStream));
        oss.str("");
    ...
    for (uint32_t serverIdx = 0; serverIdx < relayServerUdpApps.GetN(); serverIdx++)
    {
        ...
        relayServerUdpApps.Get(serverIdx)->TraceConnect(
            "RxWithAddresses",
            oss.str(),
            MakeBoundCallback(&UePacketTrace, packetTraceStream));
        oss.str("");
    ...
    {
        Simulator::Schedule((Seconds(4.0 + (10.0 * slPeriod / 1000.0) * rmDevIdx)),
                            &LteSidelinkHelper::StartRelayService,
                            proseHelper,
