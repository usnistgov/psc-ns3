...
void
TraceSinkARemoteDiscoversAllRelays(Ptr<OutputStreamWrapper> stream,
                                   uint32_t remoteId,
                                   uint32_t lastRelayId,
                                   double timeToDisc,
                                   uint32_t nPeriodstoDisc)
{
    *stream->GetStream() << Simulator::Now().GetSeconds() << "\t" << remoteId << "\t" << lastRelayId
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("discPrbNum",
                 "Number of discovery resources per subframe in the discovery pool --> N_f",
                 discPrbNum);
    cmd.AddValue("nSubframes",
                 "Number of subframes in bitmap of the discovery pool --> N_t",
                 nSubframes);
    ...
    uint32_t discPeriodInt =
        LteRrcSap::DiscPeriodAsInt(discPool.ueSelected.poolToAddModList.pools[0].pool.discPeriod);
    ...
    for (uint32_t ryDevIdx = 0; ryDevIdx < relayUeDevs.GetN(); ryDevIdx++)
    {
        if (discModel == "A")
        {
            Simulator::Schedule(Seconds(startRelayTime),
    ...
    Ptr<OutputStreamWrapper> aRemoteDiscoversAllRelaysStream =
        ascii.CreateFileStream("TimesEachRemoteDiscoversAllRelays.txt");
    *aRemoteDiscoversAllRelaysStream->GetStream()
        << "time(s)\tremoteId\tlastRelayId\ttimeToDisc(s)\tnPeriodstoDisc" << std::endl;
    ...
    for (uint32_t rmDevIdx = 0; rmDevIdx < remoteUeDevs.GetN(); rmDevIdx++)
    {
        ...
        // Randomize the start of the relay service for the Remote UEs by one discovery period (
        // 0.320 s)
        ...
        if (discModel == "A")
        {
            Simulator::Schedule(Seconds(startRemoteTimeToUse),
                                &LteSidelinkHelper::StartRelayService,
                                proseHelper,
