                                 ...
                                 Ptr<Packet> p)
{
    ...
    *stream->GetStream() << Simulator::Now().GetSeconds() << "\t" << srcL2Id << "\t" << dstL2Id
                         << "\t" << (uint32_t)lpc5smt.GetMessageType() << std::endl;
}
...
void
TraceSinkRemoteConnectionStart(Ptr<OutputStreamWrapper> stream,
                               uint32_t remoteId,
                               uint32_t relayId,
                               uint32_t connectionCount)
{
    *stream->GetStream() << Simulator::Now().GetSeconds() << "\t" << remoteId << "\t" << relayId
                         << "\t"
                         << "S"
                         << "\t" << connectionCount << std::endl;
}

// Trace sink function for logging when a Remote UE completes successfully the one-to-one connection
// procedure to connect to the Trace 'RemoteConnectionEstablished' of the
// 'LteSlUeControllerCamad2019'
void
TraceSinkRemoteConnectionEstablished(Ptr<OutputStreamWrapper> stream,
                                     uint32_t remoteId,
                                     uint32_t relayId,
                                     uint32_t connectionCount,
                                     double timeToConnect)
{
    *stream->GetStream() << Simulator::Now().GetSeconds() << "\t" << remoteId << "\t" << relayId
                         << "\t"
                         << "E"
                         << "\t" << connectionCount << "\t" << timeToConnect << std::endl;
}
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("remoteDCRqMax",
                 "Maximum Value of Direct Communication Requests retransmissions for Remote UE",
                 remoteDCRqMax);

    cmd.AddValue("relayTraffic", "The Relay UE has its own traffic", relayTraffic);
    cmd.AddValue("relayTrafficOffMean",
                 "The mean of the distribution of the 'Off' periods for the Relay UE traffic (if "
                 "set to 100 no traffic will be generated, regardless of relayTraffic flag)",
                 relayTrafficOffMean);
    ...
    Config::SetDefault("ns3::LteSlUeControllerCamad2019::MaxConnectionAttempts",
                       DoubleValue(maxConnectionAttempts));
    ...
    if (relayTraffic)
    {
        ...
        for (uint32_t relUeIdx = 0; relUeIdx < relayUeNodes.GetN(); relUeIdx++)
        {
            ...
            PacketSinkHelper packetSinkHelper("ns3::UdpSocketFactory",
                                              Inet6SocketAddress(remoteHostAddr, relUePort));
    ...
    *PC5SignalingPacketTraceStream->GetStream() << "time(s)\ttxId\tRxId\tmsgType" << std::endl;
    Ptr<OutputStreamWrapper> remoteConnectionEventsStream =
        ascii.CreateFileStream("remoteConnectionEventsTrace.txt");
    *remoteConnectionEventsStream->GetStream()
        << "time(s)\tremoteId\trelayId\teventType\tconnId\tconnTime" << std::endl;
    ...
    for (uint32_t rmDevIdx = 0; rmDevIdx < remoteUeDevs.GetN(); rmDevIdx++)
    {
        ...
        ctlr->TraceConnectWithoutContext(
            ...
            MakeBoundCallback(&TraceSinkRemoteConnectionStart, remoteConnectionEventsStream));
        ctlr->TraceConnectWithoutContext(
            "RemoteConnectionEstablished",
            MakeBoundCallback(&TraceSinkRemoteConnectionEstablished, remoteConnectionEventsStream));
    }
