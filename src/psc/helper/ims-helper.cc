...
ImsHelper::ConnectPgw(Ptr<Node> pgw)
{
    ...
    NS_LOG_DEBUG("IMS node (id " << m_imsNode->GetId() << ") connected to P-GW node (id "
                                 << pgw->GetId() << ")");
    m_pgwNode = pgw;
    ...
    if (!m_useIpv6)
    {
        ...
        Ptr<Ipv4StaticRouting> imsStaticRouting =
            ipv4RoutingHelper.GetStaticRouting(m_imsNode->GetObject<Ipv4>());
    ...
    else
    {
        ...
        Ptr<Ipv6StaticRouting> imsStaticRouting =
            ipv6RoutingHelper.GetStaticRouting(m_imsNode->GetObject<Ipv6>());
        imsStaticRouting
            ->AddNetworkRouteTo("7777:f000::", Ipv6Prefix(60), imsIpIfaces.GetAddress(0, 1), 1, 0);
    }
}
