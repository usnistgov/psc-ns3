...
uint16_t
McpttCallHelper::AddCall(ApplicationContainer clients,
                         ...
                         Time stopTime)
{
    NS_LOG_FUNCTION(this << clients.GetN() << server->GetNode()->GetId() << groupId
                         << +callType.GetType() << startTime.GetSeconds() << stopTime.GetSeconds());
    // callType not presently used
    uint16_t callId = server->AllocateCallId();
    Ptr<McpttOnNetworkFloorArbitrator> arbitrator =
        m_arbitratorFactory.Create<McpttOnNetworkFloorArbitrator>();
    ...
    for (uint32_t i = 0; i < clients.GetN(); i++)
    {
        ...
        Ptr<McpttCallMachine> callMachine =
            CreateObjectWithAttributes<McpttOnNetworkCallMachineClient>("GroupId",
                                                                        UintegerValue(groupId));
    ...
    Ptr<McpttOnNetworkFloorDualControl> dualControl =
        CreateObject<McpttOnNetworkFloorDualControl>();
    ...
    Ptr<McpttServerCallMachine> callMachine =
        CreateObjectWithAttributes<McpttServerCallMachineGroupPrearranged>("GroupId",
                                                                           UintegerValue(groupId));
    call->SetCallMachine(callMachine);
    call->SetCallId(callId);
...
void
McpttCallHelper::AddCallOffNetwork(ApplicationContainer clients,
                                   uint16_t callId,
                                   Address peerAddress,
                                   uint32_t groupId,
                                   McpttCallMsgFieldCallType callType,
                                   ...
                                   Time stopTime)
{
    NS_LOG_FUNCTION(this << clients.GetN() << +callId << peerAddress << groupId
                         << +callType.GetType() << startTime.GetSeconds() << stopTime.GetSeconds());
...
void
McpttCallHelper::ConfigureOffNetworkBasicGrpCall(ApplicationContainer& apps,
                                                 Address peerAddress,
                                                 uint32_t usersPerGroup,
                                                 uint32_t baseGroupId)
{
    ...
    for (uint32_t idx = 0; idx < apps.GetN(); idx++)
    {
        ...
        Ptr<McpttCall> call =
            pttApp->CreateCall(callFac, floorFac, McpttCall::NetworkCallType::OFF_NETWORK);
        call->SetAttribute("PeerAddress", AddressValue(peerAddress));
