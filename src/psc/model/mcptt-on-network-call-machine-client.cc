...
{
    static TypeId tid =
        TypeId("ns3::psc::McpttOnNetworkCallMachineClient")
...
McpttOnNetworkCallMachineClient::SetState(Ptr<McpttOnNetworkCallMachineClientState> state)
{
    ...
    if (m_started && (stateId != currStateId))
    {
        NS_LOG_LOGIC(GetCall()->GetOwner()->GetUserId()
                     << " moving from state " << *m_state << " to state " << *state << ".");
        m_state = state;
...
void
McpttOnNetworkCallMachineClient::SetStateChangeCb(
...
{
    NS_LOG_FUNCTION(this);
    NS_LOG_DEBUG("Release call for call ID " << GetCall()->GetCallId() << " in state "
                                             << GetStateId());
