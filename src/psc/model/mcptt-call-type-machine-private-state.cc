...
McpttCallTypeMachinePrivateState::~McpttCallTypeMachinePrivateState()
{
...
McpttCallTypeMachinePrivateState::InitiateCall(McpttCallTypeMachinePrivate& machine)
{
    ...
    NS_LOG_LOGIC(GetInstanceStateId().GetName() << "(" << this << ")"
                                                << " ignoring initate call.");
}

void
McpttCallTypeMachinePrivateState::ReceiveAccept(McpttCallTypeMachinePrivate& machine,
                                                const McpttCallMsgPrivateAccept& msg)
{
    ...
    NS_LOG_LOGIC(GetInstanceStateId().GetName()
                 << "(" << this << ")"
                 << " ignoring " << msg.GetTypeId().GetName() << " message.");
}

void
McpttCallTypeMachinePrivateState::ReceiveAcceptAck(McpttCallTypeMachinePrivate& machine,
...
void
McpttCallTypeMachinePrivateState::ReceiveEmergencyCancel(McpttCallTypeMachinePrivate& machine,
...
void
McpttCallTypeMachinePrivateState::ReceiveEmergencyCancelAck(
    McpttCallTypeMachinePrivate& machine,
...
void
McpttCallTypeMachinePrivateState::ReceiveReject(McpttCallTypeMachinePrivate& machine,
...
void
McpttCallTypeMachinePrivateState::ReceiveRelease(McpttCallTypeMachinePrivate& machine,
...
void
McpttCallTypeMachinePrivateState::ReceiveSetupRequest(McpttCallTypeMachinePrivate& machine,
...
McpttCallTypeMachinePrivateState::ReleaseCall(McpttCallTypeMachinePrivate& machine)
{
    ...
    NS_LOG_LOGIC(GetInstanceStateId().GetName() << "(" << this << ")"
                                                << " ignoring release call.");
}

void
McpttCallTypeMachinePrivateState::UpgradeTo(McpttCallTypeMachinePrivate& machine,
                                            const McpttCallMsgFieldCallType& callType)
{
    ...
    NS_LOG_LOGIC(GetInstanceStateId().GetName()
                 << "(" << this << ")"
                 << " ignoring upgrade to " << callType.GetType() << ".");
}
...
McpttCallTypeMachinePrivateStateQ0::GetInstance()
{
    NS_LOG_FUNCTION_NOARGS();

    static Ptr<McpttCallTypeMachinePrivateStateQ0> instance =
        Create<McpttCallTypeMachinePrivateStateQ0>();
...
McpttCallTypeMachinePrivateStateQ0::McpttCallTypeMachinePrivateStateQ0()
...
McpttEntityId
McpttCallTypeMachinePrivateStateQ0::GetInstanceStateId() const
{
    NS_LOG_FUNCTION(this);
...
McpttCallTypeMachinePrivateStateQ0::InitiateCall(McpttCallTypeMachinePrivate& machine)
{
    ...
    {
        machine.SetCallType(
            McpttCallMsgFieldCallType(McpttCallMsgFieldCallType::EMERGENCY_PRIVATE));
        machine.SetPriority(McpttCallMsgFieldCallType::GetCallTypePriority(
...
void
McpttCallTypeMachinePrivateStateQ0::ReceiveAccept(McpttCallTypeMachinePrivate& machine,
                                                  const McpttCallMsgPrivateAccept& msg)
{
    NS_LOG_FUNCTION(this << &msg);
...
void
McpttCallTypeMachinePrivateStateQ0::ReceiveAcceptAck(McpttCallTypeMachinePrivate& machine,
                                                     const McpttCallMsgPrivateAcceptAck& msg)
{
    NS_LOG_FUNCTION(this << &msg);
...
void
McpttCallTypeMachinePrivateStateQ0::ReceiveSetupRequest(McpttCallTypeMachinePrivate& machine,
                                                        const McpttCallMsgPrivateSetupReq& msg)
{
...
McpttCallTypeMachinePrivateStateQ1::GetInstance()
{
    NS_LOG_FUNCTION_NOARGS();

    static Ptr<McpttCallTypeMachinePrivateStateQ1> instance =
        Create<McpttCallTypeMachinePrivateStateQ1>();
...
McpttCallTypeMachinePrivateStateQ1::McpttCallTypeMachinePrivateStateQ1()
    : McpttCallTypeMachinePrivateState()
{
    NS_LOG_FUNCTION(this);
}
...
McpttEntityId
McpttCallTypeMachinePrivateStateQ1::GetInstanceStateId() const
{
    NS_LOG_FUNCTION(this);
...
void
McpttCallTypeMachinePrivateStateQ1::ReceiveAccept(McpttCallTypeMachinePrivate& machine,
                                                  const McpttCallMsgPrivateAccept& msg)
{
    NS_LOG_FUNCTION(this << &msg);
...
void
McpttCallTypeMachinePrivateStateQ1::ReceiveEmergencyCancel(
    McpttCallTypeMachinePrivate& machine,
    const McpttCallMsgPrivateEmergCancel& msg)
{
    NS_LOG_FUNCTION(this << &msg);
    ...
    if (myCallId.GetCallId() == theirCallId.GetCallId())
    {
        ...
        if (machine.GetState()->GetInstanceStateId() ==
...
void
McpttCallTypeMachinePrivateStateQ1::ReceiveEmergencyCancelAck(
    McpttCallTypeMachinePrivate& machine,
    const McpttCallMsgPrivateEmergCancelAck& msg)
{
    NS_LOG_FUNCTION(this << &msg);
...
void
McpttCallTypeMachinePrivateStateQ1::ReceiveSetupRequest(McpttCallTypeMachinePrivate& machine,
                                                        const McpttCallMsgPrivateSetupReq& msg)
{
    NS_LOG_FUNCTION(this << &msg);
...
void
McpttCallTypeMachinePrivateStateQ1::UpgradeTo(McpttCallTypeMachinePrivate& machine,
                                              const McpttCallMsgFieldCallType& callType)
{
    NS_LOG_FUNCTION(this << &machine << &callType);
...
McpttCallTypeMachinePrivateStateQ2::GetInstance()
{
    NS_LOG_FUNCTION_NOARGS();

    static Ptr<McpttCallTypeMachinePrivateStateQ2> instance =
        Create<McpttCallTypeMachinePrivateStateQ2>();
...
McpttCallTypeMachinePrivateStateQ2::McpttCallTypeMachinePrivateStateQ2()
    : McpttCallTypeMachinePrivateState()
{
    NS_LOG_FUNCTION(this);
}
...
McpttEntityId
McpttCallTypeMachinePrivateStateQ2::GetInstanceStateId() const
{
    NS_LOG_FUNCTION(this);
...
void
McpttCallTypeMachinePrivateStateQ2::ReceiveAccept(McpttCallTypeMachinePrivate& machine,
                                                  const McpttCallMsgPrivateAccept& msg)
{
    NS_LOG_FUNCTION(this << &msg);
...
void
McpttCallTypeMachinePrivateStateQ2::ReceiveEmergencyCancel(
    McpttCallTypeMachinePrivate& machine,
    const McpttCallMsgPrivateEmergCancel& msg)
{
    NS_LOG_FUNCTION(this << &msg);
...
void
McpttCallTypeMachinePrivateStateQ2::ReceiveReject(McpttCallTypeMachinePrivate& machine,
                                                  const McpttCallMsgPrivateReject& msg)
{
    NS_LOG_FUNCTION(this << &msg);
...
void
McpttCallTypeMachinePrivateStateQ2::ReceiveSetupRequest(McpttCallTypeMachinePrivate& machine,
                                                        const McpttCallMsgPrivateSetupReq& msg)
{
    NS_LOG_FUNCTION(this << &msg);
