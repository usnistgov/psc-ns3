...
#include <cmath>
#include "psc-scm-urbanmacrocell-propagation-loss-model.h"
#include <ns3/node.h>
...
NS_LOG_COMPONENT_DEFINE ("PscScmUrbanmacrocellPropagationLossModel");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (PscScmUrbanmacrocellPropagationLossModel);


TypeId
PscScmUrbanmacrocellPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PscScmUrbanmacrocellPropagationLossModel")
    .SetParent<BuildingsPropagationLossModel> ()
    .AddConstructor<PscScmUrbanmacrocellPropagationLossModel> ()
    .AddAttribute ("Frequency",
                    "The propagation frequency in Hz",
                    DoubleValue (700e6),
                    MakeDoubleAccessor (&PscScmUrbanmacrocellPropagationLossModel::m_frequency),
                    MakeDoubleChecker<double> ())
    .AddAttribute ("ShadowingEnabled",
                   "Activate or deactivate the shadowing computation",
                   BooleanValue (true),
                   MakeBooleanAccessor (&PscScmUrbanmacrocellPropagationLossModel::m_isShadowingEnabled),
                   MakeBooleanChecker ())
    ;
...
PscScmUrbanmacrocellPropagationLossModel::PscScmUrbanmacrocellPropagationLossModel ()
...
double
PscScmUrbanmacrocellPropagationLossModel::GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  NS_LOG_FUNCTION (this);
  // Pathloss
  double loss = 0.0;
...
PscScmUrbanmacrocellPropagationLossModel::PathlossType
PscScmUrbanmacrocellPropagationLossModel::GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b, bool check)
{
  NS_LOG_FUNCTION (this);
  // Frequency in GHz
  double fc = m_frequency / 1e9;
...
double
PscScmUrbanmacrocellPropagationLossModel::GetShadowing (Ptr<MobilityModel> a, Ptr<MobilityModel> b)
const
{
  NS_LOG_FUNCTION (this);
...
double
PscScmUrbanmacrocellPropagationLossModel::EvaluateSigma (Ptr<MobilityBuildingInfo> a, Ptr<MobilityBuildingInfo> b) const
{
  NS_LOG_FUNCTION (this);
...
int64_t
PscScmUrbanmacrocellPropagationLossModel::DoAssignStreams (int64_t stream)
{
  NS_LOG_FUNCTION (this);
  return 0;
}
