...
{
    static TypeId tid =
        TypeId("ns3::psc::McpttOnNetworkFloorTowardsParticipant")
            .SetParent<Object>()
            ...
            .AddAttribute(
                "PeerAddress",
                "The Ipv4 address of the node that the peer application is on.",
            ...
            .AddAttribute(
                "PeerUserId",
                "The MCPTT User ID of the node that the peer application is on.",
            ...
            .AddAttribute(
                ...
                MakeBooleanChecker())
            .AddAttribute("T8",
                          "The delay to use for timer T8 (Time value)",
                          TimeValue(Seconds(1)),
      ...
      m_t8(CreateObject<McpttTimer>(McpttEntityId(8, "T8")))
{
    ...
    m_floorChannel->SetRxPktCb(
        MakeCallback(&McpttOnNetworkFloorTowardsParticipant::ReceiveFloorPkt, this));
    m_mediaChannel->SetRxPktCb(
        MakeCallback(&McpttOnNetworkFloorTowardsParticipant::ReceiveMediaPkt, this));
...
void
McpttOnNetworkFloorTowardsParticipant::ChangeState(
    Ptr<McpttOnNetworkFloorTowardsParticipantState> state)
{
    ...
    {
        NS_LOG_LOGIC("TowardsParticipant " << GetPeerUserId() << " moving from state " << *m_state
                                           << " to state " << *state);
        ...
        m_stateChangeTrace(GetOwner()->GetOwner()->GetCallMachine()->GetUserId(),
                           GetOwner()->GetOwner()->GetCallId(),
                           selected,
...
McpttOnNetworkFloorTowardsParticipant::DoSend(McpttMsg& msg)
{
    ...
    GetOwner()->GetOwner()->GetOwner()->TraceMessageSend(GetOwner()->GetOwner()->GetCallId(),
                                                         pkt,
...
void
McpttOnNetworkFloorTowardsParticipant::ReceiveFloorQueuePositionRequest(
    const McpttFloorMsgQueuePositionRequest& msg)
{
    NS_LOG_FUNCTION(this << msg);
    NS_LOG_LOGIC("Received " << msg.GetInstanceTypeId() << ".");
    m_state->ReceiveFloorQueuePositionRequest(Ptr<McpttOnNetworkFloorTowardsParticipant>(this),
                                              msg);
}
...
McpttOnNetworkFloorTowardsParticipant::ReceiveFloorPkt(Ptr<Packet> pkt, Address from)
{
    ...
    if (subtype == McpttFloorMsgRequest::SUBTYPE)
    {
        ...
        GetOwner()->GetOwner()->GetOwner()->TraceMessageReceive(GetOwner()->GetOwner()->GetCallId(),
                                                                pkt,
                                                                reqMsg.GetInstanceTypeId());
...
McpttOnNetworkFloorTowardsParticipant::SendFloorDeny(McpttFloorMsgDeny& msg)
{
    ...
    NS_LOG_LOGIC("McpttOnNetworkFloorTowardsParticipant(" << this << ") sending "
                                                          << msg.GetInstanceTypeId() << ".");
...
void
McpttOnNetworkFloorTowardsParticipant::SetState(
    Ptr<McpttOnNetworkFloorTowardsParticipantState> state)
{
    NS_LOG_FUNCTION(this << state);
...
void
McpttOnNetworkFloorTowardsParticipant::SetStateChangeCb(
