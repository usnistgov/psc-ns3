...
McpttFloorMsgField::Deserialize(Buffer::Iterator& buff)
{
    ...
    NS_LOG_LOGIC("Read " << bytesRead << " bytes (id=" << (uint32_t)id
...
McpttFloorMsgFieldQueuePositionInfo::McpttFloorMsgFieldQueuePositionInfo(uint8_t position,
                                                                         uint8_t priority)
    : McpttFloorMsgField(3, 2),
      m_position(position),
      m_priority(priority)
...
McpttFloorMsgFieldQueuePositionInfo::Serialize(Buffer::Iterator& buff) const
{
    ...
    NS_LOG_LOGIC("McpttFloorMsgFieldQueuePositionInfo wrote two bytes (position="
                 << (uint32_t)position << ";priority=" << (uint32_t)priority << ";)");
}
...
McpttFloorMsgFieldTrackInfo::AddRef(uint32_t ref)
{
    ...
    NS_LOG_LOGIC("McpttFloorMsgFieldTrackInfo added ref = " << ref << " for a total of "
                                                            << refs.size() << " reference(s).");
...
McpttFloorMsgFieldTrackInfo::Deserialize(Buffer::Iterator& buff)
{
    ...
    NS_LOG_LOGIC("McpttFloorMsgFieldTrackInfo read two bytes (queueCap="
                 << (uint32_t)queueCap << ";ptLength=" << (uint32_t)ptLength << ";).");
    ...
    NS_LOG_LOGIC("McpttFloorMsgFieldTrackInfo read " << (uint32_t)ptLengthWithPadding << " bytes ("
                                                     << (ptLengthWithPadding - ptLength)
                                                     << " were for padding).");
    ...
    NS_LOG_LOGIC("McpttFloorMsgFieldTrackInfo read " << refCount << " reference(s) ("
                                                     << (refCount * 4) << " bytes).");
...
McpttFloorMsgFieldTrackInfo::Serialize(Buffer::Iterator& buff) const
{
    ...
    NS_LOG_LOGIC("McpttFloorMsgFieldTrackInfo wrote two bytes (queueCap="
                 << (uint32_t)queueCap << ";ptLength=" << (uint32_t)ptLength << ";).");
    ...
    NS_LOG_LOGIC("McpttFloorMsgFieldTrackInfo wrote " << (uint32_t)ptLengthWithPadding << " bytes ("
                                                      << (ptLengthWithPadding - ptLength)
                                                      << " were for padding).");
    ...
    NS_LOG_LOGIC("McpttFloorMsgFieldTrackInfo wrote " << refs.size() << " reference(s) ("
                                                      << (refs.size() * 4) << " bytes).");
}
