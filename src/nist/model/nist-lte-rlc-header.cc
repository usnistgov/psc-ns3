...
NS_LOG_COMPONENT_DEFINE ("NistLteRlcHeader");

NS_OBJECT_ENSURE_REGISTERED (NistLteRlcHeader);

NistLteRlcHeader::NistLteRlcHeader ()
  : m_headerLength (0),
    m_framingInfo (0xff),
...
void
NistLteRlcHeader::SetFramingInfo (uint8_t framingInfo)
{
  m_framingInfo = framingInfo & 0x03;
}

void
NistLteRlcHeader::SetSequenceNumber (NistSequenceNumber10 sequenceNumber)
{
  m_sequenceNumber = sequenceNumber;
}

uint8_t
NistLteRlcHeader::GetFramingInfo () const
{
  return m_framingInfo;
}

NistSequenceNumber10
NistLteRlcHeader::GetSequenceNumber () const
{
...
void
NistLteRlcHeader::PushExtensionBit (uint8_t extensionBit)
{
...
void
NistLteRlcHeader::PushLengthIndicator (uint16_t lengthIndicator)
{
...
uint8_t
NistLteRlcHeader::PopExtensionBit (void)
{
...
uint16_t
NistLteRlcHeader::PopLengthIndicator (void)
{
...
TypeId
NistLteRlcHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NistLteRlcHeader")
    .SetParent<Header> ()
    .AddConstructor<NistLteRlcHeader> ()
  ;
  return tid;
}

TypeId
NistLteRlcHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

void NistLteRlcHeader::Print (std::ostream &os)  const
{
...
uint32_t NistLteRlcHeader::GetSerializedSize (void) const
{
  return m_headerLength;
}

void NistLteRlcHeader::Serialize (Buffer::Iterator start) const
{
...
uint32_t NistLteRlcHeader::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;
  uint8_t byte_1;
  uint8_t byte_2;
