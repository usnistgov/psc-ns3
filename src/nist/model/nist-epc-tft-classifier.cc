 ...
 *
 * Authors: 
 *   Nicola Baldo <nbaldo@cttc.es> (the NistEpcTftClassifier class)
 *   Giuseppe Piro <g.piro@poliba.it> (part of the code in NistEpcTftClassifier::Classify () 
...
NS_LOG_COMPONENT_DEFINE ("NistEpcTftClassifier");

NistEpcTftClassifier::NistEpcTftClassifier ()
{
  NS_LOG_FUNCTION (this);
}

void
NistEpcTftClassifier::Add (Ptr<NistEpcTft> tft, uint32_t id)
{
...
void
NistEpcTftClassifier::Delete (uint32_t id)
{
...
uint32_t 
NistEpcTftClassifier::Classify (Ptr<Packet> p, NistEpcTft::Direction direction)
{
  ...
  if (direction ==  NistEpcTft::UPLINK)
    {
    ...
    { 
      NS_ASSERT (direction ==  NistEpcTft::DOWNLINK);
  ...
  std::map <uint32_t, Ptr<NistEpcTft> >::const_reverse_iterator it;
  NS_LOG_LOGIC ("TFT MAP size: " << m_tftMap.size ());

  for (it = m_tftMap.rbegin (); it != m_tftMap.rend (); ++it)
    {
      NS_LOG_LOGIC ("TFT id: " << it->first );
      NS_LOG_LOGIC (" Ptr<NistEpcTft>: " << it->second);
      Ptr<NistEpcTft> tft = it->second;         
