...
NS_LOG_COMPONENT_DEFINE ("NistLteChunkProcessor");

NistLteChunkProcessor::NistLteChunkProcessor ()
{
...
void
NistLteChunkProcessor::AddCallback (NistLteChunkProcessorCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
NistLteChunkProcessor::EvaluateChunk (const SpectrumValue& sinr, Time duration)
{
...
NistLteChunkProcessor::End ()
{
  ...
  if (m_totDuration.GetSeconds () > 0)
    {
      std::vector<NistLteChunkProcessorCallback>::iterator it;
