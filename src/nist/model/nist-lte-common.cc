...
NS_LOG_COMPONENT_DEFINE ("NistLteCommon");

NistLteFlowId_t::NistLteFlowId_t ()
{
}

NistLteFlowId_t::NistLteFlowId_t (const uint16_t a, const uint8_t b)
  : m_rnti (a),
    m_lcId (b)
{
}
...
bool
operator == (const NistLteFlowId_t &a, const NistLteFlowId_t &b)
{
...
NistImsiLcidPair_t::NistImsiLcidPair_t ()
{
}

NistImsiLcidPair_t::NistImsiLcidPair_t (const uint64_t a, const uint8_t b)
  : m_imsi (a),
    m_lcId (b)
{
}
...
bool
operator == (const NistImsiLcidPair_t &a, const NistImsiLcidPair_t &b)
{
...
NistLteNistUeConfig_t::NistLteNistUeConfig_t ()
{
}
...
bool
operator == (const NistLteNistUeConfig_t &a, const NistLteNistUeConfig_t &b)
{
...
uint16_t
NistLteFfConverter::double2fpS11dot3 (double val)
{
...
double
NistLteFfConverter::fpS11dot3toDouble (uint16_t val)
{
...
double 
NistLteFfConverter::getMinFpS11dot3Value ()
{
...
static const uint32_t NistBufferSizeLevelBsrTable[64] = {
...
uint32_t
NistBufferSizeLevelBsr::BsrId2BufferSize (uint8_t val)
{
  NS_ABORT_MSG_UNLESS (val < 64, "val = " << val << " is out of range");
  return NistBufferSizeLevelBsrTable[val];
}

uint8_t
NistBufferSizeLevelBsr::BufferSize2BsrId (uint32_t val)
{
  int index = 0;
  if (NistBufferSizeLevelBsrTable[63] < val)
    {
      index = 63;
    }
  else
    {
      while (NistBufferSizeLevelBsrTable[index] < val)
        {
          NS_ASSERT (index < 64);
          index++;
        }
...
uint8_t
NistTransmissionModesLayers::TxMode2LayerNum (uint8_t txMode)
{
  uint8_t nLayer = 0;
  switch (txMode)
    {
...
double 
NistEutranMeasurementMapping::RsrpRange2Dbm (uint8_t range)
{
...
uint8_t 
NistEutranMeasurementMapping::Dbm2RsrpRange (double dbm)
{
  // 3GPP TS 36.133 section 9.1.4 RSRP Measurement Report Mapping
...
double 
NistEutranMeasurementMapping::RsrqRange2Db (uint8_t range)
{
...
uint8_t 
NistEutranMeasurementMapping::Db2RsrqRange (double db)
{
  // 3GPP TS 36.133 section 9.1.7 RSRQ Measurement Report Mapping
...
double 
NistEutranMeasurementMapping::QuantizeRsrp (double v)
{
  return RsrpRange2Dbm (Dbm2RsrpRange (v));
}

double 
NistEutranMeasurementMapping::QuantizeRsrq (double v)
{
  return RsrqRange2Db (Db2RsrqRange (v));
}

double
NistEutranMeasurementMapping::IeValue2ActualHysteresis (uint8_t hysteresisIeValue)
{
...
uint8_t
NistEutranMeasurementMapping::ActualHysteresis2IeValue (double hysteresisDb)
{
...
double
NistEutranMeasurementMapping::IeValue2ActualA3Offset (int8_t a3OffsetIeValue)
{
...
int8_t
NistEutranMeasurementMapping::ActualA3Offset2IeValue (double a3OffsetDb)
{
...
double
NistEutranMeasurementMapping::IeValue2ActualQRxLevMin (int8_t qRxLevMinIeValue)
{
...
double
NistEutranMeasurementMapping::IeValue2ActualQQualMin (int8_t qQualMinIeValue)
{
