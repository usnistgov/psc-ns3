...
NS_LOG_COMPONENT_DEFINE ("NistTraceFadingLossModel");

NS_OBJECT_ENSURE_REGISTERED (NistTraceFadingLossModel);
  


NistTraceFadingLossModel::NistTraceFadingLossModel ()
...
TypeId
NistTraceFadingLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NistTraceFadingLossModel")
    .SetParent<SpectrumPropagationLossModel> ()
    .AddConstructor<NistTraceFadingLossModel> ()
    .AddAttribute ("TraceFilename",
                   "Name of file to load a trace from.",
                   StringValue (""),
                   MakeStringAccessor (&NistTraceFadingLossModel::SetTraceFileName),
                   MakeStringChecker ())
    .AddAttribute ("TraceLength",
                  "The total length of the fading trace (default value 10 s.)",
                  TimeValue (Seconds (10.0)),
                  MakeTimeAccessor (&NistTraceFadingLossModel::SetTraceLength),
                  MakeTimeChecker ())
    .AddAttribute ("SamplesNum",
                  "The number of samples the trace is made of (default 10000)",
                   UintegerValue (10000),
                   MakeUintegerAccessor (&NistTraceFadingLossModel::m_samplesNum),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("WindowSize",
                  "The size of the window for the fading trace (default value 0.5 s.)",
                  TimeValue (Seconds (0.5)),
                  MakeTimeAccessor (&NistTraceFadingLossModel::m_windowSize),
                  MakeTimeChecker ())
    .AddAttribute ("RbNum",
                    "The number of RB the trace is made of (default 100)",
                    UintegerValue (100),
                   MakeUintegerAccessor (&NistTraceFadingLossModel::m_rbNum),
    ...
    .AddAttribute ("RngStreamSetSize",
                   ...
                   MakeUintegerAccessor (&NistTraceFadingLossModel::m_streamSetSize),
...
void
NistTraceFadingLossModel::SetTraceFileName (std::string fileName)
{
...
void 
NistTraceFadingLossModel::SetTraceLength (Time t)
{
  m_traceLength = t;
}

void 
NistTraceFadingLossModel::DoInitialize ()
{
  LoadTrace ();
}
...
void
NistTraceFadingLossModel::LoadTrace ()
{
...
Ptr<SpectrumValue>
NistTraceFadingLossModel::DoCalcRxPowerSpectralDensity (
...
int64_t
NistTraceFadingLossModel::AssignStreams (int64_t stream)
{
