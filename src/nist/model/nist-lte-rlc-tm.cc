...
NS_LOG_COMPONENT_DEFINE ("NistLteRlcTm");

NS_OBJECT_ENSURE_REGISTERED (NistLteRlcTm);
...
TypeId
NistLteRlcTm::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NistLteRlcTm")
    .SetParent<NistLteRlc> ()
    .AddConstructor<NistLteRlcTm> ()
    .AddAttribute ("MaxTxBufferSize",
                   "Maximum Size of the Transmission Buffer (in Bytes)",
                   UintegerValue (2 * 1024 * 1024),
                   MakeUintegerAccessor (&NistLteRlcTm::m_maxTxBufferSize),
...
void
NistLteRlcTm::DoTransmitPdcpPdu (Ptr<Packet> p)
{
...
void
NistLteRlcTm::DoNotifyTxOpportunity (uint32_t bytes, uint8_t layer, uint8_t harqId)
{
  ...
  if (! m_txBuffer.empty ())
    {
      m_rbsTimer.Cancel ();
      m_rbsTimer = Simulator::Schedule (MilliSeconds (10), &NistLteRlcTm::ExpireRbsTimer, this);
    }
}

void
NistLteRlcTm::DoNotifyHarqDeliveryFailure ()
{
  NS_LOG_FUNCTION (this);
}

void
NistLteRlcTm::DoReceivePdu (Ptr<Packet> p)
{
  NS_LOG_FUNCTION (this << m_rnti << (uint32_t) m_lcid << p->GetSize ());
...
void
NistLteRlcTm::DoReportBufferNistStatus (void)
{
  Time holDelay (0);
  ...
  NS_LOG_LOGIC ("Send ReportBufferNistStatus = " << r.txQueueSize << ", " << r.txQueueHolDelay );
  m_macSapProvider->ReportBufferNistStatus (r);
}

void
NistLteRlcTm::ExpireRbsTimer (void)
{
