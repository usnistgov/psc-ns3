...
#ifndef NIST_EPC_ENB_APPLICATION_H
#define NIST_EPC_ENB_APPLICATION_H
 ...
 */
class NistEpcEnbApplication : public Application
{

  friend class MemberNistEpcEnbS1SapProvider<NistEpcEnbApplication>;
  friend class MemberNistEpcS1apSapEnb<NistEpcEnbApplication>;
...
public:
   ...
   */
  NistEpcEnbApplication (Ptr<Socket> lteSocket, Ptr<Socket> s1uSocket, Ipv4Address enbS1uAddress, Ipv4Address sgwS1uAddress, uint16_t cellId);

  /**
   * Destructor
   * 
   */
  virtual ~NistEpcEnbApplication (void);
   ...
   */
  void SetS1SapUser (NistEpcEnbS1SapUser * s);

  /** 
   * 
   * \return the S1 SAP Provider
   */
  NistEpcEnbS1SapProvider* GetS1SapProvider ();
   ...
   */
  void SetS1apSapMme (NistEpcS1apSapMme * s);

  /** 
   * 
   * \return the ENB side of the S1-AP SAP 
   */
  NistEpcS1apSapEnb* GetS1apSapEnb ();
  ...
  public:
    NistEpsFlowId_t ();
    NistEpsFlowId_t (const uint16_t a, const uint8_t b);

    friend bool operator == (const NistEpsFlowId_t &a, const NistEpsFlowId_t &b);
    friend bool operator < (const NistEpsFlowId_t &a, const NistEpsFlowId_t &b);
  };
...
private:
  ...
  void DoPathSwitchRequest (NistEpcEnbS1SapProvider::NistPathSwitchRequestParameters params);
  void DoUeContextRelease (uint16_t rnti);
  
  // S1-AP SAP ENB methods
  void DoInitialContextSetupRequest (uint64_t mmeUeS1Id, uint16_t enbUeS1Id, std::list<NistEpcS1apSapEnb::NistErabToBeSetupItem> erabToBeSetupList);
  void DoPathSwitchRequestAcknowledge (uint64_t enbUeS1Id, uint64_t mmeUeS1Id, uint16_t cgi, std::list<NistEpcS1apSapEnb::NistErabSwitchedInUplinkItem> erabToBeSwitchedInUplinkList);
   ...
   * 
   */
  std::map<uint32_t, NistEpsFlowId_t> m_teidRbidMap;
   ...
   * 
   */
  NistEpcS1apSapEnb* m_s1apSapEnb;
...
#endif /* NIST_EPC_ENB_APPLICATION_H */
