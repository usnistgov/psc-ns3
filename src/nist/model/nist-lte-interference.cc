...
NS_LOG_COMPONENT_DEFINE ("NistLteInterference");

NistLteInterference::NistLteInterference ()
  : m_receiving (false),
    m_lastSignalId (0),
...
TypeId
NistLteInterference::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NistLteInterference")
    .SetParent<Object> ()
  ;
  return tid;
}
...
void
NistLteInterference::StartRx (Ptr<const SpectrumValue> rxPsd)
{ 
  ...
  if (m_receiving == false)
    {
      ...
      m_receiving = true;
      for (std::list<Ptr<NistLteChunkProcessor> >::const_iterator it = m_rsPowerChunkProcessorList.begin (); it != m_rsPowerChunkProcessorList.end (); ++it)
        {
          (*it)->Start ();
        }
      for (std::list<Ptr<NistLteChunkProcessor> >::const_iterator it = m_interfChunkProcessorList.begin (); it != m_interfChunkProcessorList.end (); ++it)
        {
          (*it)->Start ();
        }
      for (std::list<Ptr<NistLteChunkProcessor> >::const_iterator it = m_sinrChunkProcessorList.begin (); it != m_sinrChunkProcessorList.end (); ++it)
        {
          (*it)->Start (); 
        }
    }
...
void
NistLteInterference::AddSignal (Ptr<const SpectrumValue> spd, const Time duration)
{
  ...
  Simulator::Schedule (duration, &NistLteInterference::DoSubtractSignal, this, spd, signalId);
}


void
NistLteInterference::DoAddSignal  (Ptr<const SpectrumValue> spd)
{ 
...
void
NistLteInterference::DoSubtractSignal  (Ptr<const SpectrumValue> spd, uint32_t signalId)
{ 
  NS_LOG_FUNCTION (this << *spd);
  ConditionallyEvaluateChunk ();   
...
void
NistLteInterference::ConditionallyEvaluateChunk ()
{
  NS_LOG_FUNCTION (this);
  if (m_receiving)
    {
...
void
NistLteInterference::SetNoisePowerSpectralDensity (Ptr<const SpectrumValue> noisePsd)
{
...
void
NistLteInterference::AddRsPowerChunkProcessor (Ptr<NistLteChunkProcessor> p)
{
...
void
NistLteInterference::AddSinrChunkProcessor (Ptr<NistLteChunkProcessor> p)
{
  NS_LOG_FUNCTION (this << p);
...
void
NistLteInterference::AddInterferenceChunkProcessor (Ptr<NistLteChunkProcessor> p)
{
  NS_LOG_FUNCTION (this << p);
