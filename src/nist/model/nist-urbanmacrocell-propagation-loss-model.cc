...
NS_LOG_COMPONENT_DEFINE ("NistUrbanmacrocellPropagationLossModel");

namespace ns3 {

NS_OBJECT_ENSURE_REGISTERED (NistUrbanmacrocellPropagationLossModel);


TypeId
NistUrbanmacrocellPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NistUrbanmacrocellPropagationLossModel")
    .SetParent<BuildingsPropagationLossModel> ()
    .AddConstructor<NistUrbanmacrocellPropagationLossModel> ()
    .AddAttribute ("Frequency",
                    "The propagation frequency in Hz",
                    DoubleValue (700e6),
                    MakeDoubleAccessor (&NistUrbanmacrocellPropagationLossModel::m_frequency),
                    MakeDoubleChecker<double> ())
    .AddAttribute ("BuildingHeight",
                    "The average building heigth",
                    DoubleValue (20),
                    MakeDoubleAccessor (&NistUrbanmacrocellPropagationLossModel::m_buildingHeight),
                    MakeDoubleChecker<double> ())
    .AddAttribute ("StreetWidth",
                    "The average street width",
                    DoubleValue (20),
                    MakeDoubleAccessor (&NistUrbanmacrocellPropagationLossModel::m_streetWidth),
                    MakeDoubleChecker<double> ())    
    .AddAttribute ("ShadowingEnabled",
                   "Activate or deactivate the shadowing computation",
                   BooleanValue (true),
                   MakeBooleanAccessor (&NistUrbanmacrocellPropagationLossModel::m_isShadowingEnabled),
                   MakeBooleanChecker ())
    .AddAttribute ("LosEnabled",
                    "when Line-of-Sight is enabled, all UEs are considered in Los with eNB. Otherwise, use randomness",
                    BooleanValue (false),
                    MakeBooleanAccessor (&NistUrbanmacrocellPropagationLossModel::m_isLosEnabled),
                    MakeBooleanChecker ())
    ;
...
NistUrbanmacrocellPropagationLossModel::NistUrbanmacrocellPropagationLossModel ()
...
double
NistUrbanmacrocellPropagationLossModel::GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  // Pathloss
  double loss = 0.0;
  // Frequency in GHz
  ...
  if (a->GetObject<Node> ()->GetDevice (0)->GetObject<NistLteUeNetDevice> () != 0)
  {
    hms = a->GetPosition ().z;
  }
  else if (a->GetObject<Node> ()->GetDevice (0)->GetObject<NistLteEnbNetDevice> () != 0)
  {
    hbs = a->GetPosition ().z;
  }
  else 
  {
    NS_FATAL_ERROR ("The node " << a->GetObject<Node> ()->GetId () << " has neither a LteUeNetDevice nor a LteEnbNetDevice ");
  }

  if (b->GetObject<Node> ()->GetDevice (0)->GetObject<NistLteUeNetDevice> () != 0)
  {
    hms = b->GetPosition ().z;
  }
  else if (b->GetObject<Node> ()->GetDevice (0)->GetObject<NistLteEnbNetDevice> () != 0)
  {
    hbs = b->GetPosition ().z;
  }
  else 
  {
    NS_FATAL_ERROR ("The node " << b->GetObject<Node> ()->GetId () << " has neither a LteUeNetDevice nor a LteEnbNetDevice ");
  }
...
NistUrbanmacrocellPropagationLossModel::PathlossType
NistUrbanmacrocellPropagationLossModel::GetLoss (Ptr<MobilityModel> a, Ptr<MobilityModel> b, bool check)
{
  // Frequency in GHz
  double fc = m_frequency / 1e9;
  // Distance between the two nodes in meter
...
double
NistUrbanmacrocellPropagationLossModel::GetShadowing (Ptr<MobilityModel> a, Ptr<MobilityModel> b)
const
{
...
double
NistUrbanmacrocellPropagationLossModel::EvaluateSigma (Ptr<MobilityBuildingInfo> a, Ptr<MobilityBuildingInfo> b) const
{
...
int64_t
NistUrbanmacrocellPropagationLossModel::DoAssignStreams (int64_t stream)
{
  return 0;
}
