...
#include "ns3/nist-lte-rlc-am-header.h"
...
NS_LOG_COMPONENT_DEFINE ("NistLteRlcAm");

NS_OBJECT_ENSURE_REGISTERED (NistLteRlcAm);
...
TypeId
NistLteRlcAm::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NistLteRlcAm")
    .SetParent<NistLteRlc> ()
    .AddConstructor<NistLteRlcAm> ()
    .AddAttribute ("PollRetransmitTimer",
                   "Value of the t-PollRetransmit (See section 7.3 of 3GPP TS 36.322)",
                   TimeValue (MilliSeconds (100)),
                   MakeTimeAccessor (&NistLteRlcAm::m_pollRetransmitTimerValue),
                   MakeTimeChecker ())
    .AddAttribute ("TxOpportunityForRetxAlwaysBigEnough",
                   ...
                   BooleanValue (false),
                   MakeBooleanAccessor (&NistLteRlcAm::m_txOpportunityForRetxAlwaysBigEnough),
                   MakeBooleanChecker ())
...
void
NistLteRlcAm::DoTransmitPdcpPdu (Ptr<Packet> p)
{
  NS_LOG_FUNCTION (this << m_rnti << (uint32_t) m_lcid << p->GetSize ());

  /** Store arrival time */
  Time now = Simulator::Now ();
  NistRlcTag timeTag (now);
...
void
NistLteRlcAm::DoNotifyTxOpportunity (uint32_t bytes, uint8_t layer, uint8_t harqId)
{
  ...
  if ( m_statusPduRequested && ! m_statusProhibitTimer.IsRunning () )
    {
      if (bytes < m_statusNistPduBufferSize)
        {
          // Stingy MAC: We need more bytes for the STATUS PDU
          NS_LOG_LOGIC ("TxOpportunity (size = " << bytes << ") too small for the STATUS PDU (size = " << m_statusNistPduBufferSize << ")");
          NS_ASSERT_MSG (false, "TxOpportunity (size = " << bytes << ") too small for the STATUS PDU (size = " << m_statusNistPduBufferSize << ")\n"
                             << "Your MAC scheduler is assigned too few resource blocks.");
          return;
        }
      ...
      NistLteRlcAmHeader rlcAmHeader;
      rlcAmHeader.SetControlPdu (NistLteRlcAmHeader::STATUS_PDU);
  ...
  while ( firstSegment && (firstSegment->GetSize () > 0) && (nextSegmentSize > 0) )
    {
      ...
      if ( (firstSegment->GetSize () > nextSegmentSize) ||
         ...
         )
        {
          ...
          rlcAmHeader.PushExtensionBit (NistLteRlcAmHeader::DATA_FIELD_FOLLOWS);
      ...
      else // (firstSegment->GetSize () < m_nextSegmentSize) && (m_txBuffer.size () > 0)
        {
          ...
          rlcAmHeader.PushExtensionBit (NistLteRlcAmHeader::E_LI_FIELDS_FOLLOWS);
  ...
  rlcAmHeader.SetResegmentationFlag (NistLteRlcAmHeader::PDU);
  rlcAmHeader.SetLastSegmentFlag (NistLteRlcAmHeader::LAST_PDU_SEGMENT);
  ...
  if ( (tag.GetNistStatus () == NistLteRlcSduNistStatusTag::FULL_SDU) ||
    ...
    {
      framingInfo |= NistLteRlcAmHeader::FIRST_BYTE;
    }
  else
    {
      framingInfo |= NistLteRlcAmHeader::NO_FIRST_BYTE;
    }
  (*it)->AddPacketTag (tag);
  ...
  if ( (tag.GetNistStatus () == NistLteRlcSduNistStatusTag::FULL_SDU) ||
    ...
    {
      framingInfo |= NistLteRlcAmHeader::LAST_BYTE;
    }
  else
    {
      framingInfo |= NistLteRlcAmHeader::NO_LAST_BYTE;
    }
  (*it)->AddPacketTag (tag);
  ...
  rlcAmHeader.SetPollingBit (NistLteRlcAmHeader::STATUS_REPORT_NOT_REQUESTED);
  ...
  if ( (m_pduWithoutPoll >= m_pollPdu) || (m_byteWithoutPoll >= m_pollByte) ||
    ...
    {
      rlcAmHeader.SetPollingBit (NistLteRlcAmHeader::STATUS_REPORT_IS_REQUESTED);
      m_pduWithoutPoll = 0;
      m_byteWithoutPoll = 0;
      ...
      if (! m_pollRetransmitTimer.IsRunning () )
        {
          ...
          m_pollRetransmitTimer = Simulator::Schedule (m_pollRetransmitTimerValue,
                                                       &NistLteRlcAm::ExpirePollRetransmitTimer, this);
        }
      else
        {
...
void
NistLteRlcAm::DoNotifyHarqDeliveryFailure ()
{
  NS_LOG_FUNCTION (this);
}

void
NistLteRlcAm::DoReceivePdu (Ptr<Packet> p)
{
  NS_LOG_FUNCTION (this << m_rnti << (uint32_t) m_lcid << p->GetSize ());
  ...
  if ( rlcAmHeader.IsDataPdu () )
    {
      ...
      NistSequenceNumber10 seqNumber = rlcAmHeader.GetSequenceNumber ();

      if ( rlcAmHeader.GetResegmentationFlag () == NistLteRlcAmHeader::SEGMENT )
        {
          NS_LOG_LOGIC ("PDU segment received ( SN = " << seqNumber << " )");
        }
      else if ( rlcAmHeader.GetResegmentationFlag () == NistLteRlcAmHeader::PDU )
        {
      ...
      if ( rlcAmHeader.GetPollingBit () == NistLteRlcAmHeader::STATUS_REPORT_IS_REQUESTED )
        {
          m_statusPduRequested = true;
          m_statusNistPduBufferSize = 4;
      ...
      std::map <uint16_t, NistPduBuffer>::iterator it = m_rxonBuffer.find (m_vrMs.GetValue ());
      ...
      if ( seqNumber == m_vrR )
        {
          std::map <uint16_t, NistPduBuffer>::iterator it = m_rxonBuffer.find (seqNumber.GetValue ());
          if ( it != m_rxonBuffer.end () &&
          ...
          if ( m_vrH > m_vrR )
            {
              ...
              m_reorderingTimer = Simulator::Schedule (Time ("0.1s"),
                                                       &NistLteRlcAm::ExpireReorderingTimer ,this);
              m_vrX = m_vrH;
...
//         }
  ...
  else if ( rlcAmHeader.IsControlPdu () )
    {
      ...
      NistSequenceNumber10 ackSn = rlcAmHeader.GetAckSn ();
//       NistSequenceNumber10 seqNumber = m_vtA;
...
bool
NistLteRlcAm::IsInsideReceivingWindow (NistSequenceNumber10 seqNumber)
{
...
void
NistLteRlcAm::ReassembleAndDeliver (Ptr<Packet> packet)
{
  NistLteRlcAmHeader rlcAmHeader;
  packet->RemoveHeader (rlcAmHeader);
  uint8_t framingInfo = rlcAmHeader.GetFramingInfo ();
  NistSequenceNumber10 currSeqNumber = rlcAmHeader.GetSequenceNumber ();
  bool expectedSnLost;
      ...
      switch (m_reassemblingState)
        {
          case WAITING_S0_FULL:
                  switch (framingInfo)
                    {
                      case (NistLteRlcAmHeader::FIRST_BYTE | NistLteRlcAmHeader::LAST_BYTE):
                              m_reassemblingState = WAITING_S0_FULL;
                      ...
                      case (NistLteRlcAmHeader::FIRST_BYTE | NistLteRlcAmHeader::NO_LAST_BYTE):
                      ...
                      case (NistLteRlcAmHeader::NO_FIRST_BYTE | NistLteRlcAmHeader::LAST_BYTE):
                      case (NistLteRlcAmHeader::NO_FIRST_BYTE | NistLteRlcAmHeader::NO_LAST_BYTE):
                      default:
                              /**
...
/// \todo To remove
// void
// NistLteRlcAm::ReassembleOutsideWindow (void)
// {
...
/// \todo To remove
// void
// NistLteRlcAm::ReassembleSnLessThan (uint16_t seqNumber)
// {
...
void
NistLteRlcAm::DoReportBufferNistStatus (void)
{
  NS_LOG_FUNCTION (this);
  ...
  if ( m_txonBufferSize > 0 )
    {
      NistRlcTag txonQueueHolTimeTag;
  ...
  NistRlcTag retxQueueHolTimeTag;
  ...
  if ( m_statusPduRequested && ! m_statusProhibitTimer.IsRunning () )
    {
      r.statusPduSize = m_statusNistPduBufferSize;
    }
  else
    {
      r.statusPduSize = 0;
  ...
  if ( r.txQueueSize != 0 || r.retxQueueSize != 0 || r.statusPduSize != 0 )
    {
      NS_LOG_INFO ("Send ReportBufferNistStatus: " << r.txQueueSize << ", " << r.txQueueHolDelay << ", " 
    ...
    {
      NS_LOG_INFO ("ReportBufferNistStatus don't needed");
    }
}


void
NistLteRlcAm::ExpireReorderingTimer (void)
{
  NS_LOG_FUNCTION (this);
...
void
NistLteRlcAm::ExpirePollRetransmitTimer (void)
{
  NS_LOG_FUNCTION (this);
