...
NS_LOG_COMPONENT_DEFINE ("NistA2A4RsrqHandoverAlgorithm");

NS_OBJECT_ENSURE_REGISTERED (NistA2A4RsrqHandoverAlgorithm);
...
NistA2A4RsrqHandoverAlgorithm::NistA2A4RsrqHandoverAlgorithm ()
  : m_a2MeasId (0),
    m_a4MeasId (0),
    ...
    m_handoverManagementSapUser (0)
{
  ...
  m_handoverManagementSapProvider = new MemberNistLteHandoverManagementSapProvider<NistA2A4RsrqHandoverAlgorithm> (this);
}
...
TypeId
NistA2A4RsrqHandoverAlgorithm::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::NistA2A4RsrqHandoverAlgorithm")
    .SetParent<NistLteHandoverAlgorithm> ()
    .AddConstructor<NistA2A4RsrqHandoverAlgorithm> ()
    .AddAttribute ("ServingCellThreshold",
                   ...
                   UintegerValue (30),
                   MakeUintegerAccessor (&NistA2A4RsrqHandoverAlgorithm::m_servingCellThreshold),
    ...
    .AddAttribute ("NeighbourCellOffset",
                   ...
                   UintegerValue (1),
                   MakeUintegerAccessor (&NistA2A4RsrqHandoverAlgorithm::m_neighbourCellOffset),
...
void
NistA2A4RsrqHandoverAlgorithm::SetNistLteHandoverManagementSapUser (NistLteHandoverManagementSapUser* s)
{
...
NistA2A4RsrqHandoverAlgorithm::GetNistLteHandoverManagementSapProvider ()
{
  NS_LOG_FUNCTION (this);
...
void
NistA2A4RsrqHandoverAlgorithm::DoInitialize ()
{
  NS_LOG_FUNCTION (this);

  NS_LOG_LOGIC (this << " requesting Event A2 measurements"
                     << " (threshold=" << (uint16_t) m_servingCellThreshold << ")");
  NistLteRrcSap::NistReportConfigEutra reportConfigA2;
  reportConfigA2.eventId = NistLteRrcSap::NistReportConfigEutra::EVENT_A2;
  reportConfigA2.threshold1.choice = NistLteRrcSap::NistThresholdEutra::THRESHOLD_RSRQ;
  reportConfigA2.threshold1.range = m_servingCellThreshold;
  reportConfigA2.triggerQuantity = NistLteRrcSap::NistReportConfigEutra::RSRQ;
  reportConfigA2.reportInterval = NistLteRrcSap::NistReportConfigEutra::MS240;
  ...
  reportConfigA4.threshold1.choice = NistLteRrcSap::NistThresholdEutra::THRESHOLD_RSRQ;
  reportConfigA4.threshold1.range = 0; // intentionally very low threshold
  reportConfigA4.triggerQuantity = NistLteRrcSap::NistReportConfigEutra::RSRQ;
  reportConfigA4.reportInterval = NistLteRrcSap::NistReportConfigEutra::MS480;
...
void
NistA2A4RsrqHandoverAlgorithm::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
void
NistA2A4RsrqHandoverAlgorithm::DoReportUeMeas (uint16_t rnti,
...
void
NistA2A4RsrqHandoverAlgorithm::EvaluateHandover (uint16_t rnti,
...
} // end of EvaluateNistMeasurementReport


bool
NistA2A4RsrqHandoverAlgorithm::IsValidNeighbour (uint16_t cellId)
{
...
void
NistA2A4RsrqHandoverAlgorithm::UpdateNeighbourMeasurements (uint16_t rnti,
                                                        uint16_t cellId,
                                                        uint8_t rsrq)
{
  ...
  if (it2 != it1->second.end ())
  ...
  else
    {
      ...
      neighbourCellMeasures = Create<NistUeMeasure> ();
      neighbourCellMeasures->m_cellId = cellId;
      neighbourCellMeasures->m_rsrp = 0;
      neighbourCellMeasures->m_rsrq = rsrq;
