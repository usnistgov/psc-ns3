...
NS_LOG_COMPONENT_DEFINE ("NistLteAnr");

NS_OBJECT_ENSURE_REGISTERED (NistLteAnr);


NistLteAnr::NistLteAnr (uint16_t servingCellId)
  : m_anrSapUser (0),
    m_threshold (0),
    m_measId (0),
    m_servingCellId (servingCellId)
{
  ...
  m_anrSapProvider = new MemberNistLteAnrSapProvider<NistLteAnr> (this);
}


NistLteAnr::~NistLteAnr ()
{
...
TypeId
NistLteAnr::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::NistLteAnr")
    .SetParent<Object> ()
    .AddAttribute ("Threshold",
                   "Minimum RSRQ range value required for detecting a neighbour cell",
                   UintegerValue (0),
                   MakeUintegerAccessor (&NistLteAnr::m_threshold),
...
void
NistLteAnr::AddNeighbourRelation (uint16_t cellId)
{
  ...
  NistNeighbourRelation_t neighbourRelation;
...
void
NistLteAnr::RemoveNeighbourRelation (uint16_t cellId)
{
  NS_LOG_FUNCTION (this << m_servingCellId << cellId);
...
void
NistLteAnr::SetNistLteAnrSapUser (NistLteAnrSapUser* s)
{
...
NistLteAnrSapProvider*
NistLteAnr::GetNistLteAnrSapProvider ()
{
...
NistLteAnr::DoInitialize ()
{
  ...
  reportConfig.eventId = NistLteRrcSap::NistReportConfigEutra::EVENT_A4;
  ...
  reportConfig.reportInterval = NistLteRrcSap::NistReportConfigEutra::MS480;
  m_measId = m_anrSapUser->AddUeMeasReportConfigForAnr (reportConfig);
}


void
NistLteAnr::DoDispose ()
{
  NS_LOG_FUNCTION (this);
...
void
NistLteAnr::DoReportUeMeas (NistLteRrcSap::NistMeasResults measResults)
{
...
void
NistLteAnr::DoAddNeighbourRelation (uint16_t cellId)
{
...
bool
NistLteAnr::DoGetNoRemove (uint16_t cellId) const
{
  NS_LOG_FUNCTION (this << m_servingCellId << cellId);
...
bool
NistLteAnr::DoGetNoHo (uint16_t cellId) const
{
  NS_LOG_FUNCTION (this << m_servingCellId << cellId);
...
bool
NistLteAnr::DoGetNoX2 (uint16_t cellId) const
{
  NS_LOG_FUNCTION (this << m_servingCellId << cellId);
...
const NistLteAnr::NistNeighbourRelation_t *
NistLteAnr::Find (uint16_t cellId) const
{
