...
#ifndef NIST_LTE_STATS_CALCULATOR_H_
#define NIST_LTE_STATS_CALCULATOR_H_
...
class NistLteStatsCalculator : public Object
{
public:
  /**
   * Constructor
   */
  NistLteStatsCalculator ();

  /**
   * Destructor
   */
  virtual ~NistLteStatsCalculator ();
...
protected:
  ...
  /**
   * Retrieves IMSI from NistLteNetDevice path in the attribute system
   * @param path Path in the attribute system to get
   * @return the IMSI associated with the given path
   */
  static uint64_t FindImsiFromNistLteNetDevice (std::string path);
...
#endif /* NIST_LTE_STATS_CALCULATOR_H_ */
