...
#include <ns3/nist-rem-spectrum-phy.h>
...
NS_LOG_COMPONENT_DEFINE ("NistRadioEnvironmentMapHelper");

NS_OBJECT_ENSURE_REGISTERED (NistRadioEnvironmentMapHelper);

NistRadioEnvironmentMapHelper::NistRadioEnvironmentMapHelper ()
{
}
...
void
NistRadioEnvironmentMapHelper::DoDispose ()
{
  NS_LOG_FUNCTION (this);
}

TypeId
NistRadioEnvironmentMapHelper::GetTypeId (void)
{
  NS_LOG_FUNCTION ("NistRadioEnvironmentMapHelper::GetTypeId");
  static TypeId tid = TypeId ("ns3::NistRadioEnvironmentMapHelper")
    .SetParent<Object> ()
    .AddConstructor<NistRadioEnvironmentMapHelper> ()
    .AddAttribute ("ChannelPath", "The path to the channel for which the Radio Environment Map is to be generated",
                   ...
                   MakeStringAccessor (&NistRadioEnvironmentMapHelper::m_channelPath),
                   MakeStringChecker ())
    .AddAttribute ("OutputFile", "the filename to which the Radio Environment Map is saved",
                   StringValue ("rem.out"),
                   MakeStringAccessor (&NistRadioEnvironmentMapHelper::m_outputFile),
                   MakeStringChecker ())
    .AddAttribute ("XMin", "The min x coordinate of the map.",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&NistRadioEnvironmentMapHelper::m_xMin),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("YMin", "The min y coordinate of the map.",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&NistRadioEnvironmentMapHelper::m_yMin),
                   MakeDoubleChecker<double> ())
   .AddAttribute ("XMax", "The max x coordinate of the map.",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&NistRadioEnvironmentMapHelper::m_xMax),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("YMax", "The max y coordinate of the map.",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&NistRadioEnvironmentMapHelper::m_yMax),
                   MakeDoubleChecker<double> ())
   .AddAttribute ("XRes", "The resolution (number of points) of the map along the x axis.",
                   UintegerValue (100),
                   MakeUintegerAccessor (&NistRadioEnvironmentMapHelper::m_xRes),
    ...
    .AddAttribute ("YRes", "The resolution (number of points) of the map along the y axis.",
                   UintegerValue (100),
                   MakeUintegerAccessor (&NistRadioEnvironmentMapHelper::m_yRes),
    ...
    .AddAttribute ("Z", "The value of the z coordinate for which the map is to be generated",
		   DoubleValue (0.0),
                   MakeDoubleAccessor (&NistRadioEnvironmentMapHelper::m_z),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("StopWhenDone", "If true, Simulator::Stop () will be called as soon as the REM has been generated",
		   BooleanValue (true),
                   MakeBooleanAccessor (&NistRadioEnvironmentMapHelper::m_stopWhenDone),
                   MakeBooleanChecker ())
    .AddAttribute ("NoisePower",
                   ...
                   MakeDoubleAccessor (&NistRadioEnvironmentMapHelper::m_noisePower),
                   MakeDoubleChecker<double> ())
...
uint8_t 
NistRadioEnvironmentMapHelper::GetBandwidth () const
{
  return m_bandwidth;
}

void 
NistRadioEnvironmentMapHelper::SetBandwidth (uint8_t bw)
{
  switch (bw)
    { 
    case 6:
...
void 
NistRadioEnvironmentMapHelper::Install ()
{
  NS_LOG_FUNCTION (this);
  if (!m_rem.empty ())
    {
      NS_FATAL_ERROR ("only one REM supported per instance of NistRadioEnvironmentMapHelper");
    }
  ...
  Simulator::Schedule (Seconds (startDelay),
                       &NistRadioEnvironmentMapHelper::DelayedInstall,
                       this);
}
...
NistRadioEnvironmentMapHelper::DelayedInstall ()
{
  ...
  for (uint32_t i = 0; i < m_maxPointsPerIteration; ++i)
    {
      NistRemPoint p;
      p.phy = CreateObject<NistRemSpectrumPhy> ();
      ...
      p.phy->SetRxSpectrumModel (NistLteSpectrumValueHelper::GetSpectrumModel (m_earfcn, m_bandwidth));
          ...
          if ((numPointsCurrentIteration == m_maxPointsPerIteration)
              || ((x > m_xMax - 0.5*m_xStep) && (y > m_yMax - 0.5*m_yStep)) )
            {
              Simulator::Schedule (Seconds (remIterationStartTime), 
                                   &NistRadioEnvironmentMapHelper::RunOneIteration,
  ...
  Simulator::Schedule (Seconds (remIterationStartTime), 
                       &NistRadioEnvironmentMapHelper::Finalize,
                       this);
}

  
void 
NistRadioEnvironmentMapHelper::RunOneIteration (double xMin, double xMax, double yMin, double yMax)
{
  NS_LOG_FUNCTION (this << xMin << xMax << yMin << yMax);
  std::list<NistRemPoint>::iterator remIt = m_rem.begin ();
  double x;
  double y;
  ...
  if (remIt != m_rem.end ())
    {
      ...
      NS_LOG_LOGIC ("deactivating NistRemSpectrumPhys that are unneeded in the last iteration");
  ...
  Simulator::Schedule (Seconds (0.0005), &NistRadioEnvironmentMapHelper::PrintAndReset, this);  
}

void 
NistRadioEnvironmentMapHelper::PrintAndReset ()
{
  NS_LOG_FUNCTION (this);
  
  for (std::list<NistRemPoint>::iterator it = m_rem.begin ();
       it != m_rem.end ();
       ++it)
    {
      if (!(it->phy->IsActive ()))
        {
          // should occur only upon last iteration when some NistRemPoint
