...
NS_LOG_COMPONENT_DEFINE ("NistLteProseHelper");


NS_OBJECT_ENSURE_REGISTERED (NistLteProseHelper);

NistLteProseHelper::NistLteProseHelper ()
{
  NS_LOG_FUNCTION (this);
}

NistLteProseHelper::~NistLteProseHelper (void)
{
  NS_LOG_FUNCTION (this);
}

TypeId NistLteProseHelper::GetTypeId (void)
{
  static TypeId
    tid =
    TypeId ("ns3::NistLteProseHelper")
    .SetParent<Object> ()
    .AddConstructor<NistLteProseHelper> ()
...
void 
NistLteProseHelper::SetLteHelper (Ptr<NistLteHelper> h)
{
...
NetDeviceContainer 
NistLteProseHelper::RemoveNetDevice (NetDeviceContainer container, Ptr<NetDevice> item)
{
...
NistLteProseHelper::AssociateForGroupcast (double txPower, double ulEarfcn, double ulBandwidth, NetDeviceContainer ues, double rsrpThreshold, int ngroups, int nreceivers, SrsrpMethod_t compMethod)
{
      ...
      while (numReceiversAssociated < nreceivers && candidateRx.GetN() > 0)
        {
          ...
          double rsrpRx = 0;
          if (compMethod == NistLteProseHelper::SRSRP_STD)
            {
...
std::vector < NetDeviceContainer > 
NistLteProseHelper::AssociateForBroadcast (double txPower, double ulEarfcn, double ulBandwidth, NetDeviceContainer ues, double rsrpThreshold, uint32_t ntransmitters, SrsrpMethod_t compMethod)
{
  std::vector < NetDeviceContainer > groups; //groups created
  Ptr<UniformRandomVariable> ueSelector = CreateObject<UniformRandomVariable> (); //random variable to select Tx/Rx nodes
...
std::vector < NetDeviceContainer > 
NistLteProseHelper::AssociateForBroadcastWithTxEnabledToReceive (double txPower, double ulEarfcn, double ulBandwidth, NetDeviceContainer ues, double rsrpThreshold, uint32_t ntransmitters, SrsrpMethod_t compMethod)
{
  std::vector < NetDeviceContainer > groups; //groups created
  Ptr<UniformRandomVariable> ueSelector = CreateObject<UniformRandomVariable> (); //random variable to select Tx/Rx nodes
...
std::vector < NetDeviceContainer > 
NistLteProseHelper::AssociateForBroadcastWithWrapAround (double txPower, double ulEarfcn, double ulBandwidth, NetDeviceContainer ues, double rsrpThreshold, uint32_t ntransmitters, Ptr<NistLteHexGridEnbTopologyHelper> topologyHelper, SrsrpMethod_t compMethod)
{
  std::vector < NetDeviceContainer > groups; //groups created
  Ptr<UniformRandomVariable> ueSelector = CreateObject<UniformRandomVariable> (); //random variable to select Tx/Rx nodes
...
void
NistLteProseHelper::PrintGroups (std::vector < NetDeviceContainer > groups)
{
...
void
NistLteProseHelper::PrintGroups (std::vector < NetDeviceContainer > groups, Ptr< OutputStreamWrapper > stream)
{
		  ...
		  for (uint32_t i = 1; i < (*gIt).GetN(); i++)
			{
			  ...
			  *stream->GetStream () << (*gIt).Get (0)->GetNode()->GetId() << "\t" << (*gIt).Get (i)->GetNode()->GetId() << "\t" << (*gIt).Get(0)->GetObject<NistLteUeNetDevice> ()->GetImsi () << "\t" << (*gIt).Get(i)->GetObject<NistLteUeNetDevice> ()->GetImsi () <<std::endl;
			} 
        }
      }
}
...
void 
NistLteProseHelper::ActivateSidelinkBearer (Time activationTime, NetDeviceContainer ues, Ptr<NistSlTft> tft)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_lteHelper, "Sidelink activation requires LteHelper to be registered with the NistLteProseHelper");
  Simulator::Schedule (activationTime, &NistLteProseHelper::DoActivateSidelinkBearer, this, ues, tft);
}

void 
NistLteProseHelper::DoActivateSidelinkBearer (NetDeviceContainer ues, Ptr<NistSlTft> tft)
{
  NS_LOG_FUNCTION (this);
