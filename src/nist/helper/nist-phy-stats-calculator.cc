...
NS_LOG_COMPONENT_DEFINE ("NistPhyStatsCalculator");

NS_OBJECT_ENSURE_REGISTERED (NistPhyStatsCalculator);

NistPhyStatsCalculator::NistPhyStatsCalculator ()
...
TypeId
NistPhyStatsCalculator::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NistPhyStatsCalculator")
    .SetParent<NistLteStatsCalculator> ()
    .AddConstructor<NistPhyStatsCalculator> ()
    .AddAttribute ("DlRsrpSinrFilename",
                   ...
                   MakeStringAccessor (&NistPhyStatsCalculator::SetCurrentCellRsrpSinrFilename),
                   MakeStringChecker ())
    .AddAttribute ("UlSinrFilename",
                   "Name of the file where the UE SINR statistics will be saved.",
                   StringValue ("UlSinrStats.txt"),
                   MakeStringAccessor (&NistPhyStatsCalculator::SetUeSinrFilename),
                   MakeStringChecker ())
    .AddAttribute ("UlInterferenceFilename",
                   ...
                   MakeStringAccessor (&NistPhyStatsCalculator::SetInterferenceFilename),
                   MakeStringChecker ())
  ;
  return tid;
}
...
void
NistPhyStatsCalculator::SetCurrentCellRsrpSinrFilename (std::string filename)
{
  m_RsrpSinrFilename = filename;
}

std::string
NistPhyStatsCalculator::GetCurrentCellRsrpSinrFilename (void)
{
  return m_RsrpSinrFilename;
}

void
NistPhyStatsCalculator::SetUeSinrFilename (std::string filename)
{
  m_ueSinrFilename = filename;
}

std::string
NistPhyStatsCalculator::GetUeSinrFilename (void)
{
  return m_ueSinrFilename;
}

void
NistPhyStatsCalculator::SetInterferenceFilename (std::string filename)
{
  m_interferenceFilename = filename;
}

std::string
NistPhyStatsCalculator::GetInterferenceFilename (void)
{
...
void
NistPhyStatsCalculator::ReportCurrentCellRsrpSinr (uint16_t cellId, uint64_t imsi, uint16_t rnti,
...
void
NistPhyStatsCalculator::ReportUeSinr (uint16_t cellId, uint64_t imsi, uint16_t rnti,  double sinrLinear)
{
...
void
NistPhyStatsCalculator::ReportInterference (uint16_t cellId, Ptr<SpectrumValue> interference)
{
...
void
NistPhyStatsCalculator::ReportCurrentCellRsrpSinrCallback (Ptr<NistPhyStatsCalculator> phyStats,
...
void
NistPhyStatsCalculator::ReportUeSinr (Ptr<NistPhyStatsCalculator> phyStats, std::string path,
              uint16_t cellId, uint16_t rnti, double sinrLinear)
{
  ...
  std::string pathEnbMac  = path.substr (0, path.find ("NistLteEnbPhy/ReportUeSinr"));
  pathEnbMac += "NistLteEnbMac/DlScheduling";
...
void
NistPhyStatsCalculator::ReportInterference (Ptr<NistPhyStatsCalculator> phyStats, std::string path,
