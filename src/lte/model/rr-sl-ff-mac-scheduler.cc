...
TypeId
RrSlFfMacScheduler::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::RrSlFfMacScheduler")
...
void
RrSlFfMacScheduler::DoCschedCellConfigReq(
...
void
RrSlFfMacScheduler::DoCschedUeConfigReq(
    const struct FfMacCschedSapProvider::CschedUeConfigReqParameters& params)
{
    ...
    auto ueIt = m_uesDestinations.find(params.m_rnti);
    if (ueIt == m_uesDestinations.end())
    {
        m_uesDestinations.insert(
            std::pair<uint16_t, std::vector<uint32_t>>(params.m_rnti, params.m_slDestinations));
    }
    else
    {
...
void
RrSlFfMacScheduler::DoCschedLcReleaseReq(
    const struct FfMacCschedSapProvider::CschedLcReleaseReqParameters& params)
{
    NS_LOG_FUNCTION(this);
    for (uint32_t i = 0; i < params.m_logicalChannelIdentity.size(); i++)
    {
...
RrSlFfMacScheduler::DoCschedPoolConfigReq(
    const struct FfMacCschedSapProvider::CschedPoolConfigReqParameters& params)
{
    ...
    auto it = m_poolAllocations.find(params.m_group);
...
void
RrSlFfMacScheduler::DoCschedPoolReleaseReq(
    const struct FfMacCschedSapProvider::CschedPoolReleaseReqParameters& params)
{
    NS_LOG_FUNCTION(this << params.m_group);
    auto it = m_poolAllocations.find(params.m_group);
...
void
RrSlFfMacScheduler::DoCschedDiscPoolConfigReq(
    const struct FfMacCschedSapProvider::CschedDiscPoolConfigReqParameters& params)
{
    NS_LOG_FUNCTION(this);
...
void
RrSlFfMacScheduler::DoCschedDiscPoolReleaseReq(
    const struct FfMacCschedSapProvider::CschedDiscPoolReleaseReqParameters& params)
{
    NS_LOG_FUNCTION(this);
    NS_FATAL_ERROR("method not implemented");
}
...
void
RrSlFfMacScheduler::DoSchedDlRlcBufferReq(
...
void
RrSlFfMacScheduler::DoSchedDlMacBufferReq(
...
bool
RrSlFfMacScheduler::SortRlcBufferReq(FfMacSchedSapProvider::SchedDlRlcBufferReqParameters i,
...
RrSlFfMacScheduler::HarqProcessAvailability(uint16_t rnti)
{
    ...
    // Return true if it is a valid HARQ proc id
...
void
RrSlFfMacScheduler::DoSchedDlTriggerReq(
    const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    for (uint32_t i = 0; i < m_dlInfoListBuffered.size(); i++)
    {
        ...
        if (retx.at(0) || retx.at(1))
        {
            ...
            if (rv == 3)
            {
                ...
                for (uint32_t k = 0; k < (*itRlcPdu).second.size(); k++)
                {
            ...
            bool free = true;
            for (uint32_t j = 0; j < dciRbg.size(); j++)
            {
            ...
            else
            {
                ...
                if (j == dciRbg.size())
                {
                    ...
                    uint32_t rbgMask = 0;
                    for (uint32_t k = 0; k < dciRbg.size(); k++)
                    {
            ...
            for (uint32_t k = 0; k < (*itRlcPdu).second.at(0).at(dci.m_harqProcess).size(); k++)
            {
    ...
    do
    {
        ...
        uint8_t nLayer = TransmissionModesLayers::TxMode2LayerNum((*itTxMode).second);
...
void
RrSlFfMacScheduler::DoSchedDlRachInfoReq(
...
void
RrSlFfMacScheduler::DoSchedDlCqiInfoReq(
    ...
    const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
    ...
    for (poolIt = m_poolAllocations.begin(); poolIt != m_poolAllocations.end(); poolIt++)
    {
        if (!poolIt->second.m_init)
        {
            ...
            poolIt->second.m_nextScPeriod =
                poolIt->second.m_pool->GetNextScPeriod(frameNo - 1, subframeNo - 1);
            ...
            NS_LOG_INFO(this << " Init SC period for pool of group " << poolIt->first
                             << ". Next period at " << poolIt->second.m_nextScPeriod.frameNo << "/"
                             << poolIt->second.m_nextScPeriod.subframeNo);
        ...
        for (it = poolIt->second.m_ceSlBsrRxed.begin(); it != poolIt->second.m_ceSlBsrRxed.end();
             it++)
        {
            if (poolIt->second.m_nextAllocation.find((*it).first) ==
                poolIt->second.m_nextAllocation.end())
            {
                // new allocation
                ...
                alloc.m_psschTx = poolIt->second.m_pool->GetPsschTransmissions(tmp,
                                                                               alloc.m_slItrp,
                                                                               alloc.m_rbStart,
                                                                               alloc.m_rbLen);

                for (auto txIt = alloc.m_psschTx.begin(); txIt != alloc.m_psschTx.end(); txIt++)
                {
                ...
                poolIt->second.m_nextAllocation.insert(
                    std::pair<uint16_t, PoolUserAllocation>(it->first, alloc));
                NS_LOG_INFO(this << " Adding Sidelink allocation for RNTI " << it->first
                                 << " in pool " << poolIt->first
                                 << " RbStart=" << (uint32_t)alloc.m_rbStart
                                 << ", rbLen=" << (uint32_t)alloc.m_rbLen
                                 << ", itrp=" << (uint32_t)alloc.m_slItrp);

                SlDciListElement_s dci;
                dci.m_rnti = alloc.m_rnti;
                dci.m_hopping = false;
                dci.m_hoppingInfo =
                    (0xFF); // frequency hopping not supported for Mode 1. Disable frequency hopping
                ...
                for (auto txIt = alloc.m_psschTx.begin(); txIt != alloc.m_psschTx.end(); txIt++)
                {
                    NS_LOG_INFO(this << " Subframe " << txIt->subframe.frameNo << "/"
                                     << txIt->subframe.subframeNo
            ...
            // a enhanced version would adapt the size of the allocation depending on the buffer
            // status
            else
            {
    ...
    for (poolIt = m_poolAllocations.begin(); poolIt != m_poolAllocations.end(); poolIt++)
    {
        if (frameNo == poolIt->second.m_nextScPeriod.frameNo &&
            subframeNo == poolIt->second.m_nextScPeriod.subframeNo)
        {
            // move allocation
            ...
            poolIt->second.m_nextScPeriod.frameNo++;
            poolIt->second.m_nextScPeriod.subframeNo++;
            NS_LOG_INFO(this << " Starting new SC period for pool of group " << poolIt->first
                             << ". Next period at " << poolIt->second.m_nextScPeriod.frameNo << "/"
                             << poolIt->second.m_nextScPeriod.subframeNo);
            NS_LOG_INFO(this << " Number of UEs allocated = "
                             << poolIt->second.m_currentAllocation.size());
        }
        // check if there are PSCCH allocations in this subframe
        std::list<uint8_t> rbPscch =
            poolIt->second.m_pool->GetPscchOpportunities(frameNo - 1, subframeNo - 1);
        ...
        for (auto userIt = poolIt->second.m_currentAllocation.begin();
             userIt != poolIt->second.m_currentAllocation.end();
             userIt++)
        {
            // check if this user is transmitting in this subframe
            auto allocIt = userIt->second.m_psschTx.begin();
            if (allocIt != userIt->second.m_psschTx.end() &&
                (*allocIt).subframe.frameNo == frameNo &&
                (*allocIt).subframe.subframeNo == subframeNo)
            {
                // User transmitting in this subframe, reserve RBs
                NS_LOG_INFO(this << " Reserving RBs for Sidelink PSSCH from "
                                 << (uint32_t)(*allocIt).rbStart << " to "
                                 << (uint32_t)((*allocIt).rbStart + (*allocIt).nbRb - 1));
    ...
    if (m_harqOn)
    {
        ...
        for (uint32_t i = 0; i < params.m_ulInfoList.size(); i++)
        {
    ...
    if (nflows == 0)
    {
        if (!ret.m_dciList.empty() || !ret.m_sldciList.empty())
        {
...
void
RrSlFfMacScheduler::DoSchedUlNoiseInterferenceReq(
...
void
RrSlFfMacScheduler::DoSchedUlSrInfoReq(
...
void
RrSlFfMacScheduler::DoSchedUlMacCtrlInfoReq(
    const struct FfMacSchedSapProvider::SchedUlMacCtrlInfoReqParameters& params)
{
    ...
    for (unsigned int i = 0; i < params.m_macCeList.size(); i++)
    {
        ...
        else if (params.m_macCeList.at(i).m_macCeType == MacCeListElement_s::SLBSR)
        {
            ...
            auto ueIt = m_uesDestinations.find(rnti);
            NS_ASSERT_MSG(ueIt != m_uesDestinations.end(), "No destinations found for " << rnti);

            NS_ASSERT_MSG(!ueIt->second.empty(), "Destinations empty for " << rnti);

            // check the report
            for (uint32_t dest = 0; dest < ueIt->second.size(); ++dest)
            {
                ...
                if (buffer > 0)
                {
                    ...
                    auto poolIt = m_poolAllocations.find(destination);
                    NS_ASSERT_MSG(poolIt != m_poolAllocations.end(),
                                  "Sidelink destination " << destination << "unknown.");

                    itSlBsr = poolIt->second.m_ceSlBsrRxed.find(rnti);
                    if (itSlBsr == poolIt->second.m_ceSlBsrRxed.end())
                    {
                        // create the new entry
                        poolIt->second.m_ceSlBsrRxed.insert(
                            std::pair<uint16_t, uint32_t>(rnti, buffer));
                        NS_LOG_INFO(this << " Insert RNTI " << rnti << " Sidelink queue "
                                         << buffer);
                    }
                    else
                    {
                        ...
                        NS_LOG_INFO(this << " Update RNTI " << rnti << " Sidelink queue "
                                         << buffer);
                    }
                }
            }
    ...
    const struct FfMacSchedSapProvider::SchedUlCqiInfoReqParameters& params)
{
    ...
    case UlCqi_s::SRS: {
        ...
        for (uint32_t i = 0; i < params.m_vendorSpecificList.size(); i++)
        {
