    ...
    const struct FfMacCschedSapProvider::CschedUeConfigReqParameters& params)
{
    ...
    std::map<uint16_t, std::vector<uint32_t>>::iterator ueIt =
        m_uesDestinations.find(params.m_rnti);
    ...
    const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
             ...
             it++)
        {
                ...
                poolIt->second.m_nextAllocation.end())
            {
                ...
                for (std::list<SidelinkCommResourcePool::SidelinkTransmissionInfo>::iterator txIt =
                         alloc.m_psschTx.begin();
                     txIt != alloc.m_psschTx.end();
                     txIt++)
                {
    ...
    for (poolIt = m_poolAllocations.begin(); poolIt != m_poolAllocations.end(); poolIt++)
    {
        ...
        for (std::map<uint16_t, PoolUserAllocation>::iterator userIt =
                 poolIt->second.m_currentAllocation.begin();
        ...
        else if (params.m_macCeList.at(i).m_macCeType == MacCeListElement_s::SLBSR)
        {
            ...
            for (uint8_t dest = 0; dest < ueIt->second.size(); ++dest)
            {
                ...
                if (buffer > 0)
                {
                    ...
                    std::map<uint32_t, PoolInfo>::iterator poolIt =
                        m_poolAllocations.find(destination);
