...
class LteSlUeRrc : public Object
{
    ...
    struct LteSlCellConfiguration
    {
        ...
        bool haveSib18;  ///< Have Sib18 ? May need to be replaced with another struct containing
                         ///< frame information
  ...
  public:
    ...
    struct AppServiceInfo
    {
        ...
        Time lastRspTimestamp; ///< time of the last response, used to avoid sending multiple times
                               ///< in the same sidelink period
    };
  ...
  public:
     ...
     */
    uint8_t GetDiscTxResources() const;
    /**
     ...
     */
    uint16_t GetDiscInterFreq() const;
    /**
     ...
     * Returns the list of Sidelink communication destinations this UE is interested in transmitting
     * to \return the list of destinations
     */
     ...
     */
    void StartRelayService(uint32_t serviceCode,
     ...
     * \param [in] p
     */
    typedef void (*PC5SignalingPacketTracedCallback)(uint32_t srcL2Id,
                                                     uint32_t dstL2Id,
                                                     Ptr<Packet> p);
  ...
  protected:
    ...
    /**
     * Process received Direct Communication Request message in One to One Relay Direct
     * Communication State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU
     * packet
     */
    void ProcessDirectCommunicationRequest(uint32_t L2Id, Ptr<Packet> pdcpSdu);

    /**
     * Process received Direct Communication Accept message in One to One Relay Direct Communication
     * State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU packet
     */
    void ProcessDirectCommunicationAccept(uint32_t L2Id, Ptr<Packet> pdcpSdu);

    /**
     * Process received Direct Communication Reject message in One to One Relay Direct Communication
     * State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU packet
     */
    void ProcessDirectCommunicationReject(uint32_t L2Id, Ptr<Packet> pdcpSdu);

    /**
     * Process received Direct Communication Keepalive message in One to One Relay Direct
     * Communication State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU
     * packet
     */
    ...
    /**
     * Process received Direct Communication Keepalive Acknowledgment message in One to One Relay
     * Direct Communication State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received
     * PDCP SDU packet
     */
    void ProcessDirectCommunicationKeepaliveAck(uint32_t L2Id, Ptr<Packet> pdcpSdu);

    /**
     * Process received Direct Communication Release message in One to One Relay Direct
     * Communication State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU
     * packet
     */
    ...
    /**
     * Process received Direct Communication Release Accept message in One to One Relay Direct
     * Communication State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU
     * packet
     */
    ...
    /**
     * Process received Direct Security Mode Command message in One to One Relay Direct
     * Communication State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU
     * packet
     */
    ...
    /**
     * Process received Direct Security Mode Complete message in One to One Relay Direct
     * Communication State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU
     * packet
     */
    ...
    /**
     * Process received Direct Security Mode Reject message in One to One Relay Direct Communication
     * State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU packet
     */
    void ProcessDirectSecurityModeReject(uint32_t L2Id, Ptr<Packet> pdcpSdu);

    /**
     * Process received Direct Rekeying Request message in One to One Relay Direct Communication
     * State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU packet
     */
    void ProcessDirectRekeyingRequest(uint32_t L2Id, Ptr<Packet> pdcpSdu);

    /**
     * Process received Direct Rekeying Response message in One to One Relay Direct Communication
     * State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU packet
     */
    void ProcessDirectRekeyingResponse(uint32_t L2Id, Ptr<Packet> pdcpSdu);

    /**
     * Process received Direct Rekeying Trigger message in One to One Relay Direct Communication
     * State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU packet
     */
    void ProcessDirectRekeyingTrigger(uint32_t L2Id, Ptr<Packet> pdcpSdu);

    /**
     * Process received Remote UE Info Request message in One to One Relay Direct Communication
     * State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU packet
     */
    void ProcessRemoteUeInfoRequest(uint32_t L2Id, Ptr<Packet> pdcpSdu);

    /**
     * Process received Remote UE Info Response message in One to One Relay Direct Communication
     * State Machine \param L2Id The Layer 2 ID \param pdcpSdu The received PDCP SDU packet
     */
     ...
     */
    void ReleaseO2OConnection(Ptr<LteSlO2oCommParams> context,
    ...
    GetO2OContext(uint32_t peerUeId, uint32_t contextId);
  ...
  private:
     ...
     */
    std::vector<std::pair<LteRrcSap::SlCommResourcePool, Ptr<SidelinkRxCommResourcePool>>>
        rxPools; ///<
    /**
     ...
     */
    std::vector<std::pair<LteRrcSap::SlDiscResourcePool, Ptr<SidelinkRxDiscResourcePool>>>
        monitorPools;
    /**
