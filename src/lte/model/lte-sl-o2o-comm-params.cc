...
LteSlO2oCommParams::LteSlO2oCommParams()
{
    ...
    m_stateString.emplace_back("RELAY_IDLE");
    m_stateString.emplace_back("RELAY_SETUP_REQUEST");
    m_stateString.emplace_back("RELAY_SECURE_ESTABLISHED");
    m_stateString.emplace_back("RELAY_INIT_RELEASE");
    m_stateString.emplace_back("REMOTE_IDLE");
    m_stateString.emplace_back("REMOTE_INIT_SETUP");
    m_stateString.emplace_back("REMOTE_SECURE_ESTABLISHED");
    m_stateString.emplace_back("REMOTE_INIT_RELEASE");
...
void
LteSlO2oCommParams::DoInitialize()
{
    NS_LOG_FUNCTION(this);
}

TypeId
LteSlO2oCommParams::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::LteSlO2oCommParams")
            .SetParent<Object>()
            .AddConstructor<LteSlO2oCommParams>()
            .AddAttribute("RelayT4111",
                          "Duration of Timer 4111 (Wait for Direct Security Mode Complete) for "
                          "Relay UE in milliseconds",
                          UintegerValue(200),
                          MakeUintegerAccessor(&LteSlO2oCommParams::SetRelayT4111Duration),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("RelayT4103",
                          "Duration of Timer 4103 (Wait for Direct Communication Release Accept) "
                          "for Relay UE in milliseconds",
                          UintegerValue(100),
                          MakeUintegerAccessor(&LteSlO2oCommParams::SetRelayT4103Duration),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("RelayT4108",
                          "Duration of Timer 4108 (Wait for Direct Communication Keepalive or PC5 "
                          "Data) for Relay UE in milliseconds",
                          UintegerValue(2000),
                          MakeUintegerAccessor(&LteSlO2oCommParams::SetRelayT4108Duration),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("RelayTRUIR",
                          "Duration of Remote UE Info Request Timer (Wait for Remote UE Info "
                          "Response) for Relay UE in milliseconds",
                          UintegerValue(100),
            ...
            .AddAttribute("RemoteT4100",
                          "Duration of Timer 4100 (Wait for Direct Communication Accept) for "
                          "Remote UE in milliseconds",
                          UintegerValue(400),
                          MakeUintegerAccessor(&LteSlO2oCommParams::SetRemoteT4100Duration),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute(
                "RemoteT4101",
                "Duration of Timer 4101 (Wait for Direct Communication Keepalive Ack or PC5 Data "
                "for Direct Communication Keepalive retransmission) for Remote UE in milliseconds",
                UintegerValue(100),
                MakeUintegerAccessor(&LteSlO2oCommParams::SetRemoteT4101Duration),
                MakeUintegerChecker<uint32_t>())
            .AddAttribute("RemoteT4102",
                          "Duration of Timer 4102 (Wait for Direct Communication Keepalive "
                          "transmission) for Remote UE in milliseconds",
                          UintegerValue(1000),
                          MakeUintegerAccessor(&LteSlO2oCommParams::SetRemoteT4102Duration),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("RemoteT4103",
                          "Duration of Timer 4103 (Wait for Direct Communication Release Accept) "
                          "for Remote UE in milliseconds",
                          UintegerValue(100),
