...
class LteSlBasicUeController : public LteSlUeController
{
  public:
    ...
    ~LteSlBasicUeController() override;
    ...
    void SetLteSlUeCtrlSapUser(LteSlUeCtrlSapUser* s) override;
    LteSlUeCtrlSapProvider* GetLteSlUeCtrlSapProvider() override;
     ...
     */
    typedef void (*RelayUeSelectionTracedCallback)(uint64_t imsi,
    ...
    {
        RANDOM_NO_RESELECTION = 0, // Default; Random selection among valid Relay UEs only if not
     ...
     * \param [in] status
     * \param [in] interface index. Default to 0 which is not used for PC5. Valid only
     * SECURE_ESTABLISHED and TERMINATED
     */
    typedef void (*Pc5ConnectionStatusTracedCallback)(
        uint32_t selfUeId,
        uint32_t peerUeId,
  ...
  protected:
    ...
    TracedCallback<uint32_t,
                   uint32_t,
                   LteSlUeRrc::RelayRole,
                   LteSlBasicUeController::Pc5ConnectionStatus,
                   uint32_t>
        m_tracePc5ConnectionStatus;
