...
{
    static TypeId tid =
        TypeId("ns3::LteSlBasicUeController")
            ...
            .AddAttribute(
                "RelayUeSelectionAlgorithm",
                "The algorithm to be used to evaluate Relay UE (re)selection upon measurements "
                "report (RandomNoReselection/MaxSDRsrpNoReselection/MaxSDRsrp)",
                ...
                MakeEnumChecker(
                    LteSlBasicUeController::RelayUeSelectionAlgorithm::RANDOM_NO_RESELECTION,
                    "RandomNoReselection",
                    LteSlBasicUeController::RelayUeSelectionAlgorithm::MAX_SDRSRP_NO_RESELECTION,
                    "MaxSDRsrpNoReselection",
                    LteSlBasicUeController::RelayUeSelectionAlgorithm::MAX_SDRSRP,
                    "MaxSDRsrp"))
            .AddTraceSource(
                "RelayUeSelection",
...
void
LteSlBasicUeController::DoRecvRelayServiceDiscovery(uint32_t serviceCode,
...
void
LteSlBasicUeController::DoPc5ConnectionStarted(uint32_t peerUeId,
                                               uint32_t selfUeId,
                                               LteSlUeRrc::RelayRole role)
{
    NS_LOG_FUNCTION(this << peerUeId << selfUeId << role);
    m_tracePc5ConnectionStatus(selfUeId,
                               peerUeId,
                               role,
                               LteSlBasicUeController::Pc5ConnectionStatus::STARTED,
                               0);
}

void
LteSlBasicUeController::DoPc5SecuredEstablished(uint32_t peerUeId,
                                                uint32_t selfUeId,
                                                LteSlUeRrc::RelayRole role)
{
    NS_LOG_FUNCTION(this << peerUeId << selfUeId << role);
    ...
    if (it == m_lteSlUeNetDeviceMap.end())
    {
        ...
        m_tracePc5ConnectionStatus(selfUeId,
                                   peerUeId,
                                   role,
                                   LteSlBasicUeController::Pc5ConnectionStatus::SECURE_ESTABLISHED,
                                   ipInterfaceIndex);
    }
}

void
LteSlBasicUeController::DoPc5ConnectionTerminated(uint32_t peerUeId,
                                                  uint32_t selfUeId,
                                                  LteSlUeRrc::RelayRole role)
{
    NS_LOG_FUNCTION(this << peerUeId << selfUeId << role);
    ...
    m_tracePc5ConnectionStatus(selfUeId,
                               peerUeId,
                               role,
                               LteSlBasicUeController::Pc5ConnectionStatus::TERMINATED,
                               ipInterfaceIndex);
...
void
LteSlBasicUeController::DoPc5ConnectionAborted(uint32_t peerUeId,
                                               uint32_t selfUeId,
                                               LteSlUeRrc::RelayRole role,
                                               LteSlO2oCommParams::UeO2ORejectReason reason)
{
    ...
    m_tracePc5ConnectionStatus(selfUeId,
                               peerUeId,
                               role,
                               LteSlBasicUeController::Pc5ConnectionStatus::ABORTED,
                               0);
...
void
LteSlBasicUeController::DoRecvRemoteUeReport(uint64_t localImsi,
                                             uint32_t peerUeId,
                                             uint64_t remoteImsi)
{
...
uint64_t
LteSlBasicUeController::DoRelayUeSelection(std::map<uint64_t, double> validRelays,
                                           uint32_t serviceCode,
                                           uint64_t currentRelayId)
{
    NS_LOG_FUNCTION(this);
    NS_LOG_DEBUG("UE IMSI " << m_netDevice->GetRrc()->GetImsi() << " Current Relay UE ID "
                            << currentRelayId << " for SC " << serviceCode);
    ...
    else
    {
        ...
        case LteSlBasicUeController::RelayUeSelectionAlgorithm::RANDOM_NO_RESELECTION:
            ...
            // - The Relay UE ID of the Relay UE to which the UE is already connected
            {
                ...
                if (currentRelayId == 0 && !validRelays.empty())
                {
        ...
        case LteSlBasicUeController::RelayUeSelectionAlgorithm::MAX_SDRSRP_NO_RESELECTION:
            ...
            // - The Relay UE ID of the Relay UE to which the UE is already connected
            {
                ...
                if (currentRelayId == 0 && !validRelays.empty())
                {
                    ...
                    {
                        NS_LOG_DEBUG("  Relay UE ID " << vaReIt->first << " SD-RSRP "
                                                      << vaReIt->second);
        ...
        if (selectedRelayId != currentRelayId)
        {
            ...
            m_traceRelayUeSelection(m_netDevice->GetRrc()->GetImsi(),
                                    serviceCode,
                                    currentRelayId,
                                    selectedRelayId);
        }
