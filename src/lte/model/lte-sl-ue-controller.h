...
class LteSlUeController : public Object
{
  ...
  protected:
     ...
     */
    virtual void DoRecvRelayServiceDiscovery(uint32_t serviceCode,
     ...
     */
    virtual void DoPc5ConnectionStarted(uint32_t peerUeId,
                                        uint32_t selfUeId,
     ...
     * \param role The role of this node (remote or relay)
     */
    virtual void DoPc5SecuredEstablished(uint32_t peerUeId,
                                         uint32_t selfUeId,
                                         LteSlUeRrc::RelayRole role) = 0;
     ...
     * \param role The role of this node (remote or relay)
     */
    virtual void DoPc5ConnectionTerminated(uint32_t peerUeId,
                                           uint32_t selfUeId,
                                           LteSlUeRrc::RelayRole role) = 0;
     ...
     */
    virtual void DoPc5ConnectionAborted(uint32_t peerUeId,
                                        uint32_t selfUeId,
     ...
     */
    virtual void DoRecvRemoteUeReport(uint64_t localImsi,
                                      uint32_t peerUeId,
                                      uint64_t remoteImsi) = 0;
     ...
     * code \param serviceCode the relay service code \param currentRelayId the Id of the Realy UE
     * to which the UE is currently connected for the service code (Zero if none) \return
     * selectedRelayId the Id of the selected Realy UE
     */
    virtual uint64_t DoRelayUeSelection(std::map<uint64_t, double> validRelays,
