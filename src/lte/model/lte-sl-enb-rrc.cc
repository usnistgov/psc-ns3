...
TypeId
LteSlEnbRrc::GetTypeId()
{
    static TypeId tid = TypeId("ns3::LteSlEnbRrc").SetParent<Object>().AddConstructor<LteSlEnbRrc>()
...
LteSlEnbRrc::GetSystemInformationType18()
{
    ...
    for (it = m_activePoolMap.begin(); it != m_activePoolMap.end(); it++)
    {
        ...
        {
            // check single pool
            if (!IsPoolInList(it->second.m_poolSetup.scheduled.commTxConfig,
                              sib18.commConfig.commRxPool.pools,
                              sib18.commConfig.commRxPool.nbPools))
            {
                // add pool
                sib18.commConfig.commRxPool.pools[sib18.commConfig.commRxPool.nbPools] =
                    it->second.m_poolSetup.scheduled.commTxConfig;
                sib18.commConfig.commRxPool.nbPools++;
            }
        }
        else if (it->second.m_poolSetup.setup == LteRrcSap::SlCommTxResourcesSetup::UE_SELECTED)
        {
            if (it->second.m_poolSetup.ueSelected.havePoolToAdd)
            {
                ...
                {
                    if (!IsPoolInList(
                            it->second.m_poolSetup.ueSelected.poolToAddModList.pools[i].pool,
                            sib18.commConfig.commRxPool.pools,
                            sib18.commConfig.commRxPool.nbPools))
                    {
                        // add pool
        ...
        else
        {
            NS_FATAL_ERROR("Unable to find resource allocation type while building SIB 18."
                           " Please make sure to properly configure the resource allocation type "
                           "while configuring the pool");
        }
    }
    ...
    if (m_defaultPoolInfo.m_isSet)
    {
        ...
        {
            sib18.commConfig.commRxPool.pools[sib18.commConfig.commRxPool.nbPools] =
                m_defaultPoolInfo.m_poolSetup.scheduled.commTxConfig;
            sib18.commConfig.commRxPool.nbPools++;
        }
        else if (m_defaultPoolInfo.m_poolSetup.setup ==
                 LteRrcSap::SlCommTxResourcesSetup::UE_SELECTED)
        {
            sib18.commConfig.commRxPool.pools[sib18.commConfig.commRxPool.nbPools] =
                m_defaultPoolInfo.m_poolSetup.ueSelected.poolToAddModList.pools[0].pool;
            sib18.commConfig.commRxPool.nbPools++;
        }
    }
...
bool
LteSlEnbRrc::IsPoolInList(LteRrcSap::SlCommResourcePool pool,
                          LteRrcSap::SlCommResourcePool* pools,
                          int nbPool)
{
    bool found = false;
...
{
    NS_LOG_FUNCTION(this);
    m_preconfigDedicatedPoolMap.insert(
        std::pair<uint32_t, LteRrcSap::SlCommTxResourcesSetup>(group, pool));
}
...
LteSlEnbRrc::GetSystemInformationType19()
{
    ...
    { // check single pool
        if (!IsPoolInList(m_discPoolList.scheduled.discTxConfig,
                          sib19.discConfig.discRxPool.pools,
                          sib19.discConfig.discRxPool.nbPools))
        {
            // add pool
            sib19.discConfig.discRxPool.pools[sib19.discConfig.discRxPool.nbPools] =
                m_discPoolList.scheduled.discTxConfig;
            sib19.discConfig.discRxPool.nbPools++;
        }
    }
    else if (m_discPoolList.setup == LteRrcSap::SlDiscTxResourcesSetup::UE_SELECTED)
    {
        if (m_discPoolList.ueSelected.havePoolToAdd)
        {
            ...
            {
                if (!IsPoolInList(m_discPoolList.ueSelected.poolToAddModList.pools[i].pool,
                                  sib19.discConfig.discRxPool.pools,
                                  sib19.discConfig.discRxPool.nbPools))
                {
                    // add pool
                    sib19.discConfig.discRxPool.pools[sib19.discConfig.discRxPool.nbPools] =
                        m_discPoolList.ueSelected.poolToAddModList.pools[i].pool;
                    sib19.discConfig.discRxPool.nbPools++;
                }
            }
        }
    ...
    else
    {
        NS_FATAL_ERROR("Unable to find resource allocation type while building SIB 19."
                       " Please make sure to properly configure the resource allocation type while "
...
bool
LteSlEnbRrc::IsPoolInList(LteRrcSap::SlDiscResourcePool pool,
                          LteRrcSap::SlDiscResourcePool* pools,
                          int nbPool)
{
    bool found = false;
    for (int i = 0; i < nbPool && !found; i++)
...
void
LteSlEnbRrc::SetDefaultPool(LteRrcSap::SlCommTxResourcesSetup pool)
{
    NS_LOG_FUNCTION(this);
