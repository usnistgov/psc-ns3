...
TypeId
LteSlUeRrc::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::LteSlUeRrc")
            .SetParent<Object>()
            ...
            .AddAttribute("RelaySolFreq",
                          "Frequency of Model B Relay Discovery Solicitation messages (in number "
...
bool
LteSlUeRrc::IsDiscEnabled() const
{
...
LteSlUeRrc::IsTxInterested()
{
    ...
    return !m_slrbMap[m_sourceL2Id].empty();
}

bool
LteSlUeRrc::IsRxInterested()
{
    return !m_rxGroup.empty();
}

bool
LteSlUeRrc::IsMonitoringInterested()
{
    // Device is monitoring if model A/monitoring OR model B (since it needs to monitor for
    // requests/responses)
...
{
    // device is announcing if model A/announcing OR model B (since it needs to send
    // requests/responses) maybe we can use a variable that is update when calling start/stop
...
uint8_t
LteSlUeRrc::GetDiscTxResources() const
{
...
uint16_t
LteSlUeRrc::GetDiscInterFreq() const
{
...
LteSlUeRrc::AddSidelinkRadioBearer(Ptr<LteSidelinkRadioBearerInfo> slb)
{
    ...
    if (srcIt == m_slrbMap.end())
    {
        ...
        m_slrbMap.insert(
            std::pair<uint32_t, std::map<uint32_t, std::list<Ptr<LteSidelinkRadioBearerInfo>>>>(
                slb->m_sourceL2Id,
                empty));
    ...
    if (groupIt == m_slrbMap[slb->m_sourceL2Id].end())
    {
        ...
        m_slrbMap[slb->m_sourceL2Id].insert(
            std::pair<uint32_t, std::list<Ptr<LteSidelinkRadioBearerInfo>>>(slb->m_destinationL2Id,
                                                                            emptyList));
    }

    NS_LOG_LOGIC("Adding SLRB " << slb->m_sourceL2Id << "->" << slb->m_destinationL2Id
                                << " LCID=" << (uint16_t)slb->m_logicalChannelIdentity);
...
{
    NS_ASSERT_MSG(m_slrbMap[src][group].size() <= 1,
                  "Currently supporting only one SLRB per pair of source/destination");
    bool deleted = m_slrbMap[src].erase(group) > 0;
    if (m_slrbMap[src].empty())
    {
    ...
    if (itInfo != m_appServicesMap.end())
    {
        ...
        if (msgType == LteSlDiscHeader::DISC_RESTRICTED_QUERY && itInfo->second.role == Discoveree)
        {
            if (Simulator::Now().GetMilliSeconds() >
                itInfo->second.lastRspTimestamp.GetMilliSeconds() +
                    m_activeDiscTxPool->GetDiscPeriod())
            {
                TransmitApp(appCode);
...
void
LteSlUeRrc::StartRelayService(uint32_t serviceCode,
                              LteSlUeRrc::DiscoveryModel model,
                              LteSlUeRrc::RelayRole role)
{
    NS_LOG_FUNCTION(this << serviceCode << model << role);
    NS_ASSERT_MSG(m_relayServicesMap.find(serviceCode) == m_relayServicesMap.end(),
                  "Cannot add already existing service " << serviceCode);
...
LteSlUeRrc::StopRelayService(uint32_t serviceCode)
{
    ...
    // The following code is used to implement "Rx Upper, Tx DCR" type scenarios in the One-to-One
    // Communication State Machine When the Remote or the Relay decides to stop its Relay Service,
...
LteSlUeRrc::TransmitRelayMessage(uint32_t serviceCode)
{
    ...
    if (it->second.model == ModelA && it->second.role == RelayUE)
    {
        ...
        NS_LOG_DEBUG("period " << m_activeDiscTxPool->GetDiscPeriod() << ", " << period);
        it->second.txTimer =
            Simulator::Schedule(period, &LteSlUeRrc::TransmitRelayMessage, this, serviceCode);
    }
    else if (it->second.model == ModelB && it->second.role == RemoteUE)
    {
        ...
        NS_LOG_DEBUG(m_relaySolFreq << "x period "
                                    << m_relaySolFreq * m_activeDiscTxPool->GetDiscPeriod() << ", "
                                    << period);
        it->second.txTimer =
            Simulator::Schedule(period, &LteSlUeRrc::TransmitRelayMessage, this, serviceCode);
    }
...
LteSlUeRrc::IsMonitoringRelayServiceCode(uint8_t msgType, uint32_t serviceCode)
{
    ...
    // This function will probably have to be updated to include the role or message type we are
    // looking for for example in model B both devices are monitoring but not for the same type of
    // messages
    std::map<uint32_t, RelayServiceInfo>::iterator it;
    it = m_relayServicesMap.find(serviceCode);
    if (it != m_relayServicesMap.end())
...
void
LteSlUeRrc::RecvRelayServiceDiscovery(uint32_t serviceCode,
                                      uint64_t announcerInfo,
                                      uint32_t proseRelayUeId,
                                      uint8_t statusIndicator)
{
    NS_LOG_FUNCTION(this << serviceCode << announcerInfo << proseRelayUeId
                         << (uint16_t)statusIndicator);
    ...
    m_slUeCtrlSapProvider->RecvRelayServiceDiscovery(serviceCode,
                                                     announcerInfo,
                                                     proseRelayUeId,
                                                     statusIndicator);
}
...
void
LteSlUeRrc::RecvRelayServiceDiscovery(uint32_t serviceCode,
                                      uint64_t discovererInfo,
                                      uint8_t urdsComposition,
                                      uint32_t proseRelayUeId)
{
    NS_LOG_FUNCTION(this << serviceCode << discovererInfo << (uint16_t)urdsComposition
                         << proseRelayUeId);
    ...
    if (itInfo != m_relayServicesMap.end())
    {
        ...
        if (Simulator::Now().GetMilliSeconds() >
            itInfo->second.lastRspTimestamp.GetMilliSeconds() + m_activeDiscTxPool->GetDiscPeriod())
        {
...
LteSlUeRrc::StartRelayDirectCommunication(uint32_t serviceCode, uint32_t proseRelayUeId)
{
    ...
    NS_ASSERT_MSG(cIt == m_o2oCommContexts.end(),
                  "Cannot start relay communication when existing link exists");
    ...
    m_o2oCommContexts.insert(
        std::pair<LteSlO2oCommParams::LteSlPc5ContextId, Ptr<LteSlO2oCommParams>>(pc5Context,
                                                                                  o2ocp));
    ...
    if (GetSidelinkRadioBearer(m_sourceL2Id, proseRelayUeId) == nullptr)
    {
    ...
    else
    {
        ...
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Sent DirectCommunicationRequest to " << proseRelayUeId);
...
LteSlUeRrc::RecvPc5DataMessage(uint32_t srcL2Id, uint32_t dstL2Id, Ptr<Packet> pdcpSdu)
{
    ...
    if (dstL2Id == m_sourceL2Id)
    {
        ...
        for (it = m_o2oCommContexts.begin(); it != m_o2oCommContexts.end(); it++)
        {
            ...
            if (o2ocp->GetContextId().peerL2Id == srcL2Id &&
                (o2ocp->GetState() == LteSlO2oCommParams::RELAY_SECURE_ESTABLISHED ||
                 o2ocp->GetState() == LteSlO2oCommParams::REMOTE_SECURE_ESTABLISHED))
            {
                NS_LOG_DEBUG("In " << m_sourceL2Id << " ["
                                   << o2ocp->GetStateString(o2ocp->GetState())
                                   << "] Received PC5 User data from " << srcL2Id);
...
LteSlUeRrc::RecvPc5SignalingMessage(uint32_t srcL2Id, uint32_t dstL2Id, Ptr<Packet> pdcpSdu)
{
    ...
    // Condition currently not implemented: Fails direct link authentication / Tx DCRj (#2), Stop
    // T4111 Condition currently not implemented: Rx CELL_ID_ANNOUNCEMENT_REQUEST / Start T4107, Tx
    // CELL_ID_ANNOUNCEMENT_RESPONSE Condition currently not implemented: T4107 expires / Stop cell
    // ID announcement in PC5_DISCOVERY message Condition currently not implemented: T4105 expires /
    // Remove TMGI and MBMS SAIs Condition currently not implemented: Rx TMGI_MONITORING_REQUEST /
    // Tx TMGI_MONITORING_RESPONSE, Start T4105

    // Condition currently not implemented: Rx DSMCm (cannot be accepted) / Tx DSMRj (#7, #8 or #9),
    // Stop T4100
}
...
LteSlUeRrc::ProcessDirectCommunicationRequest(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    if (o2ocp == nullptr)
    {
        ...
        m_o2oCommContexts.insert(
            std::pair<LteSlO2oCommParams::LteSlPc5ContextId, Ptr<LteSlO2oCommParams>>(cId, o2ocp));
        m_slUeCtrlSapProvider->Pc5ConnectionStarted(L2Id, m_sourceL2Id, LteSlUeRrc::RelayUE);
    }

    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Received DirectCommunicationRequest from " << L2Id << " context "
                       << dcrq.GetSequenceNumber());

    NS_ASSERT_MSG(dcrq.GetSequenceNumber() == o2ocp->GetContextId().contextId,
                  "Message is not a retransmission");
    ...
    case LteSlO2oCommParams::RELAY_SECURE_ESTABLISHED:
        // This can happen if the message is late and the Relay UE has changed of state in the
        // meantime
        NS_LOG_DEBUG("In State " << o2ocp->GetStateString(o2ocp->GetState()) << " for " << L2Id
                                 << " context " << dcrq.GetSequenceNumber()
    ...
    // Create a sidelink bearer to transmit messages if not already available
    if (GetSidelinkRadioBearer(m_sourceL2Id, L2Id) == nullptr)
    {
    ...
    else
    {
        ...
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Sent  DirectSecurityModeCommand to " << L2Id << " context "
                           << dsmcm.GetSequenceNumber());
...
LteSlUeRrc::ProcessDirectCommunicationAccept(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    if (it == m_o2oCommContexts.end())
    {
        ...
        // meantime e.g., due to Timer4100Expiry and the Remote UE has reached max number of direct
        // communication request retransmissions, or due to the reception of a
        // DirectCommunicationReject due to Timer4111Expiry in the Relay UE side
        NS_LOG_DEBUG(
            "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " context " << dca.GetSequenceNumber());
        return;
    }
    ...
    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Received Message: DirectCommunicationAccept from " << L2Id
                       << " context " << dca.GetSequenceNumber());
...
LteSlUeRrc::ProcessDirectCommunicationReject(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    if (it == m_o2oCommContexts.end())
    {
        NS_LOG_DEBUG(
            "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << dcrj.GetSequenceNumber());
    }
    else
    {
        Ptr<LteSlO2oCommParams> o2ocp = it->second;
        ...
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Received DirectCommunicationReject (cause="
                           << (uint16_t)dcrj.GetPc5SignallingCauseValue() << ") from " << L2Id);
        ...
        m_slUeCtrlSapProvider->Pc5ConnectionAborted(
            L2Id,
            m_sourceL2Id,
            LteSlUeRrc::RemoteUE,
            (LteSlO2oCommParams::UeO2ORejectReason)dcrj.GetPc5SignallingCauseValue());
    }
}
...
LteSlUeRrc::ProcessDirectCommunicationKeepalive(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    if (it == m_o2oCommContexts.end())
    {
        NS_LOG_DEBUG(
            "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << dck.GetSequenceNumber());
    }
    else
    {
        Ptr<LteSlO2oCommParams> o2ocp = it->second;
        ...
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Received DirectCommunicationKeepalive from " << L2Id);
        ...
        if (o2ocp->GetState() == LteSlO2oCommParams::RELAY_SECURE_ESTABLISHED)
        {
            ...
            m_rrc->SendPc5Signaling(p, L2Id);
            NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                               << "] Sent DirectCommunicationKeepaliveAck to " << L2Id);
...
LteSlUeRrc::ProcessDirectCommunicationKeepaliveAck(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    if (it == m_o2oCommContexts.end())
    {
        NS_LOG_DEBUG(
            "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << dcka.GetSequenceNumber());
    }
    else
    {
        Ptr<LteSlO2oCommParams> o2ocp = it->second;
        ...
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << " ] Received DirectCommunicationKeepaliveAck from " << L2Id);

        NS_ASSERT_MSG(o2ocp->GetState() == LteSlO2oCommParams::REMOTE_SECURE_ESTABLISHED ||
...
LteSlUeRrc::ProcessDirectCommunicationRelease(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    if (it == m_o2oCommContexts.end())
    {
        ...
        NS_LOG_DEBUG(
            "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " context " << dcr.GetSequenceNumber());
    }
    else
    {
        Ptr<LteSlO2oCommParams> o2ocp = it->second;
        ...
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Received DirectCommunicationRelease from " << L2Id);
            ...
            o2ocp->GetState() == LteSlO2oCommParams::REMOTE_INIT_RELEASE)
        {
            ...
            m_rrc->SendPc5Signaling(p, L2Id);
            NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                               << "] Sent DirectCommunicationReleaseAccept to " << L2Id);

            if (o2ocp->GetState() == LteSlO2oCommParams::RELAY_SECURE_ESTABLISHED ||
                o2ocp->GetState() == LteSlO2oCommParams::RELAY_INIT_RELEASE)
            {
                m_slUeCtrlSapProvider->Pc5ConnectionTerminated(L2Id,
                                                               m_sourceL2Id,
                                                               LteSlUeRrc::RelayUE);
            }
            else
...
LteSlUeRrc::ProcessDirectCommunicationReleaseAccept(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    NS_ASSERT_MSG(
        it == m_o2oCommContexts.end() ||
            ...
            o2ocp->GetState() == LteSlO2oCommParams::REMOTE_INIT_RELEASE,
        "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << dcra.GetSequenceNumber());

    if (it != m_o2oCommContexts.end())
    {
        Ptr<LteSlO2oCommParams> o2ocp = it->second;

        NS_LOG_DEBUG("Received Message: DirectCommunicationReleaseAccept"
                     << " At: " << m_sourceL2Id << " In State: "
                     << o2ocp->GetStateString(o2ocp->GetState()) << " From: " << L2Id);
...
LteSlUeRrc::ProcessDirectSecurityModeCommand(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    if (it == m_o2oCommContexts.end())
    {
        ...
        NS_LOG_DEBUG(
            "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << dsmcm.GetSequenceNumber());
        return;
    }
    ...
    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Received DirectSecurityModeCommand from " << L2Id << " context "
                       << dsmcm.GetSequenceNumber() << " lsb " << (uint16_t)dsmcm.GetLsb());
    ...
    if (o2ocp->GetState() == LteSlO2oCommParams::REMOTE_SECURE_ESTABLISHED)
    {
        ...
        // meantime
        NS_LOG_DEBUG("In State " << o2ocp->GetStateString(o2ocp->GetState()) << " for " << L2Id
                                 << " context " << dsmcm.GetSequenceNumber()
        ...
        o2ocp->GetSecurityModeState() == LteSlO2oCommParams::EMPTY)
    {
        ...
        m_rrc->SendPc5Signaling(p, L2Id);
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Sent DirectSecurityModeComplete to " << L2Id);
    }
}
...
LteSlUeRrc::ProcessDirectSecurityModeComplete(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    if (it == m_o2oCommContexts.end())
    {
        ...
        // meantime e.g., due to Timer4111Expiry
        NS_LOG_DEBUG(
            "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << dsmcp.GetSequenceNumber());
        return;
    }
    ...
    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Received DirectSecurityModeComplete from " << L2Id << " context "
                       << dsmcp.GetSequenceNumber() << " Lsb " << (uint16_t)dsmcp.GetLsb());
    ...
    if (o2ocp->GetState() == LteSlO2oCommParams::RELAY_SECURE_ESTABLISHED)
    {
        ...
        // meantime
        NS_LOG_DEBUG("In State " << o2ocp->GetStateString(o2ocp->GetState()) << " for " << L2Id
                                 << " context " << dsmcp.GetSequenceNumber()
        ...
        o2ocp->GetSecurityModeId() == dsmcp.GetLsb())
    {
        ...
        m_rrc->SendPc5Signaling(p, L2Id);
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Sent DirectCommunicationAccept to " << L2Id);
        o2ocp->SetSecurityModeState(LteSlO2oCommParams::COMPLETED);
        ...
        // "... if the target UE already has an existing link established to the UE known to use
        // this Layer 2 ID and the new request contains an identical User Info as the known user,
        // the UE shall process the new request. However, the target UE shall only delete the
        // existing link context after the new link setup procedure succeeds, or the link keepalive
        ...
        for (it2 = m_o2oCommContexts.begin(); it2 != m_o2oCommContexts.end(); it2++)
        {
            ...
            {
                NS_LOG_DEBUG("Removing previous context for peerL2Id "
                             << cId2.peerL2Id << " contextId " << cId2.contextId);
                o2ocp2->ClearTimers();
        ...
        if (m_isRuirqEnabled)
        {
            ...
            m_rrc->SendPc5Signaling(p, L2Id);
            NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                               << "] Sent RemoteUeInfoRequest to " << L2Id);
...
LteSlUeRrc::ProcessDirectSecurityModeReject(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    NS_ASSERT_MSG(
        it != m_o2oCommContexts.end(),
        "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << dsmrj.GetSequenceNumber());
    Ptr<LteSlO2oCommParams> o2ocp = it->second;

    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Received DirectSecurityModeReject from " << L2Id);

    if (o2ocp->GetState() == LteSlO2oCommParams::RELAY_SETUP_REQUEST)
    {
        if (dsmrj.GetPc5SignallingCauseValue() == 7 || dsmrj.GetPc5SignallingCauseValue() == 8 ||
            dsmrj.GetPc5SignallingCauseValue() == 9)
        {
            o2ocp->GetRelayT4111()->Remove();
...
LteSlUeRrc::ProcessDirectRekeyingRequest(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    NS_ASSERT_MSG(
        it != m_o2oCommContexts.end(),
        "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << drrq.GetSequenceNumber());
    Ptr<LteSlO2oCommParams> o2ocp = it->second;
    UpdateKeepAlive(o2ocp);

    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Received DirectRekeyingRequest from " << L2Id);
}
...
LteSlUeRrc::ProcessDirectRekeyingResponse(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    NS_ASSERT_MSG(
        it != m_o2oCommContexts.end(),
        "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << drrs.GetSequenceNumber());
    Ptr<LteSlO2oCommParams> o2ocp = it->second;
    UpdateKeepAlive(o2ocp);

    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Received DirectRekeyingResponse from " << L2Id);
}
...
LteSlUeRrc::ProcessDirectRekeyingTrigger(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    NS_ASSERT_MSG(
        it != m_o2oCommContexts.end(),
        "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << drt.GetSequenceNumber());
    Ptr<LteSlO2oCommParams> o2ocp = it->second;
    UpdateKeepAlive(o2ocp);

    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Received DirectRekeyingTrigger from " << L2Id);
}
...
LteSlUeRrc::ProcessRemoteUeInfoRequest(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    if (it == m_o2oCommContexts.end())
    {
        ...
        NS_LOG_DEBUG(
            "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << ruirq.GetSequenceNumber());
        return;
    }
    Ptr<LteSlO2oCommParams> o2ocp = it->second;
    UpdateKeepAlive(o2ocp);
    ...
    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Received RemoteUeInfoRequest from " << L2Id);
    ...
    if (o2ocp->GetState() == LteSlO2oCommParams::REMOTE_SECURE_ESTABLISHED)
    {
        ...
        m_rrc->SendPc5Signaling(p, L2Id);
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Sent RemoteUeInfoResponseto " << L2Id);
...
LteSlUeRrc::ProcessRemoteUeInfoResponse(uint32_t L2Id, Ptr<Packet> pdcpSdu)
{
    ...
    if (it == m_o2oCommContexts.end())
    {
        ...
        NS_LOG_DEBUG(
            "Could not find the appropriate One-to-One Communication Context for the Layer-2 ID "
            << L2Id << " and context " << ruirs.GetSequenceNumber());
        return;
    }
    Ptr<LteSlO2oCommParams> o2ocp = it->second;
    UpdateKeepAlive(o2ocp);
    ...
    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Received RemoteUeInfoResponse from " << L2Id);

    if (o2ocp->GetState() == LteSlO2oCommParams::RELAY_SECURE_ESTABLISHED &&
        o2ocp->GetRelayTRUIR()->IsRunning())
    {
...
LteSlUeRrc::Timer4100Expiry(LteSlO2oCommParams::LteSlPc5ContextId cId)
{
    ...
    if (o2ocp->GetRemoteDcrqCount() < o2ocp->GetRemoteDcrqMax())
    {
        ...
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Sent DirectCommunicationRequest (Retrans "
                           << (o2ocp->GetRemoteDcrqCount() + 1) << "/" << o2ocp->GetRemoteDcrqMax()
                           << ") to " << cId.peerL2Id);
    ...
    {
        o2ocp->SetRemoteDcrqCount(0);
        o2ocp->GetRemoteT4100()
            ->Remove(); // Aborting timer this time, Relay Reselection is also possible

        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Max DirectCommunicationRequest Retransmissions reached to "
                           << cId.peerL2Id);

        // inform controller that connection aborted. We used OTHER_ERRORS even though it was not
        // sent by the relay UE
        m_slUeCtrlSapProvider->Pc5ConnectionAborted(cId.peerL2Id,
                                                    m_sourceL2Id,
                                                    LteSlUeRrc::RemoteUE,
                                                    LteSlO2oCommParams::OTHER_ERRORS);
...
LteSlUeRrc::Timer4111Expiry(LteSlO2oCommParams::LteSlPc5ContextId cId)
{
    ...
    if (o2ocp->GetState() == LteSlO2oCommParams::RELAY_SETUP_REQUEST)
    {
        ...
        m_rrc->SendPc5Signaling(p, cId.peerL2Id);
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Sent DirectCommunicationReject (Reason=Auth Failure) to "
                           << cId.peerL2Id);
...
LteSlUeRrc::Timer4108Expiry(LteSlO2oCommParams::LteSlPc5ContextId cId)
{
    ...
    m_rrc->SendPc5Signaling(p, cId.peerL2Id);
    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Sent DirectCommunicationRelease to " << cId.peerL2Id);
...
LteSlUeRrc::Timer4103Expiry(LteSlO2oCommParams::LteSlPc5ContextId cId)
{
    ...
    NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                       << "] Timer4103Expiry for peerL2Id " << cId.peerL2Id << " context "
                       << cId.contextId);
    ...
    if (o2ocp->GetState() == LteSlO2oCommParams::RELAY_INIT_RELEASE)
    {
        if (o2ocp->GetRelayDcrCount() < o2ocp->GetRelayDcrMax())
        {
            ...
            m_rrc->SendPc5Signaling(p, cId.peerL2Id);
            NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                               << "] Sent DirectCommunicationRelease ("
                               << (o2ocp->GetRelayDcrCount() + 1) << "/" << o2ocp->GetRelayDcrMax()
                               << ") to " << cId.peerL2Id);
        ...
        else
        {
            o2ocp->ClearTimers();
            m_o2oCommContexts.erase(cId);
            NS_LOG_DEBUG("RelayDcrMaximum reached for peerL2Id "
                         << cId.peerL2Id << " context " << cId.contextId << ", releasing locally ");
            m_slUeCtrlSapProvider->Pc5ConnectionTerminated(cId.peerL2Id,
                                                           m_sourceL2Id,
                                                           LteSlUeRrc::RelayUE); // Release locally
        }
    }
    ...
    if (o2ocp->GetState() == LteSlO2oCommParams::REMOTE_INIT_RELEASE)
    {
        if (o2ocp->GetRemoteDcrCount() < o2ocp->GetRemoteDcrMax())
        {
            ...
            NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                               << "] Sent DirectCommunicationRelease ("
                               << (o2ocp->GetRemoteDcrCount() + 1) << "/"
                               << o2ocp->GetRemoteDcrMax() << ") to " << cId.peerL2Id);
        ...
        else
        {
            ...
            m_slUeCtrlSapProvider->Pc5ConnectionTerminated(cId.peerL2Id,
                                                           m_sourceL2Id,
                                                           LteSlUeRrc::RemoteUE); // Release locally
        }
    }
}
...
LteSlUeRrc::Timer4102Expiry(LteSlO2oCommParams::LteSlPc5ContextId cId)
{
    ...
    if (o2ocp->GetState() == LteSlO2oCommParams::REMOTE_SECURE_ESTABLISHED)
    {
        ...
        m_rrc->SendPc5Signaling(p, cId.peerL2Id);
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                           << "] Sent DirectCommunicationKeepalive to " << cId.peerL2Id);
...
LteSlUeRrc::Timer4101Expiry(LteSlO2oCommParams::LteSlPc5ContextId cId)
{
    ...
    if (o2ocp->GetState() == LteSlO2oCommParams::REMOTE_SECURE_ESTABLISHED)
    {
        if (o2ocp->GetRemoteDckCount() < o2ocp->GetRemoteDckMax())
        {
            ...
            m_rrc->SendPc5Signaling(p, cId.peerL2Id);
            NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                               << "] Sent DirectCommunicationKeepalive ("
                               << (o2ocp->GetRemoteDckCount() + 1) << "/"
                               << o2ocp->GetRemoteDckMax() << ") to " << cId.peerL2Id);
        ...
        {
            Ptr<Packet> p = Create<Packet>();
            dcr.SetReleaseReason(
                LteSlO2oCommParams::COMM_NO_LONGER_AVAILABLE); // Release Reason #3 added to the
                                                               // header
            o2ocp->SetDcrRetrans(dcr);
            p->AddHeader(dcr);
            m_rrc->SendPc5Signaling(p, cId.peerL2Id);
...
LteSlUeRrc::TimerRUIRExpiry(LteSlO2oCommParams::LteSlPc5ContextId cId)
{
    ...
    if (o2ocp->GetState() == LteSlO2oCommParams::RELAY_SECURE_ESTABLISHED)
    {
        NS_ASSERT_MSG(o2ocp->GetRelayRuirCount() != o2ocp->GetRelayRuirMax(),
                      "Remote UE Info Request count = Remote UE Info Request maximum is currently "
                      "not handled in the standards"); // See TS 24.334
        if (o2ocp->GetRelayRuirCount() < o2ocp->GetRelayRuirMax())
        {
            ...
            m_rrc->SendPc5Signaling(p, cId.peerL2Id);
            NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                               << "] Sent RemoteUeInfoRequest (" << (o2ocp->GetRelayRuirCount() + 1)
                               << "/" << o2ocp->GetRelayRuirMax() << ") to " << cId.peerL2Id);
...
LteSlUeRrc::IsOneToOnePeer(uint32_t proseUeId)
{
    ...
    return std::find(m_oneToOnePeer.begin(), m_oneToOnePeer.end(), proseUeId) !=
           m_oneToOnePeer.end();
}
...
LteSlUeRrc::NotifySidelinkRadioBearerActivated(uint32_t peerUeId)
{
    ...
    for (it = m_o2oCommContexts.begin(); it != m_o2oCommContexts.end(); it++)
    {
        ...
        {
            NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                               << "] with peer " << o2ocp->GetContextId().peerL2Id << " context "
                               << o2ocp->GetContextId().contextId);
            ...
            case LteSlO2oCommParams::REMOTE_IDLE:
                ...
                NS_LOG_DEBUG("In " << m_sourceL2Id << " ["
                                   << o2ocp->GetStateString(o2ocp->GetState())
                                   << "] Sent DirectCommunicationRequest to " << peerUeId);
            ...
            case LteSlO2oCommParams::RELAY_IDLE:
                ...
                m_rrc->SendPc5Signaling(p, peerUeId);
                NS_LOG_DEBUG("In "
                             << m_sourceL2Id << " [" << o2ocp->GetStateString(o2ocp->GetState())
                             << "] Sent DirectSecurityModeCommand to " << peerUeId << " context "
                             << o2ocp->GetDsmcmRetrans().GetSequenceNumber());
                o2ocp->SetSecurityModeState(LteSlO2oCommParams::COMMANDED);
...
LteSlUeRrc::UpdateKeepAlive(Ptr<LteSlO2oCommParams> context)
{
    ...
    if (context->GetState() == LteSlO2oCommParams::REMOTE_SECURE_ESTABLISHED)
    {
        if (!context->GetRemoteT4102()->IsRunning())
        {
            ...
            NS_ASSERT_MSG(context->GetRemoteT4101()->IsRunning(),
                          "No ongoing keep alive while T4102 has expired");
...
void
LteSlUeRrc::ReleaseO2OConnection(Ptr<LteSlO2oCommParams> context,
                                 LteSlO2oCommParams::UeO2OReleaseReason reason)
{
        ...
        context->GetState() == LteSlO2oCommParams::REMOTE_SECURE_ESTABLISHED)
    {
        ...
        NS_LOG_DEBUG("In " << m_sourceL2Id << " [" << context->GetStateString(context->GetState())
                           << "] Sent DirectCommunicationRelease to "
                           << context->GetContextId().peerL2Id);
    }
    ...
    for (itInfo = m_relayServicesMap.begin(); itInfo != m_relayServicesMap.end(); ++itInfo)
    {
        if (itInfo->second.role == RemoteUE)
        {
            ...
            NS_LOG_LOGIC(this << " Remote UE L2ID " << m_sourceL2Id
                              << " checking if it should (re)select Relay UE for Service Code "
                              << serviceCode);
            // Check if the UE is connected to a Relay UE (has currently a selected sidelink Relay
            // UE) This code assumes that the Remote UE can be connected only to one Relay UE per
            // service code
            uint64_t currentRelayId = 0;
            Ptr<LteSlO2oCommParams> currentRelayIdContext;
            if (!m_o2oCommContexts.empty())
            {
                std::map<LteSlO2oCommParams::LteSlPc5ContextId, Ptr<LteSlO2oCommParams>>::iterator
                    it;
                for (it = m_o2oCommContexts.begin(); it != m_o2oCommContexts.end(); ++it)
                {
                    if (it->second->GetDcrqRetrans().GetRelayServiceCode() == serviceCode)
                    {
                        ...
                        {
                            NS_LOG_LOGIC(this << " UE L2ID " << m_sourceL2Id
                                              << " is in a direct communication release procedure "
                                                 "with Relay UE ID "
                                              << it->first.peerL2Id << " for Service Code "
                                              << serviceCode << "."
                                              << " Skipping Selection... ");
                            return;
                        }
            ...
            {
                NS_LOG_LOGIC(this << " UE L2ID " << m_sourceL2Id
                                  << " does not have currently any connection");
            }
            ...
            for (vaReIt = validRelays.begin(); vaReIt != validRelays.end(); vaReIt++)
            {
                ...
                {
                    validRelaysForSc.insert(
                        std::pair<uint64_t, double>(vaReIt->first.first, vaReIt->second));
                }
            }

            selectedRelayId = m_slUeCtrlSapProvider->RelayUeSelection(validRelaysForSc,
                                                                      serviceCode,
                                                                      currentRelayId);

            if (selectedRelayId != 0)
            {
                NS_LOG_LOGIC(this << " Remote UE L2ID " << m_sourceL2Id << " selected Relay UE ID: "
                                  << selectedRelayId << " for Service Code " << serviceCode);
                if (currentRelayId != 0)
                {
                    ...
                    {
                        // the Remote UE keeps connected to the current Relay UE for this service
                        // code
                    ...
                    else
                    {
                        ...
                        ReleaseO2OConnection(currentRelayIdContext,
                                             LteSlO2oCommParams::COMM_NO_LONGER_NEEDED);
                    }
                }
