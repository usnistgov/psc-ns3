...
UeManager::RecvSidelinkUeInformation(LteRrcSap::SidelinkUeInformation msg)
{
    ...
    if (msg.haveCommTxResourceReq)
    {
        if (msg.slCommTxResourceReq.slDestinationInfoList.nbDestinations >= 1)
        {
            ...
            {
                m_slDestinations.push_back(
                    msg.slCommTxResourceReq.slDestinationInfoList.SlDestinationIdentity[i]);
            }
            ...
            if (it2 == m_rrc->m_sidelinkConfiguration->m_activePoolMap.end())
            {
                ...
                else
                {
                    ...
                    {
                        dedicatedResource.setup =
                            m_rrc->m_sidelinkConfiguration->m_defaultPoolInfo.m_poolSetup;
                    }
                    else
                    {
                        NS_LOG_ERROR("No pre-provisioned or default pool found for group "
                                     << msg.slCommTxResourceReq.slDestinationInfoList
                                            .SlDestinationIdentity[0]);
                    }
                }

                Ptr<SidelinkTxCommResourcePool> m_slPool =
                    CreateObject<SidelinkTxCommResourcePool>();
                if (dedicatedResource.setup.setup == LteRrcSap::SlCommTxResourcesSetup::SCHEDULED)
                {
                    ...
                    {
                        m_slPool->SetScheduledTxParameters(
                            m_rnti,
                            dedicatedResource.setup.scheduled.macMainConfig,
                            dedicatedResource.setup.scheduled.commTxConfig,
                            0,
                            dedicatedResource.setup.scheduled.mcs);
                    }
                    else
                    {
                        m_slPool->SetScheduledTxParameters(
                ...
                else
                {
                    ...
                    NS_ASSERT(dedicatedResource.setup.ueSelected.havePoolToAdd &&
                              dedicatedResource.setup.ueSelected.poolToAddModList.nbPools > 0);
                    m_slPool->SetPool(
                        dedicatedResource.setup.ueSelected.poolToAddModList.pools[0].pool);
                }
                ...
                m_rrc->m_sidelinkConfiguration->m_activePoolMap.insert(
                    std::pair<uint32_t, LteSlEnbRrc::ActivePoolInfo>(
                        msg.slCommTxResourceReq.slDestinationInfoList.SlDestinationIdentity[0],
                        newPoolInfo));
    ...
    if (msg.haveDiscTxResourceReq)
    {
        if (msg.discTxResourceReq > 0)
        {
            ...
            if (m_rrc->m_sidelinkConfiguration->m_discPoolList.setup ==
                LteRrcSap::SlDiscTxResourcesSetup::SCHEDULED)
            { // add fields to discPool
                discPool->SetPool(
                    m_rrc->m_sidelinkConfiguration->m_discPoolList.scheduled.discTxConfig);
                LteRrcSap::SlDiscResourcePool scheDiscResPool =
                    m_rrc->m_sidelinkConfiguration->m_discPoolList.scheduled.discTxConfig;
                LteRrcSap::SlTfIndexPairList scheDiscResources =
                    m_rrc->m_sidelinkConfiguration->m_discPoolList.scheduled.discTfIndexList;
                LteRrcSap::SlHoppingConfigDisc scheDiscHopping =
                    m_rrc->m_sidelinkConfiguration->m_discPoolList.scheduled.discHoppingConfigDisc;
                discPool->SetScheduledTxParameters(scheDiscResPool,
                                                   scheDiscResources,
                                                   scheDiscHopping);
            }
            else
            {
                // when we have ue-selected, we can have multiple pools with same criteria of
                // selection either random , or rsrpBased
                //  ue-selected pool
                NS_ASSERT(m_rrc->m_sidelinkConfiguration->m_discPoolList.ueSelected.havePoolToAdd &&
                          m_rrc->m_sidelinkConfiguration->m_discPoolList.ueSelected.poolToAddModList
                                  .nbPools > 0);
                // select one pool

                // random selection: choose randomly using uniform distribution an entry of
                // m_discPoolList.ueSelected.poolToAddModList
                if (m_rrc->m_sidelinkConfiguration->m_discPoolList.ueSelected.poolToAddModList
                        .pools[0]
                        .pool.txParameters.ueSelectedResourceConfig.poolSelection.selection ==
                    LteRrcSap::PoolSelection::RANDOM)
                {
                    uint32_t randPoolIndex =
                        m_rand->GetInteger(0,
                                           m_rrc->m_sidelinkConfiguration->m_discPoolList.ueSelected
                                                   .poolToAddModList.nbPools -
                                               1);
                    LteRrcSap::SlDiscResourcePool randomPool =
                        m_rrc->m_sidelinkConfiguration->m_discPoolList.ueSelected.poolToAddModList
                            .pools[randPoolIndex]
                            .pool;
                    // add fields to discPool
                ...
                else
                {
                    ...
                    // m_discPoolList.ueSelected.poolToAddModList.pools[i].pool.ueSelectedResourceConfig.poolSelectionRsrpBased.threshLow
                    // and
                    ...
                    while ((i < m_rrc->m_sidelinkConfiguration->m_discPoolList.ueSelected
                                    .poolToAddModList.nbPools) &&
                           (!poolFound))
                    {
                        ueSelectedDiscResPool = m_rrc->m_sidelinkConfiguration->m_discPoolList
                                                    .ueSelected.poolToAddModList.pools[i]
                                                    .pool;
                        uint32_t lowRsrp =
                            ueSelectedDiscResPool.txParameters.ueSelectedResourceConfig
                                .poolSelectionRsrpBased.threshLow;
                        uint32_t highRsrp =
                            ueSelectedDiscResPool.txParameters.ueSelectedResourceConfig
                                .poolSelectionRsrpBased.threshHigh;
                        // check the rsrp values : Value 0 corresponds to -infinity, value 1 to
                        // -110dBm, value 2 to -100dBm, and so on (i.e. in steps of 10dBm) until
                        // value 6, which corresponds to -60dBm, while value 7 corresponds to
                        // +infinity.
                        NS_ASSERT_MSG(
                            lowRsrp <= highRsrp,
                            "Invalid Rsrp limits : lower bound is greater than upper bound");
                        NS_ASSERT_MSG((lowRsrp != 7) && (highRsrp != 0),
                                      "invalid RSRP limits values");
