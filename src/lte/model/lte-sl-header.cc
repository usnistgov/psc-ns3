...
void
LteSlDiscHeader::SetRelayAnnouncementParameters(uint32_t serviceCode,
...
void
LteSlDiscHeader::SetRelaySoliciationParameters(uint32_t serviceCode,
...
void
LteSlDiscHeader::SetRelayResponseParameters(uint32_t serviceCode,
                                            uint64_t discovereeInfo,
                                            uint32_t relayUeId,
                                            uint32_t status)
{
    // DISC_RELAY_RESPONSE;
...
uint8_t
LteSlDiscHeader::BuildDiscoveryMsgType() const
{
    uint8_t msgType = ((m_discoveryType & 0x03) << 6) | ((m_discoveryContentType & 0x0F) << 2) |
                      (m_discoveryModel & 0x03);
...
TypeId
LteSlDiscHeader::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::LteSlDiscHeader").SetParent<Header>().AddConstructor<LteSlDiscHeader>();
    return tid;
}

TypeId
LteSlDiscHeader::GetInstanceTypeId() const
{
    return GetTypeId();
}
...
uint32_t
LteSlDiscHeader::GetSerializedSize() const
{
    return 29;
}
...
void
LteSlSciHeader::SetSciFormat0Params(bool hopping,
                                    uint8_t rbStart,
                                    uint8_t rbLen,
                                    uint8_t hoppingInfo,
                                    uint8_t trp,
...
bool
LteSlSciHeader::IsHopping() const
{
    return m_hopping;
}

uint8_t
LteSlSciHeader::GetRbStart() const
{
    return m_rbStart;
}

uint8_t
LteSlSciHeader::GetRbLen() const
{
    return m_rbLen;
}

uint8_t
LteSlSciHeader::GetHoppingInfo() const
{
    return m_hoppingInfo;
}

uint8_t
LteSlSciHeader::GetTrp() const
{
    return m_trp;
}

uint8_t
LteSlSciHeader::GetMcs() const
{
    return m_mcs;
}

uint8_t
LteSlSciHeader::GetGroupDstId() const
{
    return m_groupDstId;
}

TypeId
LteSlSciHeader::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::LteSlSciHeader").SetParent<Header>().AddConstructor<LteSlSciHeader>();
    return tid;
}

TypeId
LteSlSciHeader::GetInstanceTypeId() const
{
    return GetTypeId();
}
...
uint32_t
LteSlSciHeader::GetSerializedSize() const
{
    return 6;
}
