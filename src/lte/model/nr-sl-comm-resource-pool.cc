      ...
      for (uint16_t poolIndex = 0; poolIndex < slTxPoolLocal.size (); ++poolIndex)
        {
          ...
          std::list <LteRrcSap::SlResourceReservePeriod> listLocal = slTxPoolLocal.at (poolIndex).slResourcePool.slUeSelectedConfigRp.slResourceReservePeriodList;
          std::list <LteRrcSap::SlResourceReservePeriod> listOther = slTxPoolOther.at (poolIndex).slResourcePool.slUeSelectedConfigRp.slResourceReservePeriodList;
...
NrSlCommResourcePool::ValidateResvPeriod (uint8_t bwpId, uint16_t poolId, Time resvPeriod, Time slotLength) const
{
  ...
  [[maybe_unused]] NrSlCommResourcePool::BwpAndPoolIt ret = ValidateBwpAndPoolId (bwpId, poolId);
  const LteRrcSap::SlResourcePoolNr pool = GetSlResourcePoolNr (bwpId, poolId);
  std::list <LteRrcSap::SlResourceReservePeriod> resvPeriodList = pool.slUeSelectedConfigRp.slResourceReservePeriodList;
