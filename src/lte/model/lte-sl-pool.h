...
class SidelinkCommResourcePool : public Object
{
  public:
    ...
    struct SubframeInfo
    {
        ...
        friend bool operator<(const SubframeInfo& lhs, const SubframeInfo& rhs)
        {
            return lhs.frameNo < rhs.frameNo ||
                   (lhs.frameNo == rhs.frameNo && lhs.subframeNo < rhs.subframeNo);
        }
    };
    ...
    ~SidelinkCommResourcePool() override;
    /**
     ...
     * \param [in] newState
     */
    typedef void (*ScPeriodTracedCallback)(const uint32_t frameNo,
                                           const uint32_t subframeNo,
                                           const uint32_t nextScPeriod);
     ...
     */
    std::list<SidelinkCommResourcePool::SidelinkTransmissionInfo> GetPsschTransmissions(
        SubframeInfo periodStart,
        uint8_t itrp,
        uint8_t rbStart,
        uint8_t rbLen);
    ...
    /**
     * Computes all valid rbStart values for a single Lcrb (length of contiguous RBs) value from the
     * range [0,10] \param rbLen the size of the contiguous resource block \return A vector of all
     * valid rbStart index positions for that Lcrb value
     */
  ...
  protected:
     ...
     */
    SidelinkCommResourcePool::SidelinkTransmissionInfo TranslatePscch(
    ...
    LteRrcSap::SlTfResourceConfig
        m_scTfResourceConfig;       ///< control pool information (subframes and PRBs)
    ...
    LteRrcSap::SlTrptSubset
        m_trptSubset; ///< normally only used for UE selected pools because it is assumed that all
                      ///< subframes are available in scheduled pools
  ...
  private:
     ...
     */
    std::vector<std::vector<uint32_t>> GetNprbType2(uint8_t rbStart,
                                                    uint8_t rbLen,
                                                    std::vector<uint32_t> psschTimeIndexes);

    /**
     * Computes the hopping sub-band used for the given subframe when type 2 frequency hopping is
     * enabled \param i The subframe number \return The sub-band index
     */
    ...
    std::vector<uint8_t>
        m_goldSequence; ///< Pseudo random sequence used for frequency hopping Type 2.
...
class SidelinkTxCommResourcePool : public SidelinkCommResourcePool
{
  public:
     ...
     */
    void SetScheduledTxParameters(uint16_t slrnti,
  ...
  protected:
    ...
    uint8_t m_poolIdentity; ///< identity of the individual pool entry configured for sidelink
                            ///< transmission
...
class SidelinkDiscResourcePool : public Object
{
  public:
    ...
    ~SidelinkDiscResourcePool() override;
    /**
     * Register this type.
     * \return The object TypeId.
     */
     ...
     * \return The subframe and RBs information for transmission on PSDCH for the resource npsdch of
     * the resource pool
     */
    std::list<SidelinkDiscResourcePool::SidelinkTransmissionInfo> GetPsdchTransmissions(
        uint32_t npsdch);
     ...
     * \param prob The set transmit probability of type LteRrcSap::TxProbability for the discovery
     * pool \return The Tx probability value for a discovery pool as an unsigned integer
     */
  ...
  protected:
     ...
     */
    SidelinkDiscResourcePool::SidelinkTransmissionInfo TranslatePsdch(
  ...
  private:
    ...
    std::vector<std::set<uint32_t>>
        m_resourceConflictsVector; ///< Matrix indicating resources using the same subframes
    bool m_preconfigured;          ///< Indicates if the pool is preconfigured
};
...
class SidelinkTxDiscResourcePool : public SidelinkDiscResourcePool
{
  public:
     ...
     */
    void SetScheduledTxParameters(LteRrcSap::SlDiscResourcePool discPool,
  ...
  protected:
    ...
    LteRrcSap::PoolSelectionRsrpBased
        m_poolSelectionRsrpBased;             ///< parameters for the RSRP based selection
