...
class LteUeRrc : public Object
{
  ...
  public:
     ...
     * announcing) \param appCodes app code payloads to be removed \param role UE role (discovered
     * or discoveree)
     */
    ...
    typedef void (*SendSLSSTracedCallback)(const uint64_t imsi,
                                           const uint64_t slssid,
                                           const uint16_t txOffset,
                                           const bool inCoverage,
                                           const uint16_t frame,
                                           const uint16_t subframe);
  ...
  private:
     ...
     */
    void DoReportSlssMeasurements(LteUeCphySapUser::UeSlssMeasurementsParameters params,
                                  uint16_t slssId,
                                  uint16_t offset);
    /**
     ...
     *
     * Notify the successful change of timing/SyncRef, and store the selected/current SyncRef
     * information
     */
     ...
     * Transmits a SidelinkUEInformation message to the eNodeB
      \param txComm Indicates if there has been a change in sidelink communication related
     parameters for transmission
     * \param rxComm Indicates if there has been a change in sidelink communication related
     parameters for reception
     * \param txDisc Indicates if there has been a change in sidelink discovery related parameters
     for transmission
     * \param rxDisc Indicates if there has been a change in sidelink discovery related parameters
     for reception
     */
     ...
     * Exporting SlChangeOfSyncRefStatParameters containing IMSI, prevSLSSID, prevRxOffset,
     * prevFrameNo, prevSubframeNo, currSLSSID, currRxOffset, currFrameNo and currSubframeNo.
     */
     ...
     */
    void SaveSlssMeasurements(uint16_t slssid,
                              uint16_t offset,
                              double srsrp,
                              bool useLayer3Filtering);
    /**
     ...
     * \param layer3FilterCoefficient L3 filter coefficient to be used. Zero if no L3 filtering is
     * required
     */
    void SaveSdRsrpMeasurements(uint64_t relayId,
