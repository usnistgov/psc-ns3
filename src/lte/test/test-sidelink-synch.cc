...
class SidelinkSynchTestCase : public TestCase
{
  public:
     ...
     */
    SidelinkSynchTestCase(uint32_t topologyIndex,
                          bool onoff,
                          bool bidirectionalTft,
                          uint64_t stream,
                          std::string description);
    ~SidelinkSynchTestCase() override;
  ...
  private:
     ...
     */
    static std::string BuildNameString(uint32_t topologyIndex,
                                       bool onoff,
                                       bool bidirectionalTft,
                                       uint64_t stream,
                                       std::string description = "");
...
SidelinkSynchTestCase::SidelinkSynchTestCase(uint32_t topologyIndex,
                                             bool onoff,
                                             bool bidirectionalTft,
                                             uint64_t stream,
                                             std::string description)
    ...
    for (uint16_t i = 0; i < m_txUeDevs.GetN(); i++)
    {
        ...
        if (param.currSlssid / 10 == synchRefImsi)
        {
            ...
            NS_TEST_ASSERT_MSG_EQ(
                param.currFrameNo,
                ueRrc->GetFrameNumber(),
                "\nIMSI " << param.imsi << " Frame number mismatched with the selected SynchRef");

            NS_TEST_ASSERT_MSG_EQ(param.currSubframeNo,
                                  ueRrc->GetSubFrameNumber(),
                                  "\nIMSI "
                                      << param.imsi
                                      << " subframe number mismatched with the selected SynchRef");
        }
    }

    NS_TEST_ASSERT_MSG_EQ(synchRefFound,
                          true,
                          "unable to find the NetDevice of synchRef with SLSSID"
                              << param.prevSlssid << " in NetDevice container");
}

void
SidelinkSynchTestCase::DoRun()
{
    /*
      LogLevel logLevel = (LogLevel)(LOG_PREFIX_FUNC | LOG_PREFIX_TIME | LOG_PREFIX_NODE |
      LOG_LEVEL_ALL); LogComponentEnable ("LteUePhy", logLevel); LogComponentEnable ("LteUeRrc",
      logLevel);
    */
    ...
    Config::SetDefault("ns3::LteUePhy::UeSlssScanningPeriod",
                       TimeValue(MilliSeconds(scaningPeriod)));
    Config::SetDefault("ns3::LteUePhy::UeSlssMeasurementPeriod",
                       TimeValue(MilliSeconds(measPeriod)));
    /* END Synchronization*/

    // Configure the UE for UE_SELECTED scenario
    Config::SetDefault("ns3::LteUeMac::SlGrantMcs", UintegerValue(16));
    Config::SetDefault("ns3::LteUeMac::SlGrantSize",
                       UintegerValue(10)); // The number of RBs allocated per UE for Sidelink
    ...
    for (uint32_t i = 0; i < ueNodes.GetN(); i++)
    {
        ...
        NS_LOG_INFO("UE with node id = [" << ueNodes.Get(i)->GetId() << "]"
    ...
    preconfiguration.preconfigSync.filterCoefficient =
        0; // k = 4 ==> a = 0.5, k = 0 ==> a = 1 No filter;
    /* END Synchronization*/
    ...
    for (uint32_t i = 0; i < ueDevs.GetN(); i++)
    {
        ...
        ueDevs.Get(i)->GetObject<LteUeNetDevice>()->GetPhy()->SetFirstScanningTime(
            MilliSeconds(firstScanningTime));
        uint64_t imsi = ueDevs.Get(i)->GetObject<LteUeNetDevice>()->GetRrc()->GetImsi();
        NS_LOG_INFO("IMSI " << imsi << " SLSSID " << tempSlssid << " First Scan "
                            << firstScanningTime << " ms");
    }
    ...
    for (uint32_t i = 0; i < rxUeDevs.GetN(); ++i)
    {
        ...
        ueRrc->TraceConnectWithoutContext(
            "ChangeOfSyncRef",
            MakeCallback(&SidelinkSynchTestCase::NotifyChangeOfSyncRef, this));
    }
   ...
   */
    ...
    Time simTime = clientAppStartTime + MilliSeconds(firstScanningTime) +
                   MilliSeconds(scaningPeriod + measPeriod);
...
std::string
SidelinkSynchTestCase::BuildNameString(uint32_t topologyIndex,
                                       bool onoff,
                                       bool bidirectionalTft,
                                       uint64_t stream,
                                       std::string description)
    ...
    : TestSuite("sidelink-synch", SYSTEM)
{
    ...
    {
        AddTestCase(new SidelinkSynchTestCase(1, true, false, i, "Test parameters : "),
                    TestCase::QUICK);
    }
    ...
    {
        AddTestCase(new SidelinkSynchTestCase(1, true, true, i, "Test parameters : "),
                    TestCase::QUICK);
    }
    ...
    {
        AddTestCase(new SidelinkSynchTestCase(2, true, false, i, "Test parameters : "),
                    TestCase::QUICK);
    }
    ...
    {
        AddTestCase(new SidelinkSynchTestCase(2, true, true, i, "Test parameters : "),
                    TestCase::QUICK);
    }
}
