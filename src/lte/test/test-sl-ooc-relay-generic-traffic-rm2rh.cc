...
class SlOocRelayGenericTrafficRm2RhTestCase : public TestCase
{
  public:
    ...
    ~SlOocRelayGenericTrafficRm2RhTestCase() override;
...
SlOocRelayGenericTrafficRm2RhTestCase::SlOocRelayGenericTrafficRm2RhTestCase(
    ...
    uint32_t nRemoteUesPerRelay)
    : TestCase(
          BuildNameString(nRelayUes, nRemoteUesPerRelay, "sl-ooc-relay-generic-traffic-rm2rh: ")),
...
void
SlOocRelayGenericTrafficRm2RhTestCase::UePacketTrace(std::string remUeIdxStr,
                                                     Ptr<const Packet> p,
                                                     const Address& srcAddrs,
                                                     const Address& dstAddrs)
{
...
void
SlOocRelayGenericTrafficRm2RhTestCase::DoRun()
{
    /*
    ...
    for (uint16_t remUeIdx = 0; remUeIdx < remoteUeNodes.GetN(); remUeIdx++)
    {
        ...
        singleClientApp.Get(0)->TraceConnect(
            "RxWithAddresses",
            oss.str(),
            MakeCallback(&SlOocRelayGenericTrafficRm2RhTestCase::UePacketTrace, this));
        oss.str("");
    ...
    NS_LOG_DEBUG("Ratio of Remote UEs with traffic: "
                 << nRemoteFlows << " / " << m_nRelayUes * m_nRemoteUesPerRelay << " = "
                 << nRemoteFlows / (m_nRelayUes * m_nRemoteUesPerRelay));
    NS_LOG_DEBUG("Ratio of Remote UEs with enough packets: "
                 << nRemoteFlowsWithEnoughPackets << " / " << m_nRelayUes * m_nRemoteUesPerRelay
                 << " = " << nRemoteFlowsWithEnoughPackets / (m_nRelayUes * m_nRemoteUesPerRelay));

    NS_TEST_ASSERT_MSG_GT(nRemoteFlows / (m_nRelayUes * m_nRemoteUesPerRelay),
                          0.5,
                          "Not enough Remote UEs with traffic");
    NS_TEST_ASSERT_MSG_GT(nRemoteFlowsWithEnoughPackets / (m_nRelayUes * m_nRemoteUesPerRelay),
                          0.5,
...
std::string
SlOocRelayGenericTrafficRm2RhTestCase::BuildNameString(uint32_t nRelayUes,
                                                       uint32_t nRemoteUesPerRelay,
                                                       std::string description)
{
    std::ostringstream oss;
