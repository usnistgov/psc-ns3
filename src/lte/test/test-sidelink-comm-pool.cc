...
class SidelinkCommPoolNextFrameSubframeTestCase : public TestCase
{
  public:
     ...
     */
    static std::string BuildNameString(LteSlResourcePoolFactory pfactory,
                                       uint32_t frameNo,
                                       uint32_t subframeNo);
     ...
     */
    SidelinkCommPoolNextFrameSubframeTestCase(LteSlResourcePoolFactory pfactory,
                                              uint32_t frameNo,
                                              uint32_t subframeNo,
                                              uint32_t expectedNextFrameNo,
...
std::string
SidelinkCommPoolNextFrameSubframeTestCase::BuildNameString(LteSlResourcePoolFactory pfactory,
                                                           uint32_t frameNo,
                                                           uint32_t subframeNo)
{
...
SidelinkCommPoolNextFrameSubframeTestCase::SidelinkCommPoolNextFrameSubframeTestCase(
...
void
SidelinkCommPoolNextFrameSubframeTestCase::DoRun()
{
    ...
    SidelinkCommResourcePool::SubframeInfo actualNextScPeriod =
        txpool->GetNextScPeriod(m_frameNo, m_subframeNo);

    NS_LOG_INFO("Actual NextScPeriod = " << actualNextScPeriod.frameNo << "/"
                                         << actualNextScPeriod.subframeNo);

    NS_TEST_EXPECT_MSG_EQ(((m_expectedNextFrameNo == actualNextScPeriod.frameNo) &&
                           (m_expectedNextSubframeNo == actualNextScPeriod.subframeNo)),
                          true,
                          "Expected and Actual Next Frame & Subframe numbers are not equal");
}
...
class SidelinkCommPoolResourceOpportunityTestCase : public TestCase
{
  public:
     ...
     */
    static std::string BuildNameString(LteSlResourcePoolFactory pfactory,
                                       uint32_t pscchResourceNo,
                                       uint32_t pscchTransmissionNo);
     ...
     */
    SidelinkCommPoolResourceOpportunityTestCase(LteSlResourcePoolFactory pfactory,
                                                uint32_t pscchResourceNo,
                                                uint32_t pscchTransmissionNo,
                                                uint32_t expectedFrameNo,
                                                uint32_t expectedSubframeNo,
                                                uint32_t expectedRbStart,
...
std::string
SidelinkCommPoolResourceOpportunityTestCase::BuildNameString(LteSlResourcePoolFactory pfactory,
                                                             uint32_t pscchResourceNo,
                                                             uint32_t pscchTransmissionNo)
{
    std::ostringstream oss;
...
SidelinkCommPoolResourceOpportunityTestCase::SidelinkCommPoolResourceOpportunityTestCase(
    LteSlResourcePoolFactory pfactory,
    uint32_t pscchResourceNo,
    uint32_t pscchTransmissionNo,
    uint32_t expectedFrameNo,
...
void
SidelinkCommPoolResourceOpportunityTestCase::DoRun()
{
    ...
    std::list<SidelinkCommResourcePool::SidelinkTransmissionInfo> txInfo =
        txpool->GetPscchTransmissions(m_pscchResourceNo);
    ...
    NS_TEST_EXPECT_MSG_EQ(
        ((m_expectedFrameNo == actualFrameNo) && (m_expectedSubframeNo == actualSubframeNo) &&
         (m_expectedRbStart == actualRbStart) && (m_expectedNbCount == actualRbCount)),
        true,
        "Expected and Actual Frame no, Subframe no, rbStart and NbCount are not equal");
}
...
class SidelinkCommPoolSubframeOpportunityTestCase : public TestCase
{
  public:
     ...
     */
    SidelinkCommPoolSubframeOpportunityTestCase(LteSlResourcePoolFactory pfactory,
                                                uint32_t frameNo,
                                                uint32_t subframeNo,
                                                int32_t expectedPscchSubframeOp1,
...
std::string
SidelinkCommPoolSubframeOpportunityTestCase::BuildNameString(LteSlResourcePoolFactory pfactory,
                                                             uint32_t frameNo,
                                                             uint32_t subframeNo)
{
    std::ostringstream oss;
...
SidelinkCommPoolSubframeOpportunityTestCase::SidelinkCommPoolSubframeOpportunityTestCase(
    LteSlResourcePoolFactory pfactory,
    uint32_t frameNo,
    uint32_t subframeNo,
    int32_t expectedPscchSubframeOp1,
...
void
SidelinkCommPoolSubframeOpportunityTestCase::DoRun()
{
    NS_LOG_FUNCTION(this << BuildNameString(m_pfactory, m_frameNo, m_subframeNo));
    ...
    else
    {
        ...
        if ((actualPscchSubframeOp1 != m_expectedPscchSubframeOp1) ||
            (actualPscchSubframeOp2 != m_expectedPscchSubframeOp2))
        {
            flag = false;
        }
    }

    NS_TEST_EXPECT_MSG_EQ(flag,
                          true,
                          "Expected and Actual PSCCH opportunities for subframe are not equal");
}
...
class SidelinkCommPoolPsschTestCase : public TestCase
{
  public:
     ...
     * \param expectedNbCount Expected total number of RBs
     */
    SidelinkCommPoolPsschTestCase(LteSlResourcePoolFactory pfactory,
                                  uint32_t frameNo,
                                  uint32_t subframeNo,
                                  uint32_t iTrp,
                                  uint32_t rbStart,
...
std::string
SidelinkCommPoolPsschTestCase::BuildNameString(LteSlResourcePoolFactory pfactory,
                                               uint32_t frameNo,
                                               uint32_t subframeNo,
                                               uint32_t iTrp,
                                               uint32_t rbStart,
...
SidelinkCommPoolPsschTestCase::SidelinkCommPoolPsschTestCase(LteSlResourcePoolFactory pfactory,
                                                             uint32_t frameNo,
                                                             uint32_t subframeNo,
                                                             uint32_t iTrp,
                                                             uint32_t rbStart,
                                                             ...
                                                             uint32_t expectedRbStart,
                                                             uint32_t expectedNbCount)
    : TestCase(BuildNameString(pfactory,
                               frameNo,
                               subframeNo,
                               iTrp,
                               rbStart,
...
void
SidelinkCommPoolPsschTestCase::DoRun()
{
    NS_LOG_FUNCTION(this << BuildNameString(m_pfactory,
                                            m_frameNo,
                                            m_subframeNo,
                                            m_iTrp,
                                            m_rbStart,
                                            m_rbLen,
                                            m_psschTransmissionNo));

    LteRrcSap::SlCommResourcePool pool = m_pfactory.CreatePool();
    Ptr<SidelinkTxCommResourcePool> txpool = CreateObject<SidelinkTxCommResourcePool>();
    txpool->SetPool(pool);

    SidelinkCommResourcePool::SubframeInfo nextScPeriod =
        txpool->GetNextScPeriod(m_frameNo, m_subframeNo);
    std::list<SidelinkCommResourcePool::SidelinkTransmissionInfo> txInfo =
        txpool->GetPsschTransmissions(nextScPeriod, m_iTrp, m_rbStart, m_rbLen);
    ...
    NS_TEST_EXPECT_MSG_EQ(
        ((m_expectedFrameNo == actualFrameNo) && (m_expectedSubframeNo == actualSubframeNo) &&
         (m_expectedRbStart == actualRbStart) && (m_expectedNbCount == actualRbCount)),
        true,
        "Expected and Actual Frame no, Subframe no, and rbStart for PSSCH are not equal");
}
    ...
    : TestSuite("sidelink-comm-pool", UNIT)
{
    ...
    // The current subframe is less than the offset value. The next period will be the first one and
    // will start at 80 ms.
    AddTestCase(new SidelinkCommPoolNextFrameSubframeTestCase(pfactory, 0, 5, 8, 0),
                TestCase::QUICK);

    // Move to the end of the frameNo range to test roll over
    AddTestCase(new SidelinkCommPoolNextFrameSubframeTestCase(pfactory, 1023, 0, 8, 0),
                TestCase::QUICK);

    // Other example: expected next period: 1004/0
    AddTestCase(new SidelinkCommPoolNextFrameSubframeTestCase(pfactory, 1002, 0, 1004, 0),
                TestCase::QUICK);
    ...
    AddTestCase(new SidelinkCommPoolResourceOpportunityTestCase(pfactory, 0, 0, 0, 0, 10, 1),
                TestCase::QUICK);
    // PscchResourceNo:0,PscchTransmissionNo:1
    AddTestCase(new SidelinkCommPoolResourceOpportunityTestCase(pfactory, 0, 1, 0, 1, 40, 1),
                TestCase::QUICK);
    // PscchResourceNo:1,PscchTransmissionNo:0
    AddTestCase(new SidelinkCommPoolResourceOpportunityTestCase(pfactory, 1, 0, 0, 0, 40, 1),
                TestCase::QUICK);
    // PscchResourceNo:1,PscchTransmissionNo:1
    AddTestCase(new SidelinkCommPoolResourceOpportunityTestCase(pfactory, 1, 1, 0, 1, 10, 1),
                TestCase::QUICK);
    ...
    // If we go to 80 ms (frame 8, subframe 0), we should be on the first subframe of the control
    // channel
    AddTestCase(new SidelinkCommPoolSubframeOpportunityTestCase(pfactory, 8, 0, 10, 40),
                TestCase::QUICK);
    // If we go to 81 ms (frame 8, subframe 1), we should be on the second subframe of the control
    // channel
    AddTestCase(new SidelinkCommPoolSubframeOpportunityTestCase(pfactory, 8, 1, 10, 40),
                TestCase::QUICK);
    // If we go to 82 ms (frame 8, subframe 2), we should be outside the control channel
    AddTestCase(new SidelinkCommPoolSubframeOpportunityTestCase(pfactory, 8, 2, -1, -1),
                TestCase::QUICK);
    ...
    //                                           |
    //                                        6th D2D subframe
    // The first transmission should occur on: periodStart (80) + dataOffset (8) + TRP subframe (11)
    // = 99 The second one (and followings) will occur every 16 ms

    // psschTransmissionNo: Input 0 for 1st transmission, 1 for 2nd transmission, 2 for 3rd
    // transmission, 3 for 4th transmission

    // psschTransmissionNo:0
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 6, 0, 9, 9, 2, 6),
                TestCase::QUICK);
    // psschTransmissionNo:1
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 6, 1, 11, 5, 2, 6),
                TestCase::QUICK);
    // psschTransmissionNo:2
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 6, 2, 13, 1, 2, 6),
                TestCase::QUICK);
    // psschTransmissionNo:3
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 6, 3, 14, 7, 2, 6),
                TestCase::QUICK);
    ...
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 0, 9, 9, 2, 3),
                TestCase::QUICK);
    // psschTransmissionNo:1, 2nd
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 1, 11, 5, 7, 3),
                TestCase::QUICK);
    // psschTransmissionNo:2, 3rd
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 2, 13, 1, 2, 3),
                TestCase::QUICK);
    // psschTransmissionNo:3, 4th
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 3, 14, 7, 7, 3),
                TestCase::QUICK);
    ...
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 0, 9, 9, 2, 3),
                TestCase::QUICK);
    // psschTransmissionNo:1, 2nd
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 1, 11, 5, 17, 3),
                TestCase::QUICK);
    // psschTransmissionNo:2, 3rd
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 2, 13, 1, 2, 3),
                TestCase::QUICK);
    // psschTransmissionNo:3, 4th
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 3, 14, 7, 17, 3),
                TestCase::QUICK);
    ...
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 0, 9, 9, 2, 3),
                TestCase::QUICK);
    // psschTransmissionNo:1, 2nd
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 1, 11, 5, 12, 3),
                TestCase::QUICK);
    // psschTransmissionNo:2, 3rd
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 2, 13, 1, 2, 3),
                TestCase::QUICK);
    // psschTransmissionNo:3, 4th
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 3, 14, 7, 12, 3),
                TestCase::QUICK);
    ...
    // psschTransmissionNo:0, 1st
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 0, 9, 9, 42, 3),
                TestCase::QUICK);
    // psschTransmissionNo:1, 2nd
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 1, 11, 5, 45, 3),
                TestCase::QUICK);
    // psschTransmissionNo:2, 3rd
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 2, 13, 1, 5, 3),
                TestCase::QUICK);
    // psschTransmissionNo:3, 4th
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 3, 14, 7, 5, 3),
                TestCase::QUICK);

    pfactory.SetDataHoppingSubbands("ns4"); // 4 sub-bands
    // psschTransmissionNo:0, 1st
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 0, 9, 9, 47, 3),
                TestCase::QUICK);
    // psschTransmissionNo:1, 2nd
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 1, 11, 5, 45, 3),
                TestCase::QUICK);
    ...
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 2, 13, 1, 5, 3),
                TestCase::QUICK);
    // psschTransmissionNo:3, 4th
    AddTestCase(new SidelinkCommPoolPsschTestCase(pfactory, 0, 5, 5, 2, 3, 3, 14, 7, 0, 3),
                TestCase::QUICK);
}
