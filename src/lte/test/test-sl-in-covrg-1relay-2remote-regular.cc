...
class SlInCovrg1Relay2RemoteRegularTestCase : public TestCase
{
  public:
    ...
    ~SlInCovrg1Relay2RemoteRegularTestCase() override;
    /**
...
void
SlInCovrg1Relay2RemoteRegularTestCase::DataPacketSinkTxNode(Ptr<const Packet> p,
                                                            const Address& src,
                                                            const Address& dst)
{
    m_txPacketNum++;
...
void
SlInCovrg1Relay2RemoteRegularTestCase::DataPacketSinkRxNode(Ptr<const Packet> p,
                                                            const Address& src,
                                                            const Address& dst)
{
    m_rxPacketNum++;
...
void
SlInCovrg1Relay2RemoteRegularTestCase::DoRun()
{
    ...
    for (uint32_t i = 0; i < remoteUeDevs.GetN(); i++)
    {
        ...
        txWithAddresses << "/NodeList/" << remoteUeNodes.Get(i)->GetId()
                        << "/ApplicationList/0/TxWithAddresses";
        Config::ConnectWithoutContext(
            txWithAddresses.str(),
            MakeCallback(&SlInCovrg1Relay2RemoteRegularTestCase::DataPacketSinkTxNode, this));

        std::ostringstream rxWithAddresses;
        rxWithAddresses << "/NodeList/" << remoteUeNodes.Get(i)->GetId()
                        << "/ApplicationList/0/RxWithAddresses";
        Config::ConnectWithoutContext(
            rxWithAddresses.str(),
            MakeCallback(&SlInCovrg1Relay2RemoteRegularTestCase::DataPacketSinkRxNode, this));
    }
