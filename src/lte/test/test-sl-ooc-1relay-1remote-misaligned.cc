...
NS_LOG_COMPONENT_DEFINE("TestSlOoc1Relay1RemoteMisaligned");

/**
 * \ingroup lte-test
 * \ingroup tests
 *
 * \brief Sidelink ooc 1-relay 1-remote misaligned test case.
 */
class SlOoc1Relay1RemoteMisalignedTestCase : public TestCase
{
  public:
    /**
     * Constructor
     ...
     * \param preconfigurationRemote preconfigurationRemote
     */
    SlOoc1Relay1RemoteMisalignedTestCase(double simTime,
                                         double echoClientMaxPackets,
                                         double echoClientInterval,
                                         double echoClientPacketSize,
                                         LteRrcSap::SlPreconfiguration preconfigurationRemote);
    ~SlOoc1Relay1RemoteMisalignedTestCase() override;
    /**
  ...
  private:
    ...
    LteRrcSap::SlPreconfiguration m_preconfigurationRemote; ///< preconfigurationRemote
...
SlOoc1Relay1RemoteMisalignedTestCase::SlOoc1Relay1RemoteMisalignedTestCase(
    double simTime,
    double echoClientMaxPackets,
    double echoClientInterval,
    double echoClientPacketSize,
...
void
SlOoc1Relay1RemoteMisalignedTestCase::DataPacketSinkTxNode(Ptr<const Packet> p,
                                                           const Address& src,
                                                           const Address& dst)
{
...
void
SlOoc1Relay1RemoteMisalignedTestCase::DataPacketSinkRxNode(Ptr<const Packet> p,
                                                           const Address& src,
                                                           const Address& dst)
{
    if (Inet6SocketAddress::ConvertFrom(src).GetIpv6() == m_dstAddressToCompareLater &&
        Inet6SocketAddress::ConvertFrom(dst).GetIpv6() == m_srcAddressToCompareLater)
    {
        m_rxPacket = true;
    }
}
...
void
SlOoc1Relay1RemoteMisalignedTestCase::PC5SignalingPacketTraceFunction(uint32_t srcL2Id,
                                                                      uint32_t dstL2Id,
                                                                      Ptr<Packet> p)
{
...
void
SlOoc1Relay1RemoteMisalignedTestCase::DoRun()
{
    ...
    Config::SetDefault("ns3::LteUeMac::UseSetTrp",
                       BooleanValue(false)); // use default Trp index of 0
    ...
    enbSidelinkConfiguration->AddPreconfiguredDedicatedPool(
        1,
        pool); // update to use L2ID for remote UE
    enbSidelinkConfiguration->AddPreconfiguredDedicatedPool(
        2,
        pool); // update to use L2ID for remote UE
    ...
    if (useRelay)
    {
        preconfigurationRemote = m_preconfigurationRemote;
    }
    ...
    Ptr<Ipv6StaticRouting> pgwStaticRouting =
        ipv6RoutingHelper.GetStaticRouting(pgw->GetObject<Ipv6>());
    pgwStaticRouting
        ->AddNetworkRouteTo("7777:f00e::", Ipv6Prefix(64), Ipv6Address("7777:f00e::1"), 1, 0);
    ...
    Config::ConnectWithoutContext(
        "/NodeList/*/DeviceList/*/LteUeRrc/SidelinkConfiguration/PC5SignalingPacketTrace",
        MakeCallback(&SlOoc1Relay1RemoteMisalignedTestCase::PC5SignalingPacketTraceFunction, this));

    std::ostringstream txWithAddresses;
    txWithAddresses << "/NodeList/" << ueDevs.Get(1)->GetNode()->GetId()
                    << "/ApplicationList/0/TxWithAddresses";
    Config::ConnectWithoutContext(
        txWithAddresses.str(),
        MakeCallback(&SlOoc1Relay1RemoteMisalignedTestCase::DataPacketSinkTxNode, this));

    std::ostringstream rxWithAddresses;
    rxWithAddresses << "/NodeList/" << ueDevs.Get(1)->GetNode()->GetId()
                    << "/ApplicationList/0/RxWithAddresses";
    Config::ConnectWithoutContext(
        rxWithAddresses.str(),
        MakeCallback(&SlOoc1Relay1RemoteMisalignedTestCase::DataPacketSinkRxNode, this));
    ...
    NS_TEST_ASSERT_MSG_EQ((m_recvRUIRq && m_recvRUIRs),
                          false,
                          "Error in sending and receiving RemoteUeInfoRequest and "
                          "RemoteUeInfoResponse. Test misaligned scenario failed");
    NS_TEST_ASSERT_MSG_EQ((m_txPacket && m_rxPacket),
                          false,
                          "Atleast one packet was sent and received (echoed) at the Remote UE. "
                          "Test misaligned scenario failed");
}
 ...
 * \ingroup tests
 *
 * \brief Sidelink ooc 1-relay 1-remote misaligned test suite.
 */
class SlOoc1Relay1RemoteMisalignedTestSuite : public TestSuite
{
  public:
    SlOoc1Relay1RemoteMisalignedTestSuite();
};

SlOoc1Relay1RemoteMisalignedTestSuite::SlOoc1Relay1RemoteMisalignedTestSuite()
    : TestSuite("sl-ooc-1relay-1remote-misaligned", SYSTEM)
{
    // LogComponentEnable ("TestSlOoc1Relay1RemoteMisaligned", LOG_LEVEL_ALL);
    ...
    //-Control
    preconfCommPoolFactory.SetControlPeriod("sf80");
    ...
    // Misalignment can be brought about by modifying the Sidelink period, Resource Blocks and
    // Subframes etc in the preconfigurationRemote variable above. Misalignment causes the test to
    // fail. Pass the variables simTime, echoClientMaxPackets, echoClientInterval,
    // echoClientPacketSize, preconfigurationRemote
    AddTestCase(
        new SlOoc1Relay1RemoteMisalignedTestCase(10.0, 20, 0.2, 150, preconfigurationRemote),
        TestCase::QUICK);

    /*
    The following tabulates the modifications that can be made to the above preconfiguration
    variables and the responses that can be expected from running the test scenario. Variable -
    Value - Response SetDiscCpLen - NORMAL - PASS SetDiscCpLen - EXTENDED - PASS

          SetDiscPeriod - rf32 - PASS
          SetDiscPeriod - rf64 - PASS

          SetDiscPrbEnd - 40 - PASS
          SetDiscPrbEnd - 38 - ABORT
          SetDiscPrbEnd - 30 - ABORT

          SetDiscBitmap - 0x11111 - PASS
          SetDiscBitmap - 0x10101 - ABORT
          SetDiscBitmap - 0x11110 - ABORT
          SetDiscBitmap - 0x01111 - ABORT
          SetDiscBitmap - 0x111111 - ABORT

          SetDiscTxProbability - p100 - PASS
          SetDiscTxProbability - p75 - PASS
          SetDiscTxProbability - p50 - PASS
          SetDiscTxProbability - p25 - PASS

          SetControlPeriod - sf80 - PASS
          SetControlPeriod - sf40 - FAIL
          SetControlPeriod - sf70 - PASS
          SetControlPeriod - sf120 - PASS

          SetControlBitmap - 0x00000000FF - PASS
          SetControlBitmap - 0x00000000FE - PASS
          SetControlBitmap - 0x00000000F0 - PASS
          SetControlBitmap - 0x0000000FFF - PASS

          SetControlPrbEnd - 49 - PASS
          SetControlPrbEnd - 40 - PASS
          SetControlPrbEnd - 30 - PASS

          SetDataBitmap - 0xFFFFFFFFFF - PASS
          SetDataBitmap - 0xFFFFFFFFFE - PASS
          SetDataBitmap - 0xFFFFFFFFF0 - PASS
          SetDataBitmap - 0xFFFFFFF0FF - PASS

          SetDataPrbEnd - 49 - PASS
          SetDataPrbEnd - 40 - PASS
          SetDataPrbEnd - 30 - PASS
    */
}

static SlOoc1Relay1RemoteMisalignedTestSuite staticSlOoc1Relay1RemoteMisalignedTestSuite;
