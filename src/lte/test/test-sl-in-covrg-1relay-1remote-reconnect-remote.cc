...
class SlInCovrg1Relay1RemoteReconnectRemoteTestCase : public TestCase
{
  public:
     ...
     */
    SlInCovrg1Relay1RemoteReconnectRemoteTestCase(Time simTime,
                                                  Time startRelayTime,
                                                  Time startRemoteTime,
                                                  Time stopRemoteTime,
                                                  Time restartRemoteTime);
    ~SlInCovrg1Relay1RemoteReconnectRemoteTestCase() override;
    /**
...
void
SlInCovrg1Relay1RemoteReconnectRemoteTestCase::DataPacketSinkTxNode(Ptr<const Packet> p,
                                                                    const Address& src,
                                                                    const Address& dst)
{
...
void
SlInCovrg1Relay1RemoteReconnectRemoteTestCase::DataPacketSinkRxNode(Ptr<const Packet> p,
                                                                    const Address& src,
                                                                    const Address& dst)
{
...
void
SlInCovrg1Relay1RemoteReconnectRemoteTestCase::PC5SignalingPacketTraceFunction(uint32_t srcL2Id,
                                                                               uint32_t dstL2Id,
                                                                               Ptr<Packet> p)
{
...
void
SlInCovrg1Relay1RemoteReconnectRemoteTestCase::DoRun()
{
    bool useRelay = true;
    ...
    if (useRelay)
    {
        ...
        Simulator::Schedule(m_startRemoteTime,
                            ...
                            LteSlUeRrc::ModelA,
                            LteSlUeRrc::RemoteUE);
        Simulator::Schedule(m_stopRemoteTime,
                            &LteSidelinkHelper::StopRelayService,
                            proseHelper,
                            ueDevs.Get(1),
                            33);
        Simulator::Schedule(m_restartRemoteTime,
                            &LteSidelinkHelper::StartRelayService,
                            proseHelper,
                            ueDevs.Get(1),
                            33,
    ...
    Config::ConnectWithoutContext(
        txWithAddresses.str(),
        MakeCallback(&SlInCovrg1Relay1RemoteReconnectRemoteTestCase::DataPacketSinkTxNode, this));

    std::ostringstream rxWithAddresses;
    rxWithAddresses << "/NodeList/" << remoteHost->GetId() << "/ApplicationList/0/RxWithAddresses";
    Config::ConnectWithoutContext(
        rxWithAddresses.str(),
        MakeCallback(&SlInCovrg1Relay1RemoteReconnectRemoteTestCase::DataPacketSinkRxNode, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(
        ...
        "stopping and restarting relay service for Remote UE failed");

    bool addressesMatch = (m_addressBeforeStartRemoteTime == m_addressBeforeRestartRemoteTime &&
                           m_addressBeforeStopRemoteTime == m_addressAfterRestartRemoteTime);
    NS_TEST_ASSERT_MSG_EQ(addressesMatch,
                          true,
                          "Error in receiving packets with correct addresses at the Remote Host. "
                          "Test stopping and restarting relay service for Remote UE failed");
}
    ...
    : TestSuite("sl-in-covrg-1relay-1remote-reconnect-remote", SYSTEM)
{
    ...
    // Pass the variables simTime, startRelayTime, startRemoteTime, stopRemoteTime,
    // restartRemoteTime
    ...
    AddTestCase(new SlInCovrg1Relay1RemoteReconnectRemoteTestCase(simTime,
                                                                  startRelayTime,
                                                                  startRemoteTime,
                                                                  stopRemoteTime,
                                                                  restartRemoteTime),
...
static SlInCovrg1Relay1RemoteReconnectRemoteTestSuite
    staticSlInCovrg1Relay1RemoteReconnectRemoteTestSuite;
