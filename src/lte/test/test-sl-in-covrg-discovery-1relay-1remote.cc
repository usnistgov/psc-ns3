...
class SlInCovrgDiscovery1Relay1RemoteTestCase : public TestCase
{
  public:
    ...
    ~SlInCovrgDiscovery1Relay1RemoteTestCase() override;
    /**
...
SlInCovrgDiscovery1Relay1RemoteTestCase::SlInCovrgDiscovery1Relay1RemoteTestCase(double simTime,
                                                                                 bool useModelB)
...
void
SlInCovrgDiscovery1Relay1RemoteTestCase::DiscoveryMonitoringCallback(uint64_t imsi,
                                                                     uint16_t cellId,
                                                                     uint16_t rnti,
                                                                     LteSlDiscHeader discMsg)
{
    ...
    {
        if (m_imsiRelayservicecodeVector[i].imsi == imsi &&
            m_imsiRelayservicecodeVector[i].relayservicecode == discMsg.GetRelayServiceCode())
        {
            found = true;
            break;
        }
    ...
    if (m_imsiRelayservicecodeVector.empty() || !found)
    {
...
void
SlInCovrgDiscovery1Relay1RemoteTestCase::DoRun()
{
    ...
    Config::ConnectWithoutContext(
        ...
        MakeCallback(&SlInCovrgDiscovery1Relay1RemoteTestCase::DiscoveryMonitoringCallback, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(
        allDiscovered,
        true,
        "Error in Remote UE being discovered by Relay UE. Test relay service discovery failed");
}
