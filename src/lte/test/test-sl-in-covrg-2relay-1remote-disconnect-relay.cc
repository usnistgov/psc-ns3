...
NS_LOG_COMPONENT_DEFINE("TestSlInCovrg2Relay1RemoteDisconnectRelay");

/**
 * \ingroup lte-test
 * \ingroup tests
 *
 * \brief Sidelink in-covrg 2-relay 1-remote disconnect relay test case.
 */
class SlInCovrg2Relay1RemoteDisconnectRelayTestCase : public TestCase
{
  public:
    /**
     * Constructor
     ...
     */
    SlInCovrg2Relay1RemoteDisconnectRelayTestCase(double simTime);
    ~SlInCovrg2Relay1RemoteDisconnectRelayTestCase() override;
    /**
...
SlInCovrg2Relay1RemoteDisconnectRelayTestCase::SlInCovrg2Relay1RemoteDisconnectRelayTestCase(
    double simTime)
...
void
SlInCovrg2Relay1RemoteDisconnectRelayTestCase::DataPacketSinkTxNode(Ptr<const Packet> p,
                                                                    const Address& src,
                                                                    const Address& dst)
{
}
...
void
SlInCovrg2Relay1RemoteDisconnectRelayTestCase::DataPacketSinkRxNode(Ptr<const Packet> p,
                                                                    const Address& src,
                                                                    const Address& dst)
{
...
void
SlInCovrg2Relay1RemoteDisconnectRelayTestCase::DoRun()
{
    ...
    if (relayTraffic)
    {
        for (uint16_t relUeIdx = 0; relUeIdx < relayUeNodes.GetN(); relUeIdx++)
        {
            ...
            ApplicationContainer singleClientApp =
                echoClientHelper.Install(relayUeNodes.Get(relUeIdx));
    ...
    if (useRelay)
    {
        ...
        {
            Simulator::Schedule(Seconds(2.0 + 4 * ryDevIdx),
                                ...
                                LteSlUeRrc::ModelA,
                                LteSlUeRrc::RelayUE);
            Simulator::Schedule(Seconds(6.0 + 4 * ryDevIdx),
        ...
        {
            Simulator::Schedule((Seconds(4.0 + rmDevIdx)),
                                &LteSidelinkHelper::StartRelayService,
                                proseHelper,
    ...
    for (uint32_t i = 0; i < remoteUeDevs.GetN(); i++)
    {
        ...
        txWithAddresses << "/NodeList/" << remoteHost->GetId()
                        << "/ApplicationList/0/TxWithAddresses";
        Config::ConnectWithoutContext(
            txWithAddresses.str(),
            MakeCallback(&SlInCovrg2Relay1RemoteDisconnectRelayTestCase::DataPacketSinkTxNode,
                         this));

        std::ostringstream rxWithAddresses;
        rxWithAddresses << "/NodeList/" << remoteHost->GetId()
                        << "/ApplicationList/0/RxWithAddresses";
        Config::ConnectWithoutContext(
            rxWithAddresses.str(),
            MakeCallback(&SlInCovrg2Relay1RemoteDisconnectRelayTestCase::DataPacketSinkRxNode,
                         this));
    }
    ...
    NS_TEST_ASSERT_MSG_EQ(m_addressList.size(),
                          3,
                          "Test failed. Expected packets at the remote host from three different "
                          "IP addresses, namely remote uplink IP address, remote IP address under "
                          "first relay and remote IP address under second relay.");
 ...
 * \ingroup tests
 *
 * \brief Sidelink in-covrg 2-relay 1-remote disconnect relay test suite.
 */
class SlInCovrg2Relay1RemoteDisconnectRelayTestSuite : public TestSuite
{
  public:
    SlInCovrg2Relay1RemoteDisconnectRelayTestSuite();
};

SlInCovrg2Relay1RemoteDisconnectRelayTestSuite::SlInCovrg2Relay1RemoteDisconnectRelayTestSuite()
    : TestSuite("sl-in-covrg-2relay-1remote-disconnect-relay", SYSTEM)
{
    // Test regular scenario
    // 1 Remote UE detects, connects, and uses 1 Relay UE to send and receive traffic.
    // After 6 seconds, the Relay UE is disconnected. The Remote UE connects to another Relay UE to
    // send and receive traffic. After 10 seconds, the other Relay UE is also disconnected. The
    // Remote UE continues to use uplink to send and receive traffic . Pass the variables simTime
    AddTestCase(new SlInCovrg2Relay1RemoteDisconnectRelayTestCase(20.0), TestCase::QUICK);
}

static SlInCovrg2Relay1RemoteDisconnectRelayTestSuite
    staticSlInCovrg2Relay1RemoteDisconnectRelayTestSuite;
