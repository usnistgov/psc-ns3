...
public:
  ...
  virtual ~SlInCovrg2Relay1RemoteDisconnectRelayTestCase ();
  /**
...
SlInCovrg2Relay1RemoteDisconnectRelayTestCase::SlInCovrg2Relay1RemoteDisconnectRelayTestCase (double simTime)
...
void
SlInCovrg2Relay1RemoteDisconnectRelayTestCase::DataPacketSinkTxNode (Ptr<const Packet> p, const Address &src, const Address &dst)
{
}

void
SlInCovrg2Relay1RemoteDisconnectRelayTestCase::DataPacketSinkRxNode (Ptr<const Packet> p, const Address &src, const Address &dst)
{
  m_addressList.push_back (Inet6SocketAddress::ConvertFrom (src).GetIpv6 ());
}

void
SlInCovrg2Relay1RemoteDisconnectRelayTestCase::DoRun (void)
{
  ...
  for (uint32_t i = 0; i < remoteUeDevs.GetN (); i++)
    {
      ...
      Config::ConnectWithoutContext (txWithAddresses.str (), MakeCallback (&SlInCovrg2Relay1RemoteDisconnectRelayTestCase::DataPacketSinkTxNode, this));

      std::ostringstream rxWithAddresses;
      rxWithAddresses << "/NodeList/" << remoteHost->GetId () << "/ApplicationList/0/RxWithAddresses";
      Config::ConnectWithoutContext (rxWithAddresses.str (), MakeCallback (&SlInCovrg2Relay1RemoteDisconnectRelayTestCase::DataPacketSinkRxNode, this));
    }
  ...
  NS_TEST_ASSERT_MSG_EQ (m_addressList.size (), 3, "Test failed. Expected packets at the remote host from three different IP addresses, namely remote uplink IP address, remote IP address under first relay and remote IP address under second relay.");
  ...
  : TestSuite ("sl-in-covrg-2relay-1remote-disconnect-relay", SYSTEM)
{
  ...
  //After 6 seconds, the Relay UE is disconnected. The Remote UE connects to another Relay UE to send and receive traffic.
  //After 10 seconds, the other Relay UE is also disconnected. The Remote UE continues to use uplink to send and receive traffic .
  //Pass the variables simTime
  AddTestCase (new SlInCovrg2Relay1RemoteDisconnectRelayTestCase (20.0), TestCase::QUICK);
}

static SlInCovrg2Relay1RemoteDisconnectRelayTestSuite staticSlInCovrg2Relay1RemoteDisconnectRelayTestSuite;
