...
class SlInCovrgDiscoveryMultiAppTestCase : public TestCase
{
  public:
     ...
     */
    SlInCovrgDiscoveryMultiAppTestCase(double simTime,
                                       bool useModelB,
                                       LteSlDiscResourcePoolFactory pfactory);
    ~SlInCovrgDiscoveryMultiAppTestCase() override;
    /**
     ...
     */
    void DiscoveryMonitoringCallback(uint64_t imsi,
                                     uint16_t cellId,
                                     uint16_t rnti,
...
void
SlInCovrgDiscoveryMultiAppTestCase::DiscoveryMonitoringCallback(uint64_t imsi,
                                                                uint16_t cellId,
                                                                uint16_t rnti,
                                                                LteSlDiscHeader discMsg)
{
    ...
    if (m_appcodeVector.empty() || !found)
    {
...
void
SlInCovrgDiscoveryMultiAppTestCase::DoRun()
{
    ...
    else
    {
        Simulator::Schedule(Seconds(2.0),
                            ...
                            ueNetDevice0,
                            apps,
                            LteSlUeRrc::Discoveree);
        Simulator::Schedule(Seconds(2.0),
                            &LteSidelinkHelper::StartDiscoveryApps,
                            proseHelper,
                            ueNetDevice1,
                            apps,
                            LteSlUeRrc::Discoverer);
    }
    ...
    Config::ConnectWithoutContext(
        ...
        MakeCallback(&SlInCovrgDiscoveryMultiAppTestCase::DiscoveryMonitoringCallback, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(enoughAppsDiscovered,
                          true,
                          "Error in correct number of applications discovered by other UE. Test "
                          "multi application discovery failed");
}
    ...
    : TestSuite("sl-in-covrg-discovery-multi-app", SYSTEM)
{
    ...
    // Test application discovery model A and model B with 2 applications in one node, enough
    // resources to send both discovery packets in one period.
    ...
    // Test application discovery model A and model B with 2 applications in one node, not enough
    // resources to send both discovery packets in one period. Only one will make it in a period.
    // with Tx probability of 100 %, only one application would be discovered.
