 ...
 */
std::string
LteSlPc5SignallingMessageTypeTestCase::BuildNameString(LteSlPc5SignallingMessageType nlpc5smt)
{
 ...
 * \param nlpc5smt LteSlPc5SignallingMessageType
 */
LteSlPc5SignallingMessageTypeTestCase::LteSlPc5SignallingMessageTypeTestCase(
...
void
LteSlPc5SignallingMessageTypeTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_nlpc5smt.GetMessageType() == nlpc5smt_peek.GetMessageType()),
                          true,
                          "Input and Peeked msgType are not equal");
}
 ...
 */
std::string
DirectCommunicationRequestTestCase::BuildNameString(DirectCommunicationRequest ndcrq)
{
    std::ostringstream oss;
 ...
 * \param ndcrq DirectCommunicationRequest
 */
DirectCommunicationRequestTestCase::DirectCommunicationRequestTestCase(
    DirectCommunicationRequest ndcrq)
...
void
DirectCommunicationRequestTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndcrq.GetMessageIdentifier() == ndcrq_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcrq.GetSequenceNumber() == ndcrq_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcrq.GetUserInfo() == ndcrq_peek.GetUserInfo()),
                          true,
                          "Input and Peeked User Info are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcrq.GetUserInfoType() == ndcrq_peek.GetUserInfoType()),
                          true,
                          "Input and Peeked User Info Type are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcrq.GetIpAddressConfig() == ndcrq_peek.GetIpAddressConfig()),
                          true,
                          "Input and Peeked IP Address Config are not equal");
    NS_TEST_EXPECT_MSG_EQ(
        (m_ndcrq.GetMaximumInactivityPeriod() == ndcrq_peek.GetMaximumInactivityPeriod()),
        true,
        "Input and Peeked Maximum Inactivity Period are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcrq.GetNonce1() == ndcrq_peek.GetNonce1()),
                          true,
                          "Input and Peeked Nonce1 are not equal");
    NS_TEST_EXPECT_MSG_EQ(
        (m_ndcrq.GetUeSecurityCapabilities() == ndcrq_peek.GetUeSecurityCapabilities()),
        true,
        "Input and Peeked UE Security Capabilities are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcrq.GetMsbKdId() == ndcrq_peek.GetMsbKdId()),
                          true,
                          "Input and Peeked MSB Kd ID are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcrq.GetKdId() == ndcrq_peek.GetKdId()),
                          true,
                          "Input and Peeked Kd ID are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcrq.GetRelayServiceCode() == ndcrq_peek.GetRelayServiceCode()),
                          true,
                          "Input and Peeked Relay Service Code are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcrq.GetSignature() == ndcrq_peek.GetSignature()),
                          true,
                          "Input and Peeked Signature are not equal");
    NS_TEST_EXPECT_MSG_EQ(
        (m_ndcrq.GetLinkLocalIpv6Address() == ndcrq_peek.GetLinkLocalIpv6Address()),
        true,
        "Input and Peeked Link Local IPv6 Address are not equal");
}
 ...
 */
std::string
DirectCommunicationAcceptTestCase::BuildNameString(DirectCommunicationAccept ndca)
{
    std::ostringstream oss;
...
void
DirectCommunicationAcceptTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndca.GetMessageIdentifier() == ndca_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndca.GetSequenceNumber() == ndca_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndca.GetIpAddressConfig() == ndca_peek.GetIpAddressConfig()),
                          true,
                          "Input and Peeked IP Address Config are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndca.GetLinkLocalIpv6Address() == ndca_peek.GetLinkLocalIpv6Address()),
                          true,
                          "Input and Peeked Link Local IPv6 Address are not equal");
}
 ...
 */
std::string
DirectCommunicationRejectTestCase::BuildNameString(DirectCommunicationReject ndcrj)
{
    std::ostringstream oss;
 ...
 * \param ndcrj DirectCommunicationReject
 */
DirectCommunicationRejectTestCase::DirectCommunicationRejectTestCase(
    DirectCommunicationReject ndcrj)
...
void
DirectCommunicationRejectTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndcrj.GetMessageIdentifier() == ndcrj_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcrj.GetSequenceNumber() == ndcrj_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ(
        (m_ndcrj.GetPc5SignallingCauseValue() == ndcrj_peek.GetPc5SignallingCauseValue()),
        true,
        "Input and Peeked PC5 Signalling Cause Value are not equal");
}
 ...
 */
std::string
DirectCommunicationKeepaliveTestCase::BuildNameString(DirectCommunicationKeepalive ndck)
{
    std::ostringstream oss;
 ...
 * \param ndck DirectCommunicationKeepalive
 */
DirectCommunicationKeepaliveTestCase::DirectCommunicationKeepaliveTestCase(
    DirectCommunicationKeepalive ndck)
...
void
DirectCommunicationKeepaliveTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndck.GetMessageIdentifier() == ndck_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndck.GetSequenceNumber() == ndck_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndck.GetKeepaliveCounter() == ndck_peek.GetKeepaliveCounter()),
                          true,
                          "Input and Peeked Keepalive Counter are not equal");
    NS_TEST_EXPECT_MSG_EQ(
        (m_ndck.GetMaximumInactivityPeriod() == ndck_peek.GetMaximumInactivityPeriod()),
        true,
        "Input and Peeked Maximum Inactivity Period are not equal");
}
 ...
 */
std::string
DirectCommunicationKeepaliveAckTestCase::BuildNameString(DirectCommunicationKeepaliveAck ndcka)
{
    std::ostringstream oss;
 ...
 * \param ndcka DirectCommunicationKeepaliveAck
 */
DirectCommunicationKeepaliveAckTestCase::DirectCommunicationKeepaliveAckTestCase(
    DirectCommunicationKeepaliveAck ndcka)
...
void
DirectCommunicationKeepaliveAckTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndcka.GetMessageIdentifier() == ndcka_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcka.GetSequenceNumber() == ndcka_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcka.GetKeepaliveCounter() == ndcka_peek.GetKeepaliveCounter()),
                          true,
                          "Input and Peeked Keepalive Counter are not equal");
}
 ...
 */
std::string
DirectCommunicationReleaseTestCase::BuildNameString(DirectCommunicationRelease ndcr)
{
    std::ostringstream oss;
 ...
 * \param ndcr DirectCommunicationRelease
 */
DirectCommunicationReleaseTestCase::DirectCommunicationReleaseTestCase(
    DirectCommunicationRelease ndcr)
...
void
DirectCommunicationReleaseTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndcr.GetMessageIdentifier() == ndcr_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcr.GetSequenceNumber() == ndcr_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcr.GetReleaseReason() == ndcr_peek.GetReleaseReason()),
                          true,
                          "Input and Peeked Release Reason are not equal");
}
 ...
 */
std::string
DirectCommunicationReleaseAcceptTestCase::BuildNameString(DirectCommunicationReleaseAccept ndcra)
{
    std::ostringstream oss;
 ...
 * \param ndcra DirectCommunicationReleaseAccept
 */
DirectCommunicationReleaseAcceptTestCase::DirectCommunicationReleaseAcceptTestCase(
...
void
DirectCommunicationReleaseAcceptTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndcra.GetMessageIdentifier() == ndcra_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndcra.GetSequenceNumber() == ndcra_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
}
 ...
 */
std::string
DirectSecurityModeCommandTestCase::BuildNameString(DirectSecurityModeCommand ndsmcm)
{
    std::ostringstream oss;
 ...
 * \param ndsmcm DirectSecurityModeCommand
 */
DirectSecurityModeCommandTestCase::DirectSecurityModeCommandTestCase(
    DirectSecurityModeCommand ndsmcm)
...
void
DirectSecurityModeCommandTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcm.GetMessageIdentifier() == ndsmcm_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcm.GetSequenceNumber() == ndsmcm_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ(
        (m_ndsmcm.GetUeSecurityCapabilities() == ndsmcm_peek.GetUeSecurityCapabilities()),
        true,
        "Input and Peeked UE Security Capabilities are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcm.GetNonce2() == ndsmcm_peek.GetNonce2()),
                          true,
                          "Input and Peeked Nonce2 are not equal");
    NS_TEST_EXPECT_MSG_EQ(
        (m_ndsmcm.GetChosenAlgorithmsIntegrity() == ndsmcm_peek.GetChosenAlgorithmsIntegrity()),
        true,
        "Input and Peeked Chosen Algorithms Integrity are not equal");
    NS_TEST_EXPECT_MSG_EQ(
        (m_ndsmcm.GetChosenAlgorithmsCyphering() == ndsmcm_peek.GetChosenAlgorithmsCyphering()),
        true,
        "Input and Peeked Chosen Algorithms Cyphering are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcm.GetLsb() == ndsmcm_peek.GetLsb()),
                          true,
                          "Input and Peeked LSB are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcm.GetMsb() == ndsmcm_peek.GetMsb()),
                          true,
                          "Input and Peeked MSB are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcm.GetKdFreshness() == ndsmcm_peek.GetKdFreshness()),
                          true,
                          "Input and Peeked Kd Freshness are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcm.GetGpi() == ndsmcm_peek.GetGpi()),
                          true,
                          "Input and Peeked GPI are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcm.GetUserInfo() == ndsmcm_peek.GetUserInfo()),
                          true,
                          "Input and Peeked User Info are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcm.GetUserInfoType() == ndsmcm_peek.GetUserInfoType()),
                          true,
                          "Input and Peeked User Info Type are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcm.GetSignature() == ndsmcm_peek.GetSignature()),
                          true,
                          "Input and Peeked Signature are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcm.GetEncryptedPayload() == ndsmcm_peek.GetEncryptedPayload()),
                          true,
                          "Input and Peeked Encrypted Payload are not equal");
}
 ...
 */
std::string
DirectSecurityModeCompleteTestCase::BuildNameString(DirectSecurityModeComplete ndsmcp)
{
    std::ostringstream oss;
 ...
 * \param ndsmcp DirectSecurityModeComplete
 */
DirectSecurityModeCompleteTestCase::DirectSecurityModeCompleteTestCase(
    DirectSecurityModeComplete ndsmcp)
...
void
DirectSecurityModeCompleteTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcp.GetMessageIdentifier() == ndsmcp_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcp.GetSequenceNumber() == ndsmcp_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmcp.GetLsb() == ndsmcp_peek.GetLsb()),
                          true,
                          "Input and Peeked LSB are not equal");
}
 ...
 */
std::string
DirectSecurityModeRejectTestCase::BuildNameString(DirectSecurityModeReject ndsmrj)
{
    std::ostringstream oss;
...
void
DirectSecurityModeRejectTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndsmrj.GetMessageIdentifier() == ndsmrj_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmrj.GetSequenceNumber() == ndsmrj_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ(
        (m_ndsmrj.GetPc5SignallingCauseValue() == ndsmrj_peek.GetPc5SignallingCauseValue()),
        true,
        "Input and Peeked PC5 Signalling Cause Value are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmrj.GetRand() == ndsmrj_peek.GetRand()),
                          true,
                          "Input and Peeked RAND are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndsmrj.GetAuts() == ndsmrj_peek.GetAuts()),
                          true,
                          "Input and Peeked AUTS are not equal");
}
 ...
 */
std::string
DirectRekeyingRequestTestCase::BuildNameString(DirectRekeyingRequest ndrrq)
{
    std::ostringstream oss;
...
void
DirectRekeyingRequestTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndrrq.GetMessageIdentifier() == ndrrq_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndrrq.GetSequenceNumber() == ndrrq_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ(
        (m_ndrrq.GetUeSecurityCapabilities() == ndrrq_peek.GetUeSecurityCapabilities()),
        true,
        "Input and Peeked UE Security Capabilities are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndrrq.GetNonce1() == ndrrq_peek.GetNonce1()),
                          true,
                          "Input and Peeked Nonce1 are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndrrq.GetMsb() == ndrrq_peek.GetMsb()),
                          true,
                          "Input and Peeked MSB are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndrrq.GetAuthFlag() == ndrrq_peek.GetAuthFlag()),
                          true,
                          "Input and Peeked Auth Flag are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndrrq.GetPrukId() == ndrrq_peek.GetPrukId()),
                          true,
                          "Input and Peeked PRUK ID are not equal");
}
 ...
 */
std::string
DirectRekeyingResponseTestCase::BuildNameString(DirectRekeyingResponse ndrrs)
{
    std::ostringstream oss;
...
void
DirectRekeyingResponseTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndrrs.GetMessageIdentifier() == ndrrs_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndrrs.GetSequenceNumber() == ndrrs_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
}
 ...
 */
std::string
DirectRekeyingTriggerTestCase::BuildNameString(DirectRekeyingTrigger ndrt)
{
    std::ostringstream oss;
...
void
DirectRekeyingTriggerTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_ndrt.GetMessageIdentifier() == ndrt_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_ndrt.GetSequenceNumber() == ndrt_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
}
 ...
 */
std::string
RemoteUeInfoRequestTestCase::BuildNameString(RemoteUeInfoRequest nruirq)
{
    std::ostringstream oss;
...
void
RemoteUeInfoRequestTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_nruirq.GetMessageIdentifier() == nruirq_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_nruirq.GetSequenceNumber() == nruirq_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_nruirq.GetRemoteUeInfoType() == nruirq_peek.GetRemoteUeInfoType()),
                          true,
                          "Input and Peeked Remote UE Info Type are not equal");
}
 ...
 */
std::string
RemoteUeInfoResponseTestCase::BuildNameString(RemoteUeInfoResponse nruirs)
{
    std::ostringstream oss;
...
void
RemoteUeInfoResponseTestCase::DoRun()
{
    ...
    NS_TEST_EXPECT_MSG_EQ((m_nruirs.GetMessageIdentifier() == nruirs_peek.GetMessageIdentifier()),
                          true,
                          "Input and Peeked Message Identifier are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_nruirs.GetSequenceNumber() == nruirs_peek.GetSequenceNumber()),
                          true,
                          "Input and Peeked Sequence Number are not equal");
    NS_TEST_EXPECT_MSG_EQ((m_nruirs.GetImei() == nruirs_peek.GetImei()),
                          true,
                          "Input and Peeked IMEI are not equal");
}
    ...
    : TestSuite("lte-pc5-signalling-header", UNIT)
{
    ...
    uint32_t kdId = 13;
    uint32_t relayServiceCode = 14;
    uint32_t keepaliveCounter = 15;
    uint32_t maxInactivityPeriod = 16;
    uint8_t ipAddressConfig = 0;
    uint8_t pc5SignallingCauseValue = 4;
    uint8_t releaseReason = 3;
    uint8_t authFlag = 1;
    uint8_t remoteUeInfoType = 2;
    uint16_t lsb = 0;
    uint16_t msb = 0;
    ...
    AddTestCase(new LteSlPc5SignallingMessageTypeTestCase(nlpc5smt),
                TestCase::QUICK); // Default values

    nlpc5smt.SetMessageType(1);
    AddTestCase(new LteSlPc5SignallingMessageTypeTestCase(nlpc5smt),
                TestCase::QUICK); // Set MessageType

    // Test cases for DirectCommunicationRequest class
    DirectCommunicationRequest ndcrq;
    DirectCommunicationRequest ndcrq1;
    DirectCommunicationRequest ndcrq2;
    DirectCommunicationRequest ndcrq3;
    DirectCommunicationRequest ndcrq4;
    DirectCommunicationRequest ndcrq5;
    DirectCommunicationRequest ndcrq6;
    ...
    AddTestCase(new DirectCommunicationRequestTestCase(ndcrq3),
                TestCase::QUICK); // linkLocalIpv6Address set
    ...
    AddTestCase(new DirectCommunicationRequestTestCase(ndcrq5),
                TestCase::QUICK); // kdId, relayServiceCode, signature, linkLocalIpv6Address set
    ...
    AddTestCase(new DirectCommunicationRequestTestCase(ndcrq6),
                TestCase::QUICK); // userinfo, nonce1, kdId, relayServiceCode, signature,
                                  // linkLocalIpv6Address set

    // Test cases for DirectCommunicationAccept class
    DirectCommunicationAccept ndca;
    DirectCommunicationAccept ndca1;
    DirectCommunicationAccept ndca2;
    DirectCommunicationAccept ndca3;
    ...
    AddTestCase(new DirectCommunicationAcceptTestCase(ndca1),
                TestCase::QUICK); // ipAddressConfig set

    ndca2.SetLinkLocalIpv6Address(linkLocalIpv6Address);
    AddTestCase(new DirectCommunicationAcceptTestCase(ndca2),
                TestCase::QUICK); // linkLocalIpv6Address set

    ndca3.SetIpAddressConfig(ipAddressConfig);
    ndca3.SetLinkLocalIpv6Address(linkLocalIpv6Address);
    AddTestCase(new DirectCommunicationAcceptTestCase(ndca3),
                TestCase::QUICK); // ipAddressConfig, linkLocalIpv6Address set

    // Test cases for DirectCommunicationReject class
    DirectCommunicationReject ndcrj;
    DirectCommunicationReject ndcrj1;
    ...
    AddTestCase(new DirectCommunicationRejectTestCase(ndcrj1),
                TestCase::QUICK); // pc5SignallingCauseValue set

    // Test cases for DirectCommunicationKeepalive class
    DirectCommunicationKeepalive ndck;
    DirectCommunicationKeepalive ndck1;
    DirectCommunicationKeepalive ndck2;
    DirectCommunicationKeepalive ndck3;
    ...
    AddTestCase(new DirectCommunicationKeepaliveTestCase(ndck1),
                TestCase::QUICK); // keepaliveCounter set

    ndck2.SetMaximumInactivityPeriod(maxInactivityPeriod);
    AddTestCase(new DirectCommunicationKeepaliveTestCase(ndck2),
                TestCase::QUICK); // maxInactivityPeriod set

    ndck3.SetKeepaliveCounter(keepaliveCounter);
    ndck3.SetMaximumInactivityPeriod(maxInactivityPeriod);
    AddTestCase(new DirectCommunicationKeepaliveTestCase(ndck3),
                TestCase::QUICK); // keepaliveCounter, maxInactivityPeriod set

    // Test cases for DirectCommunicationKeepaliveAck class
    DirectCommunicationKeepaliveAck ndcka;
    DirectCommunicationKeepaliveAck ndcka1;

    AddTestCase(new DirectCommunicationKeepaliveAckTestCase(ndcka),
                TestCase::QUICK); // Default values

    ndcka1.SetKeepaliveCounter(keepaliveCounter);
    AddTestCase(new DirectCommunicationKeepaliveAckTestCase(ndcka1),
                TestCase::QUICK); // keepaliveCounter set

    // Test cases for DirectCommunicationRelease class
    DirectCommunicationRelease ndcr;
    DirectCommunicationRelease ndcr1;
    ...
    AddTestCase(new DirectCommunicationReleaseTestCase(ndcr1), TestCase::QUICK); // releaseReason
                                                                                 // set

    // Test cases for DirectCommunicationReleaseAccept class
    DirectCommunicationReleaseAccept ndcra;

    AddTestCase(new DirectCommunicationReleaseAcceptTestCase(ndcra),
                TestCase::QUICK); // Default values

    // Test cases for DirectSecurityModeCommand class
    DirectSecurityModeCommand ndsmcm;
    DirectSecurityModeCommand ndsmcm1;
    DirectSecurityModeCommand ndsmcm2;
    DirectSecurityModeCommand ndsmcm3;
    DirectSecurityModeCommand ndsmcm4;
    DirectSecurityModeCommand ndsmcm5;
    DirectSecurityModeCommand ndsmcm6;
    DirectSecurityModeCommand ndsmcm7;
    DirectSecurityModeCommand ndsmcm8;
    ...
    AddTestCase(new DirectSecurityModeCommandTestCase(ndsmcm7),
                TestCase::QUICK); // encryptedPayload set
    ...
    AddTestCase(new DirectSecurityModeCommandTestCase(ndsmcm8),
                TestCase::QUICK); // encryptedPayload set

    // Test cases for DirectSecurityModeComplete class
    DirectSecurityModeComplete ndsmcp;
    DirectSecurityModeComplete ndsmcp1;
    ...
    DirectSecurityModeReject ndsmrj;
    DirectSecurityModeReject ndsmrj1;
    DirectSecurityModeReject ndsmrj2;
    DirectSecurityModeReject ndsmrj3;
    DirectSecurityModeReject ndsmrj4;
    ...
    AddTestCase(new DirectSecurityModeRejectTestCase(ndsmrj1),
                TestCase::QUICK); // pc5SignallingCauseValue set
    ...
    AddTestCase(new DirectSecurityModeRejectTestCase(ndsmrj4),
                TestCase::QUICK); // pc5SignallingCauseValue, rand, auts set

    // Test cases for DirectRekeyingRequest class
    DirectRekeyingRequest ndrrq;
    DirectRekeyingRequest ndrrq1;
    DirectRekeyingRequest ndrrq2;
    DirectRekeyingRequest ndrrq3;
    DirectRekeyingRequest ndrrq4;
    ...
    AddTestCase(new DirectRekeyingRequestTestCase(ndrrq4),
                TestCase::QUICK); // nonce1, authFlag, prukId set
    ...
    RemoteUeInfoRequest nruirq;
    RemoteUeInfoRequest nruirq1;
    ...
    RemoteUeInfoResponse nruirs;
    RemoteUeInfoResponse nruirs1;
