...
class SlOocRelayGenericTrafficRm2RmTestCase : public TestCase
{
  public:
    ...
    ~SlOocRelayGenericTrafficRm2RmTestCase() override;
  ...
  private:
     ...
     */
    static std::string BuildNameString(uint32_t nRelayUes,
                                       uint32_t nRemoteUesPerRelay,
     ...
     */
    void UePacketTrace(std::string remUeIdxStr,
                       Ptr<const Packet> p,
...
SlOocRelayGenericTrafficRm2RmTestCase::SlOocRelayGenericTrafficRm2RmTestCase(
    ...
    uint32_t nRemoteUesPerRelay)
    : TestCase(
          BuildNameString(nRelayUes, nRemoteUesPerRelay, "sl-ooc-relay-generic-traffic-rm2rm: ")),
...
void
SlOocRelayGenericTrafficRm2RmTestCase::UePacketTrace(std::string remUeIdxStr,
                                                     Ptr<const Packet> p,
                                                     const Address& srcAddrs,
                                                     const Address& dstAddrs)
{
...
void
SlOocRelayGenericTrafficRm2RmTestCase::ChangeUdpEchoClientRemote(Ptr<Node> newRemoteNode,
...
void
SlOocRelayGenericTrafficRm2RmTestCase::DoRun()
{
    /*
    ...
    NS_LOG_DEBUG("Starting Test with: "
                 ...
                 << ", m_relayNetwork= " << m_relayNetwork << ", m_relayPrefix= " << m_relayPrefix
    ...
    double timeBetweenRelayStarts =
        1.0 + m_nRemoteUesPerRelay *
                  ((2 + 2 * m_nRemoteUesPerRelay) * 0.04 + timeBetweenRemoteStarts); // s
    ...
    for (uint16_t remUeIdx = 1; remUeIdx < remoteUeNodes.GetN(); remUeIdx++)
    {
        ...
        UdpEchoClientHelper echoClientHelper(
            dummyAddr); // The address will be changed in the function ChangeRemote
        ...
        oss << remUeIdx;
        singleClientApp.Get(0)->TraceConnect(
            "RxWithAddresses",
            oss.str(),
            MakeCallback(&SlOocRelayGenericTrafficRm2RmTestCase::UePacketTrace, this));
        oss.str("");
    ...
    for (uint32_t ryDevIdx = 0; ryDevIdx < relayUeDevs.GetN(); ryDevIdx++)
    {
        ...
        Simulator::Schedule(Seconds(startTimeRelay[ryDevIdx]),
                            ...
                            LteSlUeRrc::ModelA,
                            LteSlUeRrc::RelayUE);
        NS_LOG_DEBUG("Relay UE " << ryDevIdx << " node id = ["
                                 << relayUeNodes.Get(ryDevIdx)->GetId()
    ...
    // Remember that the 1st Remote UE is the EchoServer (flows to check =
    // (m_nRelayUes*m_nRemoteUesPerRelay - 1)
    NS_LOG_DEBUG("Ratio of Remote UEs with traffic: "
                 << nRemoteFlows << " / " << (m_nRelayUes * m_nRemoteUesPerRelay - 1) << " = "
                 << nRemoteFlows / (m_nRelayUes * m_nRemoteUesPerRelay - 1));
    NS_LOG_DEBUG("Ratio of Remote UEs with enough packets: "
                 << nRemoteFlowsWithEnoughPackets << " / "
                 << (m_nRelayUes * m_nRemoteUesPerRelay - 1) << " = "
                 << nRemoteFlowsWithEnoughPackets / (m_nRelayUes * m_nRemoteUesPerRelay - 1));

    NS_TEST_ASSERT_MSG_GT(nRemoteFlows / (m_nRelayUes * m_nRemoteUesPerRelay - 1),
                          0.5,
                          "Not enough Remote UEs with traffic");
    NS_TEST_ASSERT_MSG_GT(nRemoteFlowsWithEnoughPackets / (m_nRelayUes * m_nRemoteUesPerRelay - 1),
                          0.5,
                          "Not enough Remote UEs with enough packets");
...
std::string
SlOocRelayGenericTrafficRm2RmTestCase::BuildNameString(uint32_t nRelayUes,
                                                       uint32_t nRemoteUesPerRelay,
                                                       std::string description)
{
    std::ostringstream oss;
    oss << description << " nRelayUes= " << nRelayUes
        << " nRemoteUesPerRelay= " << nRemoteUesPerRelay;
    return oss.str();
}
