...
std::string
LteNistPhyErrorModelTestCase::BuildNameString(LteNistErrorModel::LtePhyChannel channel,
                                              LteNistErrorModel::LteFadingModel fadingChannel,
                                              LteNistErrorModel::LteTxMode txmode,
                                              uint16_t mcs,
                                              double newSinr,
                                              HarqProcessInfoList_t harqHistory)
{
    ...
    oss << ", tx mode=" << txmode << ", mcs=" << mcs << ", newSinr=" << newSinr << " W ("
        << 10 * std::log10(newSinr) << " dB)"
        << ", harqHistory size=" << harqHistory.size();
    if (!harqHistory.empty())
    {
        oss << ", last HarqSinr" << harqHistory[harqHistory.size() - 1].m_sinr << " W ("
            << 10 * std::log10(harqHistory[harqHistory.size() - 1].m_sinr) << " dB)";
    }

    return oss.str();
}

LteNistPhyErrorModelTestCase::LteNistPhyErrorModelTestCase(
    LteNistErrorModel::LtePhyChannel channel,
    LteNistErrorModel::LteFadingModel fadingChannel,
    LteNistErrorModel::LteTxMode txmode,
    uint16_t mcs,
    double newSinr,
...
LteNistPhyErrorModelTestCase::DoRun()
{
    NS_LOG_FUNCTION(
        this
        << BuildNameString(m_channel, m_fadingChannel, m_txmode, m_mcs, m_newSinr, m_harqHistory));
    ...
    case LteNistErrorModel::PSCCH:
        ...
        break;
    case LteNistErrorModel::PSSCH:
        tbStat = LteNistErrorModel::GetPsschBler(m_fadingChannel,
                                                 m_txmode,
                                                 m_mcs,
                                                 m_newSinr,
                                                 m_harqHistory);
    ...
    case LteNistErrorModel::PSDCH:
        tbStat =
            LteNistErrorModel::GetPsdchBler(m_fadingChannel, m_txmode, m_newSinr, m_harqHistory);
        break;
    case LteNistErrorModel::PUSCH:
        tbStat = LteNistErrorModel::GetPuschBler(m_fadingChannel,
                                                 m_txmode,
                                                 m_mcs,
                                                 m_newSinr,
                                                 m_harqHistory);
    ...
    : TestSuite("nist-phy-error-model", UNIT)
{
    ...
    AddTestCase(new LteNistPhyErrorModelTestCase(LteNistErrorModel::PSSCH,
    ...
    // Testing PSCCH
    harqInfoList.erase(harqInfoList.begin());
    AddTestCase(new LteNistPhyErrorModelTestCase(LteNistErrorModel::PSCCH,
                                                 LteNistErrorModel::AWGN,
                                                 LteNistErrorModel::SISO,
                                                 0,
                                                 std::pow(10, (-7 / 10.0)),
