...
class SlInCovrgDiscoveryRegularTestCase : public TestCase
{
  public:
     ...
     */
    SlInCovrgDiscoveryRegularTestCase(double simTime,
                                      bool useModelB,
                                      LteSlDiscResourcePoolFactory pfactory);
    ~SlInCovrgDiscoveryRegularTestCase() override;
    /**
...
void
SlInCovrgDiscoveryRegularTestCase::DiscoveryMonitoringCallback(uint64_t imsi,
                                                               uint16_t cellId,
                                                               uint16_t rnti,
                                                               LteSlDiscHeader discMsg)
{
    ...
    {
        if (m_imsiAppcodeVector[i].imsi == imsi &&
            m_imsiAppcodeVector[i].appcode == discMsg.GetApplicationCode())
        {
            found = true;
            break;
        }
    ...
    if (m_imsiAppcodeVector.empty() || !found)
    {
        imsiAppcode iA;
        iA.imsi = imsi;
...
void
SlInCovrgDiscoveryRegularTestCase::DoRun()
{
    ...
    Config::ConnectWithoutContext(
        ...
        MakeCallback(&SlInCovrgDiscoveryRegularTestCase::DiscoveryMonitoringCallback, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(
        allDiscovered,
        true,
        "Error in all UEs being discovered by other UEs. Test application discovery failed");
}
    ...
    : TestSuite("sl-in-covrg-discovery-regular", SYSTEM)
{
    ...
    // Test application discovery model A and model B. Check that UEs discover each other. Test that
    // applications are discovered by each UE.
    ...
    // Reset number of retransmissions to 0, Reset number of repetitions to 1, Set transmit
    // probability to 75 percent Pass the variables simTime, useModelB, pfactory
    pfactory.SetNumRetx(0);
    pfactory.SetNumRepetition(1);
