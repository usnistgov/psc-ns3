...
NS_LOG_COMPONENT_DEFINE("TestSlInCovrg1Relay1RemoteReconnectRelay");

/**
 * \ingroup lte-test
 * \ingroup tests
 *
 * \brief Sidelink in-covrg 1-relay 1-remote reconnect relay test case.
 */
class SlInCovrg1Relay1RemoteReconnectRelayTestCase : public TestCase
{
  public:
    /**
     * Constructor
     ...
     * \param stopRelayTime stopRelayTime
     * \param restartRelayTime restartRelayTime
     */
    SlInCovrg1Relay1RemoteReconnectRelayTestCase(Time simTime,
                                                 Time startRelayTime,
                                                 Time startRemoteTime,
                                                 Time stopRelayTime,
                                                 Time restartRelayTime);
    ~SlInCovrg1Relay1RemoteReconnectRelayTestCase() override;
    /**
  ...
  private:
    ...
    Time m_stopRelayTime;                        ///< stopRelayTime
    Time m_restartRelayTime;                     ///< restartRelayTime
    Ipv6Address m_addressBeforeStartRemoteTime;  ///< addressBeforeStartRemoteTime
    Ipv6Address m_addressBeforeStopRelayTime;    ///< addressBeforeStopRelayTime
    Ipv6Address m_addressBeforeRestartRelayTime; ///< addressBeforeRestartRelayTime
    Ipv6Address m_addressAfterRestartRelayTime;  ///< addressAfterRestartRelayTime
};

SlInCovrg1Relay1RemoteReconnectRelayTestCase::SlInCovrg1Relay1RemoteReconnectRelayTestCase(
    Time simTime,
    Time startRelayTime,
    Time startRemoteTime,
    Time stopRelayTime,
...
void
SlInCovrg1Relay1RemoteReconnectRelayTestCase::DataPacketSinkTxNode(Ptr<const Packet> p,
                                                                   const Address& src,
                                                                   const Address& dst)
{
    NS_LOG_INFO("Packet sent! " << Inet6SocketAddress::ConvertFrom(src).GetIpv6() << " "
                                << Inet6SocketAddress::ConvertFrom(dst).GetIpv6());
}

void
SlInCovrg1Relay1RemoteReconnectRelayTestCase::DataPacketSinkRxNode(Ptr<const Packet> p,
                                                                   const Address& src,
                                                                   const Address& dst)
{
    NS_LOG_INFO("Packet received! " << Inet6SocketAddress::ConvertFrom(src).GetIpv6() << " "
                                    << Inet6SocketAddress::ConvertFrom(dst).GetIpv6());
    ...
    if (Simulator::Now() > m_startRemoteTime && Simulator::Now() < m_stopRelayTime)
    {
        m_addressBeforeStopRelayTime = Inet6SocketAddress::ConvertFrom(src).GetIpv6();
    }
    if (Simulator::Now() > m_stopRelayTime && Simulator::Now() < m_restartRelayTime)
    {
        m_addressBeforeRestartRelayTime = Inet6SocketAddress::ConvertFrom(src).GetIpv6();
    }
    if (Simulator::Now() > m_restartRelayTime)
    {
        m_addressAfterRestartRelayTime = Inet6SocketAddress::ConvertFrom(src).GetIpv6();
    }
}

void
SlInCovrg1Relay1RemoteReconnectRelayTestCase::PC5SignalingPacketTraceFunction(uint32_t srcL2Id,
                                                                              uint32_t dstL2Id,
                                                                              Ptr<Packet> p)
{
...
void
SlInCovrg1Relay1RemoteReconnectRelayTestCase::DoRun()
{
    bool useRelay = true;
    ...
    Config::SetDefault("ns3::LteSlO2oCommParams::RemoteT4100", UintegerValue(200));
    ...
    pgwStaticRouting->AddNetworkRouteTo("7777:f00e::", Ipv6Prefix(60), Ipv6Address("::"), 1, 0);
    ...
    if (useRelay)
    {
        ...
        Simulator::Schedule(m_startRelayTime,
                            ...
                            LteSlUeRrc::ModelA,
                            LteSlUeRrc::RelayUE);
        Simulator::Schedule(m_startRemoteTime,
                            &LteSidelinkHelper::StartRelayService,
                            proseHelper,
                            ueDevs.Get(1),
                            33,
                            LteSlUeRrc::ModelA,
                            LteSlUeRrc::RemoteUE);
        Simulator::Schedule(m_stopRelayTime,
                            &LteSidelinkHelper::StopRelayService,
                            proseHelper,
                            ueDevs.Get(0),
                            33);
        Simulator::Schedule(m_restartRelayTime,
                            &LteSidelinkHelper::StartRelayService,
                            proseHelper,
                            ueDevs.Get(0),
                            33,
    ...
    Config::ConnectWithoutContext(
        txWithAddresses.str(),
        MakeCallback(&SlInCovrg1Relay1RemoteReconnectRelayTestCase::DataPacketSinkTxNode, this));

    std::ostringstream rxWithAddresses;
    rxWithAddresses << "/NodeList/" << remoteHost->GetId() << "/ApplicationList/0/RxWithAddresses";
    Config::ConnectWithoutContext(
        rxWithAddresses.str(),
        MakeCallback(&SlInCovrg1Relay1RemoteReconnectRelayTestCase::DataPacketSinkRxNode, this));
    ...
    NS_TEST_ASSERT_MSG_EQ(
        addressesAllOnes,
        false,
        "All the IPv6 addresses for comparison are still having default values of ones. Test "
        "stopping and restarting relay service for Relay UE failed");

    bool addressesMatch = (m_addressBeforeStartRemoteTime == m_addressBeforeRestartRelayTime &&
                           m_addressBeforeStopRelayTime == m_addressAfterRestartRelayTime);
    NS_TEST_ASSERT_MSG_EQ(
        addressesMatch,
        true,
        "Error in receiving packets with correct addresses at the Remote Host."
        "Test stopping and restarting relay service for Relay UE failed "
            << m_addressBeforeStartRemoteTime << " " << m_addressBeforeRestartRelayTime << " "
            << m_addressBeforeStopRelayTime << ", " << m_addressAfterRestartRelayTime);
}
 ...
 * \ingroup tests
 *
 * \brief Sidelink in-covrg 1-relay 1-remote reconnect relay test suite.
 */
class SlInCovrg1Relay1RemoteReconnectRelayTestSuite : public TestSuite
{
  public:
    SlInCovrg1Relay1RemoteReconnectRelayTestSuite();
};
    ...
    : TestSuite("sl-in-covrg-1relay-1remote-reconnect-relay", SYSTEM)
{
    ...
    Time stopRelayTime = Seconds(7.0);
    Time restartRelayTime = Seconds(8.0);
    AddTestCase(new SlInCovrg1Relay1RemoteReconnectRelayTestCase(simTime,
                                                                 startRelayTime,
                                                                 startRemoteTime,
                                                                 stopRelayTime,
                                                                 restartRelayTime),
...
static SlInCovrg1Relay1RemoteReconnectRelayTestSuite
    staticSlInCovrg1Relay1RemoteReconnectRelayTestSuite;
