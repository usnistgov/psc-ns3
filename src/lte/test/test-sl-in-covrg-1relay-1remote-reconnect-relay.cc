...
public:
   ...
   */
  SlInCovrg1Relay1RemoteReconnectRelayTestCase (Time simTime, Time startRelayTime, Time startRemoteTime, Time stopRelayTime, Time restartRelayTime);
  virtual ~SlInCovrg1Relay1RemoteReconnectRelayTestCase ();
  /**
...
void
SlInCovrg1Relay1RemoteReconnectRelayTestCase::PC5SignalingPacketTraceFunction (uint32_t srcL2Id, uint32_t dstL2Id, Ptr<Packet> p)
{
...
void
SlInCovrg1Relay1RemoteReconnectRelayTestCase::DoRun (void)
{
  bool useRelay = true;
  ...
  if (useRelay)
    {
      ...
      Simulator::Schedule (m_stopRelayTime, &LteSidelinkHelper::StopRelayService, proseHelper, ueDevs.Get (0), 33);
  ...
  bool addressesAllOnes = (m_addressBeforeStartRemoteTime == Ipv6Address::GetOnes ()
                           ...
                           && m_addressBeforeRestartRelayTime == Ipv6Address::GetOnes ()
                           && m_addressAfterRestartRelayTime == Ipv6Address::GetOnes ());
  NS_TEST_ASSERT_MSG_EQ (addressesAllOnes, false, "All the IPv6 addresses for comparison are still having default values of ones. Test stopping and restarting relay service for Relay UE failed");
  ...
  : TestSuite ("sl-in-covrg-1relay-1remote-reconnect-relay", SYSTEM)
{
  ...
  AddTestCase (new SlInCovrg1Relay1RemoteReconnectRelayTestCase (simTime, startRelayTime, startRemoteTime, stopRelayTime, restartRelayTime), TestCase::QUICK);
}

static SlInCovrg1Relay1RemoteReconnectRelayTestSuite staticSlInCovrg1Relay1RemoteReconnectRelayTestSuite;
