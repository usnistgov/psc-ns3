...
class SidlelinkDiscPoolNextFrameSubframeTestCase : public TestCase
{
  public:
     ...
     */
    static std::string BuildNameString(LteSlDiscResourcePoolFactory pfactory,
                                       uint32_t frameNo,
                                       uint32_t subframeNo);
     ...
     */
    SidlelinkDiscPoolNextFrameSubframeTestCase(LteSlDiscResourcePoolFactory pfactory,
                                               uint32_t frameNo,
                                               uint32_t subframeNo,
...
std::string
SidlelinkDiscPoolNextFrameSubframeTestCase::BuildNameString(LteSlDiscResourcePoolFactory pfactory,
                                                            uint32_t frameNo,
                                                            uint32_t subframeNo)
{
...
SidlelinkDiscPoolNextFrameSubframeTestCase::SidlelinkDiscPoolNextFrameSubframeTestCase(
...
void
SidlelinkDiscPoolNextFrameSubframeTestCase::DoRun()
{
    ...
    SidelinkDiscResourcePool::SubframeInfo actualNextDiscPeriod =
        txpool->GetNextDiscPeriod(m_frameNo, m_subframeNo);

    NS_LOG_INFO("Actual NextDiscPeriod = " << actualNextDiscPeriod.frameNo << "/"
                                           << actualNextDiscPeriod.subframeNo);

    NS_TEST_EXPECT_MSG_EQ(((m_expectedNextFrameNo == actualNextDiscPeriod.frameNo) &&
                           (m_expectedNextSubframeNo == actualNextDiscPeriod.subframeNo)),
                          true,
...
class SidelinkDiscPoolResourceOpportunityTestCase : public TestCase
{
  public:
     ...
     */
    SidelinkDiscPoolResourceOpportunityTestCase(LteSlDiscResourcePoolFactory pfactory,
                                                uint32_t psdchResourceNo,
...
std::string
SidelinkDiscPoolResourceOpportunityTestCase::BuildNameString(LteSlDiscResourcePoolFactory pfactory,
                                                             uint32_t psdchResourceNo)
{
    std::ostringstream oss;
...
SidelinkDiscPoolResourceOpportunityTestCase::SidelinkDiscPoolResourceOpportunityTestCase(
    LteSlDiscResourcePoolFactory pfactory,
    uint32_t psdchResourceNo,
    uint32_t expectedFrameNo,
    uint32_t expectedSubframeNo,
...
void
SidelinkDiscPoolResourceOpportunityTestCase::DoRun()
{
    ...
    std::list<SidelinkDiscResourcePool::SidelinkTransmissionInfo> txInfo =
        txpool->GetPsdchTransmissions(m_psdchResourceNo);
    ...
    NS_TEST_EXPECT_MSG_EQ(
        ((m_expectedFrameNo == actualFrameNo) && (m_expectedSubframeNo == actualSubframeNo) &&
         (m_expectedRbStart == actualRbStart) && (m_expectedNbCount == actualNbCount)),
        true,
...
class SidlelinkDiscPoolRbsPerSubframeTestCase : public TestCase
{
  public:
     ...
     */
    SidlelinkDiscPoolRbsPerSubframeTestCase(LteSlDiscResourcePoolFactory pfactory,
                                            uint32_t frameNo,
                                            uint32_t subframeNo,
...
std::string
SidlelinkDiscPoolRbsPerSubframeTestCase::BuildNameString(LteSlDiscResourcePoolFactory pfactory,
                                                         uint32_t frameNo,
                                                         uint32_t subframeNo)
{
    std::ostringstream oss;
...
SidlelinkDiscPoolRbsPerSubframeTestCase::SidlelinkDiscPoolRbsPerSubframeTestCase(
    LteSlDiscResourcePoolFactory pfactory,
    uint32_t frameNo,
    uint32_t subframeNo,
    uint32_t expectedTotalRbs,
...
void
SidlelinkDiscPoolRbsPerSubframeTestCase::DoRun()
{
    NS_LOG_FUNCTION(this << BuildNameString(m_pfactory, m_frameNo, m_subframeNo));
    ...
    NS_LOG_INFO("Actual total number of RBs = "
                << totalRbs.size() << " Expected total number of Rbs = " << m_expectedTotalRbs);

    NS_TEST_EXPECT_MSG_EQ(totalRbs.size(),
                          m_expectedTotalRbs,
                          "Total number of RBs are not as expected");
    ...
    NS_TEST_ASSERT_MSG_EQ(flag,
                          true,
                          " The indices of resource blocks are not equal to the expected indices");
}
...
class SidlelinkDiscPoolOverlapResourcesTestCase : public TestCase
{
  public:
     ...
     */
    SidlelinkDiscPoolOverlapResourcesTestCase(LteSlDiscResourcePoolFactory pfactory,
                                              uint32_t res1,
                                              uint32_t res2,
                                              bool overlap);
...
std::string
SidlelinkDiscPoolOverlapResourcesTestCase::BuildNameString(LteSlDiscResourcePoolFactory pfactory,
                                                           uint32_t res1,
                                                           uint32_t res2)
{
    std::ostringstream oss;
...
SidlelinkDiscPoolOverlapResourcesTestCase::SidlelinkDiscPoolOverlapResourcesTestCase(
    LteSlDiscResourcePoolFactory pfactory,
    uint32_t res1,
    uint32_t res2,
    bool overlap)
...
void
SidlelinkDiscPoolOverlapResourcesTestCase::DoRun()
{
    ...
    std::list<SidelinkDiscResourcePool::SidelinkTransmissionInfo> list1 =
        txpool->GetPsdchTransmissions(m_res1);
    ...
    NS_TEST_EXPECT_MSG_EQ(flag,
                          m_expectedOverlap,
                          " The overlap status of the discovery resources "
                              << m_res1 << " and  " << m_res2
                              << " is incorrect (expected=" << (uint16_t)m_expectedOverlap << ")");
}
    ...
    : TestSuite("sidelink-disc-pool", UNIT)
{
    ...
    // will start at 80 ms.
    AddTestCase(new SidlelinkDiscPoolNextFrameSubframeTestCase(pfactory, 0, 5, 8, 0),
                TestCase::QUICK);
    AddTestCase(new SidlelinkDiscPoolNextFrameSubframeTestCase(pfactory, 8, 0, 40, 0),
                TestCase::QUICK);
    AddTestCase(new SidlelinkDiscPoolNextFrameSubframeTestCase(pfactory, 40, 0, 72, 0),
                TestCase::QUICK);

    // Move to the end of the frameNo range to test roll over
    AddTestCase(new SidlelinkDiscPoolNextFrameSubframeTestCase(pfactory, 1023, 0, 8, 0),
                TestCase::QUICK);
    // Frame and subframe number from the on going discovery period
    AddTestCase(new SidlelinkDiscPoolNextFrameSubframeTestCase(pfactory, 800, 0, 808, 0),
                TestCase::QUICK);
    ...
    // psdchResourceNo:0
    AddTestCase(new SidelinkDiscPoolResourceOpportunityTestCase(pfactory, 0, 0, 0, 10, 2),
                TestCase::QUICK);
    // psdchResourceNo:1
    AddTestCase(new SidelinkDiscPoolResourceOpportunityTestCase(pfactory, 1, 0, 1, 10, 2),
                TestCase::QUICK);

    std::vector<uint32_t> rbIndex;
    // If we go to 80 ms (frame 8, subframe 0), we should be on the first subframe of the discovery
    // channel
    //  and RB 10 and 11 should be assigned for transmission
    rbIndex.push_back(10);
    rbIndex.push_back(11);
    AddTestCase(new SidlelinkDiscPoolRbsPerSubframeTestCase(pfactory, 8, 0, 2, rbIndex),
                TestCase::QUICK);
    // If we go to 81 ms (frame 8, subframe 1), we should be on the second subframe of the discovery
    // channel and RB 10 and 11 should be assigned for transmission
    AddTestCase(new SidlelinkDiscPoolRbsPerSubframeTestCase(pfactory, 8, 1, 2, rbIndex),
                TestCase::QUICK);
    ...
    rbIndex.clear();
    AddTestCase(new SidlelinkDiscPoolRbsPerSubframeTestCase(pfactory, 8, 2, 0, rbIndex),
                TestCase::QUICK);

    // SidlelinkDiscPoolOverlapResourcesTestCase Input Format:
    // pfactory,psdchResourceNo 1,psdchResourceNo 2, expected overlap
    // These tests check that the function to detect that discovery resources overlap in time is
    // working properly. Each discovery resource maps to a set of subframes and resource blocks in
    // the discovery pool. Within a sidelink period, the UE MAC must select resources that do not
    // overlap in time (see TS 36.321 5.15.1.1) even if they use different resource blocks. For this
    // test, we create a discovery pool with 4 resources
    //  layout is
    ...
    AddTestCase(new SidlelinkDiscPoolOverlapResourcesTestCase(pfactory, 0, 0, true),
                TestCase::QUICK);
    // Resources 0 and 1 are on the same RBs but different subframes, so they don't overlap
    AddTestCase(new SidlelinkDiscPoolOverlapResourcesTestCase(pfactory, 0, 1, false),
                TestCase::QUICK);
    // Resources 0 and 2 are on different RBs but same subframe, so they overlap
    AddTestCase(new SidlelinkDiscPoolOverlapResourcesTestCase(pfactory, 0, 2, true),
                TestCase::QUICK);
    // Resources 1 and 3 are on different RBs but same subframe, so they overlap
    AddTestCase(new SidlelinkDiscPoolOverlapResourcesTestCase(pfactory, 1, 3, true),
                TestCase::QUICK);
}
