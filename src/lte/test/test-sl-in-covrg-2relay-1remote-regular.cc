...
public:
  ...
  virtual ~SlInCovrg2Relay1RemoteRegularTestCase ();
  /**
...
void
SlInCovrg2Relay1RemoteRegularTestCase::DataPacketSinkTxNode (Ptr<const Packet> p, const Address &src, const Address &dst)
{
  m_txPacketNum++;
}

void
SlInCovrg2Relay1RemoteRegularTestCase::DataPacketSinkRxNode (Ptr<const Packet> p, const Address &src, const Address &dst)
{
  m_rxPacketNum++;
...
void
SlInCovrg2Relay1RemoteRegularTestCase::DoRun (void)
{
  ...
  for (uint32_t i = 0; i < remoteUeDevs.GetN (); i++)
    {
      ...
      Config::ConnectWithoutContext (txWithAddresses.str (), MakeCallback (&SlInCovrg2Relay1RemoteRegularTestCase::DataPacketSinkTxNode, this));

      std::ostringstream rxWithAddresses;
      rxWithAddresses << "/NodeList/" << remoteHost->GetId () << "/ApplicationList/0/RxWithAddresses";
      Config::ConnectWithoutContext (rxWithAddresses.str (), MakeCallback (&SlInCovrg2Relay1RemoteRegularTestCase::DataPacketSinkRxNode, this));
    }
  ...
  NS_TEST_ASSERT_MSG_EQ (m_addressList.size (), 2, "Test failed. Expected packets at the remote host from two different IP addresses, namely remote IP address under first relay and remote IP address under second relay.");

  NS_TEST_ASSERT_MSG_EQ (m_txPacketNum, m_rxPacketNum, "Number of packets received at the Remote Host is not equal to the number of packets sent from the Remote Host");
  ...
  : TestSuite ("sl-in-covrg-2relay-1remote-regular", SYSTEM)
{
  ...
  //1 Remote UE detects, connects, and uses any one of the 2 Relay UEs to send and receive traffic.
