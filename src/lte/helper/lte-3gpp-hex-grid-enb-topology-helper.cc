...
TypeId
Lte3gppHexGridEnbTopologyHelper::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::Lte3gppHexGridEnbTopologyHelper")
            .SetParent<Object>()
...
{
    NS_ABORT_MSG_IF(m > m_d / 2,
                    "UE minimum distance from eNB must be less than ISD/2 = " << m_d / 2);
    m_minDistance = m;
}
...
{
    NS_LOG_FUNCTION(this);
    NS_ABORT_MSG_IF(c.GetN() != GetNumNodes(),
                    "To create " << m_numRings
                                 << " rings, given number of nodes should be equal to "
                                 << GetNumNodes());
...
Lte3gppHexGridEnbTopologyHelper::DropUEsUniformlyPerSector(NodeContainer ues)
{
    ...
    NS_ABORT_MSG_IF(ues.GetN() % GetNumNodes() != 0,
                    "There should be equal number of UEs per eNB. Total eNBs = " << GetNumNodes());
    NS_ABORT_MSG_IF(m_minDistance > m_d / 2,
                    "UE minimum distance from eNB must be less than ISD/2 = " << m_d / 2);
    ...
    for (uint32_t n = 0; n < eNodeBpos.size(); n++)
    {
        ...
        for (uint32_t u = 0; u < numUesPerSector; u++)
        {
            ...
            } while (!(IsInsideHex(pos.x, pos.y, m_d / 2, xPos, yPos, true) &&
                       IsInsideHex(xSector,
                                   ySector,
                                   m_d / 2 * std::cos(DegreesToRadians(30)),
                                   xPos,
                                   yPos,
                                   false)) ||
                     GetDistance(pos.x, pos.y, xPos, yPos) < m_minDistance);
            // do while( Not(xPos, yPos are in the intersection of flat top hex and a pointy top hex
            // whose center is on hex flat edge) or distance < m_minDistance)
...
Lte3gppHexGridEnbTopologyHelper::DropUEsHotspot(NodeContainer ues)
{
    ...
    for (uint32_t n = 0; n < eNodeBpos.size(); n++)
    {
        ...
        // do while( Not(xPos, yPos are in the intersection of flat top hex and a pointy top hex
        // whose center is on hex flat edge) or min distance between hotspot and eNBs not met or min
        // distance to other hotspots not met)
        ...
        // Randomly and uniformly drop 2/3 UEs within hotspotRange(default 40 m) of the selected
        // area
        for (uint32_t u = 0; u < numUesHotspot; u++)
        {
            ...
            {
                // pick random x and y
                xPos = hotspotPosition.x +
                       m_uniformRandomVariable->GetValue(-m_hotspotRange, m_hotspotRange);
                yPos = hotspotPosition.y +
                       m_uniformRandomVariable->GetValue(-m_hotspotRange, m_hotspotRange);
            } while (GetDistance(hotspotPosition.x, hotspotPosition.y, xPos, yPos) >
                     m_hotspotRange);
        ...
        // Randomly and uniformly drop the remaining 1/3 UEs to the entire macro geographical area
...
NetDeviceContainer
Lte3gppHexGridEnbTopologyHelper::DropUEsIndoorOutdoor(NodeContainer ues,
                                                      double w,
                                                      double l,
                                                      double h)
{
    ...
    NS_ABORT_MSG_IF(ues.GetN() % GetNumNodes() != 0,
                    "There should be equal number of UEs per eNB. Total eNBs = " << GetNumNodes());
    NS_ABORT_MSG_IF(h < 0,
                    "height of the building should be greater than 0. Buildings with zero height "
                    "and underground buildings not supported");
    ...
    uint32_t numOutdoorUes =
        numUesPerSector - numIndoorUes; // remaining 20% of the UEs are outdoor Table A.2.1.1-1
    uint32_t currentUe = 0;
    std::vector<Ptr<Building>> buildings;
    std::vector<Ptr<Building>> buildingsInCurrentCell; // This vector contains the pointers to the
                                                       // buildings of a single cell at a time.
    ...
    for (uint32_t n = 0; n < eNodeBpos.size(); n++)
    {
        ...
        // Drop 2 RRH buildings within each cell area. Each building is w (m) x l (m) x h (m) ,
        // i.e., width x length x height
        for (uint8_t numbuildings = 0; numbuildings < MAX_SUPPORTED_BUILDINGS; numbuildings++)
        {
            ...
            // do while( Not(x and y coordinates of the center of a building are in the intersection
            // of flat top hex and a pointy top hex whose center is on hex flat edge)
        ...
        for (uint32_t u = 0; u < numIndoorUes; u++)
        {
            ...
            NS_LOG_DEBUG("Deploying UE in building "
                         << buildingIndex << " with dimensions : " << box.xMin << ":" << box.xMax
                         << ":" << box.yMin << ":" << box.yMax << ":" << box.zMin << ":"
                         << box.zMax);
            ...
            do
            {
                ...
                yPos = m_uniformRandomVariable->GetValue(box.yMin, box.yMax);
                       ...
                       IsInsideHex(xSector,
                                   ...
                                   yPos,
                                   false) &&
                       IsAtLeastDistance(xPos, yPos, indoorUes, m_minUeToUeDistanceInBuilding) &&
                       IsInsideBuilding(xPos, yPos, selectedBuilding)));
            // do while(Not(xPos, yPos are in the intersection of flat top hex and a pointy top hex
            // whose center is on hex flat edge
            //&& UE to UE minimum distance inside the building is satisfied
            //&& UE is inside the selected building))

            NS_LOG_DEBUG("Is UE inside the selected building : "
                         << IsInsideBuilding(xPos, yPos, selectedBuilding));
            // assign position to node
            Vector pos(xPos, yPos, m_ueHeight);
            Ptr<Node> node = ues.Get(currentUe);
        ...
        // Randomly and uniformly drop the remaining 20% UEs outside in the entire macro
            ...
            do
            {
                ...
                yPos = pos.y + m_uniformRandomVariable->GetValue(-m_d, m_d);
                       ...
                       IsInsideHex(xSector,
                                   ...
                                   false)) ||
                     GetDistance(pos.x, pos.y, xPos, yPos) < m_minDistance ||
                     ((IsInsideBuilding(xPos, yPos, buildingOne) ||
                       IsInsideBuilding(xPos, yPos, buildingTwo))));
            // do while(Not(xPos, yPos in the intersection of flat top hex and a pointy top hex
            // whose center is on hex flat edge)
        ...
        NS_LOG_LOGIC("Total Indoor UEs for this sector/cell = "
                     << indoorUes.size()
                     << " ratio = " << 100 * ((double)indoorUes.size() / numUesPerSector) << "%");
        NS_LOG_LOGIC("Total buildings in this sector/cell = " << buildingsInCurrentCell.size());
        buildingsInCurrentCell.erase(
            buildingsInCurrentCell.begin(),
            buildingsInCurrentCell
                .end()); // we erase already deployed buildings before moving to a new sector/cell
    }
...
uint32_t
Lte3gppHexGridEnbTopologyHelper::GetNumNodes() const
{
...
bool
Lte3gppHexGridEnbTopologyHelper::IsInsideHex(double xCenter,
                                             double yCenter,
                                             double h,
                                             double xPos,
                                             double yPos,
                                             bool flatBottom)
{
    ...
    if (flatBottom)
    {
        double v = h;
        h = v / std::sqrt(3);
        // New conditions because our cell site hex is rotated 90 degrees, having a bottom flat side
        // edge
        if (q2x > 2 * h || q2y > v)
        {
...
bool
Lte3gppHexGridEnbTopologyHelper::IsWithinDistance(double x,
                                                  double y,
                                                  std::vector<Vector> positions,
                                                  double d)
{
...
bool
Lte3gppHexGridEnbTopologyHelper::IsAtLeastDistance(double x,
                                                   double y,
                                                   std::vector<Vector> positions,
                                                   double d)
{
...
bool
Lte3gppHexGridEnbTopologyHelper::IsInsideBuilding(double x, double y, Ptr<Building> building) const
{
    Box box = building->GetBoundaries();

    NS_ABORT_MSG_IF((m_ueHeight < box.zMin || m_ueHeight > box.zMax),
                    "UE height outside the range of building height");
    Vector pos(x, y, m_ueHeight);
    return building->IsInside(pos);
}

bool
Lte3gppHexGridEnbTopologyHelper::AreOverlapping(double x,
                                                double y,
                                                double w,
                                                double l,
                                                double h,
                                                std::vector<Ptr<Building>> buildings)
{
    ...
    {
        b = it->GetBoundaries();
        if (!((currentBuilding.xMin > b.xMax) || (b.xMin > currentBuilding.xMax) ||
              (currentBuilding.yMin > b.yMax) || (b.yMin > currentBuilding.yMax)))
        {
            overlap = true;
            break;
        }
...
Lte3gppHexGridEnbTopologyHelper::GetClosestPositionInWrapAround(Vector txPos, Vector rxPos)
{
    ...
    NS_ABORT_MSG_IF(m_numRings > 4 || m_numRings < 0,
                    "WRAP-AROUND : NO SUPPORT FOR TOPOLOGIES OTHER THAN 1 THROUGH 4 RINGS!");

    // For LTE scenario, compute the distance between the UE (txPos) and the eNB (rxPos) in the
    // central cluster along with its 6 wrap-around locations. After that, return the position of an
    // eNB with the shortest distance to UE.

    // For D2D scenario, compute the distance between the transmitter UE (txPos) and the receiver UE
    // (rxPos) along with its 6 wrap around locations. And then, return the position of the receiver
    // UE with the shortest distance to transmitter UE.
    ...
    // In LTE scenario, rxPos.z is the antenna orientation of the eNB and is different for all the
    // three eNBs of three sector site. To avoid three different keys of the map for the same sight
    // we insert m_siteHeight as z coordinate of the eNB.
    ...
    m_wAroundReplica.emplace_back(pos.x, pos.y, rxPos.z);
    ...
    std::pair<std::map<std::vector<double>, WrapAroundReplicas>::iterator, bool> insertion =
        m_mapForWAround.insert(std::make_pair(m_enbPositionsCCluster, replicas));
    ...
    if (insertion.second)
    {
        NS_LOG_DEBUG("Inserting the wrap-around replicas of hex positioned at "
                     << m_enbPositionsCCluster[0] << " , " << m_enbPositionsCCluster[1]);
    }
    else
    {
        NS_LOG_DEBUG("Insertion failed. The wrap-around replicas of hex positioned at "
                     << m_enbPositionsCCluster[0] << " , " << m_enbPositionsCCluster[1]
...
Lte3gppHexGridEnbTopologyHelper::InstallWrapAroundBuildings(std::vector<Ptr<Building>> buildings)
{
    ...
    for (it = buildings.begin(); it != buildings.end(); it++)
    {
        ...
        allBuildings.push_back(AddBuilding(b.xMin + (1.5 * ((2 * m_numRings) - 1)) * s,
                                           b.xMax + (1.5 * ((2 * m_numRings) - 1)) * s,
                                           b.yMin + 0.5 * m_d,
                                           b.yMax + 0.5 * m_d,
                                           b.zMin,
                                           b.zMax));
        allBuildings.push_back(AddBuilding(b.xMin + (1.5 * (m_numRings - 1)) * s,
                                           b.xMax + (1.5 * (m_numRings - 1)) * s,
                                           b.yMin + (1 + (1.5 * (m_numRings - 1))) * m_d,
                                           b.yMax + (1 + (1.5 * (m_numRings - 1))) * m_d,
                                           b.zMin,
                                           b.zMax));
        allBuildings.push_back(AddBuilding(b.xMin - (1.5 * m_numRings) * s,
                                           b.xMax - (1.5 * m_numRings) * s,
                                           b.yMin + (0.5 + (1.5 * (m_numRings - 1))) * m_d,
                                           b.yMax + (0.5 + (1.5 * (m_numRings - 1))) * m_d,
                                           b.zMin,
                                           b.zMax));
        allBuildings.push_back(AddBuilding(b.xMin - (1.5 * ((2 * m_numRings) - 1)) * s,
                                           b.xMax - (1.5 * ((2 * m_numRings) - 1)) * s,
...
void
Lte3gppHexGridEnbTopologyHelper::AttachWithWrapAround(Ptr<PropagationLossModel> lossModel,
                                                      ...
                                                      NetDeviceContainer enbDevices)
{
    ...
    for (uint32_t u = 0; u < ueDevices.GetN(); ++u)
    {
        ...
        for (uint32_t i = 0; i < enbDevices.GetN(); ++i)
        {
            ...
            // Save the position of the eNB to restore it after RSRP is calculated for its closest
            // position including its wrap-around locations
            posEnb = enbDev->GetNode()->GetObject<MobilityModel>()->GetPosition();
            // Find position of the eNB closest to the UE (out of the eNB and its six wrap-around
            // locations)
            ...
            NS_ABORT_MSG_IF(lteEnbDev == nullptr, "No LteEnbNetDevice found");
            Ptr<LteEnbPhy> lteEnbPhy = lteEnbDev->GetPhy();
            NS_ABORT_MSG_IF(lteEnbPhy == nullptr, "No LteEnbPhy found");
            ...
            NS_ABORT_MSG_IF(lteUeDev == nullptr, "No LteUeNetDevice found");
            Ptr<LteUePhy> lteUePhy = lteUeDev->GetPhy();
            NS_ABORT_MSG_IF(lteUePhy == nullptr, "No LteUePhy found");
            ...
            NS_LOG_DEBUG("UE : " << u << " IMSI : " << lteUeDev->GetImsi() << " to Cell id : "
                                 << lteEnbDev->GetCellId() << " Position of closest eNB : "
                                 << lteEnbDev->GetNode()->GetObject<MobilityModel>()->GetPosition()
                                 << " RSRP : " << rsrp << " dBm");
            ...
            // Restore eNB position, since we already cache the pathloss restoring the position will
            // not affect the future pathloss values. This is needed to keep intact the position of
            // the central cluster so that the remaining UEs can find the closest position
            // correctly.
        ...
        NS_LOG_DEBUG("Attached UE with IMSI : "
                     << lteUeDev->GetImsi() << " to Cell id : " << bestLteEnbDev->GetCellId()
                     << " Best eNB position  : " << bestClosestPos << " RSRP = " << bestRsrp);

        if (IsInWrapAround(bestClosestPos.x, bestClosestPos.y))
        {
            NS_LOG_DEBUG("Attached in wrap-around. UE : "
                         << u << " IMSI : " << lteUeDev->GetImsi()
                         << " to Cell id : " << bestLteEnbDev->GetCellId()
                         << " Position of best eNB in central cluster : "
                         << bestEnbDev->GetNode()->GetObject<MobilityModel>()->GetPosition()
...
{
    NS_LOG_DEBUG("Building dimensions " << x - w / 2 << " : " << x + w / 2 << " : " << y - l / 2
                                        << " : " << y + l / 2 << " : " << 0.0 << " : " << h);
...
Ptr<Building>
Lte3gppHexGridEnbTopologyHelper::AddBuilding(double xMin,
                                             double xMax,
                                             double yMin,
                                             double yMax,
                                             double zMin,
...
Lte3gppHexGridEnbTopologyHelper::IsInWrapAround(double x, double y)
{
    ...
    NS_LOG_DEBUG("xMinLastRing " << -xMaxLastRing << " xMaxLastRing " << xMaxLastRing
                                 << " yMinLastRing " << -yMaxLastRing << " yMaxLastRing "
                                 << yMaxLastRing);

    // Before checking every hexagon of central cluster do a quick check to detect if the point is
    // outside max limits.
    ...
    // if we reach here that means the point is in one of the corner hex whose center is under the
    // limits of +-xMaxLastRing,+-yMaxLastRing but still is out of central cluster. For 2 rings,
    // there will 4 such hexagon, each on one corner and for 3 and 4 rings there will be 8 and 12,
    // respectively. In any case, this function is general enough to detect any point outside the
    // hexagons of central cluster.
    ...
    for (const auto& it : nodePositions)
    {
        ...
        {
            NS_LOG_DEBUG("Point is inside of the central cluster hexagon. Hex center : "
                         << it.x << " : " << it.y);
