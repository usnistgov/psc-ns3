...
class LteSidelinkHelper : public Object
{
  public:
    ...
    struct LteSlO2OIpv6AddressValue
    {
        ...
        Ipv6Address relay_address = Ipv6Address::GetOnes();  ///< Address assigned for relay UE (or
                                                             ///< all 1s if not initialized)
        Ipv6Address remote_address = Ipv6Address::GetOnes(); ///< Address assigned for remote UE (or
                                                             ///< all 1s if not initialized)
    };
    ...
    {
        return (l.relayL2Id < r.relayL2Id) ||
               (l.relayL2Id == r.relayL2Id && l.remoteL2Id < r.remoteL2Id);
    }

    LteSidelinkHelper();
    ~LteSidelinkHelper() override;
     ...
     */
    std::vector<NetDeviceContainer> AssociateForGroupcast(
        double txPower,
        double ulEarfcn,
        double ulBandwidth,
        ...
        int nGroups,
        int nReceivers,
        SrsrpMethod_t compMethod = LteSidelinkHelper::SLRSRP_PSBCH);
     ...
     * \return The list of groups, with first NetDevice in the container being the transmitter
     */
    std::vector<NetDeviceContainer> AssociateForBroadcast(
        double txPower,
        double ulEarfcn,
        double ulBandwidth,
        NetDeviceContainer ues,
     ...
     * \return The list of groups, with first NetDevice in the container being the transmitter
     */
    std::vector<NetDeviceContainer> AssociateForBroadcastWithTxEnabledToReceive(
        double txPower,
        double ulEarfcn,
        double ulBandwidth,
        NetDeviceContainer ues,
     ...
     * \return The list of groups, with first NetDevice in the container being the transmitter
     */
    std::vector<NetDeviceContainer> AssociateForBroadcastWithWrapAround(
        double txPower,
        double ulEarfcn,
        double ulBandwidth,
        NetDeviceContainer ues,
     ...
     * announcing) \param ueDevice the targeted device \param appCodes app code payloads to be added
     * \param role UE role (discovered or discoveree)
     */
    void StartDiscoveryApps(Ptr<NetDevice> ueDevice,
                            std::list<uint32_t> appCodes,
                            LteSlUeRrc::DiscoveryRole role);

    /**
     * Stops discovery process for given applications depending on the interest (monitoring or
     * announcing) \param ueDevice the targeted device \param appCodes app code payloads to be
     * removed \param role UE role (discovered or discoveree)
     */
    void StopDiscoveryApps(Ptr<NetDevice> ueDevice,
                           std::list<uint32_t> appCodes,
                           LteSlUeRrc::DiscoveryRole role);
     ...
     */
    void StartRelayService(Ptr<NetDevice> ueDevice,
     ...
     */
    Ipv6InterfaceContainer AssignIpv6AddressForRelayCommunication(Ptr<NetDevice> dev,
                                                                  uint32_t relayL2Id,
                                                                  uint32_t remoteL2Id,
                                                                  LteSlUeRrc::RelayRole role);
     ...
     */
    Ipv6Address GetRelayIpv6AddressFromMap(uint32_t relayL2Id,
                                           uint32_t remoteL2Id,
                                           LteSlUeRrc::RelayRole role);
     ...
     */
    Ipv6Address GetSelfIpv6AddressFromMap(uint32_t relayL2Id,
                                          uint32_t remoteL2Id,
                                          LteSlUeRrc::RelayRole role);
  ...
  private:
    ...
    Ipv6AddressHelper
        m_relayIpv6ah; ///< Address helper for assigning addresses for UE relay operations
    Ipv6Prefix m_ipv6AddressRelayPrefix; ///< Prefix from which addresses are assigned for UE relay
                                         ///< operations
    Ipv6Address m_ipv6AddressRelayNetwork; ///< Base network to assign IPv6 address helper
    std::map<LteSlO2OIpv6AddressKey, LteSlO2OIpv6AddressValue>
        m_ipv6AddressValueMap; ///< Provides mapping between pairs of L2 addresses and IPv6 address
                               ///< helper, IPv6 addresses and required booleans
};
