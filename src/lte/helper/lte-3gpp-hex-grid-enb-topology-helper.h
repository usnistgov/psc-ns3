...
{
  public:
    Lte3gppHexGridEnbTopologyHelper();
    ~Lte3gppHexGridEnbTopologyHelper() override;
    ...
    /**
     * Set the LteHelper to be used to actually create the EnbNetDevices and UeNetDevices in hex
     * grid
     *
    ...
    /**
     * Drops UEs uniformly (i.e., each sector will have equal number of UEs) within the coverage of
     * the three sector cells, leaving no cell gaps, and not exceeding cell boundaries. This method
     * also makes sure that the distance between the eNB and the UE is greater the minimum
     * configured distance (i.e., the value set through the attribute "MinDistance" or directly
     * calling SetMinimumDistance.
     *
     ...
     * method. E.g, 1 ring contains 3 nodes, 2 rings have 21 nodes, 3 rings have 57 nodes. By
     * default, the function will return the value 3, as the default number of rings is set to 1.
     *
     ...
     */
    void AttachWithWrapAround(Ptr<PropagationLossModel> lossModel,
  ...
  private:
     ...
     */
    bool IsInsideHex(double xCenter,
                     double yCenter,
                     double h,
                     double xPos,
                     double yPos,
     ...
     */
    bool IsInsideBuilding(double x, double y, Ptr<Building> building) const;
     ...
     *
     * \return True if a building formed by x,y,w,l,h overlaps with already deployed buildings,
     * False otherwise
     */
    bool AreOverlapping(double x,
                        double y,
                        double w,
                        double l,
                        double h,
     ...
     * \return The pointer to the deployed buildings
     */
    Ptr<Building> AddBuilding(double xMin,
                              double xMax,
                              double yMin,
                              double yMax,
                              double zMin,
    ...
    double m_minEnodebToBuildingDistance; ///< The minimum distance between the center of the
                                          ///< building and the eNodeB
    ...
    std::vector<double> m_enbPositionsCCluster; ///< Stores the position of an eNB in the central
                                                ///< cluster of hexagonal grid
    ...
    std::map<std::vector<double>, WrapAroundReplicas>
        m_mapForWAround; ///< A map for storing position of each hexagon and its six replicas in
                         ///< wrap-around
    ...
    std::map<uint64_t, WrapAroundInfo_t>
        m_mapForWrapAroundInfo; ///< map to store wrap-around info of the UE
};
