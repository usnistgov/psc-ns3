...
TypeId
RrcStatsCalculator::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::RrcStatsCalculator")
...
std::string
RrcStatsCalculator::GetSlDiscRrcOutputFilename()
{
    return LteStatsCalculator::GetSlOutputFilename();
}

void
RrcStatsCalculator::RrcDiscoveryMonitoring(uint64_t imsi,
                                           uint16_t cellId,
                                           uint16_t rnti,
                                           LteSlDiscHeader discMsg)
{
    ...
    if (m_discoveryMonitoringRrcFirstWrite)
    {
        ...
        outFile << "Time\tIMSI\tCellId\tRNTI\tDiscType\tContentType\tDiscModel\tContent"
                << std::endl;
    }
    else
    {
    ...
    case LteSlDiscHeader::DISC_RELAY_RESPONSE: // UE-to-Network Relay Discovery Response in model B
    {
        ...
        outFile << discMsg.GetRelayServiceCode() << ";" << discMsg.GetInfo() << ";"
                << discMsg.GetRelayUeId() << ";" << (uint16_t)discMsg.GetStatusIndicator() << ";0"
                << std::endl;
    }
    break;
    case LteSlDiscHeader::DISC_RELAY_SOLICITATION: {
        // write fields, include spare (0) as the last field
        outFile << discMsg.GetRelayServiceCode() << ";" << discMsg.GetInfo() << ";"
                << (uint16_t)discMsg.GetURDSComposition() << ";" << discMsg.GetRelayUeId() << ";0"
                << std::endl;
    }
    break;
...
void
RrcStatsCalculator::DiscoveryMonitoringRrcTraceCallback(Ptr<RrcStatsCalculator> rrcStats,
                                                        std::string path,
                                                        uint64_t imsi,
                                                        uint16_t cellId,
                                                        uint16_t rnti,
