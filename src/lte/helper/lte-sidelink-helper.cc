...
TypeId
LteSidelinkHelper::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::LteSidelinkHelper").SetParent<Object>().AddConstructor<LteSidelinkHelper>()
...
LteSidelinkHelper::AssociateForGroupcast(double txPower,
                                         double ulEarfcn,
                                         double ulBandwidth,
                                         ...
                                         SrsrpMethod_t compMethod)
{
    ...
    NetDeviceContainer
        candidateTx; // list of UEs not assigned to groups that can be selected for transmission
    candidateTx.Add(ues);
    ...
    while (numGroupsAssociated < nGroups && candidateTx.GetN() > 0)
    {
        ...
        // Receiver UE is randomly selected from the remaining UEs (i.e., not already part of a
        // group).
        while (numReceiversAssociated < nReceivers && candidateRx.GetN() > 0)
        {
            ...
            {
                rsrpRx = SidelinkRsrpCalculator::CalcSlRsrpPsbch(lossModel,
                                                                 txPower,
                                                                 ulEarfcn,
                                                                 ulBandwidth,
                                                                 txPhy,
            ...
            // If receiver UE is not within RSRP* of X dBm of the transmitter UE then randomly
            // reselect the receiver UE among the UEs that are within the RSRP of X dBm of the
            // transmitter UE and are not part of a group already.
            NS_LOG_DEBUG("\tCandidate Rx= " << rx->GetNode()->GetId() << " Rsrp=" << rsrpRx
                                            << " required=" << rsrpThreshold);
...
std::vector<NetDeviceContainer>
LteSidelinkHelper::AssociateForBroadcast(double txPower,
                                         double ulEarfcn,
                                         double ulBandwidth,
                                         NetDeviceContainer ues,
                                         double rsrpThreshold,
                                         uint32_t nTransmitters,
                                         SrsrpMethod_t compMethod)
{
    std::vector<NetDeviceContainer> groups; // groups created
    ...
    for (uint32_t i = 0; i < numTransmittersSelected; i++)
    {
        ...
        // Initializing link to other transmitters to be able to receive SLSSs from other
        // transmitters
        for (uint32_t k = 0; k < numTransmittersSelected; k++)
        {
            if (k != i)
            {
                ...
                Ptr<SpectrumPhy> txPhy =
                    tx->GetObject<LteUeNetDevice>()->GetPhy()->GetUlSpectrumPhy();
                Ptr<SpectrumPhy> otherTxPhy =
                    othertx->GetObject<LteUeNetDevice>()->GetPhy()->GetUlSpectrumPhy();
                ...
                {
                    rsrpRx = SidelinkRsrpCalculator::CalcSlRsrpTxPw(lossModel,
                                                                    txPower,
                                                                    txPhy,
                                                                    otherTxPhy);
                }
...
std::vector<NetDeviceContainer>
LteSidelinkHelper::AssociateForBroadcastWithTxEnabledToReceive(double txPower,
                                                               double ulEarfcn,
                                                               double ulBandwidth,
                                                               NetDeviceContainer ues,
                                                               double rsrpThreshold,
                                                               ...
                                                               SrsrpMethod_t compMethod)
{
    ...
    for (uint32_t i = 0; i < numTransmittersSelected; i++)
    {
        ...
        for (uint32_t j = 0; j < nRxDevices; ++j)
        {
            Ptr<NetDevice> rx = remainingUes.Get(j);
            if (rx->GetNode()->GetId() !=
                tx->GetNode()->GetId()) // No loopback link possible due to half-duplex
            {
                Ptr<SpectrumPhy> txPhy =
                    tx->GetObject<LteUeNetDevice>()->GetPhy()->GetUlSpectrumPhy();
                Ptr<SpectrumPhy> rxPhy =
                    rx->GetObject<LteUeNetDevice>()->GetPhy()->GetUlSpectrumPhy();
                ...
                {
                    rsrpRx =
                        SidelinkRsrpCalculator::CalcSlRsrpTxPw(lossModel, txPower, txPhy, rxPhy);
                }
                // If receiver UE is not within RSRP* of X dBm of the transmitter UE then randomly
                // reselect the receiver UE among the UEs that are within the RSRP of X dBm of the
                // transmitter UE and are not part of a group already.
...
std::vector<NetDeviceContainer>
LteSidelinkHelper::AssociateForBroadcastWithWrapAround(
    double txPower,
    double ulEarfcn,
    double ulBandwidth,
    NetDeviceContainer ues,
    double rsrpThreshold,
    uint32_t nTransmitters,
    Ptr<Lte3gppHexGridEnbTopologyHelper> topologyHelper,
    SrsrpMethod_t compMethod)
{
    std::vector<NetDeviceContainer> groups; // groups created
    ...
    for (uint32_t i = 0; i < numTransmittersSelected; i++)
    {
        ...
        for (uint32_t j = 0; j < nRxDevices; ++j)
        {
            ...
            Vector closestPos = topologyHelper->GetClosestPositionInWrapAround(
                tx->GetNode()->GetObject<MobilityModel>()->GetPosition(),
                rxPos);
    ...
    *stream->GetStream() << "TxNID\tRxNID\tTxIMSI\tRxIMSI" << std::endl;
    ...
    for (gIt = groups.begin(); gIt != groups.end(); gIt++)
    {
        ...
        else
        {
            ...
            {
                *stream->GetStream()
                    << (*gIt).Get(0)->GetNode()->GetId() << "\t"
                    << (*gIt).Get(i)->GetNode()->GetId() << "\t"
                    << (*gIt).Get(0)->GetObject<LteUeNetDevice>()->GetImsi() << "\t"
                    << (*gIt).Get(i)->GetObject<LteUeNetDevice>()->GetImsi() << std::endl;
            }
        }
    }
}
...
void
LteSidelinkHelper::ActivateSidelinkBearer(Time activationTime,
                                          NetDeviceContainer ues,
                                          Ptr<LteSlTft> tft)
{
    NS_LOG_FUNCTION(this);
    NS_ASSERT_MSG(
        m_lteHelper,
        "Sidelink activation requires LteHelper to be registered with the LteSidelinkHelper");
    Simulator::Schedule(activationTime,
                        &LteSidelinkHelper::DoActivateSidelinkBearer,
                        this,
                        ues,
                        tft);
}
...
void
LteSidelinkHelper::StartDiscoveryApps(Ptr<NetDevice> ueDevice,
                                      std::list<uint32_t> appCodes,
                                      LteSlUeRrc::DiscoveryRole role)
{
    NS_LOG_FUNCTION(this);
...
void
LteSidelinkHelper::StopDiscoveryApps(Ptr<NetDevice> ueDevice,
                                     std::list<uint32_t> appCodes,
                                     LteSlUeRrc::DiscoveryRole role)
{
    NS_LOG_FUNCTION(this);
...
void
LteSidelinkHelper::StartRelayService(Ptr<NetDevice> ueDevice,
...
LteSidelinkHelper::SetIpv6BaseForRelayCommunication(Ipv6Address network, Ipv6Prefix prefix)
{
    ...
    // 60 is currently set as the maximum allowed number of bits for the subnet part of the address
    // provided by the user We add 4 more bits along with the user provided 60 bits for the 64 bit
    // prefix, which gives us 2^4 combinations of prefixes
    NS_ASSERT_MSG(prefix.GetPrefixLength() <= 64,
                  "Provided a network address of more than 64 bits!");

    m_relayIpv6ah.SetBase(network, Ipv6Prefix(64));
}

Ipv6InterfaceContainer
LteSidelinkHelper::AssignIpv6AddressForRelayCommunication(Ptr<NetDevice> dev,
                                                          uint32_t relayL2Id,
                                                          uint32_t remoteL2Id,
                                                          LteSlUeRrc::RelayRole role)
{
    ...
    if (role == LteSlUeRrc::RelayUE)
    {
        if (value.relay_address == Ipv6Address::GetOnes())
        {
            ...
            NS_ASSERT_MSG(value.relay_address.CombinePrefix(m_ipv6AddressRelayPrefix) ==
                              m_ipv6AddressRelayNetwork,
                          "Address assigned outside allowed values");
            m_ipv6AddressValueMap.insert(
                std::pair<LteSlO2OIpv6AddressKey, LteSlO2OIpv6AddressValue>(key, value));
    ...
    if (role == LteSlUeRrc::RemoteUE)
    {
        ...
        {
            // first time
            NS_ASSERT_MSG(value.relay_address != Ipv6Address::GetOnes(),
                          "Remote UE cannot initiate IPv6 Address allocation procedure!");
...
Ipv6Address
LteSidelinkHelper::GetRelayIpv6AddressFromMap(uint32_t relayL2Id,
                                              uint32_t remoteL2Id,
                                              LteSlUeRrc::RelayRole role)
{
    LteSlO2OIpv6AddressKey key;
...
Ipv6Address
LteSidelinkHelper::GetSelfIpv6AddressFromMap(uint32_t relayL2Id,
                                             uint32_t remoteL2Id,
                                             LteSlUeRrc::RelayRole role)
{
...
void
LteSidelinkHelper::RemoteUeContextConnected(uint64_t relayImsi,
                                            uint64_t ueImsi,
...
void
LteSidelinkHelper::RemoteUeContextDisconnected(uint64_t relayImsi,
                                               uint64_t ueImsi,
                                               uint8_t ipv6Prefix[8])
{
    NS_LOG_FUNCTION(this << relayImsi << ueImsi << ipv6Prefix);
...
LteSidelinkHelper::ConfigurePgwToUeToNetworkRelayRoute(Ptr<Node> pgw)
{
    ...
    pgwStaticRouting->AddNetworkRouteTo(m_ipv6AddressRelayNetwork,
                                        m_ipv6AddressRelayPrefix,
                                        Ipv6Address("::"),
                                        1,
                                        0);
}
