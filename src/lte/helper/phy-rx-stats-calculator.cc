...
std::string
PhyRxStatsCalculator::GetSlRxOutputFilename()
{
...
std::string
PhyRxStatsCalculator::GetSlPscchRxOutputFilename()
{
...
{
    NS_LOG_FUNCTION(this << params.m_timestamp << params.m_cellId << params.m_imsi << params.m_rnti
                         << (uint16_t)params.m_mcs << params.m_size << params.m_resPscch
                         << (uint16_t)params.m_rbLen << (uint16_t)params.m_rbStart
                         << (uint16_t)params.m_iTrp << (uint16_t)params.m_hopping
                         << (uint16_t)params.m_groupDstId << (uint16_t)params.m_correctness);
    NS_LOG_INFO("Write SL Rx PSCCH Stats in " << GetSlPscchRxOutputFilename().c_str());

    std::ofstream outFile;
    if (m_slPscchRxFirstWrite)
    {
        ...
        outFile << "% "
                   "time\tcellId\tIMSI\tRNTI\tresPscch\tsizeTb\thopping\thoppingInfo\tpsschRbLen\tp"
                   "sschStartRb\tiTrp\tmcs\tl1GroupDstId\tcorrect";
        outFile << std::endl;
    }
    else
    {
