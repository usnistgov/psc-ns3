 ...
 * |----------.-----------------.-------------|
 * | true     | -100            |   20/20     | // Remote UE activates Relay Service at the
 * beginning of the simulation (the RSRP is already below -100 dBm in the initial position)
 * |----------.-----------------.-------------|
 * | true     | -120            |   16/20     | // Remote UE activates Relay Service when 'entering'
 * to the building (the RSRP decays below -120 dBm when entering to the building, i.e., after 10 s).
 * |----------.-----------------.-------------|
 * | true     | -127            |   12/20     | // Remote UE activates Relay Service when inside of
 * the building (the RSRP decays below -127 dBm after 12 s).
 * '------------------------------------------'
 *
 ...
 */
void
RsrpMeasurementTrace(Ptr<OutputStreamWrapper> stream,
                     ...
                     double rsrp)
{
    ...
    *stream->GetStream() << Simulator::Now().GetNanoSeconds() / (double)1e9 << "\t" << imsi << "\t"
    ...
    else
    {
        ...
        // the UE is not in Remote UE role already (i.e., already monitoring Discovery Relay
        // Announcements (we are using discovery Model A)) then start UE-to-Network Relay Service as
        // Remote UE
        if (rsrp < threshold &&
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("threshold",
                 "The RSRP value (dBm) for which the Remote UE will start Relay Service",
                 threshold);
    ...
    else
    {
        ...
        double rm_vel_y = 0.0;
        remoteUeNodes.Get(0)->GetObject<ConstantVelocityMobilityModel>()->SetVelocity(
    ...
    NS_LOG_INFO("Remote UE node id = [" << remoteUeNodes.Get(0)->GetId()
                                        << "] start application at " << startTimeRemoteApp << " s");
    ...
    *packetOutputStream->GetStream()
    ...
    if (useRelay)
    {
        ...
        Simulator::Schedule(Seconds(startTimeRelay),
                            ...
                            LteSlUeRrc::RelayUE);
        NS_LOG_INFO("Relay UE node id = [" << relayUeNodes.Get(0)->GetId()
                                           << "] provides Service Code " << SERVICE_CODE
                                           << " and start service at " << startTimeRelay << " s");
        ...
        *PC5SignalingPacketTraceStream->GetStream() << "time(s)\ttxId\tRxId\tmsgType" << std::endl;
        ...
        Ptr<OutputStreamWrapper> RsrpMeasurementsStream =
            ascii.CreateFileStream("RsrpMeasurementTrace.txt");
        *RsrpMeasurementsStream->GetStream()
            << "time(s)\timsi\tcellId\trsrp\tthreshold" << std::endl;
