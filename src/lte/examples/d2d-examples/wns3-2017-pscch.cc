                       ...
                       const Address& dstAddrs)
{
    ...
    *stream->GetStream() << Simulator::Now().GetNanoSeconds() / (double)1e9 << "\t" << contex
                         << "\t" << p->GetSize() << "\t";
    if (InetSocketAddress::IsMatchingType(srcAddrs))
    {
        ...
        if (!oss.str().compare("0.0.0.0")) // srcAddrs not set
        {
            *stream->GetStream() << localAddrs << ":"
                                 ...
                                 << InetSocketAddress::ConvertFrom(dstAddrs).GetPort() << std::endl;
        }
        else
        {
            oss.str("");
            ...
            if (!oss.str().compare("0.0.0.0")) // dstAddrs not set
            {
                *stream->GetStream()
                    ...
                    << InetSocketAddress::ConvertFrom(srcAddrs).GetPort() << "\t" << localAddrs
                    << ":" << InetSocketAddress::ConvertFrom(dstAddrs).GetPort() << std::endl;
            }
            else
            {
    ...
    {
        *stream->GetStream() << Inet6SocketAddress::ConvertFrom(srcAddrs).GetIpv6() << ":"
                             << Inet6SocketAddress::ConvertFrom(srcAddrs).GetPort() << "\t"
                             << Inet6SocketAddress::ConvertFrom(dstAddrs).GetIpv6() << ":"
                             << Inet6SocketAddress::ConvertFrom(dstAddrs).GetPort() << std::endl;
    }
    else
    {
...
main(int argc, char* argv[])
{
    ...
    uint32_t numReceivers =
        0;               // Number of receivers per group. If zero, then group size is not limited.
    uint32_t mcs = 10;   // Modulation and Coding Scheme
    uint32_t rbSize = 2; // PSSCH subchannel allocation size in RBs
    uint32_t ktrp = 1;   // Transmissions opportunities in a Time Repetition Pattern(number of
                         // subframes to transmit on)
    uint32_t pscchRbs = 22; // PSCCH pool size in RBs. Note, the PSCCH occupied bandwidth will be at
    ...
    bool dropOnCollisionEnabled =
        false;                 // Drop PSCCH PSSCH messages on conflicting scheduled resources
    ...
    cmd.AddValue("dropOnCol",
                 "Drop PSCCH and PSSCH messages on collisions",
    ...
    std::cout << "CtrlErrorModel: " << ((ctrlErrorModelEnabled) ? "Enabled" : "Disabled")
              << std::endl;
    std::cout << "DropOnCollision: " << ((dropOnCollisionEnabled) ? "Enabled" : "Disabled")
              << std::endl;
    ...
    if (ueAllNodes.GetN() != 0)
    {
        ...
        {
            outFile << "eNB\t" << sectorNodes.Get(i)->GetId() << "\t"
                    << sectorNodes.Get(i)->GetObject<MobilityModel>()->GetPosition().x << "\t"
                    << sectorNodes.Get(i)->GetObject<MobilityModel>()->GetPosition().y << std::endl;
        }
        ...
        for (uint32_t i = 0; i < ueAllNodes.GetN(); ++i)
        {
            ...
            outFile << " UE\t" << ueNetDevice->GetObject<LteUeNetDevice>()->GetImsi() << "\t"
                    << ueAllNodes.Get(i)->GetObject<MobilityModel>()->GetPosition().x << "\t"
                    << ueAllNodes.Get(i)->GetObject<MobilityModel>()->GetPosition().y << std::endl;
        }
        outFile.close();
    }
    ...
    {
        // groupcast
        createdgroups = proseHelper->AssociateForGroupcast(ueTxPower,
                                                           ulEarfcn,
                                                           ulBandwidth,
                                                           ueRespondersDevs,
                                                           -112,
                                                           numGroups,
                                                           numReceivers,
                                                           LteSidelinkHelper::SLRSRP_TX_PW);
    }
    else
    {
        // broadcast
        createdgroups = proseHelper->AssociateForBroadcastWithTxEnabledToReceive(
            ueTxPower,
            ulEarfcn,
            ulBandwidth,
            ueRespondersDevs,
    ...
    for (gIt = createdgroups.begin(); gIt != createdgroups.end(); gIt++)
    {
        if (gIt->GetN() < numReceivers)
        {
            ...
            std::cerr << "Unable to satisfy group size requested for groupcast. Either the UEs are "
                         "out of reach or there are not enough UEs!"
                      << std::endl;
            return 1;
        }
        totalRxs += gIt->GetN() - 1;
    ...
    std::cout << "Average number of receivers per transmitter = " << (totalRxs / numGroups)
              << std::endl;
    std::cout << "Average number of transmitters per receiver = "
              << (totalTxPerUe / txPerUeMap.size()) << std::endl;
    ...
    *stream->GetStream() << "time\ttx/rx\tNID\tIMSI\tUEtype\tsize\tIP[src]\tIP[dst]" << std::endl;
    ...
    for (gIt = createdgroups.begin(); gIt != createdgroups.end(); gIt++)
    {
        ...
        Ptr<LteSlTft> tft =
            Create<LteSlTft>(LteSlTft::TRANSMIT, clientRespondersAddress, groupL2Address);
    ...
    for (uint16_t ac = 0; ac < clientRespondersApps.GetN(); ac++)
    {
        ...
        Ipv4Address localAddrs = activeTxUes.Get(ac)
                                     ->GetNode()
    ...
    std::cout << "PSCCH RBS: " << pscchRbs << ", PSCCH hexstring: " << pscchBitmapHexstring
              << std::endl;
