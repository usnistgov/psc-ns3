...
{
    void Trace(uint64_t imsi,
               uint16_t cellId,
               uint16_t rnti,
               LteSlDiscHeader discHeader); ///< Callback function
    uint32_t m_nRxMsgs{0};                  ///< Number of messages received
};

void
DiscoveryMonitoringTracer::Trace(uint64_t imsi,
                                 uint16_t cellId,
                                 uint16_t rnti,
...
main(int argc, char* argv[])
{
    ...
    ueRrc0->TraceConnectWithoutContext("DiscoveryMonitoring",
                                       MakeCallback(&DiscoveryMonitoringTracer::Trace, &tracer0));
    DiscoveryMonitoringTracer tracer1;
    Ptr<LteUeRrc> ueRrc1 = ueDevs.Get(1)->GetObject<LteUeNetDevice>()->GetRrc();
    ueRrc1->TraceConnectWithoutContext("DiscoveryMonitoring",
                                       MakeCallback(&DiscoveryMonitoringTracer::Trace, &tracer1));
