        ...
        else
        {
            ...
            if (oss.str() == "::") // dstAddrs not set
            {
                *stream->GetStream()
                    ...
                    << Inet6SocketAddress::ConvertFrom(srcAddrs).GetPort() << "\t"
                    << Ipv6Address::ConvertFrom(localAddrs) << ":"
...
main(int argc, char* argv[])
{
    ...
    for (uint32_t devIt = 0; devIt < ueDevs.GetN(); devIt++)
    {
        ...
        ueDevs.Get(devIt)->GetObject<LteUeNetDevice>()->GetPhy()->SetFirstScanningTime(
            MilliSeconds(base_t + (devIt * base_t)));
        std::cout << "IMSI " << ueDevs.Get(devIt)->GetObject<LteUeNetDevice>()->GetRrc()->GetImsi()
                  << " SLSSID " << devIt + 10 << " First Scan " << base_t + (devIt * base_t)
                  << " ms" << std::endl;
    }
    ...
    *stream->GetStream() << "time(sec)\ttx/rx\tNodeID\tIMSI\tPktSize(bytes)\tIP[src]\tIP[dst]"
                         << std::endl;
    ...
    if (!useIPv6)
    {
        ...
        for (uint32_t ac = 0; ac < clientApps.GetN(); ac++)
        {
            Ipv4Address localAddrs = clientApps.Get(ac)
                                         ...
                                         ->GetAddress(1, 0)
                                         .GetLocal();
            std::cout << "Tx address: " << localAddrs << " Node Id "
                      << clientApps.Get(ac)->GetNode()->GetId() << std::endl;
            oss << "tx\t" << clientApps.Get(ac)->GetNode()->GetId() << "\t"
                << clientApps.Get(ac)
                       ->GetNode()
                       ->GetDevice(0)
            ...
            clientApps.Get(ac)->TraceConnect("TxWithAddresses",
                                             oss.str(),
                                             MakeBoundCallback(&UePacketTrace, stream, localAddrs));
            oss.str("");
        }
        ...
        for (uint32_t ac = 0; ac < serverApps.GetN(); ac++)
        {
            Ipv4Address localAddrs = serverApps.Get(ac)
                                         ...
                                         ->GetAddress(1, 0)
                                         .GetLocal();
            std::cout << "Rx address: " << localAddrs << " Node Id "
                      << serverApps.Get(ac)->GetNode()->GetId() << std::endl;
