              ...
              const Address& dstAddrs)
{
    ...
    *stream->GetStream() << Simulator::Now().GetNanoSeconds() / (double)1e9 << "\t" << context
 ...
 */
void
ReportUeSdRsrpMeasurementsTrace(Ptr<OutputStreamWrapper> stream,
                                uint16_t rnti,
                                uint64_t relayId,
                                ...
                                double sdRsrp)
{
    ...
    *stream->GetStream() << Simulator::Now().GetNanoSeconds() / (double)1e9 << "\t" << rnti << "\t"
                         << relayId << "\t" << serviceCode << "\t" << sdRsrp << "\t" << std::endl;
}
 ...
 */
void
RelayUeSelectionTrace(Ptr<OutputStreamWrapper> stream,
                      uint64_t imsi,
                      uint32_t serviceCode,
                      uint64_t currentRelayId,
                      uint64_t selectedRelayId)
{
    std::ostringstream oss;
    stream->GetStream()->precision(6);

    *stream->GetStream() << Simulator::Now().GetNanoSeconds() / (double)1e9 << "\t" << imsi << "\t"
                         << serviceCode << "\t" << currentRelayId << "\t" << selectedRelayId << "\t"
                         << std::endl;
}
 ...
 */
void
Pc5ConnectionStatusTrace(Ptr<OutputStreamWrapper> stream,
                         uint32_t selfUeId,
                         uint32_t peerUeId,
                         ...
                         uint32_t iface)
{
    ...
    *stream->GetStream() << Simulator::Now().GetNanoSeconds() / (double)1e9 << "\t" << selfUeId
                         << "\t" << peerUeId << "\t" << role << "\t" << status << "\t" << std::endl;
}
 ...
 */
void
GenerateTopologyPlotFile(NodeContainer enbNode,
                         ...
                         double remoteRadius)
{
    ...
    for (uint32_t ry = 0; ry < relayUeNodes.GetN(); ry++)
    {
        ...
        std::ostringstream strForLabel;
        strForLabel << "set label \"" << relayUeNodes.Get(ry)->GetId() << "\" at " << x << "," << y
                    << " textcolor rgb \"grey\" center front offset 0,1";
        plot.AppendExtra(strForLabel.str());
    ...
    for (uint32_t rm = 0; rm < remoteUeNodes.GetN(); rm++)
    {
        ...
        std::ostringstream strForLabel;
        strForLabel << "set label \"" << remoteUeNodes.Get(rm)->GetId() << "\" at " << x << "," << y
                    << " textcolor rgb \"grey\" center front offset 0,1";
        plot.AppendExtra(strForLabel.str());
 ...
 */
void
ChangeUdpEchoClientRemote(Ptr<Node> newRemoteNode,
                          Ptr<UdpEchoClient> app,
                          uint16_t port,
                          Ipv6Address network,
                          Ipv6Prefix prefix)
{
    ...
    Ipv6Address remoteNodeSlIpAddress =
        newRemoteNode->GetObject<Ipv6L3Protocol>()->GetAddress(ipInterfaceIndex, 1).GetAddress();
    NS_LOG_INFO(" Node id = [" << app->GetNode()->GetId()
                               << "] changed the UdpEchoClient Remote Ip Address to "
                               << remoteNodeSlIpAddress);
...
{
    double simTime =
        1.0; // s //Simulation time (in seconds) updated automatically based on number of nodes
    ...
    cmd.AddValue(
        "relayRadius",
        "The radius of the circle (with center on the eNB) where the Relay UEs are positioned",
        relayRadius);
    cmd.AddValue("remoteRadius",
                 "The radius of the circle (with center on the Relay UE) where the Remote UEs are "
                 "positioned",
                 remoteRadius);
    cmd.AddValue("nRelayUes", "Number of Relay UEs in the scenario", nRelayUes);
    cmd.AddValue("nRemoteUesPerRelay",
                 "Number of Remote UEs per deployed Relay UE",
                 nRemoteUesPerRelay);
    ...
    cmd.AddValue("selectionAlgorithm",
                 "The Relay UE (re)selection algorithm the Remote UEs will use "
                 "(RandomNoReselection|MaxSDRsrpNoReselection|MaxSDRsrp)",
                 selectionAlgorithm);
    cmd.AddValue(
        "echoServerNode",
        "The node towards which the Remote UE traffic is directed to (RemoteHost|RemoteUE)",
        echoServerNode);

    cmd.Parse(argc, argv);

    if (echoServerNode != "RemoteHost" && echoServerNode != "RemoteUE")
    {
        NS_FATAL_ERROR("Wrong echoServerNode!. Options are (RemoteHost|RemoteUE).");
        return 1;
    }
    if (echoServerNode == "RemoteUE" && nRelayUes * nRemoteUesPerRelay < 2)
    {
    ...
    // Four discovery periods (4 * 0.32 s) to ensure relay selection (measurement report every 4
    // periods) One discovery period (0.32 s) to avoid some of the congestion for the connection
    // messages
    ...
    // (2+2*nRemoteUesPerRelay)*0.04 is the time in the worst case for all connection messages to go
    // through between a Remote UE and a Relay UE: 2 msgs from the Remote UE, each of them in 1
    // single SL period (0.04 s) and 2 msgs from the Relay UE, which given the SL period RR
    // scheduling, it can take in the worst case up to nRemoteUesPerRelay SL periods to be sent
    // timeBetweenRemoteStarts to ensure sequentiality of activation
    double timeBetweenRelayStarts =
        5.0 +
        nRemoteUesPerRelay * (((2 + 2 * nRemoteUesPerRelay) * 0.04) + timeBetweenRemoteStarts); // s
    ...
    for (uint32_t ryIdx = 0; ryIdx < nRelayUes; ryIdx++)
    {
        ...
        for (uint32_t rm = 0; rm < nRemoteUesPerRelay; ++rm)
        {
            ...
            NS_LOG_INFO("Remote UE Idx " << rmIdx << " start time " << startTimeRemote[rmIdx]
                                         << "s");
        }
    }
    ...
    Config::SetDefault("ns3::LteSlBasicUeController::RelayUeSelectionAlgorithm",
                       StringValue(selectionAlgorithm));
    ...
    if (echoServerNode == "RemoteHost")
    {
        echoServerAddr = internetIpIfaces.GetAddress(1, 1);
    }
    else if (echoServerNode == "RemoteUE")
    {
    ...
    *packetOutputStream->GetStream()
    ...
    for (uint16_t remUeIdx = 0; remUeIdx < remoteUeNodes.GetN(); remUeIdx++)
    {
        ...
        singleServerApp.Get(0)->TraceConnect("RxWithAddresses",
                                             oss.str(),
                                             MakeBoundCallback(&UePacketTrace, packetOutputStream));
        oss.str("");
        oss << "tx\tS\t" << echoServerNodeId;
        singleServerApp.Get(0)->TraceConnect("TxWithAddresses",
                                             oss.str(),
                                             MakeBoundCallback(&UePacketTrace, packetOutputStream));
        oss.str("");
        ...
        if (echoServerNode == "RemoteUE" && remUeIdx != 0)
        {
            // Schedule the change of the RemoteAddress to 100 ms before the start of the
            // application normally 'Remote UE (0)' should be already connected to its Relay UE so
            // we can assign its address as RemoteAddress
            Simulator::Schedule(Seconds(3.0 + startTimeRemote[remUeIdx] - 0.100),
                                &ChangeUdpEchoClientRemote,
                                remoteUeNodes.Get(0),
    ...
    for (uint32_t ryDevIdx = 0; ryDevIdx < relayUeDevs.GetN(); ryDevIdx++)
    {
        ...
        Simulator::Schedule(Seconds(startTimeRelay[ryDevIdx]),
                            &LteSidelinkHelper::StartRelayService,
                            proseHelper,
                            relayUeDevs.Get(ryDevIdx),
                            serviceCode,
                            LteSlUeRrc::ModelA,
                            LteSlUeRrc::RelayUE);
        NS_LOG_INFO("Relay UE " << ryDevIdx << " node id = [" << relayUeNodes.Get(ryDevIdx)->GetId()
                                << "] provides Service Code " << serviceCode
                                << " and start service at " << startTimeRelay[ryDevIdx] << " s");
        // Remote UEs
        for (uint32_t rm = 0; rm < nRemoteUesPerRelay; ++rm)
        {
            uint32_t rmDevIdx = ryDevIdx * nRemoteUesPerRelay + rm;
            Simulator::Schedule((Seconds(startTimeRemote[rmDevIdx])),
                                &LteSidelinkHelper::StartRelayService,
                                proseHelper,
                                remoteUeDevs.Get(rmDevIdx),
                                serviceCode,
                                LteSlUeRrc::ModelA,
                                LteSlUeRrc::RemoteUE);
            NS_LOG_INFO("Remote UE "
                        << rmDevIdx << " node id = [" << remoteUeNodes.Get(rmDevIdx)->GetId()
                        << "] interested in Service Code " << serviceCode
                        << " and start service at " << startTimeRemote[rmDevIdx] << " s");
        }
    }
    ...
    *PC5SignalingPacketTraceStream->GetStream() << "time(s)\ttxId\tRxId\tmsgType" << std::endl;
    ...
    Ptr<OutputStreamWrapper> ReportUeSdRsrpMeasurementsStream =
        ascii.CreateFileStream("ReportUeSdRsrpMeasurementsTrace.txt");
    *ReportUeSdRsrpMeasurementsStream->GetStream()
        << "time(s)\trnti\trelayId\tSC\tsdRsrp" << std::endl;
    for (uint32_t ueDevIdx = 0; ueDevIdx < remoteUeDevs.GetN(); ueDevIdx++)
    {
        ...
        phy->TraceConnectWithoutContext(
            "ReportUeSdRsrpMeasurements",
            MakeBoundCallback(&ReportUeSdRsrpMeasurementsTrace, ReportUeSdRsrpMeasurementsStream));
    }

    // Tracing Relay Selection decisions in the Remote UEs
    Ptr<OutputStreamWrapper> RelayUeSelectionStream =
        ascii.CreateFileStream("RelayUeSelectionTrace.txt");
    *RelayUeSelectionStream->GetStream() << "time(s)\timsi\tSC\tcRelayId\tsRelayId" << std::endl;
    ...
    for (uint32_t ueDevIdx = 0; ueDevIdx < remoteUeDevs.GetN(); ueDevIdx++)
    {
        ...
        ctlr->TraceConnectWithoutContext(
            "RelayUeSelection",
            MakeBoundCallback(&RelayUeSelectionTrace, RelayUeSelectionStream));
    }

    // Tracing onte-to-one connection status in all UEs (Remote UEs and Relay UEs)
    Ptr<OutputStreamWrapper> Pc5ConnectionStatusStream =
        ascii.CreateFileStream("Pc5ConnectionStatusTrace.txt");
    *Pc5ConnectionStatusStream->GetStream()
        << "time(s)\tselfUeId\tpeerUeId\trole\tstatus" << std::endl;
    ...
    for (uint32_t ueDevIdx = 0; ueDevIdx < allUeDevs.GetN(); ueDevIdx++)
    {
        ...
        ctlr->TraceConnectWithoutContext(
            "Pc5ConnectionStatus",
            MakeBoundCallback(&Pc5ConnectionStatusTrace, Pc5ConnectionStatusStream));
    }
