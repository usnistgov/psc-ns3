        ...
        else
        {
            ...
            if (oss.str() == "::") // dstAddrs not set
            {
                *stream->GetStream()
                    ...
                    << Inet6SocketAddress::ConvertFrom(srcAddrs).GetPort() << "\t" << localAddrs
...
main(int argc, char* argv[])
{
    ...
    *appStream->GetStream() << "time(s)\ttx/rx\tNodeID\tIMSI\tPktSize(bytes)\tIP[src]\tIP[dst]"
                            << std::endl;
    std::ostringstream oss;

    Ipv6Address localAddrs =
        serverApps.Get(0)->GetNode()->GetObject<Ipv6L3Protocol>()->GetAddress(1, 1).GetAddress();

    // Upward: Tx by UE / Rx by Remote Host
    oss << "rx-RH\t" << serverApps.Get(0)->GetNode()->GetId() << "\t"
        << "-";
    serverApps.Get(0)->TraceConnect("RxWithAddresses",
                                    oss.str(),
                                    MakeBoundCallback(&UePacketTrace, appStream, localAddrs));
    oss.str("");

    // Downward: Tx by Remote Host / Rx by UE
    oss << "tx-RH\t" << serverApps.Get(0)->GetNode()->GetId() << "\t"
        << "-";
    serverApps.Get(0)->TraceConnect("TxWithAddresses",
                                    oss.str(),
                                    MakeBoundCallback(&UePacketTrace, appStream, localAddrs));
    oss.str("");
    ...
    // Downward: Tx by Remote Host / Rx by UE
    oss << "rx-UE\t" << clientApps.Get(0)->GetNode()->GetId() << "\t"
        << clientApps.Get(0)->GetNode()->GetDevice(0)->GetObject<LteUeNetDevice>()->GetImsi();
    clientApps.Get(0)->TraceConnect("RxWithAddresses",
                                    oss.str(),
                                    MakeBoundCallback(&UePacketTrace, appStream, localAddrs));
    oss.str("");

    // Upward: Tx by UE / Rx by Remote Host
    oss << "tx-UE\t" << clientApps.Get(0)->GetNode()->GetId() << "\t"
        << clientApps.Get(0)->GetNode()->GetDevice(0)->GetObject<LteUeNetDevice>()->GetImsi();
    clientApps.Get(0)->TraceConnect("TxWithAddresses",
                                    oss.str(),
                                    MakeBoundCallback(&UePacketTrace, appStream, localAddrs));
    oss.str("");
