...
void
PacketSrcDstAddrsTrace(Ptr<OutputStreamWrapper> stream,
                       const Ipv4Address& localAddrs,
                       std::string contex,
                       Ptr<const Packet> p,
                       const Address& src_addrs,
                       const Address& dst_addrs)
{
    std::ostringstream oss;
    *stream->GetStream() << Simulator::Now().GetMilliSeconds() << "\t" << contex << "\t"
                         << p->GetSize() << "\t";
    ...
    if (InetSocketAddress::IsMatchingType(src_addrs)) // assumes dst_addrs is same type!
    {
        ...
        if (oss.str() == "0.0.0.0") // src_addrs not set
        {
            *stream->GetStream() << localAddrs << ":"
                                 ...
                                 << InetSocketAddress::ConvertFrom(dst_addrs).GetIpv4() << ":"
                                 << InetSocketAddress::ConvertFrom(dst_addrs).GetPort()
                                 << std::endl;
        }
        else
        {
            ...
            if (oss.str() == "0.0.0.0") // dst_addrs not set
            {
                *stream->GetStream()
                    ...
                    << InetSocketAddress::ConvertFrom(src_addrs).GetPort() << "\t" << localAddrs
                    << ":" << InetSocketAddress::ConvertFrom(dst_addrs).GetPort() << std::endl;
            }
            else
            {
                *stream->GetStream()
    ...
    {
        *stream->GetStream() << Inet6SocketAddress::ConvertFrom(src_addrs).GetIpv6() << ":"
                             << Inet6SocketAddress::ConvertFrom(src_addrs).GetPort() << "\t"
                             << Inet6SocketAddress::ConvertFrom(dst_addrs).GetIpv6() << ":"
                             << Inet6SocketAddress::ConvertFrom(dst_addrs).GetPort() << std::endl;
    }
    else
    {
...
void
NotifyChangeOfSyncRef(Ptr<OutputStreamWrapper> stream,
                      LteUeRrc::SlChangeOfSyncRefStatParameters param)
{
    *stream->GetStream() << Simulator::Now().GetMilliSeconds() << "\t" << param.imsi << "\t"
                         << param.prevSlssid << "\t" << param.prevRxOffset << "\t"
                         << param.prevFrameNo << "\t" << param.prevSubframeNo << "\t"
                         << param.currSlssid << "\t" << param.currRxOffset << "\t"
                         << param.currFrameNo << "\t" << param.currSubframeNo << std::endl;
}

void
NotifySendOfSlss(Ptr<OutputStreamWrapper> stream,
                 uint64_t imsi,
                 uint64_t slssid,
                 uint16_t txOffset,
                 bool inCoverage,
                 ...
                 uint16_t subframe)
{
    *stream->GetStream() << Simulator::Now().GetMilliSeconds() << "\t" << imsi << "\t" << slssid
                         << "\t" << txOffset << "\t" << inCoverage << "\t" << frame << "\t"
                         << subframe << std::endl;
}
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("dropOnCol",
                 "Drop PSSCH and PSCCH messages on collisions",
                 dropOnCollisionEnabled);
    cmd.AddValue("onoff", "Type of traffic", onoff);

    /*Synchronization*/
    cmd.AddValue("syncTxThreshOoC",
                 "SL Sync: Threshold defining the “inner part” of the SyncRef UE cell",
                 syncTxThreshOoC);
    cmd.AddValue("filterCoefficient",
                 "SL Sync: Weight of the last measurement => k ",
                 filterCoefficient);
    cmd.AddValue("syncRefMinHyst",
                 "SL Sync: How stronger than the minimum required the signal of a SyncRef UE "
                 "should be to consider it",
                 syncRefMinHyst);
    cmd.AddValue("syncRefDiffHyst",
                 "SL Sync: How stronger than the current selected SyncRef UE the signal of another "
                 "SyncRef UE should be to consider it",
                 syncRefDiffHyst);
    cmd.AddValue(
        "interScanTimeMin",
        "SL Sync: Min value of the uniform dist of the inter syncRef-selection-processes time",
        interScanTimeMin);
    cmd.AddValue(
        "interScanTimeMax",
        "SL Sync: Max value of the uniform dist of the inter syncRef-selection-processes time",
        interScanTimeMax);
    ...
    cmd.AddValue("evalTime",
                 "SL Sync: Length of the SyncRef evaluation period for cease/initiation of SLSS "
                 "transmission",
                 evalTime);
    cmd.AddValue("firstScanTimeMin",
                 "SL Sync: Min value of the uniform dist of the initial measurement",
                 firstScanTimeMin);
    cmd.AddValue("firstScanTimeMax",
                 "SL Sync: Max value of the uniform dist of the initial measurement",
                 firstScanTimeMax);
    cmd.AddValue(
        "unsyncSl",
        "SL Sync: unsynchronized scenario (random frame/subframe indication, and random SLSSID",
        unsyncSl);
    ...
    Config::SetDefault("ns3::LteUePhy::UeSlssInterScanningPeriodMax",
                       TimeValue(MilliSeconds(interScanTimeMax)));
    Config::SetDefault("ns3::LteUePhy::UeSlssInterScanningPeriodMin",
                       TimeValue(MilliSeconds(interScanTimeMin)));
    ...
    Config::SetDefault("ns3::LteSpectrumPhy::SlCtrlErrorModelEnabled",
                       BooleanValue(ctrlErrorModelEnabled));
    Config::SetDefault("ns3::LteSpectrumPhy::CtrlFullDuplexEnabled",
                       BooleanValue(!ctrlErrorModelEnabled));
    ...
    Config::SetDefault("ns3::LteSpectrumPhy::DropRbOnCollisionEnabled",
                       BooleanValue(dropOnCollisionEnabled));
    ...
    if (ueRespondersPerSector > 0)
    {
        ...
        // Do not set position here, the topology helper takes care of deploying the nodes when
        // called ahead!
    ...
    if (ueResponders.GetN() != 0)
    {
        ...
        {
            m_outFile << "eNB\t" << sectorNodes.Get(i)->GetId() << "\t"
                      << sectorNodes.Get(i)->GetObject<MobilityModel>()->GetPosition().x << "\t"
                      << sectorNodes.Get(i)->GetObject<MobilityModel>()->GetPosition().y
                      << std::endl;
        }
        ...
        for (uint32_t i = 0; i < ueResponders.GetN(); ++i)
        {
            ...
            m_outFile << "UE\t" << ueNetDevice->GetObject<LteUeNetDevice>()->GetImsi() << "\t"
                      << ueResponders.Get(i)->GetObject<MobilityModel>()->GetPosition().x << "\t"
                      << ueResponders.Get(i)->GetObject<MobilityModel>()->GetPosition().y
                      << std::endl;
        }
        m_outFile.close();
    }
    ...
    // createdgroups = proseHelper->AssociateForBroadcastWithTxEnabledToReceive (ueTxPower,
    // ulEarfcn, ulBandwidth, ueRespondersDevs, -1000, numGroups, LteProseHelper::SLRSRP_TX_PW);
    createdgroups = proseHelper->AssociateForBroadcast(ueTxPower,
                                                       ulEarfcn,
                                                       ulBandwidth,
                                                       ueRespondersDevs,
                                                       -112,
                                                       numGroups,
                                                       LteSidelinkHelper::SLRSRP_PSBCH);
    ...
    *stream->GetStream() << "time\ttx/rx\tNID\tIMSI\tUEtype\tsize\tIP[src]\tIP[dst]" << std::endl;
    ...
    Ipv4Address groupRespondersIpv4Address =
        Ipv4AddressGenerator::NextAddress(Ipv4Mask("255.0.0.0"));
    ...
    for (gIt = createdgroups.begin(); gIt != createdgroups.end(); gIt++)
    {
        ...
        Ptr<LteSlTft> tft =
            Create<LteSlTft>(LteSlTft::TRANSMIT, groupRespondersIpv4Address, groupL2Address);
        ...
        for (uint32_t ac = 0; ac < clientRespondersApps.GetN(); ac++)
        {
            ...
            Ipv4Address localAddrs =
                (*gIt).Get(ac)->GetNode()->GetObject<Ipv4L3Protocol>()->GetAddress(1, 0).GetLocal();
            std::cout << "Tx address: " << localAddrs << std::endl;
            clientRespondersApps.Get(ac)->TraceConnect(
                "TxWithAddresses",
                oss.str(),
                MakeBoundCallback(&PacketSrcDstAddrsTrace, stream, localAddrs));
            oss.str("");
        }
    ...
    for (uint32_t ac = 0; ac < clientRespondersSrvApps.GetN(); ac++)
    {
        ...
        oss << "r\t" << ueResponders.Get(ac)->GetId() << "\t"
            << ueResponders.Get(ac)->GetDevice(0)->GetObject<LteUeNetDevice>()->GetImsi()
            << "\tresp";
        clientRespondersSrvApps.Get(ac)->TraceConnect(
            "RxWithAddresses",
            oss.str(),
            MakeBoundCallback(&PacketSrcDstAddrsTrace, stream, localAddrs));
        oss.str("");
    ...
    preconfiguration.preconfigComm.pools[0].scPeriod.period =
        LteRrcSap::PeriodAsEnum(slPeriod).period;
    ...
    preconfiguration.preconfigComm.pools[0].scTfResourceConfig.subframeBitmap.bitmap =
        pscchTrpNumber;
    // data
    ...
    preconfiguration.preconfigComm.pools[0].dataHoppingConfig.numSubbands =
        LteRrcSap::SlHoppingConfigComm::ns4;
    preconfiguration.preconfigComm.pools[0].dataHoppingConfig.rbOffset = 0;
    // UE selected parameters
    preconfiguration.preconfigComm.pools[0].trptSubset.subset =
        std::bitset<3>(0x7); // ktrp1=1, ktrp2=1, ktrp4=1 All T-RPT set available
    ...
    preconfiguration.preconfigComm.pools[0].dataTfResourceConfig.offsetIndicator.offset =
        pscchLength; // allow for control frames
    ...
    preconfiguration.preconfigComm.pools[0].dataTxParameters.alpha =
    ...
    *streamSyncRef->GetStream() << "Time\tIMSI\tprevSLSSID\tprevRxOffset\tprevFrameNo\tprevSframeNo"
                                   "\tcurrSLSSID\tcurrRxOffset\tcurrFrameNo\tcurrSframeNo"
                                << std::endl;
    Ptr<OutputStreamWrapper> streamSendOfSlss = ascii.CreateFileStream("TxSlss.txt");
    *streamSendOfSlss->GetStream()
        << "Time\tIMSI\tSLSSID\ttxOffset\tinCoverage\tFrameNo\tSframeNo" << std::endl;
    ...
    *streamFirstScan->GetStream() << "Time\tIMSI" << std::endl;
    ...
    for (groupIt = createdgroups.begin(); groupIt != createdgroups.end(); groupIt++)
    {
        for (uint32_t i = 0; i < (*groupIt).GetN(); ++i)
        {
            ...
            if ((*groupIt).Get(i)->GetObject<LteUeNetDevice>()->GetPhy()->GetFirstScanningTime() ==
                MilliSeconds(0))
            {
                uint32_t t = 0;
                if (slSyncActive)
                {
                    ...
                    (*groupIt).Get(i)->GetObject<LteUeNetDevice>()->GetPhy()->SetFirstScanningTime(
                        MilliSeconds(t));
                }
                *streamFirstScan->GetStream()
                    << t << "\t"
                    << (*groupIt).Get(i)->GetObject<LteUeNetDevice>()->GetRrc()->GetImsi()
                    << std::endl;
            }
        }
    }
    ...
    for (uint32_t i = 0; i < ueRespondersDevs.GetN(); ++i)
    {
        ...
        ueRrc->TraceConnectWithoutContext("ChangeOfSyncRef",
                                          MakeBoundCallback(&NotifyChangeOfSyncRef, streamSyncRef));
        ueRrc->TraceConnectWithoutContext("SendSLSS",
                                          MakeBoundCallback(&NotifySendOfSlss, streamSendOfSlss));
    }
    for (uint32_t i = 0; i < ueRespondersDevs.GetN(); ++i)
    {
        Ptr<LteUeRrc> ueRrc = ueRespondersDevs.Get(i)->GetObject<LteUeNetDevice>()->GetRrc();
