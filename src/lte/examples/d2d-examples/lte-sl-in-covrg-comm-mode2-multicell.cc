...
{
    void Trace(Ptr<const Packet> p,
               const Address& srcAddrs,
               const Address& dstAddrs); ///< Callback function
    uint32_t m_nRxMsgs{0};               ///< Number of messages received
};

void
NumberOfRxMessagesTracer::Trace(Ptr<const Packet> p,
                                const Address& srcAddrs,
                                const Address& dstAddrs)
{
    m_nRxMsgs++;
 ...
 *
 * This scenario uses Mode 2 for the resource allocation, i.e., the transmitter
 * UE selects autonomously the resources to use from the pool.
 * The pool configuration and the application configuration are the same as in
 * the scenario 'lte-sl-in-covrg-comm-mode2.cc'. Please refer to the Sidelink
 * section of the LTE user documentation for more details about it.
 *
 *
 */

NS_LOG_COMPONENT_DEFINE("LteSlInCovrgCommMode2Multicell");
    ...
    *stream->GetStream() << "time(sec)\ttx/rx\tNodeID\tIMSI\tPktSize(bytes)\tIP[src]\tIP[dst]"
    ...
    if (!useIPv6)
    {
        ...
        for (uint16_t ac = 0; ac < clientApps.GetN(); ac++)
        {
            ...
            oss << "tx\t" << ueNodes.Get(0)->GetId() << "\t"
                << ueNodes.Get(0)->GetDevice(0)->GetObject<LteUeNetDevice>()->GetImsi();
        ...
        for (uint16_t ac = 0; ac < serverApps.GetN(); ac++)
        {
            ...
            oss << "rx\t" << ueNodes.Get(1)->GetId() << "\t"
                << ueNodes.Get(1)->GetDevice(0)->GetObject<LteUeNetDevice>()->GetImsi();
    ...
    for (uint16_t ac = 0; ac < serverApps.GetN(); ac++)
    {
        serverApps.Get(ac)->TraceConnectWithoutContext(
            "RxWithAddresses",
            MakeCallback(&NumberOfRxMessagesTracer::Trace, &tracer));
    }
