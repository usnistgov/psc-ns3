 ...
 * (i.e., hexagonal cells in ring topology) Parabolic3dAntennaModel and 3GPP compliant pathloss
 * models in buildings module. It only supports a downlink flow. By default, only the LTE radio
 * access will be simulated with RLC SM. Users interested to use EPC or to generate REM may
 * configure the following Global Values through command-line, e.g, command to generate REM of a
 * single hexagonal cell is :
...
static ns3::GlobalValue g_nRings(
    "nRings",
...
static ns3::GlobalValue g_enableChannelLogs(
    "enableChannelLogs",
...
static ns3::GlobalValue g_dropUEsIndoorOutdoor("dropUEsIndoorOutdoor",
                                               "Drop UEs indoor and outdoor with 2 buildings per "
                                               "cell/sector as per TR 36.843 Table A.2.1.1-1.",
                                               ns3::BooleanValue(false),
                                               ns3::MakeBooleanChecker());
static ns3::GlobalValue g_enableLteTraces(
    "enableLteTraces",
...
void
GetPathlossTrace(bool isDownlink,
                 std::string context,
                 const double ploss,
                 Ptr<Node> n1,
                 Ptr<Node> n2,
...
GetConnectionTrace(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti)
{
    NS_LOG_DEBUG(Simulator::Now().GetSeconds()
                 << " eNB CellId " << cellId << " successful connection of UE with IMSI " << imsi
                 << " RNTI " << rnti);
...
void
SchedulePathlossLogConnect()
{
    Config::Connect("/ChannelList/2/PropagationLossModel/Hybrid3gppPathlossValue",
                    MakeBoundCallback(&GetPathlossTrace, true));
    Config::Connect("/ChannelList/3/PropagationLossModel/Hybrid3gppPathlossValue",
                    MakeBoundCallback(&GetPathlossTrace, false));
}

void
ScheduleGainLogConnect()
{
...
ConnectConectionTrace()
{
    Config::Connect("/NodeList/*/DeviceList/*/LteEnbRrc/ConnectionEstablished",
                    MakeCallback(&GetConnectionTrace));
}
...
SavePathlossStats(std::string fileName, const std::vector<Pathloss>& pathloss)
{
    ...
    // At time t sec, the pathloss between node id = a in Pos (Indoor/Outdoor) with IP = x.x.x.x and
    // node id = b in Pos (Indoor/Outdoor) with IP = x.x.x.x is equal to Ploss(dB) and the distance
    // between them is Dist(m)
    ...
    for (std::vector<Pathloss>::size_type i = 0; i != pathloss.size(); i++)
    {
        ...
        outFile << pathloss[i].time.GetSeconds() << "\t" << pathloss[i].n1->GetId() << "\t" << aMob
                << "\t" << pathloss[i].n1->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal() << "\t"
                << pathloss[i].n2->GetId() << "\t" << bMob << "\t"
                << pathloss[i].n2->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal() << "\t"
                << pathloss[i].ploss << "\t" << pathloss[i].dist << std::endl;
    }
}
...
SaveGainStats(std::string fileName, const std::vector<GainLogs>& gain)
{
    ...
    outFile << "#time(sec)\ttransmitter\treceiver\ttxAntennaGain(dB)\trxAntennaGain(dB)"
               "\tpropagationGain(dB)\tpathloss(dB)"
            << std::endl;

    for (std::vector<GainLogs>::size_type i = 0; i != gain.size(); i++)
    {
        outFile << gain[i].time.GetSeconds() << "\t" << gain[i].tx->GetObject<Node>()->GetId()
                << "\t" << gain[i].rx->GetObject<Node>()->GetId() << "\t" << gain[i].txAntennaGain
                << "\t" << gain[i].rxAntennaGain << "\t" << gain[i].propagationGain << "\t"
...
SaveConnectionTrace(std::string fileName, const std::vector<ConnectionLogs>& connectionLogs)
{
    ...
    // In the logs, TxNID corresponds to a cell id of an eNB and RxNID corresponds to an IMSI of an
    // UE
...
main(int argc, char* argv[])
{
    ...
    Config::SetDefault("ns3::Hybrid3gppPropagationLossModel::ShadowingEnabled",
                       BooleanValue(false));
    ...
    // Set Harq frequency reuse to assign different RBs to each eNb of 3 sector cell. FrCellTypeId
    ...
    {
        // UEs attached to the closest  eNB **Since all the three eNBs of a cell is on the same
        // location the function AttachToClosestEnb attaches every UE to the first eNB (See lte MAC
        // stats). The REM generated with this will be different from what we get with epc
        // enabled**. lteHelper->AttachToClosestEnb (ueDevs, enbDevs);
    ...
    // Channel traces, i.e., Hybrid3gppPathlossValue (only available for
    // ns3::Hybrid3gppPropagationLossModel) and Gain traces from MultiModelSpectrumChannel.
