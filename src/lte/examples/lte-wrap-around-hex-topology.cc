 ...
 * ./waf --run "example-wrap-around-hex-topology --nUesPerSector=2 --nRings=2 --enableTopologyLogs=1
 * --enableChannelLogs=0 --simTime=1 --RngRun=1" Alternatively these values can also be configured
 * through ns-3 ConfigStore system.
 */
...
{
    Config::Connect("/ChannelList/0/PropagationLossModel/Hybrid3gppPathlossValue",
                    MakeBoundCallback(&GetPathlossTrace, true));
    Config::Connect("/ChannelList/1/PropagationLossModel/Hybrid3gppPathlossValue",
...
SavePathlossStats(std::string fileName, const std::vector<Pathloss>& pathloss)
{
    ...
    // At time t sec, the pathloss between node id = a Located in (Indoor/Outdoor) and node id = b
    // Located in (Indoor/Outdoor) is equal to Ploss(dB) and the distance between them is Dist(m)
    ...
    for (std::vector<Pathloss>::size_type i = 0; i != pathloss.size(); i++)
    {
        ...
        outFile << pathloss[i].time.GetSeconds() << "\t" << pathloss[i].n1->GetId() << "\t" << aMob
                << "\t" << pathloss[i].n2->GetId() << "\t" << bMob << "\t" << pathloss[i].ploss
...
void
SaveWrapAroundPositions(std::string filename,
                        std::map<std::vector<double>, WrapAroundReplicas> mapOfWAround)
{
...
void
SaveWrapAroundAttachInfo(std::string filename,
                         std::map<uint64_t, WrapAroundInfo_t> mapOfWAroundInfo)
{
    ...
    for (const auto& i : mapOfWAroundInfo)
    {
        outFile << i.first << " " << i.second.cellId << " " << i.second.posUe.x << " "
                << i.second.posUe.y << " " << i.second.posEnb.x << " " << i.second.posEnb.y
                << std::endl;
    }
}
...
main(int argc, char* argv[])
{
    ...
    std::map<std::vector<double>, WrapAroundReplicas> mapOfWAround =
        topoHelper->GetWrapAroundPositions();
