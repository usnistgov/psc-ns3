...
NS_LOG_COMPONENT_DEFINE ("Nist3gppTestValidation");

class NistSaveTraces : public Object
{
public:
    NistSaveTraces (std::string path);
    ~NistSaveTraces ();
...
NistSaveTraces::NistSaveTraces (std::string path)
{
...
NistSaveTraces::~NistSaveTraces ()
{
  m_outFile.close ();
}
void 
NistSaveTraces::GetGainTrace  (std::string context, Ptr<MobilityModel> tx,Ptr<MobilityModel> rx, double txAntennaGain, double rxAntennaGain, double propagationGain, double pathloss)
{
...
main (int argc, char *argv[])
{
  ...
  // 15 MHz --> 75
  // 20 MHz --> 100
  // Frequency to match 3GPP is 2GHz, we use the EARFCN from 36.101 Table 5.7.3-1 to get the correct value
  //Config::SetDefault ("ns3::NistUrbanmacrocellPropagationLossModel::Frequency", DoubleValue (2000e6));//LTE Band1: 2120MHz
  ...
  //EARFCN are left with default values, which are for LTE Band 1 frequencies close to 2GHz.
  /*
  ...
  */
  ...
  /*
  Config::SetDefault ("ns3::NistLteUePowerControl::Pcmax", DoubleValue (31.0));
  */
  ...
  Config::SetDefault ("ns3::NistLteEnbRrc::SrsPeriodicity", UintegerValue (80));//80

  /*
  // Change the default HO algorithm (NoOpHandover) to the strongest cell HO algorithm
  Config::SetDefault ("ns3::NistLteHelper::HandoverAlgorithm", StringValue ("ns3::NistA3RsrpHandoverAlgorithm"));
  Config::SetDefault ("ns3::NistA3RsrpHandoverAlgorithm::Hysteresis", DoubleValue (1.0));
  */
  ...
  cmd.AddValue ("ue", "Number of users per sector", nbUesPerSector);
  cmd.Parse(argc, argv);
  ...
  topoHelper->SetSiteHeight(25);// 3GPP specs!

  // Set pathloss model
  lteHelper->SetPathlossModelType ("ns3::NistUrbanmacrocellPropagationLossModel");
  //Next line is commented out, frequency is set above by changing default class default value
  //lteHelper->SetPathlossModelAttribute("Frequency", DoubleValue (2000e6)); // Frequency to match 3GPP os 2GHz
  ...
  lteHelper->SetSchedulerType ("ns3::NistRrFfMacScheduler");
  ...
  //NetDeviceContainer ueDevs = topoHelper->DropUEsUniformlyPerCell(ues);
  ...
  ////////////////////////////////////////
  // Save the nodes IDs and positions (file name : idip.txt)
  std::ofstream m_outFile;
  ...
  // Gain traces
  NistSaveTraces dlFileTrace ("nist-dl-gain.out");
  NistSaveTraces ulFileTrace ("nist-ul-gain.out");
  Config::Connect ("/ChannelList/0/Gain", MakeCallback (&NistSaveTraces::GetGainTrace, &dlFileTrace));
  Config::Connect ("/ChannelList/1/Gain", MakeCallback (&NistSaveTraces::GetGainTrace, &ulFileTrace));

  uint32_t status_period = 1;
  Simulator::Schedule(Seconds(status_period),&PrintStatus, status_period);
